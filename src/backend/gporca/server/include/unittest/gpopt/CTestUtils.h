...
public:
	...
	static CTableDescriptor *PtabdescPlain(CMemoryPool *mp, ULONG num_cols, IMDId *mdid,
	...
	static CTableDescriptor *PtabdescPlainWithColNameFormat(CMemoryPool *mp, ULONG num_cols,
															IMDId *mdid,
	...
	static CTableDescriptor *PtabdescCreate(CMemoryPool *mp, ULONG num_cols, IMDId *mdid,
											const CName &name, BOOL fPartitioned = false);

	// generate a get expression
	static CExpression *PexprLogicalGet(CMemoryPool *mp, CTableDescriptor *ptabdesc,
										const CWStringConst *pstrTableAlias);

	// generate a get expression over table with nullable columns
	static CExpression *PexprLogicalGetNullable(CMemoryPool *mp, OID oidTable,
												const CWStringConst *str_table_name,
												const CWStringConst *pstrTableAlias);

	// generate a get expression
	static CExpression *PexprLogicalGet(CMemoryPool *mp, CWStringConst *str_table_name,
	...
	static CExpression *PexprLogicalSelect(CMemoryPool *mp, CWStringConst *str_table_name,
										   CWStringConst *pstrTableAlias, ULONG ulTableId);
	...
	static CExpression *PexprLogicalSubqueryWithConstTableGet(CMemoryPool *mp,
	...
	// generate a correlated select expression
	static CExpression *PexprLogicalSelectCorrelated(CMemoryPool *mp, CColRefSet *outer_refs,
													 ULONG ulLevel);
	...
	static CExpression *PexprLogicalNAryJoin(CMemoryPool *mp, CWStringConst *pstrRel, ULONG *pulRel,
	...
	static CExpression *PexprLogicalProject(CMemoryPool *mp, CExpression *pexpr, CColRef *colref,
											CColRef *new_colref);
	...
	static CExpression *PexprLogicalLimit(CMemoryPool *mp, CExpression *pexpr, LINT iStart,
	...
	static CExpression *PexprLogicalSequenceProject(CMemoryPool *mp, OID oidFunc,
	...
	static CExpressionJoinsArray *PdrgpexprJoins(CMemoryPool *mp, CWStringConst *pstrRel,
	...
	static CExpression *PexprScalarNestedPreds(CMemoryPool *mp, CExpression *pexpr,
	...
	static GPOS_RESULT EresTranslate(CMemoryPool *mp, const CHAR *szQueryFileName,
	...
	static void CreateExpectedAndActualFile(CMemoryPool *mp, const CHAR *file_name,
											CWStringDynamic *strExpected,
											CWStringDynamic *strActual);

	// match given two plans using string comparison
	static BOOL FPlanMatch(CMemoryPool *mp, IOstream &os, const CDXLNode *first_child_dxlnode,
						   ULLONG ullPlanIdFst, ULLONG ullPlanSpaceSizeFst,
						   const CDXLNode *second_child_dxlnode, ULLONG ullPlanIdSnd,
						   ULLONG ullPlanSpaceSizeSnd, const CHAR *file_name);

	// compare given DXL plans
	static BOOL FPlanCompare(CMemoryPool *mp, IOstream &os, const CDXLNode *first_child_dxlnode,
							 ULLONG ullPlanIdFst, ULLONG ullPlanSpaceSizeFst,
							 const CDXLNode *second_child_dxlnode, ULLONG ullPlanIdSnd,
							 ULLONG ullPlanSpaceSizeSnd, BOOL fMatchPlans, INT iCmpSpaceSize,
	...
	static GPOS_RESULT EresRunMinidump(CMemoryPool *mp, CMDAccessor *md_accessor,
	...
	// test plan sampling
	static GPOS_RESULT EresSamplePlans(const CHAR *rgszFileNames[], ULONG ulTests,
									   ULONG *pulTestCounter, ULONG ulSessionId, ULONG ulCmdId);

	// check all enumerated plans using given PlanChecker function
	static GPOS_RESULT EresCheckPlans(const CHAR *rgszFileNames[], ULONG ulTests,
									  ULONG *pulTestCounter, ULONG ulSessionId, ULONG ulCmdId,
									  FnPlanChecker *pfpc);
	...
	static GPOS_RESULT EresCheckOptimizedPlan(const CHAR *rgszFileNames[], ULONG ulTests,
											  ULONG *pulTestCounter, ULONG ulSessionId,
	...
	static CConstraintInterval *PciGenericInterval(CMemoryPool *mp, CMDAccessor *md_accessor,
	...
	static CExpression *PexprScIdentCmpConst(CMemoryPool *mp, CExpression *pexprRelational,
											 IMDType::ECmpType cmp_type, ULONG ulVal);

	// generate a scalar expression comparing scalar identifiers
	static CExpression *PexprScIdentCmpScIdent(CMemoryPool *mp, CExpression *pexprLeft,
	...
	static GPOS_RESULT EresUnittest_RunTests(const CHAR **rgszFileNames, ULONG *pulTestCounter,
											 ULONG ulTests);

	static GPOS_RESULT EresUnittest_RunTestsWithoutAdditionalTraceFlags(const CHAR **rgszFileNames,
	...
	static CColRefSetArray *createEquivalenceClasses(CMemoryPool *mp, CColRefSet *pcrs,
													 INT setBoundary[]);
};	// class CTestUtils
...
CTestUtils::PexprJoinPartitioned(CMemoryPool *mp, BOOL fOuterPartitioned)
{
	...
	CExpression *pexprScalar = CUtils::PexprScalarEqCmp(mp, CUtils::PexprScalarIdent(mp, colref),
...
template <class T>
CExpression *
CTestUtils::PexprLogicalJoin(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight)
{
