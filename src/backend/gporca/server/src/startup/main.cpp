...
#include "unittest/gpopt/minidump/MinidumpTestArray.inl"  // auto generated inlining file
	...
	GPOS_UNITTEST_STD(CSubqueryHandlerTest), GPOS_UNITTEST_STD(CBindingTest), GPOS_UNITTEST_STD(CEngineTest),
	GPOS_UNITTEST_STD(CEquivalenceClassesTest), GPOS_UNITTEST_STD(CExpressionTest), GPOS_UNITTEST_STD(CJoinOrderTest),
	...
	GPOS_UNITTEST_STD(CTableDescriptorTest), GPOS_UNITTEST_STD(CIndexDescriptorTest), GPOS_UNITTEST_STD(CTreeMapTest),
	GPOS_UNITTEST_STD(CXformFactoryTest), GPOS_UNITTEST_STD(CXformTest),
	GPOS_UNITTEST_STD(CConstExprEvaluatorDefaultTest), GPOS_UNITTEST_STD(CConstExprEvaluatorDXLTest),
...
PvExec(void *pv)
{
	...
	if (fMinidump)
	{
		...
		CDXLNode *pdxlnPlan = CMinidumperUtils::PdxlnExecuteMinidump(
			mp, file_name, ulSegments, 1 /*ulSessionId*/, 1 /*ulCmdId*/, optimizer_config, NULL /*pceeval*/
		);
