...
public:
		...
		: m_mp(mp), m_strTableName(strTableName)
	{
		...
		m_ptabdesc =
			GPOS_NEW(mp) CTableDescriptor(mp, mdid, CName(&strTableName), convert_hash_to_random,
	...
	AddIntColumn(CWStringConst strColumnName, int attno, BOOL is_nullable)
	{
		CColumnDescriptor *pcoldesc = GPOS_NEW(m_mp) CColumnDescriptor(
			m_mp, m_pmdtypeint4, default_type_modifier, CName(&strColumnName), attno, is_nullable);
...
CTranslatorDXLToExprTest::EresUnittest_SelectQueryWithScalarOp()
{
	...
	// manually create the Expr Tree
	{
		...
		CExpression *pexprScOp =
			CUtils::PexprScalarOp(mp, pcrLeft, pexprScConst, CWStringConst(GPOS_WSZ_LIT("+")),
		...
		CExpression *pexprScCmp =
			CUtils::PexprScalarCmp(mp, pcrRight, pexprScOp, CWStringConst(GPOS_WSZ_LIT(">")),
