...
CExpressionTest::EresUnittest_SimpleOps()
{
	...
	for (ULONG i = 0; i < GPOS_ARRAY_SIZE(rgpf); i++)
	{
		...
		oss << std::endl
			<< "COPIED EXPRESSION (AFTER MAPPING " << *pcrOld << " TO " << *new_colref << "):" << std::endl;
		GPOS_TRACE(str.GetBuffer());
		str.Reset();
		pexprCopy->DbgPrint();
		pexprCopy->Release();
...
CExpressionTest::EresUnittest_BitmapGet()
{
	...
	CExpression *pexprBitmapIndex =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarBitmapIndexProbe(mp, pindexdesc, mdid), pexprIndexCond);
	...
	CExpression *pexprBitmapIndex2 =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarBitmapIndexProbe(mp, pindexdesc2, pmdid2), pexprIndexCond2);
...
//---------------------------------------------------------------------------
void
CExpressionTest::SetupPlanForFValidPlanTest(CMemoryPool *mp, CExpression **ppexprGby, CColRefSet **ppcrs,
...
CExpressionTest::EresUnittest_FValidPlanError()
{
	...
	// Test that in debug mode GPOS_ASSERT fails for non-physical expressions.
	{
		...
		CReqdPropPlan *prpp = PrppCreateRequiredProperties(mp, pcrsInvalid);
		IDatum *datum =
			GPOS_NEW(mp) gpnaucrates::CDatumInt8GPDB(CTestUtils::m_sysidDefault, 1 /*val*/, false /*is_null*/);
