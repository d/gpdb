	...
	for (ULONG ul = m_ulUnsupportedTestCounter; ul < ulTests; ul++)
	{
		...
		GPOS_TRY
		{
			...
			CDXLNode *pdxlnPlan = CMinidumperUtils::PdxlnExecuteMinidump(
				mp, filename, optimizer_config->GetCostModel()->UlHosts() /*ulSegments*/, 1 /*ulSessionId*/,
				1,					   /*ulCmdId*/
	...
	for (ULONG ul = m_ulNegativeIndexApplyTestCounter; ul < ulTests; ul++)
	{
		GPOS_TRY
		{
			...
			COptimizerConfig *optimizer_config = GPOS_NEW(mp) COptimizerConfig(
				...
				CCTEConfig::PcteconfDefault(mp), pcm, CHint::PhintDefault(mp), CWindowOids::GetWindowOids(mp));
			CDXLNode *pdxlnPlan = CMinidumperUtils::PdxlnExecuteMinidump(mp, rgszNegativeIndexApplyFileNames[ul],
