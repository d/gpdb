...
CSearchStrategyTest::EresUnittest()
{
	CUnittest rgut[] = {
		...
		GPOS_UNITTEST_FUNC_THROW(CSearchStrategyTest::EresUnittest_Timeout, gpopt::ExmaGPOPT,
...
CSearchStrategyTest::Optimize(CMemoryPool *mp, Pfpexpr pfnGenerator,
							  CSearchStageArray *search_stage_array, PfnOptimize pfnOptimize)
{
...
CSearchStrategyTest::PdrgpssRandom(CMemoryPool *mp)
{
	...
	search_stage_array->Append(GPOS_NEW(mp) CSearchStage(pxfsFst, 1000 /*ulTimeThreshold*/,
														 CCost(10E4) /*costThreshold*/));
	search_stage_array->Append(GPOS_NEW(mp) CSearchStage(pxfsSnd, 10000 /*ulTimeThreshold*/,
