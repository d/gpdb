...
CBucket *
CCardinalityTestUtils::PbucketIntegerClosedLowerBound(CMemoryPool *mp, INT iLower, INT iUpper,
...
CBucket *
CCardinalityTestUtils::PbucketInteger(CMemoryPool *mp, INT iLower, INT iUpper, BOOL is_lower_closed,
...
CBucket *
CCardinalityTestUtils::PbucketSingletonBoolVal(CMemoryPool *mp, BOOL fValue, CDouble frequency)
{
...
CHistogram *
CCardinalityTestUtils::PhistInt4Remain(CMemoryPool *mp, ULONG num_of_buckets, CDouble dNDVPerBucket,
									   BOOL fNullFreq, CDouble num_NDV_remain)
{
	...
	return GPOS_NEW(mp)
		CHistogram(mp, histogram_buckets, true, null_freq, num_NDV_remain, freq_remaining);
}
...
CPoint *
CCardinalityTestUtils::PpointGeneric(CMemoryPool *mp, OID oid, CWStringDynamic *pstrEncodedValue,
									 LINT value)
{
...
CPoint *
CCardinalityTestUtils::PpointNumeric(CMemoryPool *mp, CWStringDynamic *pstrEncodedValue,
									 CDouble value)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
	...
	CDXLDatumStatsDoubleMappable *dxl_datum = GPOS_NEW(mp) CDXLDatumStatsDoubleMappable(
		mp, mdid, default_type_modifier, false /*is_const_null*/, data, ulbaSize, value);
...
void
CCardinalityTestUtils::PrintBucket(CMemoryPool *mp, const char *pcPrefix, const CBucket *bucket)
{
...
void
CCardinalityTestUtils::PrintHist(CMemoryPool *mp, const char *pcPrefix, const CHistogram *histogram)
{
	CWStringDynamic str(mp);
	COstreamString oss(&str);
