...
CHistogramTest::EresUnittest_CHistogramInt4()
{
	...
	CHistogram *phist4 =
		histogram->MakeHistogramFilterNormalize(CStatsPred::EstatscmptEq, ppoint4, &scale_factor);
...
CHistogramTest::EresUnittest_CHistogramBool()
{
	...
	CHistogram *histogram1 =
		histogram->MakeHistogramFilterNormalize(CStatsPred::EstatscmptEq, point1, &scale_factor);
	CCardinalityTestUtils::PrintHist(mp, "histogram1", histogram1);
...
CHistogramTest::PhistExampleInt4Remain(CMemoryPool *mp)
{
	...
	return GPOS_NEW(mp) CHistogram(mp, histogram_buckets, true, 0.1 /*null_freq*/,
...
CHistogramTest::EresUnittest_MergeUnion()
{
	...
	CHistogram *result1 =
		histogram1->MakeUnionHistogramNormalize(1000, histogram2, 600, &output_rows1);


	CDouble output_rows2(0.0);
	CHistogram *result2 =
		histogram2->MakeUnionHistogramNormalize(600, histogram1, 1000, &output_rows2);
