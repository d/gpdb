...
CTableDescriptor *
CStatisticsTest::PtabdescTwoColumnSource(CMemoryPool *mp, const CName &nameTable, const IMDTypeInt4 *pmdtype,
										 const CWStringConst &strColA, const CWStringConst &strColB)
{
	CTableDescriptor *ptabdesc =
		GPOS_NEW(mp) CTableDescriptor(mp, GPOS_NEW(mp) CMDIdGPDB(GPOPT_TEST_REL_OID1, 1, 1), nameTable,
...
CStatisticsTest::EresUnittest_CStatisticsBasic()
{
	...
	if (NULL == col_factory->LookupColRef(1 /*id*/))
	{
		...
		(void) col_factory->PcrCreate(pmdtypeint4, default_type_modifier, NULL, 0 /* attno */, false /*IsNullable*/,
									  1 /* id */, CName(&strColA), pexprGet->Pop()->UlOpId(), false /*IsDistCol*/
		);
	}

	if (NULL == col_factory->LookupColRef(2 /*id*/))
	{
		(void) col_factory->PcrCreate(pmdtypeint4, default_type_modifier, NULL, 1 /* attno */, false /*IsNullable*/,
									  2 /* id */, CName(&strColB), pexprGet->Pop()->UlOpId(), false /*IsDistCol*/
		);
	}

	if (NULL == col_factory->LookupColRef(10 /*id*/))
	{
		(void) col_factory->PcrCreate(pmdtypeint4, default_type_modifier, NULL, 2 /* attno */, false /*IsNullable*/,
									  10 /* id */, CName(&strColC), pexprGet->Pop()->UlOpId(), false /*IsDistCol*/
		);
	}
	...
	CStatistics *stats = GPOS_NEW(mp)
		CStatistics(mp, col_histogram_mapping, colid_width_mapping, 1000.0 /* rows */, false /* is_empty */);
	stats->Rows();
	...
	// LASJ stats
	CStatistics *pstats5 =
		stats->CalcLASJoinStats(mp, pstats2, join_preds_stats, true /* DoIgnoreLASJHistComputation */);
...
void
CStatisticsTest::StatsFilterInt4(CMemoryPool *mp, ULONG colid, INT iLower, INT iUpper,
...
void
CStatisticsTest::StatsFilterBool(CMemoryPool *mp, ULONG colid, BOOL fValue, CStatsPredPtrArry *pdrgpstatspred)
{
	CStatsPredPoint *pstatspred1 =
...
void
CStatisticsTest::StatsFilterNumeric(CMemoryPool *mp, ULONG colid, CWStringDynamic *pstrLowerEncoded,
...
void
CStatisticsTest::StatsFilterGeneric(CMemoryPool *mp, ULONG colid, OID oid, CWStringDynamic *pstrLowerEncoded,
