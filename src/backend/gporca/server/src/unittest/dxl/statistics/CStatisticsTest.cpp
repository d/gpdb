...
CTableDescriptor *
CStatisticsTest::PtabdescTwoColumnSource(CMemoryPool *mp, const CName &nameTable,
										 const IMDTypeInt4 *pmdtype, const CWStringConst &strColA,
										 const CWStringConst &strColB)
{
	CTableDescriptor *ptabdesc = GPOS_NEW(mp)
		CTableDescriptor(mp, GPOS_NEW(mp) CMDIdGPDB(GPOPT_TEST_REL_OID1, 1, 1), nameTable,
...
CStatisticsTest::EresUnittest_CStatisticsBucketTest()
{
	...
	CDouble distinct(10.0);
	CBucket *bucket1 =
		CCardinalityTestUtils::PbucketInteger(mp, 1, 5, true, true, frequency, distinct);
	CBucket *bucket2 =
		CCardinalityTestUtils::PbucketInteger(mp, 1, 5, false, false, frequency, distinct);
...
CStatisticsTest::EresUnittest_CStatisticsBasic()
{
	...
	if (NULL == col_factory->LookupColRef(1 /*id*/))
	{
		...
		(void) col_factory->PcrCreate(pmdtypeint4, default_type_modifier, NULL, 0 /* attno */,
	...
	{
		(void) col_factory->PcrCreate(pmdtypeint4, default_type_modifier, NULL, 1 /* attno */,
	...
	{
		(void) col_factory->PcrCreate(pmdtypeint4, default_type_modifier, NULL, 2 /* attno */,
	...
	// LASJ stats
	CStatistics *pstats5 = stats->CalcLASJoinStats(mp, pstats2, join_preds_stats,
...
CStatisticsTest::Pdrgpstatspred2(CMemoryPool *mp)
{
	...
	StatsFilterNumeric(mp, 3, pstrLowerNumeric, pstrUpperNumeric, CDouble(1.0), CDouble(2.0),
					   pdrgpstatspred);
	...
	StatsFilterGeneric(mp, 4, GPDB_DATE, pstrLowerDate, pstrUpperDate, lLowerDate, lUpperDate,
					   pdrgpstatspred);
	...
	StatsFilterGeneric(mp, 5, GPDB_TIMESTAMP, pstrLowerTS, pstrUpperTS, lLowerTS, lUpperTS,
					   pdrgpstatspred);
...
CStatisticsTest::StatsFilterNumeric(CMemoryPool *mp, ULONG colid, CWStringDynamic *pstrLowerEncoded,
									...
									CDouble dValUpper, CStatsPredPtrArry *pdrgpstatspred)
{
	CStatsPredPoint *pstatspred1 = GPOS_NEW(mp)
...
CStatisticsTest::StatsFilterGeneric(CMemoryPool *mp, ULONG colid, OID oid,
									...
									CWStringDynamic *pstrUpperEncoded, LINT lValueLower,
									LINT lValueUpper, CStatsPredPtrArry *pdrgpstatspred)
{
