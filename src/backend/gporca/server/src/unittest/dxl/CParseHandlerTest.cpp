...
CParseHandlerTest::EresUnittest_ErrSAXParseException()
{
	...
	(void) CDXLUtils::GetPlanDXLNode(mp, dxl_string, CTestUtils::m_szXSDPath, &plan_id,
									 &plan_space_size);
...
CParseHandlerTest::EresUnittest_RunAllPositiveTests(const CHAR *rgszFileNames[], ULONG ulFiles,
													GPOS_RESULT (*testFunc)(CMemoryPool *,
																			const CHAR *, BOOL),
													BOOL fValidate)
{
	// create memory pool
...
//---------------------------------------------------------------------------
GPOS_RESULT
CParseHandlerTest::EresParseAndSerializePlan(CMemoryPool *mp, const CHAR *dxl_filename,
											 BOOL fValidate)
{
	...
	CDXLUtils::SerializePlan(mp, osPlan, root_dxl_node, plan_id, plan_space_size,
...
//---------------------------------------------------------------------------
GPOS_RESULT
CParseHandlerTest::EresParseAndSerializeQuery(CMemoryPool *mp, const CHAR *dxl_filename,
											  BOOL fValidate)
{
	CWStringDynamic str(mp);
	COstreamString oss(&str);
	...
	CDXLUtils::SerializeQuery(mp, osQuery, root_dxlnode, dxl_array, cte_producers,
							  true /*serialize_header_footer*/, true /*indentation*/);
...
//---------------------------------------------------------------------------
GPOS_RESULT
CParseHandlerTest::EresParseAndSerializeMetadata(CMemoryPool *mp, const CHAR *dxl_filename,
												 BOOL fValidate)
{
	CWStringDynamic str(mp);
	COstreamString oss(&str);
...
//---------------------------------------------------------------------------
GPOS_RESULT
CParseHandlerTest::EresParseAndSerializeMDRequest(CMemoryPool *mp, const CHAR *dxl_filename,
												  BOOL fValidate)
{
	CWStringDynamic str(mp);
	COstreamString oss(&str);
	...
	CDXLUtils::SerializeMDRequest(mp, pmdr, oss, true /*serialize_header_footer*/,
								  true /*indentation*/);
...
//---------------------------------------------------------------------------
GPOS_RESULT
CParseHandlerTest::EresParseAndSerializeStatistics(CMemoryPool *mp, const CHAR *dxl_filename,
												   BOOL fValidate)
{
...
//---------------------------------------------------------------------------
GPOS_RESULT
CParseHandlerTest::EresParseAndSerializeScalarExpr(CMemoryPool *mp, const CHAR *dxl_filename,
												   BOOL fValidate)
{
	// read DXL file
	CHAR *dxl_string = CDXLUtils::Read(mp, dxl_filename);
	...
	CWStringDynamic *scalar_expr_str = CDXLUtils::SerializeScalarExpr(
		mp, root_dxl_node, true /*serialize_header_footer*/, true /*indentation*/);
	GPOS_CHECK_ABORT;
