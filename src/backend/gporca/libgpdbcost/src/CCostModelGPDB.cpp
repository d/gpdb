...
//---------------------------------------------------------------------------
CCost
CCostModelGPDB::CostUnary(CMemoryPool *mp, CExpressionHandle &exprhdl, const SCostingInfo *pci,
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
								 ...
								 COperator::EopPhysicalPartitionSelector == popChild->Eopid()))
		{
			...
			if (CUtils::FPhysicalScan(scanOp))
			{
				...
				dCostChild = dCostChild + CostScanOutput(mp, dScanRows, pci->GetWidth()[ul],
...
CCost
CCostModelGPDB::CostMaxChild(CMemoryPool *, CExpressionHandle &, const SCostingInfo *pci,
							 ICostModelParams *)
{
	GPOS_ASSERT(NULL != pci);
...
CCostModelGPDB::CostIndexScan(CMemoryPool *,  // mp
							  CExpressionHandle &exprhdl, const CCostModelGPDB *pcmgpdb,
							  const SCostingInfo *pci)
{
	GPOS_ASSERT(NULL != pcmgpdb);
	GPOS_ASSERT(NULL != pci);
									...
									const CCostModelGPDB *pcmgpdb, const SCostingInfo *pci)
{
	...
	CDouble dInitRebind = pcmgpdb->GetCostModelParams()
		...
		1 < pcrsLocalUsed->Size())
	{
		...
		CDouble dInitScan = pcmgpdb->GetCostModelParams()
...
CCost
CCostModelGPDB::CostBitmapSmallNDV(const CCostModelGPDB *pcmgpdb, const SCostingInfo *pci,
								   CDouble dNDV)
{
	const DOUBLE rows = pci->Rows();
	const DOUBLE width = pci->Width();
...
CCost
CCostModelGPDB::CostBitmapLargeNDV(const CCostModelGPDB *pcmgpdb, const SCostingInfo *pci,
								   CDouble dNDV)
{
	const DOUBLE rows = pci->Rows();
	const DOUBLE width = pci->Width();
