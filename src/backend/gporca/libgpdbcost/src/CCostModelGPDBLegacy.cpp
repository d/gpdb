...
//---------------------------------------------------------------------------
CCost
CCostModelGPDBLegacy::CostSpooling(DOUBLE rows, DOUBLE width, DOUBLE num_rebinds,
								   DOUBLE *pdcostChildren, ICostModelParams *pcp)
{
	GPOS_ASSERT(NULL != pcp);
									  ...
									  const CCostModelGPDBLegacy *pcmgpdb, const SCostingInfo *pci)
{
	...
	CCost cost = CostUnary(pci->Rows(), pci->Width(), pci->NumRebinds(), pci->PdCost(),
	...
	CCost costSpooling = CostSpooling(pci->Rows(), pci->Width(), pci->NumRebinds(), pci->PdCost(),
									  pcmgpdb->GetCostModelParams());
	...
	const SCostingInfo *pci) const
{
	...
	if (FUnary(op_id))
	{
		return CostUnary(pci->Rows(), pci->Width(), pci->NumRebinds(), pci->PdCost(),
						 m_cost_model_params);
	}
