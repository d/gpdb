...
//
//---------------------------------------------------------------------------
CCostModelGPDBLegacy::CCostModelGPDBLegacy(CMemoryPool *mp, ULONG ulSegments, ICostModelParamsArray *pdrgpcp)
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDBLegacy::CostUnary(DOUBLE rows, DOUBLE width, DOUBLE num_rebinds, DOUBLE *pdcostChildren,
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDBLegacy::CostSpooling(DOUBLE rows, DOUBLE width, DOUBLE num_rebinds, DOUBLE *pdcostChildren,
								   ICostModelParams *pcp)
{
	GPOS_ASSERT(NULL != pcp);
...
CCostModelGPDBLegacy::FUnary(COperator::EOperatorId op_id)
{
	return COperator::EopPhysicalAssert == op_id || COperator::EopPhysicalComputeScalar == op_id ||
		   ...
		   COperator::EopPhysicalPartitionSelector == op_id || COperator::EopPhysicalPartitionSelectorDML == op_id ||
...
CCostModelGPDBLegacy::CostCTEProducer(CMemoryPool *,  // mp
									  CExpressionHandle &exprhdl, const CCostModelGPDBLegacy *pcmgpdb,
									  const SCostingInfo *pci)
{
	...
	CCost costSpooling =
		CostSpooling(pci->Rows(), pci->Width(), pci->NumRebinds(), pci->PdCost(), pcmgpdb->GetCostModelParams());
									  ...
									  const CCostModelGPDBLegacy *pcmgpdb, const SCostingInfo *pci)
{
	...
	CDouble dSeqIOBandwidth =
		pcmgpdb->GetCostModelParams()->PcpLookup(CCostModelParamsGPDBLegacy::EcpSeqIOBandwidth)->Get();
							  ...
							  const CCostModelGPDBLegacy *pcmgpdb, const SCostingInfo *pci)
{
	...
	CDouble dTupUpdateBandwidth =
		pcmgpdb->GetCostModelParams()->PcpLookup(CCostModelParamsGPDBLegacy::EcpTupUpdateBandwith)->Get();
									...
									const CCostModelGPDBLegacy *pcmgpdb, const SCostingInfo *pci)
{
	...
	CDouble dTupProcBandwidth =
		pcmgpdb->GetCostModelParams()->PcpLookup(CCostModelParamsGPDBLegacy::EcpTupProcBandwidth)->Get();
	GPOS_ASSERT(0 < dTupProcBandwidth);

	CCost costLocal =
		CCost(pci->NumRebinds() * (1.0 * pci->Width() + num_rows_outer * dWidthOuter) / dTupProcBandwidth);
	CCost costChild = CostSum(pci->PdCost(), pci->ChildCount());
									...
									const CCostModelGPDBLegacy *pcmgpdb, const SCostingInfo *pci)
{
	...
	CCost costLocal =
		CCost(pci->NumRebinds() * (pci->Rows() * pci->Width() + num_rows_outer * dWidthOuter) / dTupProcBandwidth);
	CCost costChild = CostSum(pci->PdCost(), pci->ChildCount());
								  ...
								  const CCostModelGPDBLegacy *pcmgpdb, const SCostingInfo *pci)
{
	...
	CCost costLocal =
		CCost(pci->NumRebinds() * (pci->Rows() * pci->Width() + dHashFactor * num_rows_outer * dWidthOuter) /
			  dTupProcBandwidth);
	CCost costChild = CostSum(pci->PdCost(), pci->ChildCount());
								 ...
								 const CCostModelGPDBLegacy *pcmgpdb, const SCostingInfo *pci)
{
	...
	CDouble dNLJOuterFactor =
		pcmgpdb->GetCostModelParams()->PcpLookup(CCostModelParamsGPDBLegacy::EcpNLJOuterFactor)->Get();
	CDouble dTupProcBandwidth =
		pcmgpdb->GetCostModelParams()->PcpLookup(CCostModelParamsGPDBLegacy::EcpTupProcBandwidth)->Get();
	GPOS_ASSERT(0 < dTupProcBandwidth);
	...
	CCost costLocal =
		CCost(pci->NumRebinds() * (num_rows_outer * dWidthOuter * dRowsInner * dWidthInner * dNLJOuterFactor) /
			  dTupProcBandwidth);
	CCost costChild = CostSum(pci->PdCost(), pci->ChildCount());
								 ...
								 const SCostingInfo *pci)
{
	...
	if (COperator::EopPhysicalMotionBroadcast == op_id)
	{
		...
		CDouble dNetBandwidth =
			pcmgpdb->GetCostModelParams()->PcpLookup(CCostModelParamsGPDBLegacy::EcpNetBandwidth)->Get();
		GPOS_ASSERT(0 < dNetBandwidth);
									...
									const SCostingInfo *pci)
{
	...
	CDouble dRandomIOBandwidth =
		pcmgpdb->GetCostModelParams()->PcpLookup(CCostModelParamsGPDBLegacy::EcpRandomIOBandwidth)->Get();
...
CCostModelGPDBLegacy::CostScan(CMemoryPool *,  // mp
							   CExpressionHandle &exprhdl, const CCostModelGPDBLegacy *pcmgpdb, const SCostingInfo *pci)
{
	GPOS_ASSERT(NULL != pcmgpdb);
	GPOS_ASSERT(NULL != pci);
...
//---------------------------------------------------------------------------
CCost
CCostModelGPDBLegacy::Cost(CExpressionHandle &exprhdl,	// handle gives access to expression properties
