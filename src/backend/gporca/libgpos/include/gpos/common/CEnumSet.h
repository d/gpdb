...
public:
	// ctor
	explicit CEnumSet<T, sentinel_index>(CMemoryPool *mp) : CBitSet(mp, sentinel_index)
	{
	}

	explicit CEnumSet<T, sentinel_index>(CMemoryPool *mp, const CEnumSet<T, sentinel_index> &enum_set)
		: CBitSet(mp, enum_set)
	{
	}
