...
private:
	...
	typedef CSyncHashtableAccessByKey<CCacheHashTableEntry, K> CCacheHashtableAccessor;
	typedef CSyncHashtableIter<CCacheHashTableEntry, K> CCacheHashtableIter;
	typedef CSyncHashtableAccessByIter<CCacheHashTableEntry, K> CCacheHashtableIterAccessor;
	...
	EvictEntries()
	{
		...
		{
			double to_free =
				static_cast<double>(static_cast<double>(m_cache_size) -
...
public:
	// ctor
	CCache(CMemoryPool *mp, BOOL unique, ULLONG cache_quota, ULONG g_clock_init_counter,
		   HashFuncPtr hash_func, EqualFuncPtr equal_func)
		: m_mp(mp),
		  m_unique(unique),
		  m_cache_size(0),
		  ...
		  m_equal_func(equal_func)
	{
		...
		m_hash_table.Init(m_mp, CACHE_HT_NUM_OF_BUCKETS,
						  ...
						  (&CCacheHashTableEntry::m_invalid_key), m_hash_func, m_equal_func);
