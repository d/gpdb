...
CFileTest::EresUnittest_Invalid()
{
	...
	GPOS_TRY
	{
		...
		CFileReader rd1;
		Unittest_CheckError(&rd1, &CFileReader::Open, (const CHAR *) szTmpFile, ulRdPerms,
		...
		// it will catch an exception
		CFileReader rd2;
		Unittest_CheckError(&rd2, &CFileReader::Open, (const CHAR *) szTmpFile, ulRdPerms,
							CException::ExmiIOError);
	}
	GPOS_CATCH_EX(ex)
	{
...
void
CFileTest::Unittest_CheckError(T *pt, R (T::*pfunc)(ARG1, ARG2), ARG1 argFirst, ARG2 argSec,
