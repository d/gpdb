...
CFileTest::EresUnittest_FileContent()
{
	...
	GPOS_TRY
	{
		...
		// test file size
		GPOS_ASSERT((wr.FileSize() == (ulLineLength * iLineNum)) && (wr.UllSizeInternal() == wr.FileSize()));
...
void
CFileTest::Unittest_CheckError(T *pt, R (T::*pfunc)(ARG1, ARG2), ARG1 argFirst, ARG2 argSec, CException::ExMinor exmi)
{
	GPOS_ASSERT(NULL != pt);
