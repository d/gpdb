...
CBitSet::Union(const CBitSet *pbsOther)
{
	...
	for (bsl_other = pbsOther->m_bsllist.First(); bsl_other != NULL; bsl_other = pbsOther->m_bsllist.Next(bsl_other))
	{
...
CBitSet::ContainsAll(const CBitSet *bs) const
{
	...
	for (bsl_other = bs->m_bsllist.First(); bsl_other != NULL; bsl_other = bs->m_bsllist.Next(bsl_other))
	{
		bsl = FindLinkByOffset(bsl_other->GetOffset(), bsl);
