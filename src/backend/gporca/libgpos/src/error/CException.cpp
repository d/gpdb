...
// ctor
CException::CException(ULONG major, ULONG minor, const CHAR *filename, ULONG line, ULONG severity_level)
	: m_major(major),
	  m_minor(minor),
...
//
//---------------------------------------------------------------------------
CException::CException(ULONG major, ULONG minor) : m_major(major), m_minor(minor), m_filename(NULL), m_line(0)
{
	m_severity_level = CException::ExsevSentinel;
	m_sql_state = GetSQLState(major, minor);
}
...
void
CException::Raise(const CHAR *filename, ULONG line, ULONG major, ULONG minor, ULONG severity_level...)
{
	// manufacture actual exception object
