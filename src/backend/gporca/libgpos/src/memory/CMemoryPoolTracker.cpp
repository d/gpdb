...
void *
CMemoryPoolTracker::NewImpl(const ULONG bytes, const CHAR *file, const ULONG line,
...
CMemoryPoolTracker::WalkLiveObjects(gpos::IMemoryVisitor *visitor)
{
	...
	while (NULL != header)
	{
		...
		visitor->Visit(user, header->m_user_size, header, header->m_alloc_size, header->m_filename,
					   header->m_line, header->m_serial,
#ifdef GPOS_DEBUG
					   &header->m_stack_desc
#else
					   NULL
