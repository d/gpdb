...
private:
	...
	void TransitionGroupExpression(CMemoryPool *pmpLocal, CGroupExpression *pgexpr,
	...
	void InsertExpressionChildren(CExpression *pexpr, CGroupArray *pdrgpgroupChildren,
...
public:
	...
	// return false if it's impossible for the operator to satisfy one or more
	BOOL FCheckEnfdProps(CMemoryPool *mp, CGroupExpression *pgexpr, COptimizationContext *poc,
	...
	BOOL FValidCTEAndPartitionProperties(CMemoryPool *mp, CExpressionHandle &exprhdl,
										 CReqdPropPlan *prpp);
	...
	BOOL FOptimizeChild(CGroupExpression *pgexprParent, CGroupExpression *pgexprChild,
						COptimizationContext *pocChild, EOptimizationLevel eol);

	// determine if a plan, rooted by given group expression, can be safely pruned based on cost bounds
	BOOL FSafeToPrune(CGroupExpression *pgexpr, CReqdPropPlan *prpp, CCostContext *pccChild,
	...
	// derive statistics
	static void DeriveStats(CMemoryPool *pmpLocal, CMemoryPool *pmpGlobal, CGroup *pgroup,
