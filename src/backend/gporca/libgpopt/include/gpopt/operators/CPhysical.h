...
protected:
	...
	CCTEReq *PcterNAry(CMemoryPool *mp, CExpressionHandle &exprhdl, CCTEReq *pcter,
	...
	CColRefSet *PcrsChildReqd(CMemoryPool *mp, CExpressionHandle &exprhdl, CColRefSet *pcrsInput,
	...
	static CDistributionSpec *PdsPassThru(CMemoryPool *mp, CExpressionHandle &exprhdl,
	...
	static CDistributionSpec *PdsRequireSingletonOrReplicated(CMemoryPool *mp,
	...
	static CDistributionSpec *PdsUnary(CMemoryPool *mp, CExpressionHandle &exprhdl,
									   CDistributionSpec *pdsInput, ULONG child_index,
									   ULONG ulOptReq);

	// helper for a simple case of computing child's required rewindability
	static CRewindabilitySpec *PrsPassThru(CMemoryPool *mp, CExpressionHandle &exprhdl,
										   CRewindabilitySpec *prsRequired, ULONG child_index);

	// pass partition propagation requirement to the child
	static CPartitionPropagationSpec *PppsRequiredPushThru(CMemoryPool *mp,
														   CExpressionHandle &exprhdl,
	...
	static CPartitionPropagationSpec *PppsRequiredPushThruNAry(
		CMemoryPool *mp, CExpressionHandle &exprhdl, CPartitionPropagationSpec *pppsRequired,
		ULONG child_index);
	...
	static CPartFilterMap *PpfmDeriveCombineRelational(CMemoryPool *mp, CExpressionHandle &exprhdl);

	// helper for common case of combining partition index maps of all relational children
	static CPartIndexMap *PpimDeriveCombineRelational(CMemoryPool *mp, CExpressionHandle &exprhdl);

	// Generate a singleton distribution spec request
	static CDistributionSpec *PdsRequireSingleton(CMemoryPool *mp, CExpressionHandle &exprhdl,
...
public:
	...
	virtual CColRefSet *PcrsRequired(CMemoryPool *mp, CExpressionHandle &exprhdl,
									 CColRefSet *pcrsRequired, ULONG child_index,
									 CDrvdPropArray *pdrgpdpCtxt, ULONG ulOptReq) = 0;

	// compute required ctes of the n-th child
	virtual CCTEReq *PcteRequired(CMemoryPool *mp, CExpressionHandle &exprhdl, CCTEReq *pcter,
								  ULONG child_index, CDrvdPropArray *pdrgpdpCtxt,
								  ULONG ulOptReq) const = 0;

	// compute distribution spec from the table descriptor
	static CDistributionSpec *PdsCompute(CMemoryPool *mp, const CTableDescriptor *ptabdesc,
	...
	virtual CDistributionSpec *PdsRequired(CMemoryPool *mp, CExpressionHandle &exprhdl,
										   CDistributionSpec *pdsRequired, ULONG child_index,
										   CDrvdPropArray *pdrgpdpCtxt, ULONG ulOptReq) const = 0;

	// compute required rewindability of the n-th child
	virtual CRewindabilitySpec *PrsRequired(CMemoryPool *mp, CExpressionHandle &exprhdl,
											CRewindabilitySpec *prsRequired, ULONG child_index,
											CDrvdPropArray *pdrgpdpCtxt, ULONG ulOptReq) const = 0;
	...
	virtual BOOL FProvidesReqdCols(CExpressionHandle &exprhdl, CColRefSet *pcrsRequired,
								   ULONG ulOptReq) const = 0;
	...
	virtual CPartFilterMap *PpfmDerive(CMemoryPool *mp, CExpressionHandle &exprhdl) const = 0;
	...
	// order matching type
	virtual CEnfdOrder::EOrderMatching Eom(CReqdPropPlan *prppInput, ULONG child_index,
	...
	virtual COperator *PopCopyWithRemappedColumns(CMemoryPool *mp, UlongToColRefMap *colref_mapping,
												  BOOL must_exist);
	...
	static CDistributionSpecSingleton *PdssMatching(CMemoryPool *mp,
