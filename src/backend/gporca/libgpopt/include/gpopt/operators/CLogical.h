...
protected:
	...
	CColRefArray *PdrgpcrCreateMapping(CMemoryPool *mp, const CColumnDescriptorArray *pdrgpcoldesc,
									   ULONG ulOpSourceId, IMDId *mdid_table = NULL) const;

	// initialize the array of partition columns
	CColRef2dArray *PdrgpdrgpcrCreatePartCols(CMemoryPool *mp, CColRefArray *colref_array,
	...
	static CColRefSet *PcrsReqdChildStats(CMemoryPool *mp, CExpressionHandle &exprhdl,
	...
	static CPropConstraint *PpcDeriveConstraintFromTable(CMemoryPool *mp,
	...
	static CPropConstraint *PpcDeriveConstraintFromTableWithPredicates(
		CMemoryPool *mp, CExpressionHandle &exprhdl, const CTableDescriptor *ptabdesc,
		const CColRefArray *pdrgpcrOutput);
	...
	static COrderSpec *PosFromIndex(CMemoryPool *mp, const IMDIndex *pmdindex,
									CColRefArray *colref_array, const CTableDescriptor *ptabdesc);
	...
	static CColRefSet *DeriveOuterReferences(CMemoryPool *mp, CExpressionHandle &exprhdl,
...
public:
	...
	virtual CColRefSet *DeriveCorrelatedApplyColumns(CMemoryPool *mp,
	...
	virtual CFunctionProp *DeriveFunctionProperties(CMemoryPool *mp,
													CExpressionHandle &exprhdl) const;

	virtual CTableDescriptor *DeriveTableDescriptor(CMemoryPool *mp,
													CExpressionHandle &exprhdl) const;
	...
	virtual CExpression *PexprPartPred(CMemoryPool *mp, CExpressionHandle &exprhdl,
									   CExpression *pexprInput, ULONG child_index) const;
	...
	static IStatistics *PstatsBaseTable(CMemoryPool *mp, CExpressionHandle &exprhdl,
	...
	static CColRefSet *PcrsDist(CMemoryPool *mp, const CTableDescriptor *ptabdesc,
								const CColRefArray *colref_array);

	// derive constraint property when expression has relational children and predicates
	static CPropConstraint *PpcDeriveConstraintFromPredicates(CMemoryPool *mp,
															  CExpressionHandle &exprhdl);
