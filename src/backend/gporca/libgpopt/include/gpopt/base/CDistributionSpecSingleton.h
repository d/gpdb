...
public:
	...
	HashValue() const
	{
		...
		return gpos::CombineHashes(gpos::HashValue<ULONG>(&ulEdt), gpos::HashValue<BOOL>(&fOnMaster));
	}
	...
	{
		return Edt() == pds->Edt() && FOnMaster() == dynamic_cast<const CDistributionSpecSingleton *>(pds)->FOnMaster();
	}
