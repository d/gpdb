...
private:
	...
	void SplitPartPredicates(CMemoryPool *mp, CExpression *pexprScalar,
							 CColRef2dArray *pdrgpdrgpcrKeys, UlongToExprMap *phmulexprEqFilter,
							 UlongToExprMap *phmulexprFilter, CExpression **ppexprResidual);

	// return a residual filter given an array of predicates and a bitset
	// indicating which predicates have already been used
	CExpression *PexprResidualFilter(CMemoryPool *mp, CExpressionArray *pdrgpexpr,
									 CBitSet *pbsUsed);

	// return an array of predicates on the given partitioning key given
	// an array of predicates on all keys
	CExpressionArray *PdrgpexprPredicatesOnKey(CMemoryPool *mp, CExpressionArray *pdrgpexpr,
