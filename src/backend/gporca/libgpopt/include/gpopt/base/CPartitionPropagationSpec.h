...
private:
	...
	BOOL FRequiresPartitionPropagation(CMemoryPool *mp, CExpression *pexpr, CExpressionHandle &exprhdl,
	...
	void SplitPartPredicates(CMemoryPool *mp, CExpression *pexprScalar, CColRef2dArray *pdrgpdrgpcrKeys,
							 UlongToExprMap *phmulexprEqFilter, UlongToExprMap *phmulexprFilter,
	...
	CExpression *PexprResidualFilter(CMemoryPool *mp, CExpressionArray *pdrgpexpr, CBitSet *pbsUsed);

	// return an array of predicates on the given partitioning key given
	// an array of predicates on all keys
	CExpressionArray *PdrgpexprPredicatesOnKey(CMemoryPool *mp, CExpressionArray *pdrgpexpr, CColRef *colref,
