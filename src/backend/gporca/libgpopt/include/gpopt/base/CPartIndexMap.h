...
private:
	...
	private:
		...
		static BOOL FDefinesPartialScans(UlongToPartConstraintMap *ppartcnstrmap, CPartConstraint *ppartcnstrRel);

	public:
		// ctor
		CPartTableInfo(ULONG scan_id, UlongToPartConstraintMap *ppartcnstrmap, EPartIndexManipulator epim, IMDId *mdid,
					   CPartKeysArray *pdrgppartkeys, CPartConstraint *ppartcnstrRel, ULONG ulPropagators);
	...
	typedef CHashMap<ULONG, CPartTableInfo, gpos::HashValue<ULONG>, gpos::Equals<ULONG>, CleanupDelete<ULONG>,
					 CleanupRelease<CPartTableInfo> >
		ScanIdToPartTableInfoMap;

	// map iterator
	typedef CHashMapIter<ULONG, CPartTableInfo, gpos::HashValue<ULONG>, gpos::Equals<ULONG>, CleanupDelete<ULONG>,
						 CleanupRelease<CPartTableInfo> >
	...
	static void ResolvePropagator(EPartIndexManipulator epimFst, ULONG ulExpectedPropagatorsFst,
								  EPartIndexManipulator epimSnd, ULONG ulExpectedPropagatorsSnd,
								  EPartIndexManipulator *pepimResult, ULONG *pulExpectedPropagatorsResult);

	// helper to add part-index id's found in first map and are unresolved based on second map
	static void AddUnresolved(CMemoryPool *mp, const CPartIndexMap &pimFst, const CPartIndexMap &pimSnd,
							  CPartIndexMap *ppimResult);

	// print part constraint map
	static IOstream &OsPrintPartCnstrMap(ULONG part_idx_id, UlongToPartConstraintMap *ppartcnstrmap, IOstream &os);
...
public:
	...
	void Insert(ULONG scan_id, UlongToPartConstraintMap *ppartcnstrmap, EPartIndexManipulator epim,
	...
	void AddRequiredPartPropagation(CPartIndexMap *ppimSource, ULONG scan_id, EPartPropagationRequestAction eppra,
	...
	static CPartIndexMap *PpimCombine(CMemoryPool *mp, const CPartIndexMap &pimFst, const CPartIndexMap &pimSnd);
