...
private:
	...
	CPartFilterMap *PpfmCombineDerived(CMemoryPool *mp, CExpressionHandle &exprhdl, CReqdPropPlan *prppInput,
									   ULONG child_index, CDrvdPropArray *pdrgpdpCtxt);

public:
	// default ctor
	CReqdPropPlan() : m_pcrs(NULL), m_peo(NULL), m_ped(NULL), m_per(NULL), m_pepp(NULL), m_pcter(NULL)
	{
	}
	...
	static CReqdPropPlan *PrppRemap(CMemoryPool *mp, CReqdPropPlan *prppInput, CDrvdPropPlan *pdpplanInput,
