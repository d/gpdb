...
private:
	...
	CColRef *PcrCreate(const CColumnDescriptor *pcoldesc, ULONG id, const CName &name,
					   ULONG ulOpSource, BOOL mark_as_used = true, IMDId *mdid_table = NULL);
...
public:
	...
	CColRef *PcrCreate(const IMDType *pmdtype, INT type_modifier, IMDId *mdid_table, INT attno,
					   BOOL is_nullable, ULONG id, const CName &name, ULONG ulOpSource,
					   BOOL isDistCol, ULONG ulWidth = gpos::ulong_max);
