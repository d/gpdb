...
public:
	...
	static CExpression *PexprScalarOp(CMemoryPool *mp, const CColRef *pcrLeft, CExpression *pexpr, CWStringConst strOp,
									  IMDId *mdid_op, IMDId *return_type_mdid = NULL);
	...
	static UlongToConstraintMap *PhmulcnstrBoolConstOnPartKeys(CMemoryPool *mp, CColRef2dArray *pdrgpdrgpcrPartKey,
															   BOOL value);
	...
	static CPartConstraint *PpartcnstrFromMDPartCnstr(CMemoryPool *mp, CMDAccessor *md_accessor,
													  ...
													  CColRefArray *pdrgpcrOutput, BOOL fDummyConstraint = false);
...
template <class T>
CExpression *
CUtils::PexprLogicalJoin(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight, CExpression *pexprPredicate)
{
...
template <class T>
CExpression *
CUtils::PexprLogicalApply(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight, CExpression *pexprPred)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pexprRight);
...
template <class T>
CExpression *
CUtils::PexprLogicalApply(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight, const CColRef *pcrInner,
...
template <class T>
CExpression *
CUtils::PexprLogicalApply(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight, CColRefArray *pdrgpcrInner,
						  COperator::EOperatorId eopidOriginSubq, CExpression *pexprPred)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pexprRight);
...
template <class T>
CExpression *
CUtils::PexprLogicalCorrelatedQuantifiedApply(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight,
											  CColRefArray *pdrgpcrInner, COperator::EOperatorId eopidOriginSubq,
...
template <class T, void (*CleanupFn)(T *)>
void
CUtils::AddRefAppend(CDynamicPtrArray<T, CleanupFn> *pdrgptOutput, CDynamicPtrArray<T, CleanupFn> *pdrgptInput,
					 ULONG ulStart)
{
...
CUtils::FMatchDynamicScan(T *pop1, COperator *pop2)
{
	...
	return pop1->ScanId() == popScan2->ScanId() && pop1->UlSecondaryScanId() == popScan2->UlSecondaryScanId() &&
		   pop1->Ptabdesc()->MDId()->Equals(popScan2->Ptabdesc()->MDId()) &&
		   pop1->PdrgpcrOutput()->Equals(popScan2->PdrgpcrOutput()) &&
		   ((!pop1->IsPartial() && !popScan2->IsPartial()) || (pop1->Ppartcnstr() == popScan2->Ppartcnstr()));
}
