...
public:
	...
	static void ValidateCTEProducerConsumerLocality(CMemoryPool *mp, CExpression *pexpr,
	...
	static CExpression *PexprCmpWithZero(CMemoryPool *mp, CExpression *pexprLeft,
										 IMDId *mdid_type_left, IMDType::ECmpType ecmptype);

	// generate an equality comparison expression for column references
	static CExpression *PexprScalarEqCmp(CMemoryPool *mp, const CColRef *pcrLeft,
										 const CColRef *pcrRight);

	// generate an equality comparison expression for two expressions
	static CExpression *PexprScalarEqCmp(CMemoryPool *mp, CExpression *pexprLeft,
	...
	static CExpression *PexprScalarArrayCmp(CMemoryPool *mp,
	...
	static CExpression *PexprScalarOp(CMemoryPool *mp, const CColRef *pcrLeft, CExpression *pexpr,
	...
	static CExpression *PexprScalarProjectElement(CMemoryPool *mp, CColRef *colref,
												  CExpression *pexpr);
	...
	static CExpression *PexprCountStarAndSum(CMemoryPool *mp, const CColRef *colref,
											 CExpression *pexprLogical);
	...
	static CExpression *PexprGbAggSum(CMemoryPool *mp, CExpression *pexprLogical,
	...
	static CExpression *PexprSafeSelect(CMemoryPool *mp, CExpression *pexprLogical,
										CExpression *pexprPredicate);
	...
	static CExpression *PexprLogicalSequenceProject(CMemoryPool *mp, CDistributionSpec *pds,
	...
	static CExpression *PexprLogicalProjectNulls(CMemoryPool *mp, CColRefArray *colref_array,
												 CExpression *pexpr,
	...
	static CExpression *PexprScalarProjListConst(CMemoryPool *mp, CColRefArray *colref_array,
	...
	// generate an aggregate expression
	static CExpression *PexprLogicalGbAggGlobal(CMemoryPool *mp, CColRefArray *colref_array,
												CExpression *pexpr, CExpression *pexprPrL);

	// generate an aggregate expression
	static CExpression *PexprLogicalGbAgg(CMemoryPool *mp, CColRefArray *colref_array,
										  CExpression *pexpr, CExpression *pexprPrL,
	...
	static CExpression *PexprScalarConstNull(CMemoryPool *mp, const IMDType *typ,
											 INT type_modifier);
	...
	template <class T>
	static CExpression *PexprLogicalJoin(CMemoryPool *mp, CExpression *pexprLeft,
										 CExpression *pexprRight, CExpression *pexprPredicate);

	// generate an apply expression
	template <class T>
	static CExpression *PexprLogicalApply(CMemoryPool *mp, CExpression *pexprLeft,
	...
	static IOstream &OsPrintDrgPcr(IOstream &os, const CColRefArray *colref_array,
	...
	static CColRefSetArray *PdrgpcrsIntersectEquivClasses(CMemoryPool *mp,
														  CColRefSetArray *pdrgpcrsFst,
														  CColRefSetArray *pdrgpcrsSnd);

	// return a copy of equivalence classes from all children
	static CColRefSetArray *PdrgpcrsCopyChildEquivClasses(CMemoryPool *mp,
														  CExpressionHandle &exprhdl);

	// return a copy of the given array of columns, excluding the columns
	// in the given colrefset
	static CColRefArray *PdrgpcrExcludeColumns(CMemoryPool *mp, CColRefArray *pdrgpcrOriginal,
	...
	static void AddRefAppend(CDynamicPtrArray<T, CleanupFn> *pdrgptOutput,
							 CDynamicPtrArray<T, CleanupFn> *pdrgptInput, ULONG ulStart = 0);
	...
	static void GenerateFileName(CHAR *buf, const CHAR *szPrefix, const CHAR *szExt, ULONG length,
								 ULONG ulSessionId, ULONG ulCmdId);

	// return the mapping of the given colref based on the given hashmap
	static CColRef *PcrRemap(const CColRef *colref, UlongToColRefMap *colref_mapping,
							 BOOL must_exist);
	...
	// and based on the given mapping
	static CColRefArray *PdrgpcrRemap(CMemoryPool *mp, CColRefArray *colref_array,
									  UlongToColRefMap *colref_mapping, BOOL must_exist);
	...
	static UlongToColRefMap *PhmulcrMapping(CMemoryPool *mp, CColRefArray *pdrgpcrFrom,
											CColRefArray *pdrgpcrTo);

	// add col ID->ColRef mappings to the given hashmap based on the
	// given ColRef arrays
	static void AddColumnMapping(CMemoryPool *mp, UlongToColRefMap *colref_mapping,
	...
	static CColRefArray *PdrgpcrCopy(CMemoryPool *mp, CColRefArray *colref_array,
	...
	static CExpression *PexprCast(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexpr,
								  IMDId *mdid_dest);

	// construct a logical join expression of the given type, with the given children
	static CExpression *PexprLogicalJoin(CMemoryPool *mp, EdxlJoinType edxljointype,
										 CExpressionArray *pdrgpexpr);
	...
	static UlongToConstraintMap *PhmulcnstrBoolConstOnPartKeys(CMemoryPool *mp,
	...
	static CPartConstraint *PpartcnstrFromMDPartCnstr(CMemoryPool *mp, CMDAccessor *md_accessor,
													  CColRef2dArray *pdrgpdrgpcrPartKey,
	...
	static BOOL FMotionOverUnresolvedPartConsumers(CMemoryPool *mp, CExpressionHandle &exprhdl,
	...
	static BOOL FEquivalanceClassesEqual(CMemoryPool *mp, CColRefSetArray *pdrgpcrsFst,
										 CColRefSetArray *pdrgpcrsSnd);
	...
	static CExpression *PexprLimit(CMemoryPool *mp, CExpression *pexpr, ULONG ulOffSet,
								   ULONG count);
	...
	static CExpressionArrays *GetCombinedExpressionArrays(CMemoryPool *mp,
...
template <class T>
CExpression *
CUtils::PexprLogicalJoin(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight,
						 CExpression *pexprPredicate)
{
	...
	return GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) T(mp), pexprLeft, pexprRight, pexprPredicate);
}
...
template <class T>
CExpression *
CUtils::PexprLogicalApply(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight,
						  ...
						  CExpression *pexprPred)
{
	...
	return GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) T(mp, colref_array, eopidOriginSubq),
						  ...
						  CExpression *pexprPred)
{
	...
	return GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) T(mp, pdrgpcrInner, eopidOriginSubq),
									pexprLeft, pexprRight, pexprScalar);
}
...
template <class T>
CExpression *
CUtils::PexprLogicalCorrelatedQuantifiedApply(CMemoryPool *mp, CExpression *pexprLeft,
											  CExpression *pexprRight, CColRefArray *pdrgpcrInner,
											  COperator::EOperatorId eopidOriginSubq,
											  CExpression *pexprPred)
{
