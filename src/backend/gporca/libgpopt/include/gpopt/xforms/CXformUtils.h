...
struct SPartDynamicIndexGetInfo
{
	...
	// ctor
	SPartDynamicIndexGetInfo(const IMDIndex *pmdindex, CPartConstraint *ppartcnstr,
...
private:
	...
	typedef CLogical *(*PDynamicIndexOpConstructor)(
		CMemoryPool *mp, const IMDIndex *pmdindex, CTableDescriptor *ptabdesc, ULONG ulOriginOpId,
		CName *pname, ULONG ulPartIndex, CColRefArray *pdrgpcrOutput,
		CColRef2dArray *pdrgpdrgpcrPart, ULONG ulSecondaryPartIndexId, CPartConstraint *ppartcnstr,
		CPartConstraint *ppartcnstrRel);

	typedef CLogical *(*PStaticIndexOpConstructor)(CMemoryPool *mp, const IMDIndex *pmdindex,
												   CTableDescriptor *ptabdesc, ULONG ulOriginOpId,
												   CName *pname, CColRefArray *pdrgpcrOutput);

	typedef CExpression *(PRewrittenIndexPath)(CMemoryPool *mp, CExpression *pexprIndexCond,
											   CExpression *pexprResidualCond,
											   const IMDIndex *pmdindex, CTableDescriptor *ptabdesc,
	...
	static CExpression *PexprAssertNotNull(CMemoryPool *mp, CExpression *pexprChild,
										   CTableDescriptor *ptabdesc, CColRefArray *colref_array);

	// create a logical assert for the check constraints on the given table
	static CExpression *PexprAssertCheckConstraints(CMemoryPool *mp, CExpression *pexprChild,
													CTableDescriptor *ptabdesc,
													CColRefArray *colref_array);

	// add a min(col) project element to the given expression list for
	// each column in the given column array
	static void AddMinAggs(CMemoryPool *mp, CMDAccessor *md_accessor, CColumnFactory *col_factory,
	...
	static CColRefSet *PcrsIndexColumns(CMemoryPool *mp, CColRefArray *colref_array,
	...
	// in the index included / key columns
	static CColRefArray *PdrgpcrIndexColumns(CMemoryPool *mp, CColRefArray *colref_array,
											 const IMDIndex *pmdindex, const IMDRelation *pmdrel,
											 EIndexCols eic);
	...
	static void CacheJoinKeys(CExpression *pexpr, CExpressionArray *pdrgpexprOuter,
	...
	template <class T>
	static void AddHashOrMergeJoinAlternative(CMemoryPool *mp, CExpression *pexprJoin,
											  CExpressionArray *pdrgpexprOuter,
											  CExpressionArray *pdrgpexprInner,
											  IMdIdArray *join_opfamilies, CXformResult *pxfres);
	...
	static CLogicalGbAgg *PopGbAggPushableBelowJoin(CMemoryPool *mp, CLogicalGbAgg *popGbAggOld,
									  ...
									  CPartConstraint *ppartcnstr, CPartConstraint *ppartcnstrRel)
	{
		return GPOS_NEW(mp) CLogicalDynamicIndexGet(
			mp, pmdindex, ptabdesc, ulOriginOpId, pname, ulPartIndex, pdrgpcrOutput,
			pdrgpdrgpcrPart, ulSecondaryPartIndexId, ppartcnstr, ppartcnstrRel);
	}
								 ...
								 COperator *popLogical)
	{
		...
		return CUtils::PexprSafeSelect(mp, GPOS_NEW(mp) CExpression(mp, popLogical, pexprIndexCond),
									   pexprResidualCond);
	}

	// create a candidate dynamic get scan to suplement the partial index scans
	static SPartDynamicIndexGetInfo *PpartdigDynamicGet(CMemoryPool *mp,
	...
	static CExpression *PexprEqualityOnBoolColumn(CMemoryPool *mp, CMDAccessor *md_accessor,
												  BOOL fNegated, CColRef *colref);
	...
	static CPartConstraint *PpartcnstrDisjunction(CMemoryPool *mp, CPartConstraint *ppartcnstrOld,
	...
	static CExpression *PexprBitmapSelectBestIndex(
		...
		CColRefSet *pcrsReqd, CColRefSet *pcrsOuterRefs, CExpression **ppexprRecheck,
	...
	static CExpressionArrays *PdrgpdrgpexprSortedPrjElemsArray(CMemoryPool *mp,
...
public:
	...
	template <class T>
	static void TransformImplementBinaryOp(CXformContext *pxfctxt, CXformResult *pxfres,
										   CExpression *pexpr);
	...
	static BOOL FProcessGPDBAntiSemiHashJoin(CMemoryPool *mp, CExpression *pexpr,
	...
	static CExpression *PexprSwapJoins(CMemoryPool *mp, CExpression *pexprTopJoin,
	...
	static BOOL FTriggersExist(CLogicalDML::EDMLOperator edmlop, CTableDescriptor *ptabdesc,
							   BOOL fBefore);
	...
	static CExpression *PexprRowTrigger(CMemoryPool *mp, CExpression *pexprChild,
	...
	static CExpression *PexprLogicalPartitionSelector(CMemoryPool *mp, CTableDescriptor *ptabdesc,
													  CColRefArray *colref_array,
	...
	static void QuantifiedToAgg(CMemoryPool *mp, CExpression *pexprSubquery,
								CExpression **ppexprNewSubquery, CExpression **ppexprNewScalar);
	...
	// create a logical assert for the check constraints on the given table
	static CExpression *PexprAssertConstraints(CMemoryPool *mp, CExpression *pexprChild,
											   CTableDescriptor *ptabdesc,
											   CColRefArray *colref_array);

	// create a logical assert for checking cardinality of update values
	static CExpression *PexprAssertUpdateCardinality(CMemoryPool *mp, CExpression *pexprDMLChild,
	...
	static CColRefArray *PdrgpcrIndexKeys(CMemoryPool *mp, CColRefArray *colref_array,
										  const IMDIndex *pmdindex, const IMDRelation *pmdrel);

	// return the set of key columns from the given array of columns which appear
	// in the index key columns
	static CColRefSet *PcrsIndexKeys(CMemoryPool *mp, CColRefArray *colref_array,
									 const IMDIndex *pmdindex, const IMDRelation *pmdrel);

	// return the set of key columns from the given array of columns which appear
	// in the index included columns
	static CColRefSet *PcrsIndexIncludedCols(CMemoryPool *mp, CColRefArray *colref_array,
											 const IMDIndex *pmdindex, const IMDRelation *pmdrel);

	// check if an index is applicable given the required, output and scalar
	// expression columns
	static BOOL FIndexApplicable(CMemoryPool *mp, const IMDIndex *pmdindex,
	...
	FGenerateApply(CXform::EXformId exfid)
	{
		return CXform::ExfSelect2Apply == exfid || CXform::ExfProject2Apply == exfid ||
			   ...
			   CXform::ExfSubqNAryJoin2Apply == exfid || CXform::ExfSequenceProject2Apply == exfid;
	}

	// helper for creating IndexGet/DynamicIndexGet expression
	static CExpression *
	PexprLogicalIndexGet(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprGet,
						 ULONG ulOriginOpId, CExpressionArray *pdrgpexprConds, CColRefSet *pcrsReqd,
						 CColRefSet *pcrsScalarExpr, CColRefSet *outer_refs,
						 const IMDIndex *pmdindex, const IMDRelation *pmdrel,
						 BOOL fAllowPartialIndex, CPartConstraint *ppcartcnstrIndex)
	{
		return PexprBuildIndexPlan(mp, md_accessor, pexprGet, ulOriginOpId, pdrgpexprConds,
								   pcrsReqd, pcrsScalarExpr, outer_refs, pmdindex, pmdrel,
								   fAllowPartialIndex, ppcartcnstrIndex, IMDIndex::EmdindBtree,
	...
	// path expressions
	static CExpression *PexprBitmapBoolOp(CMemoryPool *mp, IMDId *pmdidBitmapType,
	...
	static void CreateBitmapIndexProbeOps(
		...
		CColRefArray *pdrgpcrOutput, CColRefSet *outer_refs, CColRefSet *pcrsReqd,
		BOOL fConjunction, CExpressionArray *pdrgpexprBitmap, CExpressionArray *pdrgpexprRecheck,
	...
	static SPartDynamicIndexGetInfoArrays *PdrgpdrgppartdigCandidates(
		CMemoryPool *mp, CMDAccessor *md_accessor, CExpressionArray *pdrgpexprScalar,
		CColRef2dArray *pdrgpdrgpcrPartKey, const IMDRelation *pmdrel,
		CPartConstraint *ppartcnstrRel, CColRefArray *pdrgpcrOutput, CColRefSet *pcrsReqd,
	...
	static CPartConstraint *PpartcnstrUpdateCovered(
		...
		CExpressionArray *pdrgpexprResidual, const IMDRelation *pmdrel, const IMDIndex *pmdindex,
		CColRefSet *pcrsAcceptedOuterRefs);

	// remap the expression from the old columns to the new ones
	static CExpression *PexprRemapColumns(CMemoryPool *mp, CExpression *pexprScalar,
										  CColRefArray *pdrgpcrA, CColRefArray *pdrgpcrRemappedA,
										  CColRefArray *pdrgpcrB, CColRefArray *pdrgpcrRemappedB);
...
template <class T>
void
CXformUtils::TransformImplementBinaryOp(CXformContext *pxfctxt, CXformResult *pxfres,
										CExpression *pexpr)
{
...
template <class T>
void
CXformUtils::AddHashOrMergeJoinAlternative(CMemoryPool *mp, CExpression *pexprJoin,
										   CExpressionArray *pdrgpexprOuter,
										   ...
										   CXformResult *pxfres)
{
	...
	CExpression *pexprResult =
		GPOS_NEW(mp) CExpression(mp, op, (*pexprJoin)[0], (*pexprJoin)[1], (*pexprJoin)[2]);
...
template <class T>
void
CXformUtils::ImplementHashJoin(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != pxfctxt);
	...
	if (NULL != pdrgpexprOuter)
	{
		...
		else
		{
			...
			AddHashOrMergeJoinAlternative<T>(mp, pexpr, pdrgpexprOuter, pdrgpexprInner,
											 join_opfamilies, pxfres);
		}
	...
	CExpression *pexprResult = GPOS_NEW(mp) CExpression(
		mp, pexpr->Pop(), pexprOuter, pexprInner, CPredicateUtils::PexprConjunction(mp, pdrgpexpr));
	...
	if (0 != pdrgpexprOuter->Size())
	{
		AddHashOrMergeJoinAlternative<T>(mp, pexprResult, pdrgpexprOuter, pdrgpexprInner,
										 join_opfamilies, pxfres);
	}
	else
	{
...
template <class T>
void
CXformUtils::ImplementMergeJoin(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != pxfctxt);
