...
private:
	...
	static void
	CreateCorrelatedApply(CMemoryPool *mp, CExpression *pexprApply, CXformResult *pxfres)
	{
		...
		switch (op_id)
		{
			...
			case COperator::EopLogicalLeftSemiApplyIn:
				pexprResult = CUtils::PexprLogicalCorrelatedQuantifiedApply<
					CLogicalLeftSemiCorrelatedApplyIn>(mp, pexprOuter, pexprInner, colref_array,
...
protected:
	...
	static BOOL
	FDecorrelate(CMemoryPool *mp, CExpression *pexprApply, CExpression **ppexprInner,
				 CExpressionArray **ppdrgpexpr)
	{
		...
		*ppdrgpexpr = GPOS_NEW(mp) CExpressionArray(mp);
		...
		if (!CDecorrelator::FProcess(mp, (*pexprApply)[1], false /*fEqualityOnly*/, ppexprInner,
		...
		if ((COperator::EopLogicalLeftSemiJoin == pexprApply->Pop()->Eopid() ||
			...
			!CPredicateUtils::FValidSemiJoinCorrelations(mp, (*pexprApply)[0], (*ppexprInner),
														 (*ppdrgpexpr)))
		{
			(*ppdrgpexpr)->Release();
