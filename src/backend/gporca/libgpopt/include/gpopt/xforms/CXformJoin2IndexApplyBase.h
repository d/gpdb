...
private:
	...
	BOOL
	FCanLeftOuterIndexApply(CMemoryPool *mp, CExpression *pexprInner, CExpression *pexprScalar) const
	{
		...
		if (fCanOuterIndexApply)
		{
			...
			for (ULONG ul = 0; ul < pdrgpexpr->Size(); ul++)
			{
				...
				if (!pcrsPred->IsDisjoint(popGet->PcrsDist()) && CPredicateUtils::IsEqualityOp(pexprPred))
				{
...
public:
	// ctor
	explicit CXformJoin2IndexApplyBase<TJoin, TApply, TGet, fWithSelect, is_partial, eidxtype>(CMemoryPool *mp)
		:  // pattern
	...
	Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const
	{
		...
		if (is_partial)
		{
			CreatePartialIndexApplyAlternatives(mp, pexpr->Pop()->UlOpId(), pexprOuter, pexprInner, pexprAllPredicates,
												ptabdescInner, popDynamicGet, pxfres);
		}
		else
		{
			CreateHomogeneousIndexApplyAlternatives(mp, pexpr->Pop()->UlOpId(), pexprOuter, pexprGet,
													pexprAllPredicates, ptabdescInner, popDynamicGet, pxfres, eidxtype);
		}
		CRefCount::SafeRelease(pexprAllPredicates);
	}
