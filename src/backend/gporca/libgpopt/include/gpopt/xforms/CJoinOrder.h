...
public:
	...
	struct SComponent : public CRefCount
	{
		...
		SComponent(CExpression *expr, CBitSet *pbs, CBitSet *edge_set,
				   INT parent_loj_id = NON_LOJ_DEFAULT_ID, EPosition position = EpSentinel);
...
protected:
	...
	void AddComponent(CMemoryPool *mp, CExpression *expr, INT loj_id, EPosition position,
					  INT comp_num);
...
public:
	...
	CJoinOrder(CMemoryPool *mp, CExpressionArray *pdrgpexprComponents,
			   CExpressionArray *pdrgpexprConjuncts, BOOL include_outer_join_childs);
