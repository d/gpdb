...
//
//---------------------------------------------------------------------------
COptimizerConfig::COptimizerConfig(CEnumeratorConfig *pec, CStatisticsConfig *stats_config,
								   CCTEConfig *pcteconf, ICostModel *cost_model, CHint *phint,
...
COptimizerConfig::PoconfDefault(CMemoryPool *mp, ICostModel *pcm)
{
	...
	return GPOS_NEW(mp)
		COptimizerConfig(GPOS_NEW(mp) CEnumeratorConfig(mp, 0 /*plan_id*/, 0 /*ullSamples*/),
						 CStatisticsConfig::PstatsconfDefault(mp), CCTEConfig::PcteconfDefault(mp),
						 pcm, CHint::PhintDefault(mp), CWindowOids::GetWindowOids(mp));
}
							...
							CBitSet *pbsTrace) const
{
	...
	xml_serializer->AddAttribute(CDXLTokens::GetDXLTokenStr(EdxltokenMaxStatsBuckets),
