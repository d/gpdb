			...
			COperator::EopScalarProjectList == (*pexpr)[1]->Pop()->Eopid() && 1 == (*pexpr)[1]->Arity())
		{
			...
			if (COperator::EopScalarConst == pop->Eopid())
			{
				...
				if (NULL != datum && !datum->IsNull())
				{
					...
					if (0 ==
						clib::Strncmp((const char *) select_element_bytes, query_name_prefix, query_name_prefix_len))
					{
...
)
{
	...
	GPOS_TRY_HDL(&errhdl)
	{
...

		{
			...
			CQueryContext *pqc =
				CQueryContext::PqcGenerate(mp, pexprTranslated, pdrgpul, pdrgpmdname, true /*fDeriveStats*/);
			GPOS_CHECK_ABORT;
...
//---------------------------------------------------------------------------
CExpression *
COptimizer::PexprOptimize(CMemoryPool *mp, CQueryContext *pqc, CSearchStageArray *search_stage_array)
{
	CEngine eng(mp);
...
//---------------------------------------------------------------------------
CDXLNode *
COptimizer::CreateDXLNode(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexpr, CColRefArray *colref_array,
