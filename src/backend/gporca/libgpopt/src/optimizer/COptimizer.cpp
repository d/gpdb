...
//---------------------------------------------------------------------------
void
COptimizer::PrintQuery(CMemoryPool *mp, CExpression *pexprTranslated, CQueryContext *pqc)
{
	CAutoTrace at(mp);
...
//---------------------------------------------------------------------------
void
COptimizer::DumpSamples(CMemoryPool *mp, CEnumeratorConfig *pec, ULONG ulSessionId, ULONG ulCmdId)
{
...
//---------------------------------------------------------------------------
void
COptimizer::PrintQueryOrPlan(CMemoryPool *mp, CExpression *pexpr, CQueryContext *pqc)
{
			...
			1 == (*pexpr)[1]->Arity())
		{
			...
			if (COperator::EopScalarConst == pop->Eopid())
			{
				...
				CDatumGenericGPDB *datum =
					dynamic_cast<CDatumGenericGPDB *>(popScalarConst->GetDatum());
				...
				if (NULL != datum && !datum->IsNull())
				{
					...
					if (0 == clib::Strncmp((const char *) select_element_bytes, query_name_prefix,
...
//---------------------------------------------------------------------------
CDXLNode *
COptimizer::PdxlnOptimize(CMemoryPool *mp, CMDAccessor *md_accessor, const CDXLNode *query,
...
)
{
	...
	if (fMinidump)
	{
		...
		CMinidumperUtils::GenerateMinidumpFileName(file_name, GPOS_FILE_NAME_BUF_SIZE, ulSessionId,
												   ulCmdId, szMinidumpFileName);
	...
	GPOS_TRY_HDL(&errhdl)
	{
...

		{
			...
			CQueryContext *pqc = CQueryContext::PqcGenerate(mp, pexprTranslated, pdrgpul,
			...
			if (fMinidump)
			{
				CSerializablePlan serPlan(mp, pdxlnPlan,
...
//---------------------------------------------------------------------------
CDXLNode *
COptimizer::CreateDXLNode(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexpr,
