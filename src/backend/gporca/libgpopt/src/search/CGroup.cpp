...
void
CGroup::SetJoinKeys(CExpressionArray *pdrgpexprOuter, CExpressionArray *pdrgpexprInner,
...
CGroup::CreateScalarExpression()
{
	...
	if (CUtils::FSubquery(pop))
	{
		if (COperator::EopScalarSubquery == pop->Eopid())
		{
			...
			m_pexprScalarRep = CUtils::PexprScalarConstNull(m_mp, subquery_colref->RetrieveType(),
...
CGroup::CreateDummyCostContext()
{
	...
	COptimizationContext *poc = GPOS_NEW(m_mp)
		COptimizationContext(m_mp, this, CReqdPropPlan::PrppEmpty(m_mp),
...
//---------------------------------------------------------------------------
void
CGroup::RecursiveBuildTreeMap(CMemoryPool *mp, COptimizationContext *poc, CCostContext *pccParent,
...
CGroup::EspDerive(CMemoryPool *pmpLocal, CMemoryPool *pmpGlobal, CGroupExpression *pgexpr,
				  CReqdPropRelational *prprel, IStatisticsArray *stats_ctxt, BOOL fDeriveChildStats)
{
	...
	if (fDeriveChildStats && esp > CLogical::EspNone)
	{
		...
		IStatistics *stats = pgexpr->PstatsRecursiveDerive(pmpLocal, pmpGlobal, prprel, stats_ctxt,
						  ...
						  CReqdPropRelational *prprelInput, IStatisticsArray *stats_ctxt)
{
	...
	while (NULL != pgexprCurrent)
	{
		...
		{
			CLogical::EStatPromise espCurrent =
				EspDerive(pmpLocal, pmpGlobal, pgexprCurrent, prprelInput, stats_ctxt,
