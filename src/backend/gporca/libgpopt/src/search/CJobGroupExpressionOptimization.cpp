...
//---------------------------------------------------------------------------
void
CJobGroupExpressionOptimization::Init(CGroupExpression *pgexpr, COptimizationContext *poc, ULONG ulOptReq,
...
CJobGroupExpressionOptimization::DerivePrevChildProps(CSchedulerContext *psc)
{
	...
	COptimizationContext *pocChild = pgroupChild->PocLookupBest(
		psc->GetGlobalMemoryPool(), psc->Peng()->UlSearchStages(), m_pexprhdlPlan->Prpp(ulPrevChildIndex));
	...
	if (psc->Peng()->FSafeToPrune(m_pgexpr, m_poc->Prpp(), pccChildBest, ulPrevChildIndex, &costLowerBound))
	{
		...
		(void) m_pgexpr->PccComputeCost(psc->GetGlobalMemoryPool(), m_poc, m_ulOptReq, NULL /*pdrgpoc*/,
...
//---------------------------------------------------------------------------
BOOL
CJobGroupExpressionOptimization::FScheduleCTEOptimization(CSchedulerContext *psc, CGroupExpression *pgexpr,
														  COptimizationContext *poc, ULONG ulOptReq, CJob *pjParent)
{
