...
//
//---------------------------------------------------------------------------
CMemo::CMemo(CMemoryPool *mp) : m_mp(mp), m_aul(0), m_pgroupRoot(NULL), m_ulpGrps(0), m_pmemotmap(NULL)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CGroup *
CMemo::PgroupInsert(CGroup *pgroupTarget, CGroupExpression *pgexpr, CExpression *pexprOrigin, BOOL fNewGroup)
{
...
//---------------------------------------------------------------------------
CExpression *
CMemo::PexprExtractPlan(CMemoryPool *mp, CGroup *pgroupRoot, CReqdPropPlan *prppInput, ULONG ulSearchStages)
{
	// check stack size
	...
	CExpression *pexpr = GPOS_NEW(mp) CExpression(mp, pgexprBest->Pop(), pgexprBest, pdrgpexpr, stats, cost);
...
CMemo::BuildTreeMap(COptimizationContext *poc)
{
	...
	m_pgroupRoot->BuildTreeMap(m_mp, poc, NULL /*pccParent*/, gpos::ulong_max /*child_index*/, m_pmemotmap);
}
