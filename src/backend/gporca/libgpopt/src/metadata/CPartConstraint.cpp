...
//
//---------------------------------------------------------------------------
CPartConstraint::CPartConstraint(CMemoryPool *mp, UlongToConstraintMap *phmulcnstr, CBitSet *pbsDefaultParts,
								 BOOL is_unbounded, CColRef2dArray *pdrgpdrgpcr)
	...
	: m_phmulcnstr(NULL), m_pbsDefaultParts(NULL), m_is_unbounded(is_unbounded), m_fUninterpreted(false)
{
	...
	BOOL result GPOS_ASSERTS_ONLY = m_phmulcnstr->Insert(GPOS_NEW(mp) ULONG(0 /*ulLevel*/), pcnstr);
	GPOS_ASSERT(result);
...
//---------------------------------------------------------------------------
BOOL
CPartConstraint::FEqualConstrMaps(UlongToConstraintMap *phmulcnstrFst, UlongToConstraintMap *phmulcnstrSnd,
								  ULONG ulLevels)
{
...
//---------------------------------------------------------------------------
BOOL
CPartConstraint::FOverlapLevel(CMemoryPool *mp, const CPartConstraint *ppartcnstr, ULONG ulLevel) const
{
	GPOS_ASSERT(NULL != ppartcnstr);
...
//---------------------------------------------------------------------------
BOOL
CPartConstraint::FOverlap(CMemoryPool *mp, const CPartConstraint *ppartcnstr) const
{
	GPOS_ASSERT(NULL != ppartcnstr);
...
CPartConstraint::FSubsume(const CPartConstraint *ppartcnstr) const
{
	...
	for (ULONG ul = 0; ul < m_num_of_part_levels && fSubsumeLevel; ul++)
	{
		...
		fSubsumeLevel =
			pcnstrCurrent->Contains(pcnstrOther) && (IsDefaultPartition(ul) || !ppartcnstr->IsDefaultPartition(ul));
	}
...
CPartConstraint::PpartcnstrRemaining(CMemoryPool *mp, CPartConstraint *ppartcnstr)
{
	...
	BOOL result GPOS_ASSERTS_ONLY = phmulcnstr->Insert(GPOS_NEW(mp) ULONG(0), pcnstrRemaining);
	GPOS_ASSERT(result);
...
//---------------------------------------------------------------------------
CConstraint *
CPartConstraint::PcnstrRemaining(CMemoryPool *mp, CConstraint *pcnstrFst, CConstraint *pcnstrSnd)
{
...
CPartConstraint::PpartcnstrCopyWithRemappedColumns(CMemoryPool *mp, UlongToColRefMap *colref_mapping, BOOL must_exist)
{
	...
	for (ULONG ul = 0; ul < m_num_of_part_levels; ul++)
	{
		...
		if (NULL != pcnstr)
		{
			...
			BOOL result GPOS_ASSERTS_ONLY = phmulcnstr->Insert(GPOS_NEW(mp) ULONG(ul), pcnstrRemapped);
			GPOS_ASSERT(result);
		}
	}
...
CPartConstraint::OsPrint(IOstream &os) const
{
	...
	os << ", default partitions on levels: " << *m_pbsDefaultParts << ", unbounded: " << m_is_unbounded;
	os << ")";
	return os;
}
...
//---------------------------------------------------------------------------
BOOL
CPartConstraint::FDisjunctionPossible(CPartConstraint *ppartcnstrFst, CPartConstraint *ppartcnstrSnd)
{
	...
	fSuccess = (fSuccess && NULL != ppartcnstrFst->Pcnstr(ulLevels - 1) && NULL != ppartcnstrSnd->Pcnstr(ulLevels - 1));
...
CPartConstraint::PpartcnstrDisjunction(CMemoryPool *mp, CPartConstraint *ppartcnstrFst, CPartConstraint *ppartcnstrSnd)
{
	...
	for (ULONG ul = 0; ul < ulLevels - 1; ul++)
	{
		...
		pcnstrFst->AddRef();
		BOOL result GPOS_ASSERTS_ONLY = phmulcnstr->Insert(GPOS_NEW(mp) ULONG(ul), pcnstrFst);
		GPOS_ASSERT(result);
	...
	BOOL result GPOS_ASSERTS_ONLY = phmulcnstr->Insert(GPOS_NEW(mp) ULONG(ulLevels - 1), pcnstrDisj);
	GPOS_ASSERT(result);
...
//---------------------------------------------------------------------------
void
CPartConstraint::CopyPartConstraints(CMemoryPool *mp, UlongToPartConstraintMap *ppartcnstrmapDest,
									 UlongToPartConstraintMap *ppartcnstrmapSource)
{
	...
	while (pcmi.Advance())
	{
		...
		if (NULL == ppartcnstrDest)
		{
			...
			BOOL result GPOS_ASSERTS_ONLY = ppartcnstrmapDest->Insert(GPOS_NEW(mp) ULONG(ulKey), ppartcnstrSource);
...
CPartConstraint::PpartcnstrmapCombine(CMemoryPool *mp, UlongToPartConstraintMap *ppartcnstrmapFst,
