...
CXformUpdate2DML::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const
{
	...
	if (CXformUtils::FTriggersExist(CLogicalDML::EdmlUpdate, ptabdesc, true /*fBefore*/))
	{
		...
		pexprChild = CXformUtils::PexprRowTrigger(mp, pexprChild, CLogicalDML::EdmlUpdate, rel_mdid,
	...
	if (ptabdesc->IsPartitioned())
	{
		...
		pexprProject = CXformUtils::PexprLogicalPartitionSelector(mp, ptabdesc, pdrgpcrInsert,
																  pexprAssertConstraints);
