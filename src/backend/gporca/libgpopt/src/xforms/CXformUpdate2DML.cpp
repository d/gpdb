...
CXformUpdate2DML::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const
{
	...
	if (CXformUtils::FTriggersExist(CLogicalDML::EdmlUpdate, ptabdesc, true /*fBefore*/))
	{
		...
		pexprChild = CXformUtils::PexprRowTrigger(mp, pexprChild, CLogicalDML::EdmlUpdate, rel_mdid, true /*fBefore*/,
	...
	CExpression *pexprSplit = GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CLogicalSplit(mp, pdrgpcrDelete, pdrgpcrInsert, pcrCtid, pcrSegmentId, pcrAction, pcrTupleOid),
	...
	{
		pexprAssertConstraints = CXformUtils::PexprAssertConstraints(mp, pexprSplit, ptabdesc, pdrgpcrInsert);
	}
	else
	{
	...
	CExpression *pexprDML = GPOS_NEW(mp)
		CExpression(mp,
					GPOS_NEW(mp) CLogicalDML(mp, CLogicalDML::EdmlUpdate, ptabdesc, pdrgpcrDelete, pbsModified,
