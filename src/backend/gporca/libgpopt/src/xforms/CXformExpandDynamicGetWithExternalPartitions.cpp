	...
	const
{
	...
	if (ppartcnstrRest == NULL)
	{
		...
		GPOS_RAISE(gpopt::ExmaGPOPT, gpopt::ExmiUnsupportedOp,
				   GPOS_WSZ_LIT("Query over external partitions in multi-level partitioned table"));
	}
	...
	CLogicalDynamicGet *popPartialDynamicGet =
		GPOS_NEW(mp) CLogicalDynamicGet(mp, pnameDG,
										ptabdesc,
										popGet->ScanId(),
										popGet->PdrgpcrOutput(),
										popGet->PdrgpdrgpcrPart(),
	...
	CLogicalMultiExternalGet *popMultiExternalGet =
		GPOS_NEW(mp) CLogicalMultiExternalGet(mp, external_part_mdids,
											  pnameMEG,
											  ptabdesc,
											  popGet->ScanId(),
											  pdrgpcrNew);
