...
{
	if (0 == exprhdl.DeriveUsedColumns(2)->Size() || exprhdl.DeriveHasSubquery(2) ||
...
//---------------------------------------------------------------------------
void
CXformJoin2IndexApply::ComputeColumnSets(CMemoryPool *mp, CExpression *pexprInner,
										 CExpression *pexprScalar, CColRefSet **ppcrsScalarExpr,
										 CColRefSet **ppcrsOuterRefs, CColRefSet **ppcrsReqd) const
{
...
CXformJoin2IndexApply::CreateHomogeneousIndexApplyAlternatives(
	CMemoryPool *mp, ULONG ulOriginOpId, CExpression *pexprOuter, CExpression *pexprInner,
	CExpression *pexprScalar, CTableDescriptor *ptabdescInner, CLogicalDynamicGet *popDynamicGet,
	CXformResult *pxfres, IMDIndex::EmdindexType emdtype) const
{
	...
	if (IMDIndex::EmdindBtree == emdtype)
	{
		CreateHomogeneousBtreeIndexApplyAlternatives(
			mp, ulOriginOpId, pexprOuter, pexprInner, pexprScalar, ptabdescInner, popDynamicGet,
			pcrsScalarExpr, outer_refs, pcrsReqd, ulIndices, pxfres);
	}
	else
	{
		CreateHomogeneousBitmapIndexApplyAlternatives(mp, ulOriginOpId, pexprOuter, pexprInner,
													  pexprScalar, ptabdescInner, outer_refs,
													  pcrsReqd, pxfres);
	}
...
CXformJoin2IndexApply::CreateHomogeneousBtreeIndexApplyAlternatives(
	CMemoryPool *mp, ULONG ulOriginOpId, CExpression *pexprOuter, CExpression *pexprInner,
	CExpression *pexprScalar, CTableDescriptor *ptabdescInner, CLogicalDynamicGet *popDynamicGet,
	CColRefSet *pcrsScalarExpr, CColRefSet *outer_refs, CColRefSet *pcrsReqd, ULONG ulIndices,
	...
	for (ULONG ul = 0; ul < ulIndices; ul++)
	{
		...
		if (NULL != popDynamicGet)
		{
			ppartcnstrIndex = CUtils::PpartcnstrFromMDPartCnstr(
				mp, COptCtxt::PoctxtFromTLS()->Pmda(), popDynamicGet->PdrgpdrgpcrPart(),
				pmdindex->MDPartConstraint(), popDynamicGet->PdrgpcrOutput());
		}
		CreateAlternativesForBtreeIndex(mp, ulOriginOpId, pexprOuter, pexprInner, md_accessor,
										pdrgpexpr, pcrsScalarExpr, outer_refs, pcrsReqd, pmdrel,
										pmdindex, ppartcnstrIndex, pxfres);
	}
...
CXformJoin2IndexApply::CreateAlternativesForBtreeIndex(
	CMemoryPool *mp, ULONG ulOriginOpId, CExpression *pexprOuter, CExpression *pexprInner,
	CMDAccessor *md_accessor, CExpressionArray *pdrgpexprConjuncts, CColRefSet *pcrsScalarExpr,
	CColRefSet *outer_refs, CColRefSet *pcrsReqd, const IMDRelation *pmdrel,
	const IMDIndex *pmdindex, CPartConstraint *ppartcnstrIndex, CXformResult *pxfres) const
{
	CExpression *pexprLogicalIndexGet = CXformUtils::PexprLogicalIndexGet(
		mp, md_accessor, pexprInner, ulOriginOpId, pdrgpexprConjuncts, pcrsReqd, pcrsScalarExpr,
		outer_refs, pmdindex, pmdrel, false /*fAllowPartialIndex*/, ppartcnstrIndex);
	if (NULL != pexprLogicalIndexGet)
	{
		...
		CExpression *pexprIndexApply = GPOS_NEW(mp)
			CExpression(mp, PopLogicalApply(mp, colref_array), pexprOuter, pexprLogicalIndexGet,
...
CXformJoin2IndexApply::CreateHomogeneousBitmapIndexApplyAlternatives(
	CMemoryPool *mp, ULONG ulOriginOpId, CExpression *pexprOuter, CExpression *pexprInner,
	CExpression *pexprScalar, CTableDescriptor *ptabdescInner, CColRefSet *outer_refs,
	...
	CXformResult *pxfres) const
{
	...
	SPartDynamicIndexGetInfoArrays *pdrgpdrgppartdig = CXformUtils::PdrgpdrgppartdigCandidates(
		mp, COptCtxt::PoctxtFromTLS()->Pmda(), pdrgpexpr, popDynamicGet->PdrgpdrgpcrPart(), pmdrel,
		ppartcnstr, popDynamicGet->PdrgpcrOutput(), pcrsReqd, pcrsScalarExpr, outer_refs);
	...
	for (ULONG ul = 0; ul < ulCandidates; ul++)
	{
		...
		{
			CreatePartialIndexApplyPlan(mp, ulOriginOpId, pexprOuter, pexprScalar, outer_refs,
...
//---------------------------------------------------------------------------
void
CXformJoin2IndexApply::CreatePartialIndexApplyPlan(CMemoryPool *mp, ULONG ulOriginOpId,
	...
	for (ULONG ul = 0; ul < ulPartialIndexes; ul++)
	{
		...
		if (NULL != pmdindex)
		{
			pexprUnionAllChild = PexprIndexApplyOverCTEConsumer(
				mp, ulOriginOpId, popDynamicGet, pdrgpexprIndex, pdrgpexprResidual, pdrgpcrIndexGet,
				pmdindex, pmdrel, fFirst, ulCTEId, ppartcnstr, outer_refs, pdrgpcrOuter,
				pdrgpcrOuterNew, pdrgpcrOuterRefsInScan, pdrgpulIndexesOfRefsInScan);
		}
		else
		{
			pexprUnionAllChild = PexprJoinOverCTEConsumer(mp, ulOriginOpId, popDynamicGet, ulCTEId,
														  pexprScalar, pdrgpcrIndexGet, ppartcnstr,
												...
												CColRefArray *pdrgpcrOuterNew) const
{
	...
	CExpression *pexprJoin = GPOS_NEW(mp) CExpression(
		mp, PopLogicalJoin(mp), CXformUtils::PexprCTEConsumer(mp, ulCTEId, pdrgpcrOuterNew),
...
CXformJoin2IndexApply::PexprIndexApplyOverCTEConsumer(
	...
	CColRefArray *pdrgpcrIndexGet, const IMDIndex *pmdindex, const IMDRelation *pmdrel, BOOL fFirst,
	ULONG ulCTEId, CPartConstraint *ppartcnstr, CColRefSet *outer_refs, CColRefArray *pdrgpcrOuter,
	CColRefArray *pdrgpcrOuterNew, CColRefArray *pdrgpcrOuterRefsInScan,
	ULongPtrArray *pdrgpulIndexesOfRefsInScan) const
{
	CExpression *pexprDynamicScan = CXformUtils::PexprPartialDynamicIndexGet(
		mp, popDynamicGet, ulOriginOpId, pdrgpexprIndex, pdrgpexprResidual, pdrgpcrIndexGet,
		pmdindex, pmdrel, ppartcnstr, outer_refs, pdrgpcrOuter, pdrgpcrOuterNew);
											  ...
											  CExpression *pexprRightChild, ULONG scan_id) const
{
	...
	return GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CLogicalUnionAll(mp, pdrgpcrLeftSchema, pdrgpdrgpcrInput, scan_id),
...
//---------------------------------------------------------------------------
void
CXformJoin2IndexApply::AddUnionPlanForPartialIndexes(CMemoryPool *mp,
													 CLogicalDynamicGet *popDynamicGet,
