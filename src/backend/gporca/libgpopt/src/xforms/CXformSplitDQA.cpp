...
CXformSplitDQA::Exfp(CExpressionHandle &exprhdl) const
{
	...
	if (!CLogicalGbAgg::PopConvert(exprhdl.Pop())->FGlobal() ||
		0 == exprhdl.DeriveTotalDistinctAggs(1) || exprhdl.DeriveHasMultipleDistinctAggs(1) ||
		0 < exprhdl.DeriveOuterReferences()->Size() || NULL == exprhdl.PexprScalarExactChild(1) ||
...
CXformSplitDQA::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const
{
	...
	ExtractDistinctCols(mp, col_factory, md_accessor, pexprProjectList, pdrgpexprChildPrEl,
	...
	if (fScalarDQA)
	{
		...
		CExpression *pexprTwoStageScalarDQA =
			PexprSplitHelper(mp, col_factory, md_accessor, pexpr, pexprRelational, phmexprcr,
...
//---------------------------------------------------------------------------
CExpression *
CXformSplitDQA::PexprSplitIntoLocalDQAGlobalAgg(CMemoryPool *mp, CColumnFactory *col_factory,
												...
												CLogicalGbAgg::EAggStage aggStage)
{
	...
	CExpression *pexprGlobal = PexprMultiLevelAggregation(
		mp, pexprRelational, pdrgpexprPrElFirstStage, NULL, /* pdrgpexprPrElSecondStage */
								 ...
								 CColRefArray *pdrgpcrArgDQA, CLogicalGbAgg::EAggStage aggStage)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		else
		{
			...
			PopulatePrLMultiPhaseAgg(mp, col_factory, md_accessor, pexprPrEl,
									 pdrgpexprPrElFirstStage, pdrgpexprPrElSecondStage,
...
//---------------------------------------------------------------------------
void
CXformSplitDQA::PopulatePrLMultiPhaseAgg(CMemoryPool *mp, CColumnFactory *col_factory,
										 CMDAccessor *md_accessor, CExpression *pexprPrEl,
...
CXformSplitDQA::PcrAggFuncArgument(CMemoryPool *mp, CMDAccessor *md_accessor,
								   CColumnFactory *col_factory, CExpression *pexprArg,
...
CExpression *
CXformSplitDQA::PexprMultiLevelAggregation(
	CMemoryPool *mp, CExpression *pexprRelational, CExpressionArray *pdrgpexprPrElFirstStage,
	CExpressionArray *pdrgpexprPrElSecondStage, CExpressionArray *pdrgpexprPrElThirdStage,
	CColRefArray *pdrgpcrArgDQA, CColRefArray *pdrgpcrLastStage, BOOL fSplit2LevelsOnly,
	BOOL fAddDistinctColToLocalGb, CLogicalGbAgg::EAggStage aggStage)
{
	...
	if (fSplit2LevelsOnly)
	{
		...
		popFirstStage =
			GPOS_NEW(mp) CLogicalGbAgg(mp, pdrgpcrLocal, COperator::EgbaggtypeLocal,
	...
	else
	{
		...
		popSecondStage = GPOS_NEW(mp) CLogicalGbAgg(
			mp, pdrgpcrLocal, COperator::EgbaggtypeIntermediate, false, /* fGeneratesDuplicates */
	...
	pdrgpcrLastStage->AddRef();
	CLogicalGbAgg *popGlobalThreeStage = GPOS_NEW(mp)
		CLogicalGbAgg(mp, pdrgpcrLastStage, COperator::EgbaggtypeGlobal, /* egbaggtype */
					  aggStage);
...
void
CXformSplitDQA::ExtractDistinctCols(
	CMemoryPool *mp, CColumnFactory *col_factory, CMDAccessor *md_accessor, CExpression *pexpr,
	CExpressionArray *pdrgpexprChildPrEl, ExprToColRefMap *phmexprcr,
