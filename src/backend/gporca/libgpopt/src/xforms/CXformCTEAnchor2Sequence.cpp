...
{
	ULONG id = gpos::dyn_cast<CLogicalCTEAnchor>(exprhdl.Pop())->Id();
...
//---------------------------------------------------------------------------
void
CXformCTEAnchor2Sequence::Transform(gpos::pointer<CXformContext *> pxfctxt,
									...
									gpos::pointer<CExpression *> pexpr) const
{
	...
	gpos::pointer<CGroupExpression *> pgexprOrigin = pexpr->Pgexpr();
	...
	gpos::pointer<CLogicalCTEAnchor *> popCTEAnchor =
		gpos::dyn_cast<CLogicalCTEAnchor>(pexpr->Pop());
	...
	gpos::owner<CExpression *> pexprSequence =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalSequence(mp),
								 pexprProducer, std::move(pexprChild));

	pxfres->Add(std::move(pexprSequence));
}
