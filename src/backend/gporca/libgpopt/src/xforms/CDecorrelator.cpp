						...
						CColRefSet *outerRefsToRemove)
{
	...
	BOOL fSuccess = FProcessOperator(mp, pexpr, fEqualityOnly, ppexprDecorrelated,
								...
								CColRefSet *outerRefsToRemove)
{
	...
	switch (pexpr->Pop()->Eopid())
	{
		case COperator::EopLogicalSelect:
			result = FProcessSelect(mp, pexpr, fEqualityOnly, ppexprDecorrelated,
									pdrgpexprCorrelations, outerRefsToRemove);
			break;
		...
		case COperator::EopLogicalSequenceProject:
			result = FProcessProject(mp, pexpr, fEqualityOnly, ppexprDecorrelated,
									 pdrgpexprCorrelations, outerRefsToRemove);
			break;

		case COperator::EopLogicalAssert:
			result = FProcessAssert(mp, pexpr, fEqualityOnly, ppexprDecorrelated,
									pdrgpexprCorrelations, outerRefsToRemove);
			break;

		case COperator::EopLogicalMaxOneRow:
			result = FProcessMaxOneRow(mp, pexpr, fEqualityOnly, ppexprDecorrelated,
									   pdrgpexprCorrelations, outerRefsToRemove);
			break;
...
//---------------------------------------------------------------------------
BOOL
CDecorrelator::FProcessSelect(CMemoryPool *mp, CExpression *pexpr, BOOL fEqualityOnly,
							  CExpression **ppexprDecorrelated,
							  CExpressionArray *pdrgpexprCorrelations,
							  CColRefSet *outerRefsToRemove)
{
	...
	BOOL fSuccess = FProcessPredicate(mp, pexpr, (*pexpr)[1], fEqualityOnly, &pexprPredicate,
									  pdrgpexprCorrelations, outerRefsToRemove);
...
//---------------------------------------------------------------------------
BOOL
CDecorrelator::FProcessJoin(CMemoryPool *mp, CExpression *pexpr, BOOL fEqualityOnly,
							CExpression **ppexprDecorrelated,
							CExpressionArray *pdrgpexprCorrelations, CColRefSet *outerRefsToRemove)
{
	...
	BOOL fSuccess = FProcessPredicate(mp, pexpr, pexprOriginalInnerJoinPreds, fEqualityOnly,
									  &pexprPredicate, pdrgpexprCorrelations, outerRefsToRemove);
	...
	if (fSuccess)
	{
		...
		if (isNaryLOJ)
		{
			...
			pexprPredicate =
				naryJoin->ReplaceInnerJoinPredicates(mp, (*pexpr)[arity - 1], pexprPredicate);
		}
...
//---------------------------------------------------------------------------
BOOL
CDecorrelator::FProcessAssert(CMemoryPool *mp, CExpression *pexpr, BOOL fEqualityOnly,
							  CExpression **ppexprDecorrelated,
							  CExpressionArray *pdrgpexprCorrelations,
							  CColRefSet *outerRefsToRemove)
{
...
//---------------------------------------------------------------------------
BOOL
CDecorrelator::FProcessLimit(CMemoryPool *mp, CExpression *pexpr, BOOL fEqualityOnly,
							 CExpression **ppexprDecorrelated,
							 CExpressionArray *pdrgpexprCorrelations, CColRefSet *outerRefsToRemove)
{
