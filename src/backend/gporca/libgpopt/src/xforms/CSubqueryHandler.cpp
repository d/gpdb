...
//---------------------------------------------------------------------------
CExpression *
CSubqueryHandler::PexprReplace(CMemoryPool *mp, CExpression *pexprInput, CColRef *colref,
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryHandler::PexprSubqueryPred(CExpression *pexprOuter, CExpression *pexprSubquery,
...
CSubqueryHandler::Psd(CMemoryPool *mp, CExpression *pexprSubquery, CExpression *pexprOuter,
										...
										CExpression **ppexprResidualScalar)
{
	...
	if (psd->m_fProjectCount && !psd->m_fCorrelatedExecution)
	{
		...
		CScalarSubquery *popInnerSubq =
			GPOS_NEW(m_mp) CScalarSubquery(m_mp, psd->m_pcrCountAgg, false /*fGeneratedByExist*/,
		...
		fSuccess = FRemoveScalarSubqueryInternal(m_mp, pexprOuter, pexprNewSubq, EsqctxtValue, psd,
												 m_fEnforceCorrelatedApply, &pexprNewOuter,
												 &pexprResidualScalar);
	...
	{
		fSuccess = FRemoveScalarSubqueryInternal(m_mp, pexprOuter, pexprSubquery, esqctxt, psd,
												 m_fEnforceCorrelatedApply, ppexprNewOuter,
...
//---------------------------------------------------------------------------
BOOL
CSubqueryHandler::FRemoveScalarSubqueryInternal(CMemoryPool *mp, CExpression *pexprOuter,
												...
												CExpression **ppexprResidualScalar)
{
	...
	{
		return FGenerateCorrelatedApplyForScalarSubquery(mp, pexprOuter, pexprSubquery, esqctxt,
														 psd, fEnforceCorrelatedApply,
	...
	{
		fSuccess =
			FCreateOuterApply(mp, pexprOuter, pexprInner, pexprSubquery, NULL /* pexprPredicate */,
							  psd->m_fHasOuterRefs, ppexprNewOuter, ppexprResidualScalar,
							  false /* not null opt for quant*/);
		if (!fSuccess)
		{
			pexprInner->Release();
		}
	...
	*useNotNullableInnerOpt = false;
	...
	if (innerIsNullable || !CPredicateUtils::FBuiltInComparisonIsVeryStrict(pscalarCmp->MdIdOp()))
	{
		predToUse = GPOS_NEW(mp)
			CExpression(mp, GPOS_NEW(mp) CScalarBooleanTest(mp, CScalarBooleanTest::EbtIsNotFalse),
						pexprPredicate);
	}
	else
	{
...
//---------------------------------------------------------------------------
BOOL
CSubqueryHandler::FCreateOuterApplyForScalarSubquery(CMemoryPool *mp, CExpression *pexprOuter,
	...
	*ppexprNewOuter = pexprPrj;
	...
	if (fGeneratedByQuantified || (fHasCountAggMatchingColumn && 0 == pgbAgg->Pdrgpcr()->Size()))
	{
		...
		CExpression *pexprCoalesce = GPOS_NEW(mp) CExpression(
			mp, GPOS_NEW(mp) CScalarCoalesce(mp, pmdidInt8),
			CUtils::PexprScalarIdent(mp, pcrComputed), CUtils::PexprScalarConstInt8(mp, 0 /*val*/));
		...
		if (fGeneratedByQuantified)
		{
			...
			*ppexprResidualScalar = GPOS_NEW(mp) CExpression(
				...
				CUtils::PexprScalarEqCmp(mp, pcrComputed,
...
//---------------------------------------------------------------------------
BOOL
CSubqueryHandler::FCreateGrpCols(CMemoryPool *mp, CExpression *pexprOuter, CExpression *pexprInner,
...
CSubqueryHandler::CreateGroupByNode(CMemoryPool *mp, CExpression *pexprChild,
									CColRefArray *colref_array, BOOL fExistential, CColRef *colref,
									CExpression *pexprPredicate, CColRef **pcrCount,
									CColRef **pcrSum)
{
	GPOS_ASSERT(NULL == *pcrCount);
	...
	else
	{
		...
		*pcrCount = col_factory->PcrCreate(pmdtypeCount, popCount->TypeModifier());
		...
		CColRef *colrefForSum =
			CScalarProjectElement::PopConvert((*pexprPrjListForSum)[0]->Pop())->Pcr();
...
BOOL
CSubqueryHandler::FCreateOuterApplyForExistOrQuant(
	CMemoryPool *mp, CExpression *pexprOuter, CExpression *pexprInner, CExpression *pexprSubquery,
	CExpression *pexprPredicate, BOOL fOuterRefsUnderInner, CExpression **ppexprNewOuter,
	CExpression **ppexprResidualScalar, BOOL useNotNullableInnerOpt)
{
	...
	CColRefArray *colref_array = NULL;
	BOOL fGbOnInner = false;
	if (!FCreateGrpCols(mp, pexprOuter, pexprInner, fExistential, fOuterRefsUnderInner,
						&colref_array, &fGbOnInner))
	{
	...
	else
	{
		...
		CExpression *result = CUtils::PexprLogicalApply<CLogicalLeftOuterApply>(
			mp, pexprOuter, pexprPrjInner, pcrSubquery, COperator::EopScalarSubquery);

		*ppexprNewOuter = CreateGroupByNode(mp, result, colref_array, fExistential, colref,
											pexprPredicate, &pcrCount, &pcrSum);
	}

	// residual scalar examines introduced columns
	*ppexprResidualScalar =
		PexprScalarIf(mp, colref, pcrSum, pcrCount, pexprSubquery, useNotNullableInnerOpt);
...
//---------------------------------------------------------------------------
BOOL
CSubqueryHandler::FCreateOuterApply(CMemoryPool *mp, CExpression *pexprOuter,
									...
									CExpression **ppexprResidualScalar, BOOL useNotNullableInnerOpt)
{
	...
	if (fExistential || fQuantified)
	{
		return FCreateOuterApplyForExistOrQuant(
			mp, pexprOuter, pexprInner, pexprSubquery, pexprPredicate, fOuterRefsUnderInner,
			ppexprNewOuter, ppexprResidualScalar, useNotNullableInnerOpt);
	}

	return FCreateOuterApplyForScalarSubquery(mp, pexprOuter, pexprInner, pexprSubquery,
...
BOOL
CSubqueryHandler::FCreateCorrelatedApplyForQuantifiedSubquery(
	CMemoryPool *mp, CExpression *pexprOuter, CExpression *pexprSubquery, ESubqueryCtxt esqctxt,
	CExpression **ppexprNewOuter, CExpression **ppexprResidualScalar)
{
...
//---------------------------------------------------------------------------
BOOL
CSubqueryHandler::FCreateCorrelatedApplyForExistOrQuant(CMemoryPool *mp, CExpression *pexprOuter,
														CExpression *pexprSubquery,
														...
														CExpression **ppexprResidualScalar)
{
	...
	if (fExistential)
	{
		return FCreateCorrelatedApplyForExistentialSubquery(mp, pexprOuter, pexprSubquery, esqctxt,
															ppexprNewOuter, ppexprResidualScalar);
	}

	return FCreateCorrelatedApplyForQuantifiedSubquery(mp, pexprOuter, pexprSubquery, esqctxt,
													   ppexprNewOuter, ppexprResidualScalar);
}
...
//---------------------------------------------------------------------------
BOOL
CSubqueryHandler::FRemoveAnySubquery(CExpression *pexprOuter, CExpression *pexprSubquery,
									 ESubqueryCtxt esqctxt, CExpression **ppexprNewOuter,
									 CExpression **ppexprResidualScalar)
{
	CMemoryPool *mp = m_mp;
	...
	{
		return FCreateCorrelatedApplyForExistOrQuant(mp, pexprOuter, pexprSubquery, esqctxt,
													 ppexprNewOuter, ppexprResidualScalar);
	}
	...
	if (EsqctxtValue == esqctxt)
	{
		CExpression *pexprNewSelect =
			PexprInnerSelect(mp, colref, pexprResult, pexprPredicate, &fUseNotNullableInnerOpt);
		CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
		...
		if (!fUseCorrelated)
		{
			fSuccess = FCreateOuterApply(mp, pexprOuter, pexprSelect, pexprSubquery, pexprPredicate,
		...
		{
			pexprSelect->Release();
			fSuccess = FCreateCorrelatedApplyForExistOrQuant(mp, pexprOuter, pexprSubquery, esqctxt,
															 ppexprNewOuter, ppexprResidualScalar);
		}
	}
	else
...
//---------------------------------------------------------------------------
BOOL
CSubqueryHandler::FRemoveAllSubquery(CExpression *pexprOuter, CExpression *pexprSubquery,
									 ESubqueryCtxt esqctxt, CExpression **ppexprNewOuter,
									 CExpression **ppexprResidualScalar)
{
	CMemoryPool *mp = m_mp;
	...
	if (EsqctxtValue == esqctxt)
	{
		...
		CExpression *pexprNewInnerSelect =
			PexprInnerSelect(mp, colref, pexprInner, pexprPredicate, &fUseNotNullOptimization);
		...
		if (!fUseCorrelated)
		{
			fSuccess = FCreateOuterApply(mp, pexprOuter, pexprInnerSelect, pexprSubquery,
										 pexprPredicate, fOuterRefsUnderInner, ppexprNewOuter,
										 ppexprResidualScalar, fUseNotNullOptimization);
		}
		if (!fSuccess || fUseCorrelated)
		{
			pexprInnerSelect->Release();
...
//---------------------------------------------------------------------------
void
CSubqueryHandler::AddProjectNode(CMemoryPool *mp, CExpression *pexpr, CExpression **ppexprResult)
{
...
//---------------------------------------------------------------------------
CExpression *
CSubqueryHandler::PexprScalarIf(CMemoryPool *mp, CColRef *pcrBool, CColRef *pcrSum,
								...
								BOOL useNotNullableInnerOpt)
{
	...
	CExpression *pexprScalarIf = GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CScalarIf(mp, mdid), pexprEquality,
	...
	mdid->AddRef();
	CExpression *result =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarIf(mp, mdid), pexprRowCountGreaterZero,
	...
	if (useNotNullableInnerOpt)
	{
		...
		result = GPOS_NEW(mp) CExpression(
			mp, GPOS_NEW(mp) CScalarIf(mp, mdid), CUtils::PexprIsNull(mp, pexprScalar),
...
//---------------------------------------------------------------------------
BOOL
CSubqueryHandler::FRemoveExistentialSubquery(CMemoryPool *mp, COperator::EOperatorId op_id,
...
//---------------------------------------------------------------------------
BOOL
CSubqueryHandler::FRemoveExistsSubquery(CExpression *pexprOuter, CExpression *pexprSubquery,
										ESubqueryCtxt esqctxt, CExpression **ppexprNewOuter,
										CExpression **ppexprResidualScalar)
{
	if (m_fEnforceCorrelatedApply)
	{
		return FCreateCorrelatedApplyForExistOrQuant(m_mp, pexprOuter, pexprSubquery, esqctxt,
													 ppexprNewOuter, ppexprResidualScalar);
	}

	return FRemoveExistentialSubquery(m_mp, COperator::EopScalarSubqueryExists, pexprOuter,
...
//---------------------------------------------------------------------------
BOOL
CSubqueryHandler::FRemoveNotExistsSubquery(CExpression *pexprOuter, CExpression *pexprSubquery,
										   ESubqueryCtxt esqctxt, CExpression **ppexprNewOuter,
										   CExpression **ppexprResidualScalar)
{
	if (m_fEnforceCorrelatedApply)
	...
	return FRemoveExistentialSubquery(m_mp, COperator::EopScalarSubqueryNotExists, pexprOuter,
									  pexprSubquery, esqctxt, ppexprNewOuter, ppexprResidualScalar);
}
...
//---------------------------------------------------------------------------
BOOL
CSubqueryHandler::FRecursiveHandler(CExpression *pexprOuter, CExpression *pexprScalar,
									ESubqueryCtxt esqctxt, CExpression **ppexprNewOuter,
									CExpression **ppexprResidualScalar)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		if (!FProcess(pexprCurrentOuter, pexprScalarChild, esqctxt, &pexprNewLogical,
					  &pexprNewScalar))
		{
...
//---------------------------------------------------------------------------
BOOL
CSubqueryHandler::FProcessScalarOperator(CExpression *pexprOuter, CExpression *pexprScalar,
										 ESubqueryCtxt esqctxt, CExpression **ppexprNewOuter,
										 CExpression **ppexprResidualScalar)
{
	CMemoryPool *mp = m_mp;
	...
	switch (op_id)
	{
		case COperator::EopScalarSubquery:
			fSuccess = FRemoveScalarSubquery(pexprOuter, pexprScalar, esqctxt, ppexprNewOuter,
											 ppexprResidualScalar);
			break;
		case COperator::EopScalarSubqueryAny:
			fSuccess = FRemoveAnySubquery(pexprOuter, pexprScalar, esqctxt, ppexprNewOuter,
										  ppexprResidualScalar);
			break;
		case COperator::EopScalarSubqueryAll:
			fSuccess = FRemoveAllSubquery(pexprOuter, pexprScalar, esqctxt, ppexprNewOuter,
										  ppexprResidualScalar);
			break;
		case COperator::EopScalarSubqueryExists:
			fSuccess = FRemoveExistsSubquery(pexprOuter, pexprScalar, esqctxt, ppexprNewOuter,
											 ppexprResidualScalar);
			break;
		case COperator::EopScalarSubqueryNotExists:
			fSuccess = FRemoveNotExistsSubquery(pexprOuter, pexprScalar, esqctxt, ppexprNewOuter,
												ppexprResidualScalar);
			break;
		...
		case COperator::EopScalarSwitchCase:
			fSuccess = FRecursiveHandler(pexprOuter, pexprScalar, esqctxt, ppexprNewOuter,
										 ppexprResidualScalar);
			break;
		default:
...
)
{
	...
	return FProcessScalarOperator(pexprOuter, pexprScalar, esqctxt, ppexprNewOuter,
								  ppexprResidualScalar);
}
