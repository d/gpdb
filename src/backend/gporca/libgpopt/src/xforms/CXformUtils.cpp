...
//---------------------------------------------------------------------------
CXform::EXformPromise
CXformUtils::ExfpExpandJoinOrder(CExpressionHandle &exprhdl, const CXform *xform)
{
...
#ifdef GPOS_DEBUG
	CAutoMemoryPool amp;
	GPOS_ASSERT(!FJoinPredOnSingleChild(amp.Pmp(), exprhdl) && "join predicates are not pushed down");
#endif	// GPOS_DEBUG
...
//---------------------------------------------------------------------------
CColRefSet *
CXformUtils::PcrsFKey(CMemoryPool *mp,
...
//---------------------------------------------------------------------------
CColRefSet *
CXformUtils::PcrsFKey(CMemoryPool *mp, CExpression *pexprOuter, CExpression *pexprInner, CExpression *pexprScalar)
{
...
)
{
	...
	GPOS_ASSERT(COperator::EopLogicalDynamicIndexGet == op_id || COperator::EopLogicalDynamicBitmapTableGet == op_id ||
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprSwapJoins(CMemoryPool *mp, CExpression *pexprTopJoin, CExpression *pexprBottomJoin)
{
#ifdef GPOS_DEBUG
...
//---------------------------------------------------------------------------
CLogicalGbAgg *
CXformUtils::PopGbAggPushableBelowJoin(CMemoryPool *mp, CLogicalGbAgg *popGbAggOld, CColRefSet *pcrsOutputOuter,
...
//---------------------------------------------------------------------------
BOOL
CXformUtils::FCanPushGbAggBelowJoin(CColRefSet *pcrsGrpCols, CColRefSet *pcrsJoinOuterChildOutput,
...
//---------------------------------------------------------------------------
void
CXformUtils::ExistentialToAgg(CMemoryPool *mp, CExpression *pexprSubquery,
...
//---------------------------------------------------------------------------
void
CXformUtils::QuantifiedToAgg(CMemoryPool *mp, CExpression *pexprSubquery,
							 CExpression **ppexprNewSubquery,  // output argument for new scalar subquery
							 CExpression **ppexprNewScalar	   // output argument for new scalar expression
)
{
...
//---------------------------------------------------------------------------
void
CXformUtils::SubqueryAnyToAgg(CMemoryPool *mp, CExpression *pexprSubquery,
							  CExpression **ppexprNewSubquery,	// output argument for new scalar subquery
							  CExpression **ppexprNewScalar		// output argument for new scalar expression
)
{
	...
	if (fCanEvaluateToNull)
	{
		...
		CExpression *pexprCountEqZero = CUtils::PexprCmpWithZero(
			mp, pexprScalarIdentCount, CScalarIdent::PopConvert(pexprScalarIdentCount->Pop())->MdidType(),
			IMDType::EcmptEq);
		...
		CExpression *pexprProjected = GPOS_NEW(mp) CExpression(
			...
			GPOS_NEW(mp)
				CExpression(mp, GPOS_NEW(mp) CScalarIf(mp, pmdidInt8), pexprCountEqSum,
		...
		*ppexprNewSubquery = GPOS_NEW(mp) CExpression(
			mp,
			GPOS_NEW(mp) CScalarSubquery(mp, pcrSubquery, false /*fGeneratedByExist*/, true /*fGeneratedByQuantified*/),
			pexprPrj);
	...
	else
	{
		...
		*ppexprNewSubquery = GPOS_NEW(mp) CExpression(
			mp,
			GPOS_NEW(mp) CScalarSubquery(mp, pcrCount, false /*fGeneratedByExist*/, true /*fGeneratedByQuantified*/),
			pexprInnerNew);
		*ppexprNewScalar = CUtils::PexprCmpWithZero(mp, CUtils::PexprScalarIdent(mp, pcrCount),
...
//---------------------------------------------------------------------------
void
CXformUtils::SubqueryAllToAgg(CMemoryPool *mp, CExpression *pexprSubquery,
							  CExpression **ppexprNewSubquery,	// output argument for new scalar subquery
							  CExpression **ppexprNewScalar		// output argument for new scalar expression
)
{
	...
	CExpression *pexprSubqTest = GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CScalarIf(mp, pmdidInt4), pexprSubqPred,
					CUtils::PexprScalarConstInt4(mp, 1 /*val*/), CUtils::PexprScalarConstInt4(mp, 0 /*val*/));
	...
	CColRef *pcrInnerNullTest =
		const_cast<CColRef *>(CScalarProjectElement::PopConvert((*(*pexprPrj)[1])[1]->Pop())->Pcr());
	...
	CExpression *pexprInnerHasNulls = CUtils::PexprCmpWithZero(
		mp, pexprScalarIdentSumNulls, CScalarIdent::PopConvert(pexprScalarIdentSumNulls->Pop())->MdidType(),
		IMDType::EcmptG);
...
CXformUtils::PexprSeparateSubqueryPreds(CMemoryPool *mp, CExpression *pexpr)
{
	...
	else
	{
		...
		pdrgpexpr->Append(naryLOJOp->ReplaceInnerJoinPredicates(mp, pexprScalar,
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprLogicalPartitionSelector(CMemoryPool *mp, CTableDescriptor *ptabdesc, CColRefArray *colref_array,
										...
										CColRef *pcrSegmentId)
{
	...
	CExpression *pexprDML = GPOS_NEW(mp) CExpression(
		...
		GPOS_NEW(mp) CLogicalDML(mp, edmlop, ptabdesc, colref_array, GPOS_NEW(mp) CBitSet(mp) /*pbsModified*/,
								 pcrAction, pcrOid, pcrCtid, pcrSegmentId, NULL /*pcrTupleOid*/),
		pexprProject);
...
//---------------------------------------------------------------------------
BOOL
CXformUtils::FTriggersExist(CLogicalDML::EDMLOperator edmlop, CTableDescriptor *ptabdesc, BOOL fBefore)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
	...
	for (ULONG ul = 0; ul < ulTriggers; ul++)
	{
		...
		if (!pmdtrigger->IsEnabled() || !pmdtrigger->ExecutesOnRowLevel() || !FTriggerApplies(edmlop, pmdtrigger))
		{
			continue;
		}
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprRowTrigger(CMemoryPool *mp, CExpression *pexprChild, CLogicalDML::EDMLOperator edmlop,
							 IMDId *rel_mdid, BOOL fBefore, CColRefArray *colref_array)
{
	GPOS_ASSERT(CLogicalDML::EdmlInsert == edmlop || CLogicalDML::EdmlDelete == edmlop);
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprAssertNotNull(CMemoryPool *mp, CExpression *pexprChild, CTableDescriptor *ptabdesc,
								CColRefArray *colref_array)
{
	...
	CLogicalAssert *popAssert = GPOS_NEW(mp) CLogicalAssert(
		mp, GPOS_NEW(mp) CException(gpos::CException::ExmaSQL, gpos::CException::ExmiSQLNotNullViolation));
...
CExpression *
CXformUtils::PexprRowTrigger(CMemoryPool *mp, CExpression *pexprChild, CLogicalDML::EDMLOperator edmlop,
							 IMDId *rel_mdid, BOOL fBefore, CColRefArray *pdrgpcrOld, CColRefArray *pdrgpcrNew)
{
	...
	return GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CLogicalRowTrigger(mp, rel_mdid, type, pdrgpcrOld, pdrgpcrNew), pexprChild);
}
...
//---------------------------------------------------------------------------
CExpressionArray *
CXformUtils::PdrgpexprPartEqFilters(CMemoryPool *mp, CTableDescriptor *ptabdesc, CColRefArray *pdrgpcrSource)
{
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprAssertConstraints(CMemoryPool *mp, CExpression *pexprChild, CTableDescriptor *ptabdesc,
									CColRefArray *colref_array)
{
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprAssertCheckConstraints(CMemoryPool *mp, CExpression *pexprChild, CTableDescriptor *ptabdesc,
										 CColRefArray *colref_array)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
	const IMDRelation *pmdrel = md_accessor->RetrieveRel(ptabdesc->MDId());
	...
	if (0 < ulCheckConstraint)
	{
		...
		for (ULONG ul = 0; ul < ulCheckConstraint; ul++)
		{
			...
			CWStringConst *pstrErrMsg = PstrErrorMessage(
				mp, gpos::CException::ExmaSQL, gpos::CException::ExmiSQLCheckConstraintViolation,
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprAssertUpdateCardinality(CMemoryPool *mp, CExpression *pexprDMLChild, CExpression *pexprDML,
										  CColRef *pcrCtid, CColRef *pcrSegmentId)
{
	...
	CExpression *pexprProjElem =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectElement(mp, pcrProjElem), pexprCountStar);
	...
	CExpression *pexprProjList =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pdrgpexprProjElemsCountDistinct);

	CColRefArray *pdrgpcrGbCols = GPOS_NEW(mp) CColRefArray(mp);
	pdrgpcrGbCols->Append(pcrCtid);
	pdrgpcrGbCols->Append(pcrSegmentId);

	CExpression *pexprGbAgg = GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CLogicalGbAgg(mp, pdrgpcrGbCols, COperator::EgbaggtypeGlobal /*egbaggtype*/),
	...
	CExpression *pexprAssertConstraint =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarAssertConstraint(mp, pstrErrorMsg),
	...
	return GPOS_NEW(mp) CExpression(
		mp,
		GPOS_NEW(mp)
			CLogicalAssert(mp, GPOS_NEW(mp) CException(gpos::CException::ExmaSQL, gpos::CException::ExmiSQLDefault)),
...
//---------------------------------------------------------------------------
void
CXformUtils::AddMinAggs(CMemoryPool *mp, CMDAccessor *md_accessor, CColumnFactory *col_factory,
						CColRefArray *colref_array, ColRefToColRefMap *phmcrcr, CExpressionArray *pdrgpexpr,
						CColRefArray **ppdrgpcrNew)
{
	...
	for (ULONG ul = 0; ul < num_cols; ul++)
	{
		...
		if (NULL == new_colref)
		{
			...
			CExpression *pexprProjElemMin =
				GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectElement(mp, new_colref), pexprMinAgg);
...
//---------------------------------------------------------------------------
CColRefArray *
CXformUtils::PdrgpcrIndexKeys(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
...
//---------------------------------------------------------------------------
CColRefSet *
CXformUtils::PcrsIndexKeys(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
						   const IMDRelation *pmdrel)
{
...
//---------------------------------------------------------------------------
CColRefSet *
CXformUtils::PcrsIndexIncludedCols(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
								   const IMDRelation *pmdrel)
{
...
//---------------------------------------------------------------------------
CColRefSet *
CXformUtils::PcrsIndexColumns(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
...
//---------------------------------------------------------------------------
CColRefArray *
CXformUtils::PdrgpcrIndexColumns(CMemoryPool *mp, CColRefArray *colref_array, const IMDIndex *pmdindex,
								 const IMDRelation *pmdrel, EIndexCols eic)
{
	GPOS_ASSERT(EicKey == eic || EicIncluded == eic);
...
//---------------------------------------------------------------------------
BOOL
CXformUtils::FIndexApplicable(CMemoryPool *mp, const IMDIndex *pmdindex, const IMDRelation *pmdrel,
							  CColRefArray *pdrgpcrOutput, CColRefSet *pcrsReqd, CColRefSet *pcrsScalar,
...
CXformUtils::PexprRowNumber(CMemoryPool *mp)
{
	...
	CScalarWindowFunc *popRowNumber = GPOS_NEW(mp)
		CScalarWindowFunc(mp, GPOS_NEW(mp) CMDIdGPDB(row_number_oid), GPOS_NEW(mp) CMDIdGPDB(GPDB_INT8_OID),
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprWindowWithRowNumber(CMemoryPool *mp, CExpression *pexprWindowChild, CColRefArray *pdrgpcrInput)
{
...
void
CXformUtils::LookupJoinKeys(CMemoryPool *mp, CExpression *pexpr, CExpressionArray **ppdrgpexprOuter,
...
void
CXformUtils::CacheJoinKeys(CExpression *pexpr, CExpressionArray *pdrgpexprOuter, CExpressionArray *pdrgpexprInner,
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprAddCTEProducer(CMemoryPool *mp, ULONG ulCTEId, CColRefArray *colref_array, CExpression *pexpr)
{
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprBuildIndexPlan(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprGet, ULONG ulOriginOpId,
								 CExpressionArray *pdrgpexprConds, CColRefSet *pcrsReqd, CColRefSet *pcrsScalarExpr,
								 CColRefSet *outer_refs, const IMDIndex *pmdindex, const IMDRelation *pmdrel,
								 ...
								 PStaticIndexOpConstructor psiopc, PRewrittenIndexPath prip)
{
	...
	CPredicateUtils::ExtractIndexPredicates(mp, md_accessor, pdrgpexprConds, pmdindex, pdrgppcrIndexCols,
	...
	if (fDynamicGet)
	{
		...
		popLogicalGet =
			(*pdiopc)(mp, pmdindex, ptabdesc, ulOriginOpId, GPOS_NEW(mp) CName(mp, CName(alias)), ulPartIndex,
					  pdrgpcrOutput, pdrgpdrgpcrPart, ulSecondaryPartIndex, ppartcnstrIndex, ppartcnstrRel);
	}
	else
	{
		popLogicalGet =
			(*psiopc)(mp, pmdindex, ptabdesc, ulOriginOpId, GPOS_NEW(mp) CName(mp, CName(alias)), pdrgpcrOutput);
	}
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprScalarBitmapBoolOp(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprOriginalPred,
									 CExpressionArray *pdrgpexpr, CTableDescriptor *ptabdesc, const IMDRelation *pmdrel,
									 ...
									 BOOL isAPartialPredicate)
{
	...
	CreateBitmapIndexProbeOps(mp, md_accessor, pexprOriginalPred, pdrgpexpr, ptabdesc, pmdrel, pdrgpcrOutput,
							  outer_refs, pcrsReqd, fConjunction, pdrgpexprBitmap, pdrgpexprRecheckNew,
							  pdrgpexprResidualNew, isAPartialPredicate);
	...
	{
		ComputeBitmapTableScanResidualPredicate(mp, fConjunction, pexprOriginalPred, ppexprResidual,
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprBitmapBoolOp(CMemoryPool *mp, IMDId *pmdidBitmapType, CExpression *pexprLeft, CExpression *pexprRight,
							   BOOL fConjunction)
{
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprEqualityOnBoolColumn(CMemoryPool *mp, CMDAccessor *md_accessor, BOOL value, CColRef *colref)
{
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprBitmapLookupWithPredicateBreakDown(CMemoryPool *mp, CMDAccessor *md_accessor,
													 ...
													 CExpression **ppexprResidual)
{
	...
	CExpression *pexprResult = PexprScalarBitmapBoolOp(
		...
		fConjunction, ppexprRecheck, ppexprResidual, !fConjunction /* we are now breaking up something other than an AND
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprBitmapSelectBestIndex(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprPred,
										CTableDescriptor *ptabdesc, const IMDRelation *pmdrel,
										CColRefArray *pdrgpcrOutput, CColRefSet *pcrsReqd, CColRefSet *pcrsOuterRefs,
										...
										BOOL alsoConsiderBTreeIndexes)
{
	...
	for (ULONG ul = 0; ul < ulIndexes; ul++)
	{
		...
		if (!pmdrel->IsPartialIndex(pmdindex->MDId()) &&
			CXformUtils::FIndexApplicable(mp, pmdindex, pmdrel, pdrgpcrOutput, pcrsReqd, pcrsScalar,
										  IMDIndex::EmdindBitmap, altIndexType))
		{
			...
			CDouble selectivity =
				CFilterStatsProcessor::SelectivityOfPredicate(mp, pexprIndex, ptabdesc, pcrsOuterRefs);
	...
	if (NULL != pexprIndexFinal)
	{
		...
		return GPOS_NEW(mp)
			CExpression(mp, GPOS_NEW(mp) CScalarBitmapIndexProbe(mp, pindexdesc, pmdindex->GetIndexRetItemTypeMdid()),
						pexprIndexFinal);
	}
...
//---------------------------------------------------------------------------
void
CXformUtils::CreateBitmapIndexProbeOps(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprOriginalPred,
									   ...
									   CExpressionArray *pdrgpexprResidual, BOOL isAPartialPredicate)
{
	...
	for (ULONG ul = 0; ul < ulPredicates; ul++)
	{
		...
		CreateBitmapIndexProbesWithOrWithoutPredBreakdown(mp, md_accessor, pexprOriginalPred, pexprPred, ptabdesc,
														  pmdrel, pdrgpcrOutput, outer_refs, pcrsReqd, &pexprBitmap,
														  &pexprRecheck, pdrgpexprResidual, isAPartialPredicate);
	...
	CExpressionArray *pdrgpexprResidualResult, BOOL isAPartialPredicate)
{
	...
	while (NULL != pexprPred)
	{
		...
		if (CPredicateUtils::FBitmapLookupSupportedPredicateOrConjunct(pexprPred, pcrsOuterRefs))
		{
			...
			if (!isAPartialPredicateOrArrayCmp)
			{
				...
				for (ULONG i = 0; i < size && !isAPartialPredicateOrArrayCmp; i++)
				{
					isAPartialPredicateOrArrayCmp =
						CPredicateUtils::FArrayCompareIdentToConstIgnoreCast((*conjuncts)[i]);
				}
			...
			pexprBitmapLocal =
				PexprBitmapSelectBestIndex(pmp, pmda, pexprPred, ptabdesc, pmdrel, pdrgpcrOutput, pcrsReqd,
		...
		else
		{
			...
			pexprBitmapLocal = PexprBitmapLookupWithPredicateBreakDown(
				pmp, pmda, pexprOriginalPred, pexprPred, ptabdesc, pmdrel, pdrgpcrOutput, pcrsOuterRefs, pcrsReqd,
...
void
CXformUtils::JoinBitmapIndexProbes(CMemoryPool *pmp, CExpressionArray *pdrgpexprBitmap,
								   CExpressionArray *pdrgpexprRecheck, BOOL fConjunction, CExpression **ppexprBitmap,
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprBitmapTableGet(CMemoryPool *mp, CLogical *popGet, ULONG ulOriginOpId, CTableDescriptor *ptabdesc,
								 CExpression *pexprScalar, CColRefSet *outer_refs, CColRefSet *pcrsReqd)
{
		...
		if (fDynamicGet)
		{
			...
			popBitmapTableGet = GPOS_NEW(mp)
				CLogicalDynamicBitmapTableGet(mp, ptabdesc, ulOriginOpId, pname, popDynamicGet->ScanId(), pdrgpcrOutput,
											  popDynamicGet->PdrgpdrgpcrPart(), popDynamicGet->UlSecondaryScanId(),
											  false,  // is_partial
...
CXformUtils::PdrgpdrgppartdigCandidates(CMemoryPool *mp, CMDAccessor *md_accessor, CExpressionArray *pdrgpexprScalar,
										CColRef2dArray *pdrgpdrgpcrPartKey, const IMDRelation *pmdrel,
										CPartConstraint *ppartcnstrRel, CColRefArray *pdrgpcrOutput,
										...
										CColRefSet *pcrsAcceptedOuterRefs)
{
	...
	for (ULONG ul = 0; ul < ulIndexes; ul++)
	{
		...
		if (!CXformUtils::FIndexApplicable(mp, pmdindex, pmdrel, pdrgpcrOutput, pcrsReqd, pcrsScalarExpr,
		...
		CPartConstraint *ppartcnstr = CUtils::PpartcnstrFromMDPartCnstr(mp, md_accessor, pdrgpdrgpcrPartKey,
																		pmdindex->MDPartConstraint(), pdrgpcrOutput);
		CExpressionArray *pdrgpexprIndex = GPOS_NEW(mp) CExpressionArray(mp);
		CExpressionArray *pdrgpexprResidual = GPOS_NEW(mp) CExpressionArray(mp);
		CPartConstraint *ppartcnstrNewlyCovered =
			PpartcnstrUpdateCovered(mp, md_accessor, pdrgpexprScalar, ppartcnstrCovered, ppartcnstr, pdrgpcrOutput,
									pdrgpexprIndex, pdrgpexprResidual, pmdrel, pmdindex, pcrsAcceptedOuterRefs);
		...
		pdrgppartdig->Append(GPOS_NEW(mp)
								 SPartDynamicIndexGetInfo(pmdindex, ppartcnstr, pdrgpexprIndex, pdrgpexprResidual));
	}
...
//---------------------------------------------------------------------------
CPartConstraint *
CXformUtils::PpartcnstrUpdateCovered(CMemoryPool *mp, CMDAccessor *md_accessor, CExpressionArray *pdrgpexprScalar,
									 CPartConstraint *ppartcnstrCovered, CPartConstraint *ppartcnstr,
									 CColRefArray *pdrgpcrOutput, CExpressionArray *pdrgpexprIndex,
									 CExpressionArray *pdrgpexprResidual, const IMDRelation *pmdrel,
...
//---------------------------------------------------------------------------
CPartConstraint *
CXformUtils::PpartcnstrDisjunction(CMemoryPool *mp, CPartConstraint *ppartcnstrOld, CPartConstraint *ppartcnstrNew)
{
...
CXformUtils::PpartdigDynamicGet(CMemoryPool *mp, CExpressionArray *pdrgpexprScalar, CPartConstraint *ppartcnstrCovered,
								CPartConstraint *ppartcnstrRel)
{
	...
	return GPOS_NEW(mp)
		SPartDynamicIndexGetInfo(NULL /*pmdindex*/, ppartcnstrRest, NULL /* pdrgpexprIndex */, pdrgpexprScalar);
}
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprRemapColumns(CMemoryPool *mp, CExpression *pexpr, CColRefArray *pdrgpcrA,
							   CColRefArray *pdrgpcrRemappedA, CColRefArray *pdrgpcrB, CColRefArray *pdrgpcrRemappedB)
{
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprPartialDynamicIndexGet(CMemoryPool *mp, CLogicalDynamicGet *popGet, ULONG ulOriginOpId,
										 CExpressionArray *pdrgpexprIndex, CExpressionArray *pdrgpexprResidual,
										 CColRefArray *pdrgpcrDIG, const IMDIndex *pmdindex, const IMDRelation *pmdrel,
										 ...
										 CColRefArray *pdrgpcrOuter, CColRefArray *pdrgpcrNewOuter)
{
	...
	if (NULL != colref_mapping)
	{
		...
		ppartcnstrRel =
			popGet->PpartcnstrRel()->PpartcnstrCopyWithRemappedColumns(mp, colref_mapping, true /*must_exist*/);
	...
	CLogicalDynamicIndexGet *popIndexGet = GPOS_NEW(mp) CLogicalDynamicIndexGet(
		mp, pmdindex, ptabdesc, ulOriginOpId, GPOS_NEW(mp) CName(mp, CName(&strTableAliasName)), popGet->ScanId(),
		pdrgpcrDIG, pdrgpdrgpcrPart, COptCtxt::PoctxtFromTLS()->UlPartIndexNextVal(), ppartcnstrDIG, ppartcnstrRel);
...
CXformUtils::FJoinPredOnSingleChild(CMemoryPool *mp, CExpressionHandle &exprhdl)
{
	...
	CExpressionArray *pdrgpexprPreds =
		CPredicateUtils::PdrgpexprConjuncts(mp, exprhdl.PexprScalarExactChild(arity - 1));
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprCTEConsumer(CMemoryPool *mp, ULONG ulCTEId, CColRefArray *colref_array)
{
...
//---------------------------------------------------------------------------
CColRefArray *
CXformUtils::PdrgpcrReorderedSubsequence(CMemoryPool *mp, CColRefArray *colref_array,
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprWinFuncAgg2ScalarAgg(CMemoryPool *mp, CExpression *pexprWinFunc)
{
...
//---------------------------------------------------------------------------
void
CXformUtils::MapPrjElemsWithDistinctAggs(CMemoryPool *mp, CExpression *pexprPrjList,
...
//---------------------------------------------------------------------------
CExpressionArrays *
CXformUtils::PdrgpdrgpexprSortedPrjElemsArray(CMemoryPool *mp, ExprToExprArrayMap *phmexprdrgpexpr)
{
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprGbAggOnCTEConsumer2Join(CMemoryPool *mp, CExpression *pexprGbAgg)
{
	...
	for (ULONG ulPrjElemsArr = 0; ulPrjElemsArr < size; ulPrjElemsArr++)
	{
		...
		if (0 == ulConsumers)
		{
			...
			pexprNewGbAgg = GPOS_NEW(mp) CExpression(
				mp, GPOS_NEW(mp) CLogicalGbAgg(mp, pdrgpcrGrpCols, COperator::EgbaggtypeGlobal), pexprCTEConsumer,
		...
		else
		{
			...
			for (ULONG ul = 0; ul < ulPrjElems; ul++)
			{
				...
				CExpression *pexprNewPrjEl =
					pexprPrjEl->PexprCopyWithRemappedColumns(mp, colref_mapping, false /*must_exist*/);
		...
		if (NULL != pexprLastGbAgg)
		{
			...
			if (NULL == pexprJoin)
			{
				// create first join
				pexprJoin =
					CUtils::PexprLogicalJoin<CLogicalInnerJoin>(mp, pexprLastGbAgg, pexprNewGbAgg, pexprJoinCondition);
			}
			else
			{
				// cascade joins
				pexprJoin =
					CUtils::PexprLogicalJoin<CLogicalInnerJoin>(mp, pexprJoin, pexprNewGbAgg, pexprJoinCondition);
			}
		}
