...
//---------------------------------------------------------------------------
CColRefSet *
CXformUtils::PcrsFKey(CMemoryPool *mp, CExpression *pexprOuter, CExpression *pexprInner,
...
CXformUtils::PexprPushGbBelowJoin(CMemoryPool *mp, CExpression *pexpr)
{
	...
	BOOL fCanPush = FCanPushGbAggBelowJoin(pcrsGrpCols, pcrsOuterOutput, pcrsScalarFromOuter,
										   pcrsAggOutput, pcrsUsed, pcrsFKey);
...
//---------------------------------------------------------------------------
CLogicalGbAgg *
CXformUtils::PopGbAggPushableBelowJoin(CMemoryPool *mp, CLogicalGbAgg *popGbAggOld,
...
CXformUtils::FCanPushGbAggBelowJoin(CColRefSet *pcrsGrpCols, CColRefSet *pcrsJoinOuterChildOutput,
									...
									CColRefSet *pcrsGrpByOutput, CColRefSet *pcrsGrpByUsed,
									CColRefSet *pcrsFKey)
{
...
)
{
	...
	if (fCanEvaluateToNull)
	{
		...
		*ppexprNewSubquery = GPOS_NEW(mp)
			CExpression(mp,
						GPOS_NEW(mp) CScalarSubquery(mp, pcrSubquery, false /*fGeneratedByExist*/,
													 true /*fGeneratedByQuantified*/),
						pexprPrj);
		*ppexprNewScalar =
			CUtils::PexprCmpWithZero(mp, CUtils::PexprScalarIdent(mp, pcrSubquery),
	...
	else
	{
		...
		*ppexprNewSubquery = GPOS_NEW(mp)
			CExpression(mp,
						GPOS_NEW(mp) CScalarSubquery(mp, pcrCount, false /*fGeneratedByExist*/,
													 true /*fGeneratedByQuantified*/),
						pexprInnerNew);
		*ppexprNewScalar =
...
)
{
	...
	pmdidInt4->AddRef();
	CExpression *pexprSubqTest = GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CScalarIf(mp, pmdidInt4), pexprSubqPred,
	...
	CExpression *pexprSumTest = CUtils::PexprCmpWithZero(
		mp, pexprScalarIdentSum, CScalarIdent::PopConvert(pexprScalarIdentSum->Pop())->MdidType(),
		IMDType::EcmptEq);
	...
	*ppexprNewSubquery = GPOS_NEW(mp)
		CExpression(mp,
					...
					pexprProjected);
	*ppexprNewScalar =
		CUtils::PexprScalarCmp(mp, CUtils::PexprScalarIdent(mp, pcrSubquery),
...
CXformUtils::PexprSeparateSubqueryPreds(CMemoryPool *mp, CExpression *pexpr)
{
	...
	else
	{
		...
		pdrgpexpr->Append(naryLOJOp->ReplaceInnerJoinPredicates(
			mp, pexprScalar, CPredicateUtils::PexprConjunction(mp, pdrgpexprNonSQ)));
	}
	...
	return CUtils::PexprLogicalSelect(mp, pexprJoin,
									  CPredicateUtils::PexprConjunction(mp, pdrgpexprSQ));
}
...
CXformUtils::PexprNullIndicator(CMemoryPool *mp, CExpression *pexpr)
{
	...
	mdid->AddRef();
	return GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarIf(mp, mdid), pexprIsNull,
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprLogicalPartitionSelector(CMemoryPool *mp, CTableDescriptor *ptabdesc,
										   CColRefArray *colref_array, CExpression *pexprChild)
{
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprLogicalDMLOverProject(CMemoryPool *mp, CExpression *pexprChild,
...
//---------------------------------------------------------------------------
BOOL
CXformUtils::FTriggersExist(CLogicalDML::EDMLOperator edmlop, CTableDescriptor *ptabdesc,
							BOOL fBefore)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
...
CXformUtils::PexprRowTrigger(CMemoryPool *mp, CExpression *pexprChild,
							 CLogicalDML::EDMLOperator edmlop, IMDId *rel_mdid, BOOL fBefore,
							 CColRefArray *colref_array)
{
	...
	if (CLogicalDML::EdmlInsert == edmlop)
	{
		return PexprRowTrigger(mp, pexprChild, edmlop, rel_mdid, fBefore, NULL /*pdrgpcrOld*/,
							   colref_array);
	}

	return PexprRowTrigger(mp, pexprChild, edmlop, rel_mdid, fBefore, colref_array,
						   NULL /*pdrgpcrNew*/);
}
								...
								CTableDescriptor *ptabdesc, CColRefArray *colref_array)
{
	...
	for (ULONG ul = 0; ul < num_cols; ul++)
	{
		...
		CWStringConst *pstrErrorMsg = PstrErrorMessage(
			mp, gpos::CException::ExmaSQL, gpos::CException::ExmiSQLNotNullViolation,
	...
	CLogicalAssert *popAssert = GPOS_NEW(mp)
		CLogicalAssert(mp, GPOS_NEW(mp) CException(gpos::CException::ExmaSQL,
							 ...
							 CColRefArray *pdrgpcrOld, CColRefArray *pdrgpcrNew)
{
	...
	return GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CLogicalRowTrigger(mp, rel_mdid, type, pdrgpcrOld, pdrgpcrNew),
					pexprChild);
}
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprAssertCheckConstraints(CMemoryPool *mp, CExpression *pexprChild,
										 CTableDescriptor *ptabdesc, CColRefArray *colref_array)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
	const IMDRelation *pmdrel = md_accessor->RetrieveRel(ptabdesc->MDId());
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprAssertUpdateCardinality(CMemoryPool *mp, CExpression *pexprDMLChild,
										  ...
										  CColRef *pcrSegmentId)
{
	...
	return GPOS_NEW(mp) CExpression(
		mp,
		GPOS_NEW(mp) CLogicalAssert(mp, GPOS_NEW(mp) CException(gpos::CException::ExmaSQL,
...
CXformUtils::AddMinAggs(CMemoryPool *mp, CMDAccessor *md_accessor, CColumnFactory *col_factory,
						CColRefArray *colref_array, ColRefToColRefMap *phmcrcr,
						CExpressionArray *pdrgpexpr, CColRefArray **ppdrgpcrNew)
{
		...
		(emdindtype == IMDIndex::EmdindBitmap && IMDIndex::EmdindGin == pmdindex->IndexType()))
	{
		if (pmdrel->IsPartialIndex(pmdindex->MDId()))
		{
			...
			return false;
			 ...
			 pmdindex->IndexType() == IMDIndex::EmdindBtree && pmdrel->IsAORowOrColTable())
	{
...
CXformUtils::PexprRowNumber(CMemoryPool *mp)
{
	...
	CScalarWindowFunc *popRowNumber = GPOS_NEW(mp) CScalarWindowFunc(
		mp, GPOS_NEW(mp) CMDIdGPDB(row_number_oid), GPOS_NEW(mp) CMDIdGPDB(GPDB_INT8_OID),
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprWindowWithRowNumber(CMemoryPool *mp, CExpression *pexprWindowChild,
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprAddCTEProducer(CMemoryPool *mp, ULONG ulCTEId, CColRefArray *colref_array,
								 CExpression *pexpr)
{
...
)
{
	...
	*ppexprResult = NULL;
	...
	pexprInner->AddRef();
	pexpr->Pop()->AddRef();
	*ppexprResult = GPOS_NEW(mp) CExpression(mp, pexpr->Pop(), pexprOuter, pexprInner,
...
CExpression *
CXformUtils::PexprBuildIndexPlan(
	CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprGet, ULONG ulOriginOpId,
	...
	BOOL fAllowPartialIndex, CPartConstraint *ppartcnstrIndex, IMDIndex::EmdindexType emdindtype,
	PDynamicIndexOpConstructor pdiopc, PStaticIndexOpConstructor psiopc, PRewrittenIndexPath prip)
{
	...
	if (!FIndexApplicable(mp, pmdindex, pmdrel, pdrgpcrOutput, pcrsReqd, pcrsScalarExpr,
						  emdindtype))
	{
		GPOS_DELETE(alias);
		CRefCount::SafeRelease(ppartcnstrIndex);
	...
	CPredicateUtils::ExtractIndexPredicates(mp, md_accessor, pdrgpexprConds, pmdindex,
											pdrgppcrIndexCols, pdrgpexprIndex, pdrgpexprResidual,
											outer_refs);
	...
	if (fDynamicGet)
	{
		...
		popLogicalGet = (*pdiopc)(
			mp, pmdindex, ptabdesc, ulOriginOpId, GPOS_NEW(mp) CName(mp, CName(alias)), ulPartIndex,
			pdrgpcrOutput, pdrgpdrgpcrPart, ulSecondaryPartIndex, ppartcnstrIndex, ppartcnstrRel);
	}
	else
	{
		popLogicalGet = (*psiopc)(mp, pmdindex, ptabdesc, ulOriginOpId,
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprScalarBitmapBoolOp(CMemoryPool *mp, CMDAccessor *md_accessor,
									 ...
									 BOOL isAPartialPredicate)
{
	...
	CreateBitmapIndexProbeOps(mp, md_accessor, pexprOriginalPred, pdrgpexpr, ptabdesc, pmdrel,
	...
	JoinBitmapIndexProbes(mp, pdrgpexprBitmap, pdrgpexprRecheckNew, fConjunction,
						  &pexprBitmapBoolOp, &pexprRecheckNew);
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprBitmapBoolOp(CMemoryPool *mp, IMDId *pmdidBitmapType, CExpression *pexprLeft,
							   CExpression *pexprRight, BOOL fConjunction)
{
	...
	return GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CScalarBitmapBoolOp(mp, ebitmapboolop, pmdidBitmapType),
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprEqualityOnBoolColumn(CMemoryPool *mp, CMDAccessor *md_accessor, BOOL value,
									   CColRef *colref)
{
...
CXformUtils::PexprBitmapLookupWithPredicateBreakDown(
	...
	CExpression *pexprPred, CTableDescriptor *ptabdesc, const IMDRelation *pmdrel,
	...
	CExpression **ppexprRecheck, CExpression **ppexprResidual)
{
	...
	CExpression *pexprResult = PexprScalarBitmapBoolOp(
		mp, md_accessor, pexprOriginalPred, pdrgpexpr, ptabdesc, pmdrel, pdrgpcrOutput, outer_refs,
		pcrsReqd, fConjunction, ppexprRecheck, ppexprResidual,
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprBitmapSelectBestIndex(CMemoryPool *mp, CMDAccessor *md_accessor,
										CExpression *pexprPred, CTableDescriptor *ptabdesc,
										const IMDRelation *pmdrel, CColRefArray *pdrgpcrOutput,
										CColRefSet *pcrsReqd, CColRefSet *pcrsOuterRefs,
										CExpression **ppexprRecheck, CExpression **ppexprResidual,
	...
	for (ULONG ul = 0; ul < ulIndexes; ul++)
	{
										  ...
										  IMDIndex::EmdindBitmap, altIndexType))
		{
			...
			CPredicateUtils::ExtractIndexPredicates(
				mp, md_accessor, pdrgpexprScalar, pmdindex, pdrgpcrIndexCols, pdrgpexprIndex,
				pdrgpexprResidual, pcrsOuterRefs, alsoConsiderBTreeIndexes);
			...
			if (selectivity > AO_TABLE_BTREE_INDEX_SELECTIVITY_THRESHOLD &&
				pmdrel->IsAORowOrColTable() && pmdindex->IndexType() == IMDIndex::EmdindBtree)
			{
				pdrgpexprIndex->Release();
				pdrgpexprResidual->Release();
				continue;
...
CXformUtils::CreateBitmapIndexProbeOps(
	CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprOriginalPred,
	CExpressionArray *pdrgpexprPreds, CTableDescriptor *ptabdesc, const IMDRelation *pmdrel,
	CColRefArray *pdrgpcrOutput, CColRefSet *outer_refs, CColRefSet *pcrsReqd,
	BOOL,  // fConjunction
	...
	CExpressionArray *pdrgpexprResidual, BOOL isAPartialPredicate)
{
	...
	for (ULONG ul = 0; ul < ulPredicates; ul++)
	{
		...
		CreateBitmapIndexProbesWithOrWithoutPredBreakdown(
			mp, md_accessor, pexprOriginalPred, pexprPred, ptabdesc, pmdrel, pdrgpcrOutput,
			outer_refs, pcrsReqd, &pexprBitmap, &pexprRecheck, pdrgpexprResidual,
			isAPartialPredicate);
...
CXformUtils::CreateBitmapIndexProbesWithOrWithoutPredBreakdown(
	...
	CTableDescriptor *ptabdesc, const IMDRelation *pmdrel, CColRefArray *pdrgpcrOutput,
	CColRefSet *pcrsOuterRefs, CColRefSet *pcrsReqd, CExpression **pexprBitmapResult,
	CExpression **pexprRecheckResult, CExpressionArray *pdrgpexprResidualResult,
	BOOL isAPartialPredicate)
{
	...
	while (NULL != pexprPred)
	{
		...
		if (CPredicateUtils::FBitmapLookupSupportedPredicateOrConjunct(pexprPred, pcrsOuterRefs))
		{
			...
			pexprBitmapLocal = PexprBitmapSelectBestIndex(
				pmp, pmda, pexprPred, ptabdesc, pmdrel, pdrgpcrOutput, pcrsReqd, pcrsOuterRefs,
		...
		else
		{
			...
			pexprBitmapLocal = PexprBitmapLookupWithPredicateBreakDown(
				pmp, pmda, pexprOriginalPred, pexprPred, ptabdesc, pmdrel, pdrgpcrOutput,
				pcrsOuterRefs, pcrsReqd, &pexprRecheckLocal, &pexprResidualLocal);
	...
	if (0 < ulBitmapExpr)
	{
		JoinBitmapIndexProbes(pmp, pdrgpexprBitmapTemp, pdrgpexprRecheckTemp, true /*fConjunction*/,
...
CXformUtils::PexprSelect2BitmapBoolOp(CMemoryPool *mp, CExpression *pexpr)
{
	...
	CExpression *pexprResult =
		PexprBitmapTableGet(mp, popGet, pexpr->Pop()->UlOpId(), ptabdesc, pexprScalar,
							NULL,  // outer_refs
							pcrsReqd);
	pcrsReqd->Release();
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprBitmapTableGet(CMemoryPool *mp, CLogical *popGet, ULONG ulOriginOpId,
								 ...
								 CColRefSet *outer_refs, CColRefSet *pcrsReqd)
{
	...
	CExpression *pexprBitmap = PexprScalarBitmapBoolOp(
		mp, md_accessor, pexprScalar, pdrgpexpr, ptabdesc, pmdrel, pdrgpcrOutput, outer_refs,
	...
	if (NULL != pexprBitmap)
	{
		...
		if (fDynamicGet)
		{
			...
			popBitmapTableGet = GPOS_NEW(mp) CLogicalDynamicBitmapTableGet(
				mp, ptabdesc, ulOriginOpId, pname, popDynamicGet->ScanId(), pdrgpcrOutput,
...
CXformUtils::PdrgpdrgppartdigCandidates(CMemoryPool *mp, CMDAccessor *md_accessor,
										...
										CColRefSet *pcrsAcceptedOuterRefs)
{
	...
	for (ULONG ul = 0; ul < ulIndexes; ul++)
	{
		...
		if (!CXformUtils::FIndexApplicable(mp, pmdindex, pmdrel, pdrgpcrOutput, pcrsReqd,
										   pcrsScalarExpr, IMDIndex::EmdindBtree /*emdindtype*/) ||
		...
		CPartConstraint *ppartcnstrNewlyCovered = PpartcnstrUpdateCovered(
			mp, md_accessor, pdrgpexprScalar, ppartcnstrCovered, ppartcnstr, pdrgpcrOutput,
...
//---------------------------------------------------------------------------
CPartConstraint *
CXformUtils::PpartcnstrUpdateCovered(CMemoryPool *mp, CMDAccessor *md_accessor,
									 CExpressionArray *pdrgpexprScalar,
									 ...
									 const IMDIndex *pmdindex, CColRefSet *pcrsAcceptedOuterRefs)
{
	...
	CColRefArray *pdrgpcrIndexCols = PdrgpcrIndexKeys(mp, pdrgpcrOutput, pmdindex, pmdrel);
	CPredicateUtils::ExtractIndexPredicates(mp, md_accessor, pdrgpexprScalar, pmdindex,
											pdrgpcrIndexCols, pdrgpexprIndex, pdrgpexprResidual,
...
//---------------------------------------------------------------------------
CPartConstraint *
CXformUtils::PpartcnstrDisjunction(CMemoryPool *mp, CPartConstraint *ppartcnstrOld,
...
CXformUtils::PpartdigDynamicGet(CMemoryPool *mp, CExpressionArray *pdrgpexprScalar,
								CPartConstraint *ppartcnstrCovered, CPartConstraint *ppartcnstrRel)
{
	...
	return GPOS_NEW(mp) SPartDynamicIndexGetInfo(NULL /*pmdindex*/, ppartcnstrRest,
...
//---------------------------------------------------------------------------
CExpression *
CXformUtils::PexprPartialDynamicIndexGet(CMemoryPool *mp, CLogicalDynamicGet *popGet,
										 ULONG ulOriginOpId, CExpressionArray *pdrgpexprIndex,
										 ...
										 CColRefArray *pdrgpcrOuter, CColRefArray *pdrgpcrNewOuter)
{
	...
	if (NULL != colref_mapping)
	{
		...
		pdrgpdrgpcrPart = CUtils::PdrgpdrgpcrRemap(mp, popGet->PdrgpdrgpcrPart(), colref_mapping,
												   true /*must_exist*/);
		ppartcnstrDIG =
	...
	CLogicalDynamicIndexGet *popIndexGet = GPOS_NEW(mp) CLogicalDynamicIndexGet(
		mp, pmdindex, ptabdesc, ulOriginOpId, GPOS_NEW(mp) CName(mp, CName(&strTableAliasName)),
		popGet->ScanId(), pdrgpcrDIG, pdrgpdrgpcrPart,
		COptCtxt::PoctxtFromTLS()->UlPartIndexNextVal(), ppartcnstrDIG, ppartcnstrRel);
	...
	return CUtils::PexprSafeSelect(mp, GPOS_NEW(mp) CExpression(mp, popIndexGet, pexprIndexCond),
								   pexprResidualCond);
}
	...
	for (ULONG ulPrjElemsArr = 0; ulPrjElemsArr < size; ulPrjElemsArr++)
	{
		...
		if (0 == ulConsumers)
		{
			...
			pexprNewGbAgg = GPOS_NEW(mp) CExpression(
				mp, GPOS_NEW(mp) CLogicalGbAgg(mp, pdrgpcrGrpCols, COperator::EgbaggtypeGlobal),
				pexprCTEConsumer,
				GPOS_NEW(mp)
		...
		else
		{
			...
			pexprNewGbAgg = GPOS_NEW(mp) CExpression(
				mp, GPOS_NEW(mp) CLogicalGbAgg(mp, pdrgpcrNewGrpCols, COperator::EgbaggtypeGlobal),
				pexprNewConsumer,
				GPOS_NEW(mp)
		...
		if (NULL != pexprLastGbAgg)
		{
			...
			else
			{
				...
				pexprJoinCondition = CPredicateUtils::PexprINDFConjunction(mp, pdrgpcrLastGrpCols,
																		   pdrgpcrNewGrpCols);
			}
