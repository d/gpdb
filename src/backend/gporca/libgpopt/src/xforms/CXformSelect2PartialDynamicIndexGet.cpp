...
//---------------------------------------------------------------------------
void
CXformSelect2PartialDynamicIndexGet::Transform(CXformContext *pxfctxt, CXformResult *pxfres,
											   CExpression *pexpr) const
{
	...
	SPartDynamicIndexGetInfoArrays *pdrgpdrgppartdig = CXformUtils::PdrgpdrgppartdigCandidates(
		mp, md_accessor, pdrgpexpr, popGet->PdrgpdrgpcrPart(), pmdrel, ppartcnstr,
...
void
CXformSelect2PartialDynamicIndexGet::CreatePartialIndexGetPlan(
	CMemoryPool *mp, CExpression *pexpr, SPartDynamicIndexGetInfoArray *pdrgppartdig,
	...
	for (ULONG ul = 0; ul < ulPartialIndexes; ul++)
	{
		...
		if (NULL != pmdindex)
		{
			pexprDynamicScan = CXformUtils::PexprPartialDynamicIndexGet(
				mp, popGet, pexpr->Pop()->UlOpId(), pdrgpexprIndex, pdrgpexprResidual, pdrgpcrNew,
				pmdindex, pmdrel, ppartcnstr,
				NULL,  // pcrsAcceptedOuterRefs
				NULL,  // pdrgpcrOuter
...
//---------------------------------------------------------------------------
CExpression *
CXformSelect2PartialDynamicIndexGet::PexprSelectOverDynamicGet(CMemoryPool *mp,
															   ...
															   CPartConstraint *ppartcnstr)
{
	...
	CLogicalDynamicGet *popPartialDynamicGet =
		(CLogicalDynamicGet *) popGet->PopCopyWithRemappedColumns(mp, colref_mapping,
																  true /*must_exist*/);
	...
	CExpression *pexprSelect = GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CLogicalSelect(mp), GPOS_NEW(mp) CExpression(mp, popPartialDynamicGet),
