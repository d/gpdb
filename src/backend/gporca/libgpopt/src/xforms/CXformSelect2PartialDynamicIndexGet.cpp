...
//
//---------------------------------------------------------------------------
CXformSelect2PartialDynamicIndexGet::CXformSelect2PartialDynamicIndexGet(CMemoryPool *mp)
	:  // pattern
...
//---------------------------------------------------------------------------
void
CXformSelect2PartialDynamicIndexGet::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *pexpr) const
{
	...
	SPartDynamicIndexGetInfoArrays *pdrgpdrgppartdig =
		CXformUtils::PdrgpdrgppartdigCandidates(mp, md_accessor, pdrgpexpr, popGet->PdrgpdrgpcrPart(), pmdrel,
...
//---------------------------------------------------------------------------
void
CXformSelect2PartialDynamicIndexGet::CreatePartialIndexGetPlan(CMemoryPool *mp, CExpression *pexpr,
															   ...
															   const IMDRelation *pmdrel, CXformResult *pxfres) const
{
	...
	for (ULONG ul = 0; ul < ulPartialIndexes; ul++)
	{
		...
		{
			pexprDynamicScan =
				CXformUtils::PexprPartialDynamicIndexGet(mp, popGet, pexpr->Pop()->UlOpId(), pdrgpexprIndex,
		...
		{
			pexprDynamicScan = PexprSelectOverDynamicGet(mp, popGet, pexprScalar, pdrgpcrNew, ppartcnstr);
		}
	...
	if (0 < ulInput)
	{
		...
		if (1 < ulInput)
		{
			...
			pexprResult = GPOS_NEW(mp)
				CExpression(mp, GPOS_NEW(mp) CLogicalUnionAll(mp, pdrgpcrOuter, pdrgpdrgpcrInput, popGet->ScanId()),
							pdrgpexprInput);
		}
		else
		{
		...
		{
			pexprResult =
				GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CLogicalSelect(mp), pexprResult, pexprPredOnPartKey);
		}
...
//---------------------------------------------------------------------------
CExpression *
CXformSelect2PartialDynamicIndexGet::PexprSelectOverDynamicGet(CMemoryPool *mp, CLogicalDynamicGet *popGet,
															   CExpression *pexprScalar, CColRefArray *pdrgpcrDGet,
															   CPartConstraint *ppartcnstr)
{
	...
	CExpression *pexprSelect = GPOS_NEW(mp)
		CExpression(mp, GPOS_NEW(mp) CLogicalSelect(mp), GPOS_NEW(mp) CExpression(mp, popPartialDynamicGet),
