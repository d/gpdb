...
CJoinOrder::CJoinOrder(CMemoryPool *mp, CExpressionArray *all_components, CExpressionArray *inner_join_conjuncts,
	...
	: m_mp(mp), m_rgpedge(NULL), m_ulEdges(0), m_rgpcomp(NULL), m_ulComps(0), m_include_loj_childs(include_loj_childs)
{
...
void
CJoinOrder::AddComponent(CMemoryPool *mp, CExpression *expr, INT loj_id, EPosition position, INT comp_num)
{
	expr->AddRef();
