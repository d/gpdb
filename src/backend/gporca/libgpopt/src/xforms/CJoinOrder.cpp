...
// ctor
CJoinOrder::SComponent::SComponent(CMemoryPool *mp, CExpression *pexpr, INT parent_loj_id,
								   EPosition position)
	: m_pbs(NULL),
	  m_edge_set(NULL),
	  m_pexpr(pexpr),
...
// ctor
CJoinOrder::SComponent::SComponent(CExpression *pexpr, CBitSet *pbs, CBitSet *edge_set,
...
{
	return os << (m_loj_num > 0 ? "Edge (loj): " : "Edge : ") << (*m_pbs) << std::endl
			  << (*m_pexpr) << std::endl;
}
...
CJoinOrder::CJoinOrder(CMemoryPool *mp, CExpressionArray *all_components,
					   CExpressionArray *inner_join_conjuncts, BOOL include_loj_childs)
	: m_mp(mp),
	  m_rgpedge(NULL),
	  m_ulEdges(0),
	  m_rgpcomp(NULL),
...
//---------------------------------------------------------------------------
CJoinOrder::CJoinOrder(CMemoryPool *mp, CExpressionArray *all_components,
					   CExpressionArray *innerJoinPredConjuncts, CExpressionArray *onPreds,
...
CJoinOrder::PcompCombine(SComponent *comp1, SComponent *comp2)
{
	...
	else
	{
		...
		if (IsChildOfSameLOJ(comp1, comp2))
		{
			...
			CExpression *pexprLeft =
				comp1->Position() == CJoinOrder::EpLeft ? pexprChild1 : pexprChild2;
			CExpression *pexprRight =
				comp1->Position() == CJoinOrder::EpLeft ? pexprChild2 : pexprChild1;
			...
			pexpr = CUtils::PexprLogicalJoin<CLogicalLeftOuterJoin>(m_mp, pexprLeft, pexprRight,
																	loj_predicate);
		...
		else
		{
				...
				comp2->ParentLojId() > NON_LOJ_DEFAULT_ID)
			{
				...
				parent_loj_id = NON_LOJ_DEFAULT_ID < comp1->ParentLojId() ? comp1->ParentLojId()
																		  : comp2->ParentLojId();
				position = NON_LOJ_DEFAULT_ID < comp1->ParentLojId() ? comp1->Position()
																	 : comp2->Position();
			...
			other_conjuncts->Release();
			CExpression *predicate = CPredicateUtils::PexprConjunction(m_mp, loj_conjuncts);
			pexpr = CUtils::PexprLogicalJoin<CLogicalInnerJoin>(m_mp, pexprChild1, pexprChild2,
																predicate);
		}
	}
	...
	SComponent *join_comp =
		GPOS_NEW(m_mp) SComponent(pexpr, pbs, edge_set, parent_loj_id, position);
...
CJoinOrder::IsChildOfSameLOJ(SComponent *comp1, SComponent *comp2) const
{
	...
	BOOL child_of_same_loj =
		comp1->ParentLojId() == comp2->ParentLojId() &&
...
void
CJoinOrder::AddComponent(CMemoryPool *mp, CExpression *expr, INT loj_id, EPosition position,
						 INT comp_num)
{
	expr->AddRef();
