...
void
CXformEagerAgg::Transform(CXformContext *pxfctxt, CXformResult *pxfres, CExpression *agg_expr) const
{
	...
	//		WHERE t1.j1 = t2.j2
	//		GROUP BY t1.g1;
	CColRefSet *push_down_gb_cols =
		GPOS_NEW(mp) CColRefSet(mp, *(join_condition_expr->DeriveUsedColumns()));
	...
	(void) PopulateLowerUpperProjectList(mp, agg_proj_list_expr, &lower_expr_proj_list,
	...
	CExpression *lower_agg_expr = GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CLogicalGbAgg(mp, push_down_gb_col_array, COperator::EgbaggtypeLocal),
	...
	CExpression *new_join_expr = GPOS_NEW(mp)
		CExpression(mp, join_op, lower_agg_expr, join_inner_child_expr, join_condition_expr);
	...
	CExpression *upper_agg_expr = GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CLogicalGbAgg(mp, grouping_col_array, COperator::EgbaggtypeGlobal),
...
) const
{
	...
	for (ULONG ul = 0; ul < num_proj_elements; ul++)
	{
		...
		PopulateLowerProjectElement(
			...
			orig_agg_expr->PdrgPexpr(), orig_agg_func->IsDistinct(), &lower_proj_elem_expr);
		...
		PopulateUpperProjectElement(
			...
			orig_proj_elem->Pcr(), orig_agg_func->IsDistinct(), &upper_proj_elem_expr);
		upper_proj_elem_array->Append(upper_proj_elem_expr);
	}  // end of loop over each project element

	/* 3. create new project lists */
	*lower_proj_list =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), lower_proj_elem_array);

	*upper_proj_list =
		GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), upper_proj_elem_array);
}
...
) const
{
	...
	CScalarAggFunc *lower_agg_func =
		CUtils::PopAggFunc(mp, agg_mdid, agg_name, is_distinct, EaggfuncstageLocal, true);
...
CXformEagerAgg::PopulateUpperProjectElement(
	CMemoryPool *mp,  // memory pool
	IMDId *agg_mdid,  // original global aggregate function
	CWStringConst *agg_name, CColRef *lower_colref, CColRef *output_colref, BOOL is_distinct,
...
) const
{
	...
	CScalarAggFunc *upper_agg_func =
		CUtils::PopAggFunc(mp, agg_mdid, agg_name, is_distinct, EaggfuncstageGlobal, true);
