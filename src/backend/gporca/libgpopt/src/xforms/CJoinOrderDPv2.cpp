...
//
//---------------------------------------------------------------------------
CJoinOrderDPv2::CJoinOrderDPv2(CMemoryPool *mp, CExpressionArray *pdrgpexprAtoms,
	  ...
	  m_outer_refs(outerRefs)
{
	...
	m_top_k_expressions = GPOS_NEW(mp)
		CKHeap<SExpressionInfoArray, SExpressionInfo>(mp, GPOPT_DPV2_JOIN_ORDERING_TOPK);
...
//---------------------------------------------------------------------------
void
CJoinOrderDPv2::ComputeCost(SExpressionInfo *expr_info, CDouble join_cardinality)
{
	...
	if (expr_info->m_left_child_expr.IsValid())
	{
		...
		expr_info->m_cost_adj_PS =
			expr_info->m_cost_adj_PS + expr_info->m_right_child_expr.GetExprInfo()->m_cost_adj_PS;
	}
...
CJoinOrderDPv2::GetJoinExprForProperties(SGroupInfo *left_child, SGroupInfo *right_child,
							...
							SExpressionProperties &result_properties)
{
	...
	if (isLOJ)
	{
		join_expr = CUtils::PexprLogicalJoin<CLogicalLeftOuterJoin>(m_mp, left_expr, right_expr,
																	scalar_expr);
	}
	else
	{
		join_expr =
			CUtils::PexprLogicalJoin<CLogicalInnerJoin>(m_mp, left_expr, right_expr, scalar_expr);
	}

	return GPOS_NEW(m_mp)
		SExpressionInfo(m_mp, join_expr, left_child_expr, right_child_expr, result_properties);
}
...
CJoinOrderDPv2::GetBestExprForProperties(SGroupInfo *group_info, SExpressionProperties &props)
{
...
//---------------------------------------------------------------------------
void
CJoinOrderDPv2::AddNewPropertyToExpr(SExpressionInfo *expr_info, SExpressionProperties props)
{
...
CJoinOrderDPv2::AddExprToGroupIfNecessary(SGroupInfo *group_info, SExpressionInfo *new_expr_info)
{
	...
	for (ULONG ul = 0; ul < group_info->m_best_expr_info_array->Size(); ul++)
	{
		SExpressionInfo *expr_info = (*group_info->m_best_expr_info_array)[ul];
		BOOL old_ge_new =
			IsASupersetOfProperties(expr_info->m_properties, new_expr_info->m_properties);
		BOOL new_ge_old =
			IsASupersetOfProperties(new_expr_info->m_properties, expr_info->m_properties);
...
CJoinOrderDPv2::AddSelectNodeForRemainingEdges(CExpression *join_expr)
{
	...
	if (0 < exprArray->Size())
	{
		...
		return GPOS_NEW(m_mp)
			CExpression(m_mp, GPOS_NEW(m_mp) CLogicalSelect(m_mp), join_expr, conj);
	}
...
void
CJoinOrderDPv2::PopulateDPEInfo(SExpressionInfo *join_expr_info, SGroupInfo *part_table_group_info,
				...
				while (iter_ps.Advance())
				{
							...
							->Eopid() == COperator::EopLogicalSelect)
					{
						...
						join_expr_info->m_cost_adj_PS =
							join_expr_info->m_cost_adj_PS -
							(percent_reduction * pt_atom->m_cardinality) + broadcast_penalty;
					}
				}
				break;
			}
	...
	for (ULONG left_ix = 0; left_ix < left_size; left_ix++)
	{
		...
		for (; right_ix < right_size; right_ix++)
		{
			...
			SExpressionInfo *join_expr_info =
				GetJoinExprForProperties(left_group_info, right_group_info, reqd_properties);
			...
			if (NULL != join_expr_info)
			{
				...
				SGroupInfo *group_info =
					LookupOrCreateGroupInfo(current_level_info, join_bitset, join_expr_info);
				...
				join_expr_info =
					GetJoinExprForProperties(left_group_info, right_group_info, join_props);
...
CJoinOrderDPv2::GreedySearchJoinOrders(ULONG left_level, JoinOrderPropType algo)
{
	...
	{
		left_child_expr_info =
			GetBestExprForProperties((*left_group_info_array)[left_ix], left_reqd_properties);
	...
	for (; right_ix < right_size; right_ix++)
	{
		...
		SExpressionInfo *join_expr_info =
			GetJoinExpr(left_child_expr_info, right_child_expr_info, result_properties);
		if (NULL != join_expr_info)
		{
			// we have a valid join
			CBitSet *join_bitset = GPOS_NEW(m_mp) CBitSet(m_mp, *left_bitset);
	...
	if (join_order_exhaustive_limit < m_ulComps)
	{
		for (ULONG l = 2; l <= m_ulComps; l++)
		{
			...
			Level(l)->m_top_k_groups =
				GPOS_NEW(m_mp) CKHeap<SGroupInfoArray, SGroupInfo>(m_mp, number_of_allowed_groups);
		}
	}
...
//---------------------------------------------------------------------------
BOOL
CJoinOrderDPv2::IsRightChildOfNIJ(SGroupInfo *groupInfo, CExpression **onPredToUse,
...
CJoinOrderDPv2::OsPrint(IOstream &os) const
{
	...
	for (ULONG lev = 1; lev < num_levels; lev++)
	{
		...
		os << "CJoinOrderDPv2 - Level: " << lev << " (" << bitsets_this_level->Size()
		   << " group(s))" << std::endl;
		...
		for (ULONG c = 0; c < num_bitsets_this_level; c++)
		{
			...
			{
				os << "   Rows: " << expr_for_stats.GetExprInfo()->m_expr->Pstats()->Rows()
				   << std::endl;
			...
			for (ULONG x = 0; x < num_exprs; x++)
			{
				...
				if (!gi->IsAnAtom())
				{
					...
					os << std::endl;
					os << "   left child cost: "
					   << expr_info->m_left_child_expr.m_group_info->m_lowest_expr_cost
					   << std::endl;
					os << "   right child cost: "
					   << expr_info->m_right_child_expr.m_group_info->m_lowest_expr_cost
					   << std::endl;
				}
				os << "   Cost: ";
