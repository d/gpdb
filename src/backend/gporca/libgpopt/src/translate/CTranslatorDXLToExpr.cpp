...
//
//---------------------------------------------------------------------------
CTranslatorDXLToExpr::CTranslatorDXLToExpr(CMemoryPool *mp, CMDAccessor *md_accessor,
...
//---------------------------------------------------------------------------
CExpression *
CTranslatorDXLToExpr::PexprTranslateScalar(const CDXLNode *dxlnode, CColRefArray *colref_array,
...
CTranslatorDXLToExpr::PexprLogicalTVF(const CDXLNode *dxlnode)
{
	...
	for (ULONG ul = 0; ul < ulColumns; ul++)
	{
		...
		CColumnDescriptor *pcoldesc = GPOS_NEW(m_mp) CColumnDescriptor(
			m_mp, pmdtype, pdxlcoldesc->TypeModifier(), CName(m_mp, &strColName), attrnum,
			true,  // is_nullable
			pdxlcoldesc->Width());
...
CTranslatorDXLToExpr::PexprLogicalGet(const CDXLNode *dxlnode)
{
	...
	if (pmdrel->IsPartitioned())
	{
		...
		CPartConstraint *ppartcnstr = CUtils::PpartcnstrFromMDPartCnstr(
			m_mp, m_pmda, popDynamicGet->PdrgpdrgpcrPart(), pmdrel->MDPartConstraint(),
			colref_array, fDummyConstraint);
									   ...
									   const CColRef *pcrToCast, CColRef *pcrToReturn)
{
	...
	else
	{
		pexprCast = GPOS_NEW(m_mp) CExpression(
			m_mp,
			GPOS_NEW(m_mp) CScalarCast(m_mp, mdid_dest, pmdcast->GetCastFuncMdId(),
		...
		if (!pcrsChildOutput->FMember(colref))
		{
			...
			if (fEqualTypes)
			{
				...
				pexprChildProjElem = GPOS_NEW(m_mp) CExpression(
					m_mp, GPOS_NEW(m_mp) CScalarProjectElement(m_mp, new_colref),
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		if (0 < pdrgpexprChildProjElems->Size())
		{
			CExpression *pexprChildProject = GPOS_NEW(m_mp) CExpression(
				...
				GPOS_NEW(m_mp) CExpression(m_mp, GPOS_NEW(m_mp) CScalarProjectList(m_mp),
...
//---------------------------------------------------------------------------
CColRef *
CTranslatorDXLToExpr::PcrCreate(const CColRef *colref, const IMDType *pmdtype, INT type_modifier,
...
CTranslatorDXLToExpr::PexprLogicalDelete(const CDXLNode *dxlnode)
{
	...
	return GPOS_NEW(m_mp) CExpression(
		m_mp, GPOS_NEW(m_mp) CLogicalDelete(m_mp, ptabdesc, colref_array, pcrCtid, pcrSegmentId),
		pexprChild);
}
...
CTranslatorDXLToExpr::PexprLogicalUpdate(const CDXLNode *dxlnode)
{
	...
	return GPOS_NEW(m_mp)
		CExpression(m_mp,
					GPOS_NEW(m_mp) CLogicalUpdate(m_mp, ptabdesc, pdrgpcrDelete, pdrgpcrInsert,
	...
	while (hmiterulpdrgexpr.Advance())
	{
		...
		if (0 < colref_array->Size())
		{
			...
			pds = CDistributionSpecHashed::MakeHashedDistrSpec(
				m_mp, pdrgpexprScalarIdents, true /* fNullsCollocated */, NULL /* pdshashedEquiv */,
				NULL /* opfamilies */);
			if (NULL == pds)
			{
...
CTranslatorDXLToExpr::Ptabdesc(CDXLTableDescr *table_descr)
{
	...
	CTableDescriptor *ptabdesc = GPOS_NEW(m_mp)
		CTableDescriptor(m_mp, mdid, CName(m_mp, &strName), pmdrel->ConvertHashToRandom(),
	...
	for (ULONG ul = 0; ul < ulColumns; ul++)
	{
		...
		CColumnDescriptor *pcoldesc = GPOS_NEW(m_mp)
			CColumnDescriptor(m_mp, pmdtype, pdxlcoldesc->TypeModifier(), CName(m_mp, &strColName),
							  attrnum, is_nullable, ulWidth);
...
CTranslatorDXLToExpr::RegisterMDRelationCtas(CDXLLogicalCTAS *pdxlopCTAS)
{
	...
	CMDRelationCtasGPDB *pmdrel = GPOS_NEW(m_mp) CMDRelationCtasGPDB(
		...
		GPOS_NEW(m_mp) CMDName(m_mp, pdxlopCTAS->MdName()->GetMDName()), pdxlopCTAS->IsTemporary(),
		pdxlopCTAS->HasOids(), pdxlopCTAS->RetrieveRelStorageType(), pdxlopCTAS->Ereldistrpolicy(),
		mdcol_array, pdxlopCTAS->GetDistrColPosArray(), pdxlopCTAS->GetDistrOpfamilies(),
...
CTranslatorDXLToExpr::PexprLogicalConstTableGet(const CDXLNode *pdxlnConstTable)
{
	...
	for (ULONG ulColIdx = 0; ulColIdx < ulColumns; ulColIdx++)
	{
		...
		CColumnDescriptor *pcoldesc =
			GPOS_NEW(m_mp) CColumnDescriptor(m_mp, pmdtype, pdxlcd->TypeModifier(), name,
											 ulColIdx + 1,	// attno
											 true,			// IsNullable
											 ulWidth);
		pdrgpcoldesc->Append(pcoldesc);
...
//---------------------------------------------------------------------------
CExpression *
CTranslatorDXLToExpr::PexprScalarSubqueryQuantified(Edxlopid edxlopid, IMDId *scalar_op_mdid,
...
CTranslatorDXLToExpr::PexprScalarOp(const CDXLNode *pdxlnOpExpr)
{
	...
	CScalarOp *pscop = GPOS_NEW(m_mp)
		CScalarOp(m_mp, mdid, return_type_mdid,
...
CTranslatorDXLToExpr::PexprScalarNullIf(const CDXLNode *pdxlnNullIf)
{
	...
	return GPOS_NEW(m_mp) CExpression(m_mp, GPOS_NEW(m_mp) CScalarNullIf(m_mp, mdid_op, mdid_type),
...
CTranslatorDXLToExpr::PexprWindowFunc(const CDXLNode *pdxlnWindowRef)
{
	...
	CScalarWindowFunc *popWindowFunc = GPOS_NEW(m_mp) CScalarWindowFunc(
		m_mp, mdid_func, mdid_return_type, str_name, ews, pdxlopWinref->IsDistinct(),
