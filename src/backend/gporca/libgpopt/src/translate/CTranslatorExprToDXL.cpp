...
//
//---------------------------------------------------------------------------
CTranslatorExprToDXL::CTranslatorExprToDXL(CMemoryPool *mp, CMDAccessor *md_accessor,
										   IntPtrArray *pdrgpiSegments, BOOL fInitColumnFactory)
	: m_mp(mp),
	  m_pmda(md_accessor),
	  m_pdpplan(NULL),
	  m_pcf(NULL),
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnTranslate(CExpression *pexpr, CColRefArray *colref_array,
									 CMDNameArray *pdrgpmdname)
{
	...
	BOOL fDML = false;
	CDXLNode *dxlnode = CreateDXLNode(pexpr, colref_array, pdrgpdsBaseTables, &ulNonGatherMotions,
									  &fDML, true /*fRemap*/, true /*fRoot*/);
	...
	{
		CTranslatorExprToDXLUtils::SetDirectDispatchInfo(m_mp, m_pmda, dxlnode, pexpr,
														 pdrgpdsBaseTables);
	}
...
CTranslatorExprToDXL::CreateDXLNode(CExpression *pexpr, CColRefArray *colref_array,
									...
									ULONG *pulNonGatherMotions, BOOL *pfDML, BOOL fRemap,
									BOOL fRoot)
{
	...
	{
		CDXLNode *dxlnode =
			PdxlnTblScan(pexpr, NULL /*pcrsOutput*/, colref_array, pdrgpdsBaseTables,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnTblScan(CExpression *pexprTblScan, CColRefSet *pcrsOutput,
								   ...
								   CExpression *pexprScalar, CDXLPhysicalProperties *dxl_properties)
{
	...
	CDXLTableDescr *table_descr =
		MakeDXLTableDescr(popTblScan->Ptabdesc(), pdrgpcrOutput, pexprTblScan->Prpp());
									 ...
									 CDXLPhysicalProperties *dxl_properties, CReqdPropPlan *prpp)
{
	...
	// translate table descriptor
	CDXLTableDescr *table_descr =
		MakeDXLTableDescr(popIs->Ptabdesc(), pdrgpcrOutput, pexprIndexScan->Prpp());
										 ...
										 CReqdPropPlan *prpp)
{
	...
	// translate table descriptor
	CDXLTableDescr *table_descr =
		MakeDXLTableDescr(popIs->Ptabdesc(), pdrgpcrOutput, pexprIndexOnlyScan->Prpp());
	...
	// TODO: vrgahavan; we assume that the index are always forward access.
	// create the physical index scan operator
	CDXLPhysicalIndexOnlyScan *dxl_op = GPOS_NEW(m_mp)
		CDXLPhysicalIndexOnlyScan(m_mp, table_descr, dxl_index_descr, EdxlisdForward);
...
#ifdef GPOS_DEBUG
	pdxlnIndexOnlyScan->GetOperator()->AssertValid(pdxlnIndexOnlyScan,
...
CTranslatorExprToDXL::PdxlnBitmapBoolOp(CExpression *pexprBitmapBoolOp)
{
	...
	return GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarBitmapBoolOp(m_mp, mdid_type, edxlbitmapop),
...
void
CTranslatorExprToDXL::AddBitmapFilterColumns(
	CMemoryPool *mp, CPhysicalScan *pop, CExpression *pexprRecheckCond, CExpression *pexprScalar,
...
CDXLNode *
CTranslatorExprToDXL::PdxlnBitmapTableScan(CExpression *pexprBitmapTableScan,
										   CColRefSet *pcrsOutput, CColRefArray *colref_array,
										   CDistributionSpecArray *pdrgpdsBaseTables,
...
)
{
	...
	return PdxlnDynamicTableScan(pexprDTS, colref_array, pdrgpdsBaseTables, pexprScalarCond,
								 dxl_properties);
}
...
)
{
	...
	CDXLPhysicalProperties *dxl_properties = NULL;
	return PdxlnDynamicBitmapTableScan(pexprScan, colref_array, pdrgpdsBaseTables, pexprScalar,
									   dxl_properties);
}
												  ...
												  CDXLPhysicalProperties *dxl_properties)
{
	...
	CDXLPhysicalDynamicBitmapTableScan *pdxlopScan =
		GPOS_NEW(m_mp) CDXLPhysicalDynamicBitmapTableScan(m_mp, table_descr,
														  pop->UlSecondaryScanId(), pop->ScanId());
											...
											CReqdPropPlan *prpp)
{
	...
	// create the physical index scan operator
	CDXLNode *pdxlnDIS =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLPhysicalDynamicIndexScan(
								  ...
								  ULONG *pulNonGatherMotions, BOOL *pfDML, CDXLNode *pdxlnScalar)
{
	...
	return PdxlnResult(pexprRelational, colref_array, pdrgpdsBaseTables, pulNonGatherMotions, pfDML,
								  ...
								  CDXLPhysicalProperties *dxl_properties)
{
	...
	CDXLNode *pdxlnRelationalChild =
		CreateDXLNode(pexprRelational, colref_array, pdrgpdsBaseTables, pulNonGatherMotions, pfDML,
					  false /*fRemap*/, false /*fRoot */);
	GPOS_ASSERT(NULL != pexprRelational->Prpp());
	CColRefSet *pcrsOutput = pexprRelational->Prpp()->PcrsRequired();

	return PdxlnAddScalarFilterOnRelationalChild(pdxlnRelationalChild, pdxlnScalar, dxl_properties,
								  ...
								  ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	CDXLNode *pdxlnode = PdxlnFromFilter(pexprFilter, colref_array, pdrgpdsBaseTables,
														 ...
														 CDistributionSpecArray *pdrgpdsBaseTables)
{
		...
		isGist)
	{
		...
		if (COperator::EopPhysicalIndexScan == op_id)
		{
			pdxlnIndexScan = PdxlnIndexScan(pexprNewIndexScan, colref_array, dxl_properties,
											pexprIndexScan->Prpp());
		}
		else if (COperator::EopPhysicalIndexOnlyScan == op_id)
		{
			pdxlnIndexScan = PdxlnIndexOnlyScan(pexprNewIndexScan, colref_array, dxl_properties,
												pexprIndexScan->Prpp());
		}
		else
		{
			pdxlnIndexScan = PdxlnDynamicIndexScan(pexprNewIndexScan, colref_array, dxl_properties,
												   pexprIndexScan->Prpp());
		}
	...
	if (COperator::EopPhysicalIndexScan == op_id)
	{
		return PdxlnIndexScan(pexprIndexScan, colref_array, pdrgpdsBaseTables, &ulNonGatherMotions,
							  &fDML);
	}
	if (COperator::EopPhysicalIndexOnlyScan == op_id)
	{
		return PdxlnIndexOnlyScan(pexprIndexScan, colref_array, pdrgpdsBaseTables,
								  &ulNonGatherMotions, &fDML);
	}

	return PdxlnDynamicIndexScan(pexprIndexScan, colref_array, pdrgpdsBaseTables,
								 &ulNonGatherMotions, &fDML);
}
									  ...
									  CDXLPhysicalProperties *dxl_properties)
{
	...
	{
		return CreateDXLNode(pexprRelational, colref_array, pdrgpdsBaseTables, pulNonGatherMotions,
	...
	switch (eopidRelational)
	{
		...
		case COperator::EopPhysicalExternalScan:
		{
			...
			return PdxlnTblScan(pexprRelational, pcrsOutput, NULL /* colref_array */,
								pdrgpdsBaseTables, pexprScalar, dxl_properties /* cost info */
			);
		}
		case COperator::EopPhysicalBitmapTableScan:
		{
			...
			return PdxlnBitmapTableScan(pexprRelational, pcrsOutput, NULL /*colref_array*/,
		...
		case COperator::EopPhysicalDynamicTableScan:
		{
			...
			return PdxlnDynamicTableScan(pexprRelational, colref_array, pdrgpdsBaseTables,
		...
		{
			dxl_properties->AddRef();
			return PdxlnIndexScanWithInlinedCondition(pexprRelational, pexprScalar, dxl_properties,
													  colref_array, pdrgpdsBaseTables);
		}
		case COperator::EopPhysicalDynamicBitmapTableScan:
		{
			...
			return PdxlnDynamicBitmapTableScan(pexprRelational, colref_array, pdrgpdsBaseTables,
											   pexprScalar, dxl_properties);
		}
		default:
		{
			return PdxlnResultFromFilter(pexprFilter, colref_array, pdrgpdsBaseTables,
...
CDXLNode *
CTranslatorExprToDXL::PdxlnPartitionSelectorWithInlinedCondition(
	CExpression *pexprFilter, CColRefArray *colref_array, CDistributionSpecArray *pdrgpdsBaseTables,
	ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	GPOS_ASSERT(NULL != pexprFilter);
	...
	return PdxlnPartitionSelector(pexprRelational, colref_array, pdrgpdsBaseTables,
								  pulNonGatherMotions, pfDML, pexprCond, dxl_properties);
}
											...
											ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	CDXLNode *child_dxlnode =
		CreateDXLNode(pexprRelational, NULL /* colref_array */, pdrgpdsBaseTables,
	...
	if (one_time_filter_quals_exprs->Size() > 0)
	{
		...
		CExpression *scalar_cmp_expr =
			CPredicateUtils::PexprConjunction(m_mp, one_time_filter_quals_exprs);
									   ...
									   ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	// translate relational child expression
	CDXLNode *child_dxlnode =
		CreateDXLNode(pexprRelational, pdrgpcrRequired, pdrgpdsBaseTables, pulNonGatherMotions,
					  pfDML, true /*fRemap*/, false /*fRoot */);
	...
	CDXLNode *pdxlnCTEProducer =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLPhysicalCTEProducer(m_mp, ulCTEId, colids),
								  ...
								  ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		CDXLNode *child_dxlnode =
			CreateDXLNode(pexprChild, requiredInput, pdrgpdsBaseTables, pulNonGatherMotions, pfDML,
						  false /*fRemap*/, false /*fRoot*/);
		...
		CDXLNode *pdxlnChildProjected = PdxlnRemapOutputColumns(
			pexprChild, child_dxlnode, requiredInput /* required input columns */,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnRemapOutputColumns(CExpression *pexpr, CDXLNode *dxlnode,
											  CColRefArray *pdrgpcrRequired,
											  CColRefArray *pdrgpcrOrder)
{
	GPOS_ASSERT(NULL != pexpr);
												   ...
												   CExpression *pexprScalar)
{
	...
	else
	{
		...
		pdrgpdatum->Release();
		return CTranslatorExprToDXLUtils::PdxlnResult(m_mp, GetProperties(pexprCTG), pdxlnPrL,
													  PdxlnFilter(NULL), one_time_filter,
													  NULL	//child_dxlnode
		);
	}
}
									 ...
									 ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	return PdxlnAggregate(pexprAgg, colref_array, pdrgpdsBaseTables, pulNonGatherMotions, pfDML,
						  dxl_agg_strategy, pdrgpcrGroupingCols, NULL /*pcrsKeys*/
	);
}
										  ...
										  ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	CDXLNode *pdxlnAgg =
		PdxlnAggregate(pexprAgg, colref_array, pdrgpdsBaseTables, pulNonGatherMotions, pfDML,
								...
								ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	// translate relational child expression
	CDXLNode *child_dxlnode =
		CreateDXLNode(pexprChild, colref_array, pdrgpdsBaseTables, pulNonGatherMotions, pfDML,
					  false /*fRemap*/, false /*fRoot*/);
								 ...
								 ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	{
		return CreateDXLNode(pexprChild, colref_array, pdrgpdsBaseTables, pulNonGatherMotions,
							 pfDML, true /*fRemap*/, false /*fRoot*/);
	}
	...
	CDistributionSpecArray *pdrgpdsBaseTables, ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	if (EdxlSubPlanTypeScalar == dxl_subplan_type)
	{
		...
		BuildScalarSubplans(pdrgpcrInner, pexprInner, dxl_colref_array, pdrgpdsBaseTables,
							pulNonGatherMotions, pfDML);
	...
	CDistributionSpecArray *pdrgpdsBaseTables, ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	switch (op_id)
	{
		...
		case COperator::EopPhysicalCorrelatedNotInLeftAntiSemiNLJoin:
			pdxlnSubPlan =
				PdxlnQuantifiedSubplan(pdrgpcrInner, pexprCorrelatedNLJoin, dxl_colref_array,
									   pdrgpdsBaseTables, pulNonGatherMotions, pfDML);
			pdxlnSubPlan->AddRef();
		...
		case COperator::EopPhysicalCorrelatedLeftAntiSemiNLJoin:
			pdxlnSubPlan =
				PdxlnExistentialSubplan(pdrgpcrInner, pexprCorrelatedNLJoin, dxl_colref_array,
										pdrgpdsBaseTables, pulNonGatherMotions, pfDML);
			pdxlnSubPlan->AddRef();
			*ppdxlnScalar = pdxlnSubPlan;
			return;
											 ...
											 ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	CDXLNode *pdxlnInnerChild =
		CreateDXLNode(pexprInner, NULL /*colref_array*/, pdrgpdsBaseTables, pulNonGatherMotions,
	...
	CDXLNode *pdxlnSubPlan = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarSubPlan(m_mp, mdid, dxl_colref_array,
											  ...
											  ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	EdxlSubPlanType dxl_subplan_type = Edxlsubplantype(pexprCorrelatedNLJoin);
	GPOS_ASSERT_IMP(fCorrelatedLOJ, EdxlSubPlanTypeExists == dxl_subplan_type ||
										EdxlSubPlanTypeNotExists == dxl_subplan_type);
	...
	CDXLNode *pdxlnSubPlan = GPOS_NEW(m_mp) CDXLNode(
		m_mp, GPOS_NEW(m_mp) CDXLScalarSubPlan(m_mp, mdid, dxl_colref_array, dxl_subplan_type,
											...
											ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	{
		BuildSubplans(pexpr, dxl_colref_array, &pdxlnCond, pdrgpdsBaseTables, pulNonGatherMotions,
					  pfDML);
	}
	...
	switch (pexprOuterChild->Pop()->Eopid())
	{
		case COperator::EopPhysicalTableScan:
		{
			...
			dxlnode =
				PdxlnTblScanFromNLJoinOuter(pexprOuterChild, pdxlnCond, colref_array,
											pdrgpdsBaseTables, pulNonGatherMotions, dxl_properties);
			break;
		}
		...
		{
			dxl_properties->AddRef();
			dxlnode = PdxlnResultFromNLJoinOuter(pexprOuterChild, pdxlnCond, colref_array,
												 pdrgpdsBaseTables, pulNonGatherMotions, pfDML,
												 dxl_properties);
			break;
		}
		...
		default:
		{
			// create a result node over outer child
			dxl_properties->AddRef();
			dxlnode = PdxlnResult(pexprOuterChild, colref_array, pdrgpdsBaseTables,
								  pulNonGatherMotions, pfDML, pdxlnCond, dxl_properties);
		}
	}
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnScBoolExpr(EdxlBoolExprType boolexptype, CDXLNode *dxlnode_left,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnResultFromNLJoinOuter(CExpression *pexprOuterChildRelational,
												 ...
												 CDXLPhysicalProperties *dxl_properties)
{
	...
	CDXLNode *pdxlnRelationalNew =
		PdxlnFromFilter(pexprOuterChildRelational, colref_array, pdrgpdsBaseTables,
						pulNonGatherMotions, pfDML, dxl_properties);
	dxl_properties->Release();
	...
	switch (edxlopid)
	{
		...
		case EdxlopPhysicalSequence:
		{
			...
			pdxlnRelationalNew = PdxlnAddScalarFilterOnRelationalChild(
				pdxlnRelationalNew, pdxlnJoinCond, dxl_properties, pcrsOutput, colref_array);
		}
		break;
		default:
			pdxlnJoinCond->Release();
								  ...
								  ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	CDXLNode *pdxlnOuterChild =
		CreateDXLNode(pexprOuterChild, NULL /*colref_array*/, pdrgpdsBaseTables,
					  pulNonGatherMotions, pfDML, false /*fRemap*/, false /*fRoot*/);
	CDXLNode *pdxlnInnerChild =
		CreateDXLNode(pexprInnerChild, NULL /*colref_array*/, pdrgpdsBaseTables,
					  pulNonGatherMotions, pfDML, false /*fRemap*/, false /*fRoot*/);
									 ...
									 ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		GPOS_ASSERT(
			CPhysicalJoin::FMergeJoinCompatible(pexprPred, pexprOuterChild, pexprInnerChild));
									...
									ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		CDXLNode *child_dxlnode =
			CreateDXLNode(pexprChild, pdrgpcrChildOutput, pdrgpdsBaseTables, pulNonGatherMotions,
						  pfDML, false /*fRemap*/, false /*fRoot*/);
...
CTranslatorExprToDXL::PdxlnPartitionSelector(CExpression *pexpr, CColRefArray *colref_array,
											 ...
											 ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	return PdxlnPartitionSelector(pexpr, colref_array, pdrgpdsBaseTables, pulNonGatherMotions,
								  pfDML, NULL /*pexprScalarCond*/, NULL /*dxl_properties*/);
}
											 ...
											 CDXLPhysicalProperties *dxl_properties)
{
	...
	if (CUtils::FScalarConstTrue(pexprScalar))
	{
		return PdxlnPartitionSelectorExpand(pexpr, colref_array, pdrgpdsBaseTables,
											pulNonGatherMotions, pfDML, pexprScalarCond,
											dxl_properties);
	}

	return PdxlnPartitionSelectorFilter(pexpr, colref_array, pdrgpdsBaseTables, pulNonGatherMotions,
										pfDML, pexprScalarCond, dxl_properties);
}
												...
												ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	TranslatePartitionFilters(pexpr, true /*fPassThrough*/, &pdxlnEqFilters, &pdxlnFilters,
							  &pdxlnResidual);
	...
	CDXLNode *pdxlnSelector = CTranslatorExprToDXLUtils::PdxlnPartitionSelector(
		...
		0,	// scan_id
		GetProperties(pexpr), pdxlnPrL, pdxlnEqFilters, pdxlnFilters, pdxlnResidual,
		pdxlnPropagation, pdxlnPrintable, child_dxlnode);
												   ...
												   CDXLPhysicalProperties *dxl_properties)
{
	...
	// translate child
	CDXLNode *child_dxlnode =
		PdxlnPartitionSelectorChild(pexprChild, pexprScalarCond, dxl_properties, colref_array,
									pdrgpdsBaseTables, pulNonGatherMotions, pfDML);
	...
	CDXLNode *pdxlnSelector = CTranslatorExprToDXLUtils::PdxlnPartitionSelector(
		m_mp, rel_mdid, ulLevels, scan_id, CTranslatorExprToDXLUtils::GetProperties(m_mp), pdxlnPrL,
		pdxlnEqFilters, pdxlnFilters, pdxlnResidual, pdxlnPropagation, pdxlnPrintable);
		...
		if (NULL != pexprEqFilter)
		{
			...
			if (fRangePart)
			{
				filter_dxlnode = CTranslatorExprToDXLUtils::PdxlnRangeFilterEqCmp(
					...
					NULL /*pmdidTypeCastExpr*/, NULL /*mdid_cast_func*/, ulLevel);
			}
			else  // list partition
			{
				...
				filter_dxlnode = CTranslatorExprToDXLUtils::PdxlnListFilterScCmp(
					m_mp, m_pmda, pdxlnPartKey, pdxlnEq, pmdidTypePartKey, pmdidTypeOther,
					IMDType::EcmptEq, ulLevel, fDefaultPartition);
			}
		}
		...
		if (NULL != pexprFilter)
		{
			...
			for (ULONG ul = 0; ul < length; ul++)
			{
				CDXLNode *pdxlnScCmp = PdxlnPredOnPartKey(
					(*pdrgpexprConjuncts)[ul], pcrPartKey, pmdidTypePartKey, ulLevel, fRangePart,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnPredOnPartKey(CExpression *pexprPred, CColRef *pcrPartKey,
...
)
{
	...
	{
		return PdxlnScCmpPartKey(pexprPred, pcrPartKey, pmdidTypePartKey, ulPartLevel, fRangePart,
	...
	{
		return PdxlArrayExprOnPartKey(pexprPred, pcrPartKey, pmdidTypePartKey, ulPartLevel,
									  fRangePart, pfLTComparison, pfGTComparison, pfEQComparison);
	}

	GPOS_ASSERT(CPredicateUtils::FOr(pexprPred) || CPredicateUtils::FAnd(pexprPred));

	return PdxlnConjDisjOnPartKey(pexprPred, pcrPartKey, pmdidTypePartKey, ulPartLevel, fRangePart,
								  pfLTComparison, pfGTComparison, pfEQComparison);
}
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlArrayExprOnPartKey(CExpression *pexprPred, CColRef *pcrPartKey,
											 IMDId *pmdidTypePartKey, ULONG ulPartLevel,
											 BOOL fRangePart,
											 BOOL *pfLTComparison,	// input/output
											 BOOL *pfGTComparison,	// input/output
...
)
{
	...
	CDXLNode *dxlnode =
		PdxlnConjDisjOnPartKey(pexprDisj, pcrPartKey, pmdidTypePartKey, ulPartLevel, fRangePart,
							   pfLTComparison, pfGTComparison, pfEQComparison);
	pexprDisj->Release();
	pci->Release();
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnConjDisjOnPartKey(CExpression *pexprPred, CColRef *pcrPartKey,
											 IMDId *pmdidTypePartKey, ULONG ulPartLevel,
											 BOOL fRangePart,
											 BOOL *pfLTComparison,	// input/output
											 BOOL *pfGTComparison,	// input/output
...
)
{
	...
	for (ULONG ul = 0; ul < ulChildren; ul++)
	{
		...
		CDXLNode *child_dxlnode =
			PdxlnPredOnPartKey(pexprChild, pcrPartKey, pmdidTypePartKey, ulPartLevel, fRangePart,
							   pfLTComparison, pfGTComparison, pfEQComparison);
		...
		{
			pdxlnPred = CTranslatorExprToDXLUtils::PdxlnCombineBoolean(m_mp, pdxlnPred,
																	   child_dxlnode, edxlbet);
		}
	}
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnScCmpPartKey(CExpression *pexprScCmp, CColRef *pcrPartKey,
										IMDId *pmdidTypePartKey, ULONG ulPartLevel, BOOL fRangePart,
										BOOL *pfLTComparison,  // input/output
										BOOL *pfGTComparison,  // input/output
										BOOL *pfEQComparison   // input/output
)
{
	...
	CPredicateUtils::ExtractComponents(pexprScCmp, pcrPartKey, &pexprPartKey, &pexprOther,
									   &cmp_type);
	...
	*pfEQComparison = *pfEQComparison || IMDType::EcmptEq == cmp_type;
	...
	if (fRangePart)	 // range partition
	{
		...
		CTranslatorExprToDXLUtils::ExtractCastFuncMdids(pexprNewPartKey->Pop(), &pmdidTypeCastExpr,
														&mdid_cast_func);

		return CTranslatorExprToDXLUtils::PdxlnRangeFilterScCmp(
			m_mp, m_pmda, pdxlnOther, pmdidTypePartKey, pmdidTypeOther, pmdidTypeCastExpr,
			mdid_cast_func, cmp_type, ulPartLevel);
	}
	else  // list partition
	{
		...
		return CTranslatorExprToDXLUtils::PdxlnListFilterScCmp(
			m_mp, m_pmda, pdxlnPartKeyExpr, pdxlnOther, pmdidTypePartKeyExpr, pmdidTypeOther,
			cmp_type, ulPartLevel, true);
	}
}
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorExprToDXL::PdxlnScNullTestPartKey(IMDId *pmdidTypePartKey, ULONG ulPartLevel,
											 BOOL fRangePart, BOOL is_null)
{
	if (!fRangePart)  // list partition
	{
		...
		return GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarBoolExpr(m_mp, Edxlor),
	...
	CDXLNode *pdxlnPredicateMin = GPOS_NEW(m_mp) CDXLNode(
		...
		GPOS_NEW(m_mp)
			CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarPartBound(m_mp, ulPartLevel, pmdidTypePartKey,
															  true /*is_lower_bound*/)));
	...
	return GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarBoolExpr(m_mp, Edxland),
												  ...
												  ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	{
		case COperator::EopPhysicalDynamicTableScan:
			return PdxlnDynamicTableScan(pexprChild, colref_array, pdrgpdsBaseTables,
										 pexprScalarCond, dxl_properties);
		case COperator::EopPhysicalDynamicIndexScan:
			return PdxlnIndexScanWithInlinedCondition(pexprChild, pexprScalarCond, dxl_properties,
													  colref_array, pdrgpdsBaseTables);
		default:
			GPOS_ASSERT(COperator::EopPhysicalDynamicBitmapTableScan == pexprChild->Pop()->Eopid());
			return PdxlnDynamicBitmapTableScan(pexprChild, colref_array, pdrgpdsBaseTables,
											   pexprScalarCond, dxl_properties);
	}
}
							   ...
							   ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	*pfDML = false;
	...
	CDXLNode *child_dxlnode =
		CreateDXLNode(pexprChild, pdrgpcrSource, pdrgpdsBaseTables, pulNonGatherMotions, pfDML,
					  false /*fRemap*/, false /*fRoot*/);

	CDXLTableDescr *table_descr =
		MakeDXLTableDescr(ptabdesc, NULL /*pdrgpcrOutput*/, NULL /*requiredProperties*/);
	...
	CDXLPhysicalDML *pdxlopDML = GPOS_NEW(m_mp) CDXLPhysicalDML(
		m_mp, dxl_dml_type, table_descr, pdrgpul, action_colid, oid_colid, ctid_colid, segid_colid,
								...
								ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	for (ULONG ul = 0; ul < ulColumns; ul++)
	{
		...
		CDXLColDescr *pdxlcd = GPOS_NEW(m_mp)
			CDXLColDescr(m_mp, pmdnameCol, colref->Id(), pcd->AttrNum(), pmdidColType,
	...
	CDXLPhysicalCTAS *pdxlopCTAS = GPOS_NEW(m_mp) CDXLPhysicalCTAS(
		m_mp, mdname_schema, GPOS_NEW(m_mp) CMDName(m_mp, pmdrel->Mdname().GetMDName()),
		dxl_col_descr_array, pmdrel->GetDxlCtasStorageOption(), pmdrel->GetRelDistribution(),
		pdrgpulDistr, pmdrel->IsTemporary(), pmdrel->HasOids(), pmdrel->RetrieveRelStorageType(),
		pdrgpul, vartypemod_array);
								 ...
								 ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	CDXLNode *child_dxlnode =
		CreateDXLNode(pexprChild, pdrgpcrRequired, pdrgpdsBaseTables, pulNonGatherMotions, pfDML,
					  true /*fRemap*/, false /*fRoot*/);
	pdrgpcrRequired->Release();
	pcrsRequired->Release();

	CDXLPhysicalSplit *pdxlopSplit =
		GPOS_NEW(m_mp) CDXLPhysicalSplit(m_mp, delete_colid_array, insert_colid_array, action_colid,
...
CTranslatorExprToDXL::PdxlnScFuncExpr(CExpression *pexprFunc)
{
	...
	CDXLNode *pdxlnFuncExpr = GPOS_NEW(m_mp) CDXLNode(
		m_mp, GPOS_NEW(m_mp) CDXLScalarFuncExpr(m_mp, mdid_func, mdid_return_type,
...
CTranslatorExprToDXL::PdxlnScWindowFuncExpr(CExpression *pexprWindowFunc)
{
	...
	CDXLScalarWindowRef *pdxlopWindowref = GPOS_NEW(m_mp)
		CDXLScalarWindowRef(m_mp, mdid_func, mdid_return_type, popScWindowFunc->IsDistinct(),
								  ...
								  ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	CDXLNode *child_dxlnode =
		CreateDXLNode((*pexprSeqPrj)[0], NULL /* colref_array */, pdrgpdsBaseTables,
					  pulNonGatherMotions, pfDML, false /*fRemap*/, false /*fRoot*/);
...
CTranslatorExprToDXL::PdxlnArray(CExpression *pexpr)
{
	...
	CDXLNode *pdxlnArray = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarArray(m_mp, elem_type_mdid, array_type_mdid,
...
CTranslatorExprToDXL::PdxlnArrayRef(CExpression *pexpr)
{
	...
	CDXLNode *pdxlnArrayref = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarArrayRef(m_mp, elem_type_mdid, pop->TypeModifier(),
...
CTranslatorExprToDXL::PdxlnArrayCmp(CExpression *pexpr)
{
	...
	CDXLNode *pdxlnArrayCmp = GPOS_NEW(m_mp) CDXLNode(
		m_mp, GPOS_NEW(m_mp) CDXLScalarArrayComp(
				  m_mp, mdid_op, GPOS_NEW(m_mp) CWStringConst(m_mp, str_opname->GetBuffer()),
				  edxlarrcmpt));
...
CTranslatorExprToDXL::PdxlnProjList(const CExpression *pexprProjList,
									const CColRefSet *pcrsRequired, CColRefArray *colref_array)
{
	if (NULL == colref_array)
	{
		// no order specified
	...
	CHashMap<ULONG, CDXLNode, gpos::HashValue<ULONG>, gpos::Equals<ULONG>, CleanupDelete<ULONG>,
		...
		GPOS_NEW(m_mp) CHashMap<ULONG, CDXLNode, gpos::HashValue<ULONG>, gpos::Equals<ULONG>,
										...
										const IMdIdArray *opfamilies)
{
	...
	for (ULONG ul = 0; ul < pdrgpexpr->Size(); ul++)
	{
		...
		CDXLNode *pdxlnHashExpr =
			GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarHashExpr(m_mp, opfamily));
...
CDXLNode *
CTranslatorExprToDXL::PdxlnResult(CDXLPhysicalProperties *dxl_properties, CDXLNode *pdxlnPrL,
								  CDXLNode *child_dxlnode)
{
	...
	return CTranslatorExprToDXLUtils::PdxlnResult(
		m_mp, dxl_properties, pdxlnPrL, PdxlnFilter(NULL), pdxlnScalarOneTimeFilter,
		pdxlnMaterialize ? pdxlnMaterialize : child_dxlnode);
}
