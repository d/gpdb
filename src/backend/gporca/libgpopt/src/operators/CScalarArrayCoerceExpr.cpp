...
//
//---------------------------------------------------------------------------
CScalarArrayCoerceExpr::CScalarArrayCoerceExpr(CMemoryPool *mp, IMDId *element_func, IMDId *result_type_mdid,
...
CScalarArrayCoerceExpr::Matches(COperator *pop) const
{
	...
	return popCoerce->PmdidElementFunc()->Equals(m_pmdidElementFunc) && popCoerce->MdidType()->Equals(MdidType()) &&
		   popCoerce->TypeModifier() == TypeModifier() && popCoerce->IsExplicit() == m_is_explicit &&
