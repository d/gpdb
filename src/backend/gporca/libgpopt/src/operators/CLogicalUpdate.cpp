...
CLogicalUpdate::CLogicalUpdate(CMemoryPool *mp, CTableDescriptor *ptabdesc, CColRefArray *pdrgpcrDelete,
							   CColRefArray *pdrgpcrInsert, CColRef *pcrCtid, CColRef *pcrSegmentId,
							   CColRef *pcrTupleOid)
	: CLogical(mp),
	  m_ptabdesc(ptabdesc),
...
CLogicalUpdate::Matches(COperator *pop) const
{
	...
	return m_pcrCtid == popUpdate->PcrCtid() && m_pcrSegmentId == popUpdate->PcrSegmentId() &&
		   m_pcrTupleOid == popUpdate->PcrTupleOid() && m_ptabdesc->MDId()->Equals(popUpdate->Ptabdesc()->MDId()) &&
...
//---------------------------------------------------------------------------
COperator *
CLogicalUpdate::PopCopyWithRemappedColumns(CMemoryPool *mp, UlongToColRefMap *colref_mapping, BOOL must_exist)
{
	...
	return GPOS_NEW(mp)
		CLogicalUpdate(mp, m_ptabdesc, pdrgpcrDelete, pdrgpcrInsert, pcrCtid, pcrSegmentId, pcrTupleOid);
}
