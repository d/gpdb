...
CLogicalNAryJoin::DerivePropertyConstraint(CMemoryPool *mp, CExpressionHandle &exprhdl) const
{
	...
	for (ULONG ul = 0; ul < arity - 1; ul++)
	{
		if (IsInnerJoinChild(ul))
		{
			...
			CColRefSetArray *pdrgpcrsMerged =
				CUtils::PdrgpcrsMergeEquivClasses(mp, equivalenceClasses, pdrgpcrsChild);
	...
	if (NULL != trueInnerJoinPreds)
	{
		...
		CConstraint *pcnstr = CConstraint::PcnstrFromScalarExpr(mp, trueInnerJoinPreds,
																&equivClassesFromInnerJoinPreds);
...
//---------------------------------------------------------------------------
CExpression *
CLogicalNAryJoin::ReplaceInnerJoinPredicates(CMemoryPool *mp,
