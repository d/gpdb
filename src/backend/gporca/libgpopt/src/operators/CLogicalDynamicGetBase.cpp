...
CLogicalDynamicGetBase::CLogicalDynamicGetBase(CMemoryPool *mp, const CName *pnameAlias, CTableDescriptor *ptabdesc,
											   ...
											   CColRef2dArray *pdrgpdrgpcrPart, ULONG ulSecondaryScanId,
	  ...
	  m_pcrsDist(NULL)
{
	...
	GPOS_ASSERT_IMP(is_partial,
					NULL != m_part_constraint->PcnstrCombined() && "Partial scan with unsupported constraint type");
	  ...
	  m_pcrsDist(NULL)
{
	...
	m_part_constraint =
		GPOS_NEW(mp) CPartConstraint(mp, phmulcnstr, pbsDefaultParts, true /*is_unbounded*/, m_pdrgpdrgpcrPart);
...
//---------------------------------------------------------------------------
IStatistics *
CLogicalDynamicGetBase::PstatsDeriveFilter(CMemoryPool *mp, CExpressionHandle &exprhdl, CExpression *pexprFilter) const
{
	...
	CStatsPred *pred_stats = CStatsPredUtils::ExtractPredStats(mp, pexprFilterNew, NULL /*outer_refs*/
	);
	pexprFilterNew->Release();

	IStatistics *result_stats =
		CFilterStatsProcessor::MakeStatsFilter(mp, pstatsFullTable, pred_stats, true /* do_cap_NDVs */);
	pred_stats->Release();
