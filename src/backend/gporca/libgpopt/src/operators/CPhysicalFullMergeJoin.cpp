...
CPhysicalFullMergeJoin::CPhysicalFullMergeJoin(CMemoryPool *mp, CExpressionArray *outer_merge_clauses,
	...
	: CPhysicalJoin(mp), m_outer_merge_clauses(outer_merge_clauses), m_inner_merge_clauses(inner_merge_clauses)
{
	GPOS_ASSERT(NULL != mp);
...
CDistributionSpec *
CPhysicalFullMergeJoin::PdsRequired(CMemoryPool *mp, CExpressionHandle &exprhdl, CDistributionSpec *pdsRequired,
									ULONG child_index,
...
CRewindabilitySpec *
CPhysicalFullMergeJoin::PrsRequired(CMemoryPool *mp, CExpressionHandle &exprhdl, CRewindabilitySpec *prsRequired,
									ULONG child_index,
									CDrvdPropArray *,  // pdrgpdpCtxt
									ULONG			   // ulOptReq
) const
...
CDistributionSpec *
CPhysicalFullMergeJoin::PdsDerive(CMemoryPool *mp, CExpressionHandle &exprhdl) const
{
	...
	if (CDistributionSpec::EdtHashed == pdsOuter->Edt() && CDistributionSpec::EdtHashed == pdsInner->Edt())
	{
		...
		CDistributionSpecHashed *pds =
			GPOS_NEW(mp) CDistributionSpecHashed(pdshashedOuter->Pdrgpexpr(), false /* fNullsCollocated */);
