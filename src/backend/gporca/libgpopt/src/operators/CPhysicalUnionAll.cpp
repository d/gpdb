...
CPhysicalUnionAll::CPhysicalUnionAll(CMemoryPool *mp, CColRefArray *pdrgpcrOutput,
...
void
CPhysicalUnionAll::PopulateDistrSpecs(CMemoryPool *mp, CColRefArray *pdrgpcrOutput,
									  CColRef2dArray *pdrgpdrgpcrInput)
{
	...
	for (ULONG ulChild = 0; ulChild < arity; ulChild++)
	{
		...
		CDistributionSpec *pdshashed = CDistributionSpecHashed::MakeHashedDistrSpec(
...
//---------------------------------------------------------------------------
CCTEReq *
CPhysicalUnionAll::PcteRequired(CMemoryPool *mp, CExpressionHandle &exprhdl, CCTEReq *pcter,
...
CPhysicalUnionAll::PdsStrictRandomParallelUnionAllChildren(CMemoryPool *mp,
														   CExpressionHandle &expr_handle) const
{
	if (COperator::EopPhysicalParallelUnionAll == expr_handle.Pop()->Eopid())
	{
		...
		for (ULONG idx = 0;
			 has_motion_random && has_strict_random_spec && idx < expr_handle.Arity(); idx++)
		{
			...
			has_motion_random =
				COperator::EopPhysicalMotionRandom == expr_handle.Pop(idx /*child_index*/)->Eopid();
...
CPhysicalUnionAll::PdshashedDerive(CMemoryPool *mp, CExpressionHandle &exprhdl) const
{
	...
	{
		pdrgpulOuter = PdrgpulMap(mp, CDistributionSpecHashed::PdsConvert(pdsHashed)->Pdrgpexpr(),
								  0 /*child_index*/);
	...
	for (ULONG ulChild = 1; fSuccess && ulChild < arity; ulChild++)
	{
		...
		while (pdsChildHashed && !equi_hash_spec_matches)
		{
			pdrgpulChild = PdrgpulMap(
				mp, CDistributionSpecHashed::PdsConvert(pdsChildHashed)->Pdrgpexpr(), ulChild);
...
CColRefSet *
CPhysicalUnionAll::MapOutputColRefsToInput(CMemoryPool *mp, CColRefSet *out_col_refs,
										   ULONG child_index)
{
...
void
CheckChildDistributions(CMemoryPool *mp, CExpressionHandle &exprhdl, BOOL fSingletonChild,
