...
const WCHAR CLogicalDML::m_rgwszDml[EdmlSentinel][10] = {GPOS_WSZ_LIT("Insert"), GPOS_WSZ_LIT("Delete"),
														 GPOS_WSZ_LIT("Update")};
...
CLogicalDML::CLogicalDML(CMemoryPool *mp, EDMLOperator edmlop, CTableDescriptor *ptabdesc, CColRefArray *pdrgpcrSource,
						 CBitSet *pbsModified, CColRef *pcrAction, CColRef *pcrTableOid, CColRef *pcrCtid,
	  ...
	  m_pcrTupleOid(pcrTupleOid)
{
	...
	GPOS_ASSERT_IMP(EdmlDelete == edmlop || EdmlUpdate == edmlop, NULL != pcrCtid && NULL != pcrSegmentId);
...
CLogicalDML::Matches(COperator *pop) const
{
	...
	return m_pcrAction == popDML->PcrAction() && m_pcrTableOid == popDML->PcrTableOid() &&
		   ...
		   m_pcrTupleOid == popDML->PcrTupleOid() && m_ptabdesc->MDId()->Equals(popDML->Ptabdesc()->MDId()) &&
...
//---------------------------------------------------------------------------
COperator *
CLogicalDML::PopCopyWithRemappedColumns(CMemoryPool *mp, UlongToColRefMap *colref_mapping, BOOL must_exist)
{
	...
	return GPOS_NEW(mp) CLogicalDML(mp, m_edmlop, m_ptabdesc, colref_array, m_pbsModified, pcrAction, pcrTableOid,
...
//---------------------------------------------------------------------------
CPropConstraint *
CLogicalDML::DerivePropertyConstraint(CMemoryPool *mp, CExpressionHandle &exprhdl) const
{
	CColRefSet *pcrsOutput = GPOS_NEW(mp) CColRefSet(mp);
	pcrsOutput->Include(m_pdrgpcrSource);
