					  ...
					  CColRefArray *pdrgpcrOutput)
{
	...
	switch (ptabdesc->GetRelDistribution())
	{
		...
		case IMDRelation::EreldistrHash:
		{
			...
			pds = GPOS_NEW(mp)
				CDistributionSpecHashed(pdrgpexpr, true /*fNullsColocated*/, opfamilies);
			break;
		}
...
//---------------------------------------------------------------------------
CDistributionSpec *
CPhysical::PdsRequireSingletonOrReplicated(CMemoryPool *mp, CExpressionHandle &exprhdl,
...
//---------------------------------------------------------------------------
CDistributionSpec *
CPhysical::PdsUnary(CMemoryPool *mp, CExpressionHandle &exprhdl, CDistributionSpec *pdsRequired,
					ULONG child_index, ULONG ulOptReq)
{
	...
	CDistributionSpec *pds =
		PdsRequireSingletonOrReplicated(mp, exprhdl, pdsRequired, child_index, ulOptReq);
	if (NULL != pds)
	{
		return pds;
	}
...
//---------------------------------------------------------------------------
CColRefSet *
CPhysical::PcrsChildReqd(CMemoryPool *mp, CExpressionHandle &exprhdl, CColRefSet *pcrsRequired,
...
CPhysical::PppsRequiredPushThruNAry(CMemoryPool *mp, CExpressionHandle &exprhdl,
									CPartitionPropagationSpec *pppsReqd, ULONG child_index)
{
	...
	for (ULONG ul = 0; ul < ulPartIndexIds; ul++)
	{
		...
		CPartKeysArray *pdrgppartkeys =
			exprhdl.DerivePartitionInfo(child_index)->PdrgppartkeysByScanId(part_idx_id);
											   ...
											   CColRefSet *filter_colrefs)
{
	...
	for (ULONG ul = 0; ul < ulPartIndexIds; ul++)
	{
		...
		{
			// push requirements to child node
			ppimResult->AddRequiredPartPropagation(ppimReqd, part_idx_id,
...
CDistributionSpec *
CPhysical::PdsRequireSingleton(CMemoryPool *mp, CExpressionHandle &exprhdl, CDistributionSpec *pds,
							   ULONG child_index)
{
