...
BOOL
CPredicateUtils::FComparison(CExpression *pexpr, CColRef *colref,
...
BOOL
CPredicateUtils::FRangeComparison(CExpression *pexpr, CColRef *colref,
								  CColRefSet *pcrsAllowedRefs,	// other column references allowed in the comparison
								  BOOL allowNotEqualPreds)
{
	...
	return (IMDType::EcmptOther != cmp_type && (allowNotEqualPreds || IMDType::EcmptNEq != cmp_type));
}
...
BOOL
CPredicateUtils::FConjunctionOfEqComparisons(CMemoryPool *mp, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != pexpr);
...
CExpressionArray *
CPredicateUtils::PdrgpexprExpandDisjuncts(CMemoryPool *mp, CExpressionArray *pdrgpexprDisjuncts)
{
...
CExpressionArray *
CPredicateUtils::PdrgpexprExpandConjuncts(CMemoryPool *mp, CExpressionArray *pdrgpexprConjuncts)
{
...
void
CPredicateUtils::ExtractLikePredComponents(CExpression *pexprPred, CExpression **ppexprScIdent,
...
void
CPredicateUtils::ExtractComponents(CExpression *pexprScCmp, CColRef *pcrKey, CExpression **ppexprKey,
...
CExpression *
CPredicateUtils::PexprConjDisj(CMemoryPool *mp, CExpressionArray *pdrgpexpr, BOOL fConjunction)
{
...
CExpression *
CPredicateUtils::PexprConjDisj(CMemoryPool *mp, CExpression *pexprOne, CExpression *pexprTwo, BOOL fConjunction)
{
...
CExpression *
CPredicateUtils::PexprConjunction(CMemoryPool *mp, CExpression *pexprOne, CExpression *pexprTwo)
{
...
CExpression *
CPredicateUtils::PexprDisjunction(CMemoryPool *mp, CExpression *pexprOne, CExpression *pexprTwo)
{
...
CExpressionArray *
CPredicateUtils::PdrgpexprPlainEqualities(CMemoryPool *mp, CExpressionArray *pdrgpexpr)
{
	...
	*pecmpt = IMDType::EcmptOther;
	...
	if (CUtils::FScalarCmp(pexpr))
	{
		*pecmpt = CScalarCmp::PopConvert(pop)->ParseCmpType();
		...
		return CColRef::EcrtTable == colref->Ecrt() && !CColRefTable::PcrConvert(colref)->IsNullable();
	}
...
CExpression *
CPredicateUtils::PexprEliminateSelfComparison(CMemoryPool *mp, CExpression *pexpr)
{
...
CExpression *
CPredicateUtils::PexprINDFConjunction(CMemoryPool *mp, CColRefArray *pdrgpcrFirst, CColRefArray *pdrgpcrSecond)
{
...
// constant comparisons.
CExpression *
CPredicateUtils::PexprPartPruningPredicate(CMemoryPool *mp, const CExpressionArray *pdrgpexpr, CColRef *pcrPartKey,
...
)
{
	...
	GPOS_ASSERT(
		pexprCol == NULL || CUtils::FScalarConstTrue(pexprCol) ||
...
CExpressionArray *
CPredicateUtils::PdrgpexprAppendConjunctsDedup(CMemoryPool *mp, CExpressionArray *pdrgpexpr, CExpression *pexpr)
{
...
// on the given column
BOOL
CPredicateUtils::IsDisjunctionOfRangeComparison(CMemoryPool *mp, CExpression *pexpr, CColRef *colref,
												CColRefSet *pcrsAllowedRefs, BOOL allowNotEqualPreds)
{
	if (!FOr(pexpr))
	{
		return false;
...
// constant filters.
CExpression *
CPredicateUtils::PexprExtractPredicatesOnPartKeys(CMemoryPool *mp, CExpression *pexprScalar,
...
CExpression *
CPredicateUtils::PexprPredicateCol(CMemoryPool *mp, CConstraint *pcnstr, CColRef *colref, BOOL fUseConstraints)
{
...
// checks if the given constraint specifies a set of constants for a column
BOOL
CPredicateUtils::FColumnDisjunctionOfConst(CConstraintInterval *pcnstrInterval, const CColRef *
#ifdef GPOS_DEBUG
																					colref
#endif
)
...
CExpression *
CPredicateUtils::PexprIndexLookupKeyOnLeft(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprScalar,
...
CExpression *
CPredicateUtils::PexprIndexLookupKeyOnRight(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprScalar,
	...
	if (CUtils::FScalarCmp(pexprScalar))
	{
		...
		if (popScCmpCommute)
		{
			...
			CExpression *pexprIndexCond =
				PexprIndexLookupKeyOnLeft(mp, md_accessor, pexprCommuted, pmdindex, pdrgpcrIndex, outer_refs);
...
CExpression *
CPredicateUtils::PexprIndexLookup(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexprScalar,
								  const IMDIndex *pmdindex, CColRefArray *pdrgpcrIndex, CColRefSet *outer_refs,
								  BOOL allowArrayCmpForBTreeIndexes)
{
	...
	CExpression *pexprIndexLookupKeyOnRight =
		PexprIndexLookupKeyOnRight(mp, md_accessor, pexprScalar, pmdindex, pdrgpcrIndex, outer_refs);
	...
	BOOL allowArrayCmpForBTreeIndexes)
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		if (CUtils::FScalarIdentBoolType(pexprCond))
		{
			...
			pexprCond = CUtils::PexprScalarEqCmp(mp, pexprCond,
												 CUtils::PexprScalarConstBool(mp, true /*value*/, false /*is_null*/));
		}
		else if (FNot(pexprCond) && CUtils::FScalarIdentBoolType((*pexprCond)[0]))
		{
			...
			pexprCond->Release();
			pexprScId->AddRef();
			pexprCond = CUtils::PexprScalarEqCmp(mp, pexprScId,
...
void
CPredicateUtils::SeparateOuterRefs(CMemoryPool *mp, CExpression *pexprScalar, CColRefSet *outer_refs,
...
CExpression *
CPredicateUtils::PexprPruneSuperfluosEquality(CMemoryPool *mp, CExpression *pexpr)
{
	GPOS_CHECK_STACK_SIZE;
	GPOS_ASSERT(pexpr->Pop()->FScalar());
...
CExpression *
CPredicateUtils::PexprRemoveImpliedConjuncts(CMemoryPool *mp, CExpression *pexprScalar, CExpressionHandle &exprhdl)
{
	if (COperator::EopScalarNAryJoinPredList == pexprScalar->Pop()->Eopid())
	{
		// for a ScalarNAryJoinPredList we have to preserve that operator and
...
BOOL
CPredicateUtils::FValidSemiJoinCorrelations(CMemoryPool *mp, CExpression *pexprOuter, CExpression *pexprInner,
...
BOOL
CPredicateUtils::FSimpleEqualityUsingCols(CMemoryPool *mp, CExpression *pexprScalar, CColRefSet *pcrs)
{
	GPOS_ASSERT(NULL != pexprScalar);
...
CExpression *
CPredicateUtils::PexprReplaceColsWithNulls(CMemoryPool *mp, CExpression *pexprScalar, CColRefSet *pcrs)
{
	GPOS_CHECK_STACK_SIZE;
	GPOS_ASSERT(NULL != pexprScalar);
...
// are set to NULL
BOOL
CPredicateUtils::FNullRejecting(CMemoryPool *mp, CExpression *pexprScalar, CColRefSet *pcrs)
{
	GPOS_CHECK_STACK_SIZE;
	GPOS_ASSERT(NULL != pexprScalar);
	GPOS_ASSERT(pexprScalar->Pop()->FScalar());
	...
	if (fHasVolatileFunctions || fHasSQL || pexprScalar->DeriveHasNonScalarFunction())
	{
...
BOOL
CPredicateUtils::FCompatiblePredicates(CExpressionArray *pdrgpexprPred, const IMDIndex *pmdindex,
...
BOOL
CPredicateUtils::FCompatibleIndexPredicate(CExpression *pexprPred, const IMDIndex *pmdindex, CColRefArray *pdrgpcrIndex,
...
BOOL
CPredicateUtils::FConvertToCNF(CExpression *pexprOuter, CExpression *pexprInner, CExpression *pexprScalar)
{
	GPOS_ASSERT(NULL != pexprScalar);
...
void
CPredicateUtils::CollectGrandChildrenUnionUnionAll(CMemoryPool *mp, CExpression *pexpr, ULONG child_index,
...
//		+--CScalarConstArray
BOOL
CPredicateUtils::FBitmapLookupSupportedPredicateOrConjunct(CExpression *pexpr, CColRefSet *outer_refs)
{
