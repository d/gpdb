...
CPhysicalHashJoin::CreateHashRedistributeRequests(CMemoryPool *mp)
{
	...
	if (1 < ulExprs)
	{
		for (ULONG ul = 0; ul < ulExprs; ul++)
		{
			...
			CDistributionSpecHashed *pdshashedCurrent = GPOS_NEW(mp)
				CDistributionSpecHashed(pdrgpexprCurrent, true /* fNullsCollocated */, opfamilies);
	...
	CDistributionSpecHashed *pdshashed = GPOS_NEW(mp)
		CDistributionSpecHashed(pdrgpexpr, true /* fNullsCollocated */, m_hash_opfamilies);
...
) const
{
	...
	if (1 == child_index)
	{
		...
		{
			return GPOS_NEW(mp)
				CRewindabilitySpec(CRewindabilitySpec::ErtRescannable, prsRequired->Emht());
		}
...
//---------------------------------------------------------------------------
CDistributionSpec *
CPhysicalHashJoin::PdsMatch(CMemoryPool *mp, CDistributionSpec *pds, ULONG ulSourceChildIndex) const
{
	...
	switch (pds->Edt())
	{
		...
		case CDistributionSpec::EdtHashed:
			...
			return PdshashedMatching(mp, CDistributionSpecHashed::PdsConvert(pds),
									 ulSourceChildIndex);
...
//---------------------------------------------------------------------------
CDistributionSpec *
CPhysicalHashJoin::PdsRequiredReplicate(CMemoryPool *mp, CExpressionHandle &exprhdl,
										...
										CDrvdPropArray *pdrgpdpCtxt, ULONG ulOptReq) const
{
	...
	if (EceoLeftToRight == eceo)
	{
		...
		return CPhysicalJoin::PdsRequired(mp, exprhdl, pdsInput, child_index, pdrgpdpCtxt,
										  0 /*ulOptReq*/);
	}
		...
		CDistributionSpec::EdtHashed == pdsInput->Edt())
	{
		...
		CDistributionSpecHashed *pdshashed =
			PdshashedPassThru(mp, exprhdl, CDistributionSpecHashed::PdsConvert(pdsInput),
...
//---------------------------------------------------------------------------
CDistributionSpecHashed *
CPhysicalHashJoin::PdshashedPassThru(CMemoryPool *mp, CExpressionHandle &exprhdl,
...
//---------------------------------------------------------------------------
CDistributionSpec *
CPhysicalHashJoin::PdsRequiredRedistribute(CMemoryPool *mp, CExpressionHandle &exprhdl,
										   CDistributionSpec *,	 // pdsInput
...
CDistributionSpec *
CPhysicalHashJoin::PdsRequired(
	CMemoryPool *mp, CExpressionHandle &exprhdl, CDistributionSpec *pdsInput, ULONG child_index,
	CDrvdPropArray *pdrgpdpCtxt,
...
) const
{
	...
	if (ulOptReq < ulHashDistributeRequests)
	{
		...
		CDistributionSpec *pds =
			PdsRequiredRedistribute(mp, exprhdl, pdsInput, child_index, pdrgpdpCtxt, ulOptReq);
	...
	if (ulOptReq == ulHashDistributeRequests || ulOptReq == ulHashDistributeRequests + 1)
	{
		...
		CDistributionSpec *pds =
			PdsRequiredReplicate(mp, exprhdl, pdsInput, child_index, pdrgpdpCtxt, ulOptReq);
		if (CDistributionSpec::EdtHashed == pds->Edt())
		{
			CDistributionSpecHashed *pdsHashed = CDistributionSpecHashed::PdsConvert(pds);
			pdsHashed->ComputeEquivHashExprs(mp, exprhdl);
