	...
	BOOL must_exist)
{
	...
	gpos::owner<CColRefArray *> pdrgpcrMinimal = nullptr;
	if (nullptr != m_pdrgpcrMinimal)
	{
...
//---------------------------------------------------------------------------
BOOL
CLogicalGbAgg::Matches(gpos::pointer<COperator *> pop) const
{
	if (pop->Eopid() != Eopid())
	{
		return false;
	...
	gpos::pointer<CLogicalGbAgg *> popAgg = dynamic_cast<CLogicalGbAgg *>(pop);
