		...
		exprhdl.HasOuterRefs())
	{
		...
		if (NULL != pdshashedEquiv)
		{
			...
			CDistributionSpecHashed *pdshashedResult =
				GPOS_NEW(mp) CDistributionSpecHashed(pdrgpexprHashed, pdshashed->FNullsColocated(),
										 ...
										 ULONG ulExpectedPropagators)
{
	...
	ppim->Insert(part_idx_id, ppartcnstrmap, CPartIndexMap::EpimConsumer, ulExpectedPropagators,
				 rel_mdid, pdrgppartkeys, ppartcnstrRel);
