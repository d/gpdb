...
)
{
	...
	SplitPrjList(mp, pexprSeqPrj, &pdrgpexprDistinctAggsPrEl, &pdrgpexprOtherPrEl, &pdrgposOther,
				 &pdrgpwfOther);
	...
	*ppexprOutputSeqPrj = GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CLogicalSequenceProject(mp, pds, pdrgposOther, pdrgpwfOther),
		pexprSeqPrjChild,
