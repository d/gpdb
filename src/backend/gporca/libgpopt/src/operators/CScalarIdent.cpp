...
//---------------------------------------------------------------------------
COperator *
CScalarIdent::PopCopyWithRemappedColumns(CMemoryPool *mp, UlongToColRefMap *colref_mapping, BOOL must_exist)
{
	ULONG id = m_pcr->Id();
...
CScalarIdent::FAllowedFuncScId(CExpression *pexpr)
{
	...
	if (COperator::EopScalarFunc == pexpr->Pop()->Eopid() && COperator::EopScalarIdent == (*pexpr)[0]->Pop()->Eopid())
	{
