...
//---------------------------------------------------------------------------
COperator *
CLogicalConstTableGet::PopCopyWithRemappedColumns(CMemoryPool *mp, UlongToColRefMap *colref_mapping,
												  BOOL must_exist)
{
...
CLogicalConstTableGet::PdrgpcoldescMapping(CMemoryPool *mp, CColRefArray *colref_array) const
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		CColumnDescriptor *pcoldesc = GPOS_NEW(mp)
			CColumnDescriptor(mp, colref->RetrieveType(), colref->TypeModifier(), colref->Name(),
							  ul + 1,  //attno
							  true,	   // IsNullable
							  length);
