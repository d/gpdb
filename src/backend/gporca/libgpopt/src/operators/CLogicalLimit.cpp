...
//
//---------------------------------------------------------------------------
CLogicalLimit::CLogicalLimit(CMemoryPool *mp, COrderSpec *pos, BOOL fGlobal, BOOL fHasCount,
...
//---------------------------------------------------------------------------
COperator *
CLogicalLimit::PopCopyWithRemappedColumns(CMemoryPool *mp, UlongToColRefMap *colref_mapping,
										  BOOL must_exist)
{
...
//---------------------------------------------------------------------------
CColRefSet *
CLogicalLimit::DeriveOuterReferences(CMemoryPool *mp, CExpressionHandle &exprhdl)
{
	CColRefSet *pcrsSort = m_pos->PcrsUsed(mp);
							 ...
							 CExpressionHandle &exprhdl) const
{
	...
	if (NULL != pexprCount && CUtils::FScalarConstInt<IMDTypeInt8>(pexprCount) &&
...
//---------------------------------------------------------------------------
CColRefSet *
CLogicalLimit::PcrsStat(CMemoryPool *mp, CExpressionHandle &exprhdl, CColRefSet *pcrsInput,
...
) const
{
	...
	return CLimitStatsProcessor::CalcLimitStats(mp, dynamic_cast<CStatistics *>(child_stats),
												dRowsMax);
}
