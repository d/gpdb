...
//---------------------------------------------------------------------------
CColRefArray *
CLogical::PdrgpcrCreateMapping(CMemoryPool *mp, const CColumnDescriptorArray *pdrgpcoldesc,
									   ...
									   const CColRefArray *pdrgpcrOutput)
{
	...
	for (ULONG ul = 0; ul < ulCheckConstraint; ul++)
	{
		...
		CConstraint *pcnstr = CConstraint::PcnstrFromScalarExpr(
			mp, pexprCheckConstraint, &pdrgpcrsChild, true /* infer_nulls_as */);
		if (NULL != pcnstr)
		{
			pdrgpcnstr->Append(pcnstr);
...
//---------------------------------------------------------------------------
CPropConstraint *
CLogical::PpcDeriveConstraintRestrict(CMemoryPool *mp, CExpressionHandle &exprhdl,
...
CLogical::DeriveFunctionProperties(CMemoryPool *mp, CExpressionHandle &exprhdl) const
{
	...
	return GPOS_NEW(mp) CFunctionProp(efs, IMDFunction::EfdaNoSQL,
