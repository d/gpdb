...
gpopt::EresExceptionInit(CMemoryPool *mp)
{
	...
	CMessage rgmsg[ExmiSentinel] = {
		CMessage(CException(gpopt::ExmaGPOPT, gpopt::ExmiNoPlanFound), CException::ExsevError,
				 GPOS_WSZ_WSZLEN("No plan has been computed for required properties"), 0,
		...
		CMessage(
			CException(gpopt::ExmaGPOPT, gpopt::ExmiUnsupportedNonDeterministicUpdate), CException::ExsevNotice,
			GPOS_WSZ_WSZLEN("Feature not supported by the Pivotal Query Optimizer: non-deterministic DML statements"),
			0,
			GPOS_WSZ_WSZLEN("Feature not supported by the Pivotal Query Optimizer: non-deterministic DML statements")),

		CMessage(CException(gpopt::ExmaGPOPT, gpopt::ExmiUnsatisfiedRequiredProperties), CException::ExsevError,
				 GPOS_WSZ_WSZLEN("Plan does not satisfy required properties"), 0,
