...
CMDAccessor::InitHashtables(CMemoryPool *mp)
{
	...
	m_shtCacheAccessors.Init(
		mp, GPOPT_CACHEACC_HT_NUM_OF_BUCKETS, GPOS_OFFSET(SMDAccessorElem, m_link),
	...
	m_shtProviders.Init(mp, GPOPT_CACHEACC_HT_NUM_OF_BUCKETS, GPOS_OFFSET(SMDProviderElem, m_link),
						...
						&(SMDProviderElem::m_mdpelemInvalid), SMDProviderElem::HashValue,
...
//---------------------------------------------------------------------------
void
CMDAccessor::RecordColumnStats(CMemoryPool *mp, IMDId *rel_mdid, ULONG colid, ULONG ulPos,
							   ...
							   CStatisticsConfig *stats_config)
{
	...
	BOOL fRecordMissingStats = !isEmptyTable && fGuc && !isSystemCol && (NULL != stats_config) &&
					...
					CStatisticsConfig *stats_config)
{
	...
	while (crsiHist.Advance())
	{
		...
		RecordColumnStats(mp, rel_mdid, colid, ulPos, pcrtable->IsSystemCol(), fEmptyTable,
						  col_histogram_mapping, colid_width_mapping, stats_config);
	}
...
CMDAccessor::GetHistogram(CMemoryPool *mp, IMDId *mdid_type, const IMDColStats *pmdcolstats)
{
	...
	CHistogram *histogram =
		GPOS_NEW(mp) CHistogram(mp, buckets, true /*is_well_defined*/, null_freq,
...
CMDAccessor::Pbucket(CMemoryPool *mp, IMDId *mdid_type, const CDXLBucket *dxl_bucket)
{
	...
	return GPOS_NEW(mp) CBucket(bucket_lower_bound, bucket_upper_bound, dxl_bucket->IsLowerClosed(),
								dxl_bucket->IsUpperClosed(), dxl_bucket->GetFrequency(),
