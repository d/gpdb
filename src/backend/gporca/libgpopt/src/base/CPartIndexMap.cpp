...
//
//---------------------------------------------------------------------------
CPartIndexMap::CPartTableInfo::CPartTableInfo(ULONG scan_id, UlongToPartConstraintMap *ppartcnstrmap,
...
//---------------------------------------------------------------------------
void
CPartIndexMap::CPartTableInfo::AddPartConstraint(CMemoryPool *mp, ULONG scan_id, CPartConstraint *ppartcnstr)
{
	GPOS_ASSERT(NULL != m_ppartcnstrmap);
	BOOL result GPOS_ASSERTS_ONLY = m_ppartcnstrmap->Insert(GPOS_NEW(mp) ULONG(scan_id), ppartcnstr);
...
//---------------------------------------------------------------------------
void
CPartIndexMap::CPartTableInfo::AddPartConstraints(CMemoryPool *mp, UlongToPartConstraintMap *ppartcnstrmap)
{
...
//---------------------------------------------------------------------------
void
CPartIndexMap::Insert(ULONG scan_id, UlongToPartConstraintMap *ppartcnstrmap, EPartIndexManipulator epim,
					  ULONG ulExpectedPropagators, IMDId *mdid, CPartKeysArray *pdrgppartkeys,
					  CPartConstraint *ppartcnstrRel)
{
	...
	if (NULL == ppti)
	{
		...
		ppti = GPOS_NEW(m_mp)
			CPartTableInfo(scan_id, ppartcnstrmap, epim, mdid, pdrgppartkeys, ppartcnstrRel, ulExpectedPropagators);
		BOOL fSuccess GPOS_ASSERTS_ONLY = m_pim->Insert(GPOS_NEW(m_mp) ULONG(scan_id), ppti);
...
//---------------------------------------------------------------------------
void
CPartIndexMap::AddUnresolved(CMemoryPool *mp, const CPartIndexMap &pimFst, const CPartIndexMap &pimSnd,
...
//---------------------------------------------------------------------------
void
CPartIndexMap::ResolvePropagator(EPartIndexManipulator epimFst, ULONG ulExpectedPropagatorsFst,
								 EPartIndexManipulator epimSnd, ULONG ulExpectedPropagatorsSnd,
...
CPartIndexMap::FSubset(const CPartIndexMap *ppim) const
{
	...
	while (pimi.Advance())
	{
		...
		if (pptiFst->Epim() != pptiSnd->Epim() || pptiFst->UlExpectedPropagators() != pptiSnd->UlExpectedPropagators())
		{
			return false;
		}
	}
...
//---------------------------------------------------------------------------
void
CPartIndexMap::AddRequiredPartPropagation(CPartIndexMap *ppimSource, ULONG scan_id, EPartPropagationRequestAction eppra,
										  CPartKeysArray *pdrgppartkeys)
{
	...
	this->Insert(ppti->ScanId(), ppti->Ppartcnstrmap(), ppti->Epim(), ulExpectedPropagators, ppti->MDId(),
				 pdrgppartkeys, ppti->PpartcnstrRel());
}
...
CPartIndexMap::PpimPartitionSelector(CMemoryPool *mp, ULONG scan_id, ULONG ulExpectedFromReq) const
{
	...
	while (pimi.Advance())
	{
		...
		ppimResult->Insert(ppti->ScanId(), ppartcnstrmap, epim, ulExpectedPropagators, mdid, pdrgppartkeys,
						   ppartcnstrRel);
	}
...
//---------------------------------------------------------------------------
IOstream &
CPartIndexMap::OsPrintPartCnstrMap(ULONG scan_id, UlongToPartConstraintMap *ppartcnstrmap, IOstream &os)
{
	if (NULL == ppartcnstrmap)
	{
		return os;
