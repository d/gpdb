...
const CHAR *CPartIndexMap::CPartTableInfo::m_szManipulator[EpimSentinel] = {"propagator",
																			"consumer", "resolver"};
...
//---------------------------------------------------------------------------
void
CPartIndexMap::CPartTableInfo::AddPartConstraints(CMemoryPool *mp,
												  UlongToPartConstraintMap *ppartcnstrmap)
{
...
CPartIndexMap::Insert(ULONG scan_id, UlongToPartConstraintMap *ppartcnstrmap,
					  EPartIndexManipulator epim, ULONG ulExpectedPropagators, IMDId *mdid,
					  CPartKeysArray *pdrgppartkeys, CPartConstraint *ppartcnstrRel)
{
	CPartTableInfo *ppti = m_pim->Find(&scan_id);
	if (NULL == ppti)
	{
		// no entry is found, create a new entry
		ppti = GPOS_NEW(m_mp) CPartTableInfo(scan_id, ppartcnstrmap, epim, mdid, pdrgppartkeys,
							 ...
							 const CPartIndexMap &pimSnd, CPartIndexMap *ppimResult)
{
	...
	while (pimiFst.Advance())
	{
		...
		if (NULL != pptiSnd)
		{
			...
			ResolvePropagator(epimFst, ulPropagatorsFst, epimSnd, ulPropagatorsSnd, &epimResult,
							  &ulPropagatorsResult);
		...
		ppimResult->Insert(scan_id, ppartcnstrmap, epimResult, ulPropagatorsResult, mdid,
						   pdrgppartkeys, ppartcnstrRel);
	}
}
...
//---------------------------------------------------------------------------
void
CPartIndexMap::AddRequiredPartPropagation(CPartIndexMap *ppimSource, ULONG scan_id,
										  ...
										  CPartKeysArray *pdrgppartkeys)
{
	...
	this->Insert(ppti->ScanId(), ppti->Ppartcnstrmap(), ppti->Epim(), ulExpectedPropagators,
				 ppti->MDId(), pdrgppartkeys, ppti->PpartcnstrRel());
}
...
CPartIndexMap::PpimPartitionSelector(CMemoryPool *mp, ULONG scan_id, ULONG ulExpectedFromReq) const
{
	...
	while (pimi.Advance())
	{
		...
		ppimResult->Insert(ppti->ScanId(), ppartcnstrmap, epim, ulExpectedPropagators, mdid,
						   pdrgppartkeys, ppartcnstrRel);
	}
...
//---------------------------------------------------------------------------
IOstream &
CPartIndexMap::OsPrintPartCnstrMap(ULONG scan_id, UlongToPartConstraintMap *ppartcnstrmap,
								   IOstream &os)
{
	if (NULL == ppartcnstrmap)
	{
