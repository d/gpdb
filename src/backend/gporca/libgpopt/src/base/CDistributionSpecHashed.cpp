...
//---------------------------------------------------------------------------
CDistributionSpec *
CDistributionSpecHashed::PdsCopyWithRemappedColumns(CMemoryPool *mp, UlongToColRefMap *colref_mapping, BOOL must_exist)
{
...
//---------------------------------------------------------------------------
CDistributionSpecHashed *
CDistributionSpecHashed::PdshashedMaximal(CMemoryPool *mp, CColRefArray *colref_array, BOOL fNullsColocated)
{
...
void
CDistributionSpecHashed::ComputeEquivHashExprs(CMemoryPool *mp, CExpressionHandle &expression_handle)
{
...
CDistributionSpecHashed::Copy(CMemoryPool *mp)
{
	...
	CDistributionSpecHashed *spec_copy =
		GPOS_NEW(mp) CDistributionSpecHashed(distribution_exprs, this->FNullsColocated(), spec, m_opfamilies);
...
CDistributionSpecHashed::OsPrintWithPrefix(IOstream &os, const char *prefix) const
{
	...
	if (NULL != m_equiv_hash_exprs && m_equiv_hash_exprs->Size() > 0 && GPOS_FTRACE(EopttracePrintEquivDistrSpecs))
	{
		os << "," << std::endl;
		for (ULONG ul = 0; ul < m_equiv_hash_exprs->Size(); ul++)
		{
...
CDistributionSpecHashed::Combine(CMemoryPool *mp, CDistributionSpecHashed *other_spec)
{
	...
	for (ULONG ul = 0; ul < all_distribution_exprs->Size(); ul++)
	{
		...
		combined_hashed_spec =
			GPOS_NEW(mp) CDistributionSpecHashed(exprs, this->FNullsColocated(), combined_hashed_spec, m_opfamilies);
	}
...
CDistributionSpecHashed *
CDistributionSpecHashed::CompleteEquivSpec(CMemoryPool *mp, CDistributionSpecHashed *pdshashed, CExpression *pexprPred)
{
...
CDistributionSpecHashed *
CDistributionSpecHashed::MakeHashedDistrSpec(CMemoryPool *mp, CExpressionArray *pdrgpexpr, BOOL fNullsColocated,
											 CDistributionSpecHashed *pdshashedEquiv, IMdIdArray *opfamilies)
{
	if (GPOS_FTRACE(EopttraceConsiderOpfamiliesForDistribution) && NULL == opfamilies)
	{
