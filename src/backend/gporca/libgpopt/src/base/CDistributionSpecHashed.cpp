...
//---------------------------------------------------------------------------
CDistributionSpec *
CDistributionSpecHashed::PdsCopyWithRemappedColumns(CMemoryPool *mp,
													...
													BOOL must_exist)
{
	...
	return GPOS_NEW(mp)
		CDistributionSpecHashed(pdrgpexpr, m_fNullsColocated, pdshashed, m_opfamilies);
}
...
//---------------------------------------------------------------------------
CDistributionSpecHashed *
CDistributionSpecHashed::PdshashedMaximal(CMemoryPool *mp, CColRefArray *colref_array,
...
// expression array
BOOL
CDistributionSpecHashed::IsCoveredBy(const CExpressionArray *dist_cols_expr_array) const
{
	BOOL covers = false;
...
void
CDistributionSpecHashed::ComputeEquivHashExprs(CMemoryPool *mp,
											   CExpressionHandle &expression_handle)
{
	...
	if (NULL == equiv_distribution_all_exprs)
	{
		...
		for (ULONG distribution_key_idx = 0; distribution_key_idx < distribution_exprs->Size();
			 distribution_key_idx++)
		{
			...
			CColRefSet *equiv_cols =
				expression_handle.DerivePropertyConstraint()->PcrsEquivClass(distribution_colref);
			...
			if (NULL != equiv_cols)
			{
				...
				while (equiv_cols_iter.Advance())
				{
					...
					CExpression *predicate_expr_with_casts =
						CCastUtils::PexprAddCast(mp, predicate_expr_with_inferred_quals);
...
CDistributionSpecHashed::Copy(CMemoryPool *mp)
{
	...
	for (ULONG ul = 1; ul < equiv_distribution_exprs->Size(); ul++)
	{
		...
		distribution_exprs->AddRef();
		spec =
			GPOS_NEW(mp) CDistributionSpecHashed(distribution_exprs, this->FNullsColocated(), spec);
	}
	...
	CDistributionSpecHashed *spec_copy = GPOS_NEW(mp)
		CDistributionSpecHashed(distribution_exprs, this->FNullsColocated(), spec, m_opfamilies);
...
IOstream &
CDistributionSpecHashed::OsPrintWithPrefix(IOstream &os, const char *prefix) const
{
	os << this->SzId() << ": [ ";
	const ULONG length = m_pdrgpexpr->Size();
	for (ULONG ul = 0; ul < length; ul++)
...
CDistributionSpecHashed::Combine(CMemoryPool *mp, CDistributionSpecHashed *other_spec)
{
	...
	CExpressionArrays *all_distribution_exprs =
		CUtils::GetCombinedExpressionArrays(mp, distribution_exprs, other_distribution_exprs);
...
CDistributionSpecHashed *
CDistributionSpecHashed::CompleteEquivSpec(CMemoryPool *mp, CDistributionSpecHashed *pdshashed,
...
CDistributionSpecHashed *
CDistributionSpecHashed::MakeHashedDistrSpec(CMemoryPool *mp, CExpressionArray *pdrgpexpr,
											 BOOL fNullsColocated,
											 CDistributionSpecHashed *pdshashedEquiv,
											 IMdIdArray *opfamilies)
{
	...
	return GPOS_NEW(mp)
		CDistributionSpecHashed(pdrgpexpr, fNullsColocated, pdshashedEquiv, opfamilies);
}
// EOF
