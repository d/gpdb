...
CEnfdPartitionPropagation::CEnfdPartitionPropagation(CPartitionPropagationSpec *ppps,
													 EPartitionPropagationMatching eppm, CPartFilterMap *ppfm)
...
CEnfdPartitionPropagation::Epet(CExpressionHandle &exprhdl, CPhysical *popPhysical, BOOL fPropagationReqd) const
{
	if (fPropagationReqd)
	{
...
CEnfdPartitionPropagation::FResolved(CMemoryPool *mp, CPartIndexMap *ppim) const
{
	...
	for (ULONG ul = 0; ul < length && fResolved; ul++)
	{
		...
		if (ppim->Contains(part_idx_id))
		{
			...
			fResolved = CPartIndexMap::EpimResolver == epim || CPartIndexMap::EpimPropagator == epim ||
