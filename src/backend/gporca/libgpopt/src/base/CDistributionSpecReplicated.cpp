...
CDistributionSpecReplicated::FSatisfies(const CDistributionSpec *pdss) const
{
	...
	return !(EdtSingleton == pdss->Edt() && (dynamic_cast<const CDistributionSpecSingleton *>(pdss))->FOnMaster());
}
											 ...
											 CExpressionArray *pdrgpexpr, CExpression *pexpr)
{
	...
	pexpr->AddRef();
	CExpression *pexprMotion = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CPhysicalMotionBroadcast(mp), pexpr);
