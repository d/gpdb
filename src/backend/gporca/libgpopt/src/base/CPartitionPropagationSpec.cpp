										   ...
										   CExpressionArray *pdrgpexpr, CExpression *pexpr)
{
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		if (m_ppfm->FContainsScanId(scan_id))
		{
			...
			SplitPartPredicates(mp, pexprScalar, pdrgpdrgpcrKeys, phmulexprEqFilter,
								phmulexprFilter, &pexprResidual);
			pexprScalar->Release();
		}
		else
		{
														 ...
														 ULONG part_idx_id) const
{
	...
	ppim->Insert(part_idx_id, ppartcnstrmap, m_ppim->Epim(part_idx_id),
				 m_ppim->UlExpectedPropagators(part_idx_id), mdid, pdrgppartkeys, ppartcnstr);
...
)
{
	...
	for (ULONG ul = 0; ul < ulLevels; ul++)
	{
			...
			CPredicateUtils::FIdentCompare((*pdrgpexprKey)[0], IMDType::EcmptEq, colref))
		{
			...
			CPredicateUtils::ExtractComponents((*pdrgpexprKey)[0], colref, &pexprPartKey,
											   &pexprOther, &cmp_type);
		...
		else
		{
			...
			BOOL result GPOS_ASSERTS_ONLY = phmulexprFilter->Insert(
				GPOS_NEW(mp) ULONG(ul), CPredicateUtils::PexprConjunction(mp, pdrgpexprKey));
			GPOS_ASSERT(result);
			continue;
		}
	}
...
//---------------------------------------------------------------------------
CExpression *
CPartitionPropagationSpec::PexprResidualFilter(CMemoryPool *mp, CExpressionArray *pdrgpexpr,
											   CBitSet *pbsUsed)
{
	GPOS_ASSERT(NULL != pdrgpexpr);
...
//---------------------------------------------------------------------------
CExpressionArray *
CPartitionPropagationSpec::PdrgpexprPredicatesOnKey(CMemoryPool *mp, CExpressionArray *pdrgpexpr,
													CColRef *colref, CColRefSet *pcrsKeys,
													CBitSet **ppbs)
{
	GPOS_ASSERT(NULL != pdrgpexpr);
