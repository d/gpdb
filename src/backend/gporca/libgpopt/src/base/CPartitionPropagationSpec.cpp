...
//---------------------------------------------------------------------------
void
CPartitionPropagationSpec::AppendEnforcers(CMemoryPool *mp, CExpressionHandle &exprhdl,
										   CReqdPropPlan *
#ifdef GPOS_DEBUG
											   prpp
#endif	// GPOS_DEBUG
										   ...
										   CExpressionArray *pdrgpexpr, CExpression *pexpr)
{
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		pexprResolver = GPOS_NEW(mp) CExpression(
			mp,
			GPOS_NEW(mp) CPhysicalPartitionSelector(mp, scan_id, mdid, pdrgpdrgpcrKeys, ppartcnstrmap, ppartcnstr,
...
CPartitionPropagationSpec::PexprFilter(CMemoryPool *mp, ULONG scan_id)
{
	...
	if (CUtils::FScalarIdent(pexprScalar))
	{
		...
		pexprScalar->AddRef();
		pexprScalar = CUtils::PexprScalarEqCmp(mp, pexprScalar,
	...
	else if (CPredicateUtils::FNot(pexprScalar) && CUtils::FScalarIdent((*pexprScalar)[0]))
	{
		...
		pexprScalar =
			CUtils::PexprScalarEqCmp(mp, pexprId, CUtils::PexprScalarConstBool(mp, false /*value*/, false /*is_null*/));
	}
	else
	{
		pexprScalar->AddRef();
...
//---------------------------------------------------------------------------
BOOL
CPartitionPropagationSpec::FRequiresPartitionPropagation(CMemoryPool *mp, CExpression *pexpr,
														 CExpressionHandle &exprhdl, ULONG part_idx_id) const
{
	...
	ppim->Insert(part_idx_id, ppartcnstrmap, m_ppim->Epim(part_idx_id), m_ppim->UlExpectedPropagators(part_idx_id),
	...
	CEnfdPartitionPropagation *pepp = GPOS_NEW(mp)
		CEnfdPartitionPropagation(ppps, CEnfdPartitionPropagation::EppmSatisfy, GPOS_NEW(mp) CPartFilterMap(mp));
	CEnfdProp::EPropEnforcingType epetPartitionPropagation =
		pepp->Epet(exprhdl, CPhysical::PopConvert(pexpr->Pop()), true /*fPartitionPropagationRequired*/);
...
//---------------------------------------------------------------------------
void
CPartitionPropagationSpec::SplitPartPredicates(CMemoryPool *mp, CExpression *pexprScalar,
...
)
{
	...
	for (ULONG ul = 0; ul < ulLevels; ul++)
	{
		...
		if (length == 1 && CPredicateUtils::FIdentCompare((*pdrgpexprKey)[0], IMDType::EcmptEq, colref))
		{
			...
			pexprOther->AddRef();
			BOOL result GPOS_ASSERTS_ONLY = phmulexprEqFilter->Insert(GPOS_NEW(mp) ULONG(ul), pexprOther);
			GPOS_ASSERT(result);
			pdrgpexprKey->Release();
		}
		else
...
//---------------------------------------------------------------------------
CColRefSet *
CPartitionPropagationSpec::PcrsKeys(CMemoryPool *mp, CColRef2dArray *pdrgpdrgpcrKeys)
{
...
//---------------------------------------------------------------------------
CExpression *
CPartitionPropagationSpec::PexprResidualFilter(CMemoryPool *mp, CExpressionArray *pdrgpexpr, CBitSet *pbsUsed)
{
	GPOS_ASSERT(NULL != pdrgpexpr);
...
//---------------------------------------------------------------------------
CExpressionArray *
CPartitionPropagationSpec::PdrgpexprPredicatesOnKey(CMemoryPool *mp, CExpressionArray *pdrgpexpr, CColRef *colref,
													CColRefSet *pcrsKeys, CBitSet **ppbs)
{
	GPOS_ASSERT(NULL != pdrgpexpr);
