...
CColumnFactory::CColumnFactory() : m_mp(NULL), m_phmcrcrs(NULL), m_aul(0)
{
	...
	m_sht.Init(m_mp, GPOPT_COLFACTORY_HT_BUCKETS, GPOS_OFFSET(CColRef, m_link),
			   GPOS_OFFSET(CColRef, m_id), &(CColRef::m_ulInvalid), CColRef::HashValue,
			   CColRef::Equals);
...
//---------------------------------------------------------------------------
CColRef *
CColumnFactory::PcrCreate(const CColumnDescriptor *pcoldesc, ULONG id, const CName &name,
...
//---------------------------------------------------------------------------
CColRef *
CColumnFactory::PcrCreate(const IMDType *pmdtype, INT type_modifier, IMDId *mdid_table, INT attno,
						  BOOL is_nullable, ULONG id, const CName &name, ULONG ulOpSource,
						  BOOL isDistCol, ULONG ulWidth)
{
	CName *pnameCopy = GPOS_NEW(m_mp) CName(m_mp, name);
	CAutoP<CName> a_pnameCopy(pnameCopy);

	CColRef *colref = GPOS_NEW(m_mp) CColRefTable(pmdtype, type_modifier, attno, is_nullable, id,
												  pnameCopy, ulOpSource, isDistCol, ulWidth);
	(void) a_pnameCopy.Reset();
	CAutoP<CColRef> a_pcr(colref);
...
CColumnFactory::PcrCopy(const CColRef *colref)
{
	...
	return PcrCreate(colref->RetrieveType(), colref->TypeModifier(), colref->GetMdidTable(),
