...
CExpression *
CUtils::PexprScalarProjectElement(CMemoryPool *mp, CColRef *colref, CExpression *pexpr)
{
	GPOS_ASSERT(NULL != colref);
...
CExpression *
CUtils::PexprScalarCmp(CMemoryPool *mp, const CColRef *pcrLeft, const CColRef *pcrRight, CWStringConst strOp,
					   IMDId *mdid_op)
{
...
CExpression *
CUtils::PexprScalarCmp(CMemoryPool *mp, const CColRef *pcrLeft, CExpression *pexprRight, CWStringConst strOp,
					   IMDId *mdid_op)
{
	GPOS_ASSERT(NULL != pcrLeft);
...
CExpression *
CUtils::PexprScalarCmp(CMemoryPool *mp, const CColRef *pcrLeft, const CColRef *pcrRight, IMDType::ECmpType cmp_type)
{
	GPOS_ASSERT(NULL != pcrLeft);
	GPOS_ASSERT(NULL != pcrRight);
...
CExpression *
CUtils::PexprScalarCmp(CMemoryPool *mp, const CColRef *pcrLeft, CExpression *pexprRight, IMDType::ECmpType cmp_type)
{
	GPOS_ASSERT(NULL != pcrLeft);
	GPOS_ASSERT(NULL != pexprRight);
	GPOS_ASSERT(IMDType::EcmptOther > cmp_type);
...
CExpression *
CUtils::PexprScalarCmp(CMemoryPool *mp, CExpression *pexprLeft, const CColRef *pcrRight, IMDType::ECmpType cmp_type)
{
...
CExpression *
CUtils::PexprScalarCmp(CMemoryPool *mp, CExpression *pexprLeft, const CColRef *pcrRight, CWStringConst strOp,
					   IMDId *mdid_op)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pcrRight);
	...
	return GPOS_NEW(mp) CExpression(
		mp, GPOS_NEW(mp) CScalarCmp(mp, mdid_op, GPOS_NEW(mp) CWStringConst(mp, strOp.GetBuffer()), cmp_type),
		pexprLeft, PexprScalarIdent(mp, pcrRight));
}

// Generate a comparison expression over two expressions
CExpression *
CUtils::PexprScalarCmp(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight, CWStringConst strOp,
					   IMDId *mdid_op)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pexprRight);
...
// Generate a comparison expression over two expressions
CExpression *
CUtils::PexprScalarCmp(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight, IMDId *mdid_scop)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pexprRight);
...
// Generate a comparison expression over two expressions
CExpression *
CUtils::PexprScalarCmp(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight, IMDType::ECmpType cmp_type)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pexprRight);
	GPOS_ASSERT(IMDType::EcmptOther > cmp_type);
...
CExpression *
CUtils::PexprScalarEqCmp(CMemoryPool *mp, const CColRef *pcrLeft, const CColRef *pcrRight)
{
	GPOS_ASSERT(NULL != pcrLeft);
	GPOS_ASSERT(NULL != pcrRight);
...
CExpression *
CUtils::PexprScalarEqCmp(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pexprRight);
...
CExpression *
CUtils::PexprScalarEqCmp(CMemoryPool *mp, const CColRef *pcrLeft, CExpression *pexprRight)
{
	GPOS_ASSERT(NULL != pcrLeft);
	GPOS_ASSERT(NULL != pexprRight);
...
CExpression *
CUtils::PexprScalarEqCmp(CMemoryPool *mp, CExpression *pexprLeft, const CColRef *pcrRight)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pcrRight);
...
CExpression *
CUtils::PScalarArrayExprChildAt(CMemoryPool *mp, CExpression *pexprArray, ULONG ul)
{
	ULONG arity = pexprArray->Arity();
	if (0 == arity)
	{
...
CExpression *
CUtils::PexprScalarArrayCmp(CMemoryPool *mp, CScalarArrayCmp::EArrCmpType earrcmptype, IMDType::ECmpType ecmptype,
							CExpressionArray *pexprScalarChildren, const CColRef *colref)
{
...
CExpression *
CUtils::PexprCmpWithZero(CMemoryPool *mp, CExpression *pexprLeft, IMDId *mdid_type_left, IMDType::ECmpType ecmptype)
{
...
CExpression *
CUtils::PexprIDF(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pexprRight);
...
// generate an Is Distinct From expression
CExpression *
CUtils::PexprIDF(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight, IMDId *mdid_scop)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pexprRight);
...
CExpression *
CUtils::PexprINDF(CMemoryPool *mp, const CColRef *pcrLeft, const CColRef *pcrRight)
{
	GPOS_ASSERT(NULL != pcrLeft);
	GPOS_ASSERT(NULL != pcrRight);
...
CExpression *
CUtils::PexprINDF(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pexprRight);
...
CExpression *
CUtils::PexprINDF(CMemoryPool *mp, CExpression *pexprLeft, CExpression *pexprRight, IMDId *mdid_scop)
{
	GPOS_ASSERT(NULL != pexprLeft);
	GPOS_ASSERT(NULL != pexprRight);
...
BOOL
CUtils::FUsesNullableCol(CMemoryPool *mp, CExpression *pexprScalar, CExpression *pexprLogical)
{
...
CExpression *
CUtils::PexprScalarOp(CMemoryPool *mp, const CColRef *pcrLeft, CExpression *pexprRight, const CWStringConst strOp,
...
CExpression *
CUtils::PexprScalarBoolOp(CMemoryPool *mp, CScalarBoolOp::EBoolOperator eboolop, CExpressionArray *pdrgpexpr)
{
...
CUtils::PexprScalarConstNull(CMemoryPool *mp, const IMDType *typ, INT type_modifier)
{
	...
	switch (typ->GetDatumType())
	{
		...
		case IMDType::EtiGeneric:
			...
			datum = GPOS_NEW(mp) CDatumGenericGPDB(mp, mdid, type_modifier,
...
CScalarAggFunc *
CUtils::PopAggFunc(CMemoryPool *mp, IMDId *pmdidAggFunc, const CWStringConst *pstrAggFunc, BOOL is_distinct,
...
)
{
	...
	return GPOS_NEW(mp)
		CScalarAggFunc(mp, pmdidAggFunc, pmdidResolvedReturnType, pstrAggFunc, is_distinct, eaggfuncstage, fSplit);
}

// generate an aggregate function
CExpression *
CUtils::PexprAggFunc(CMemoryPool *mp, IMDId *pmdidAggFunc, const CWStringConst *pstrAggFunc, const CColRef *colref,
...
CUtils::PexprCountStar(CMemoryPool *mp)
{
	...
	CScalarAggFunc *popScAggFunc =
		PopAggFunc(mp, mdid, str, false /*is_distinct*/, EaggfuncstageGlobal /*eaggfuncstage*/, false /*fSplit*/);
...
CExpression *
CUtils::PexprCountStarAndSum(CMemoryPool *mp, const CColRef *colref, CExpression *pexprLogical)
{
...
BOOL
CUtils::FHasCountAggMatchingColumn(const CExpression *pexpr, const CColRef *colref, const CLogicalGbAgg **ppgbAgg)
{
	COperator *pop = pexpr->Pop();
...
CExpression *
CUtils::PexprGbAggSum(CMemoryPool *mp, CExpression *pexprLogical, CColRefArray *pdrgpcrSum)
{
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
	CColumnFactory *col_factory = COptCtxt::PoctxtFromTLS()->Pcf();
...
CExpression *
CUtils::PexprMin(CMemoryPool *mp, CMDAccessor *md_accessor, const CColRef *colref)
{
...
CExpression *
CUtils::PexprAgg(CMemoryPool *mp, CMDAccessor *md_accessor, IMDType::EAggType agg_type, const CColRef *colref,
				 BOOL is_distinct)
{
...
CExpression *
CUtils::PexprLogicalSelect(CMemoryPool *mp, CExpression *pexpr, CExpression *pexprPredicate)
{
	GPOS_ASSERT(NULL != pexpr);
	GPOS_ASSERT(NULL != pexprPredicate);

	CTableDescriptor *ptabdesc = NULL;
	if (pexpr->Pop()->Eopid() == CLogical::EopLogicalSelect || pexpr->Pop()->Eopid() == CLogical::EopLogicalGet ||
...
CExpression *
CUtils::PexprSafeSelect(CMemoryPool *mp, CExpression *pexprLogical, CExpression *pexprPred)
{
...
CExpression *
CUtils::PexprCollapseSelect(CMemoryPool *mp, CExpression *pexpr, CExpression *pexprPredicate)
{
	GPOS_ASSERT(NULL != pexpr);
	GPOS_ASSERT(NULL != pexprPredicate);
...
CExpression *
CUtils::PexprLogicalProject(CMemoryPool *mp, CExpression *pexpr, CExpression *pexprPrjList, BOOL fNewComputedCol)
{
	GPOS_ASSERT(NULL != pexpr);
...
CExpression *
CUtils::PexprLogicalSequenceProject(CMemoryPool *mp, CDistributionSpec *pds, COrderSpecArray *pdrgpos,
									CWindowFrameArray *pdrgpwf, CExpression *pexpr, CExpression *pexprPrjList)
{
...
CExpression *
CUtils::PexprLogicalProjectNulls(CMemoryPool *mp, CColRefArray *colref_array, CExpression *pexpr,
...
CExpression *
CUtils::PexprScalarProjListConst(CMemoryPool *mp, CColRefArray *colref_array, IDatumArray *pdrgpdatum,
								 UlongToColRefMap *colref_mapping)
{
...
CExpression *
CUtils::PexprAddProjection(CMemoryPool *mp, CExpression *pexpr, CExpression *pexprProjected)
{
	GPOS_ASSERT(pexpr->Pop()->FLogical());
...
CExpression *
CUtils::PexprAddProjection(CMemoryPool *mp, CExpression *pexpr, CExpressionArray *pdrgpexprProjected,
...
CExpression *
CUtils::PexprLogicalGbAgg(CMemoryPool *mp, CColRefArray *colref_array, CExpression *pexprRelational,
...
CExpression *
CUtils::PexprLogicalGbAggGlobal(CMemoryPool *mp, CColRefArray *colref_array, CExpression *pexprRelational,
...
CColRefSetArray *
CUtils::AddEquivClassToArray(CMemoryPool *mp, const CColRefSet *pcrsNew, const CColRefSetArray *pdrgpcrs)
{
...
CColRefSetArray *
CUtils::PdrgpcrsMergeEquivClasses(CMemoryPool *mp, CColRefSetArray *pdrgpcrsFst, CColRefSetArray *pdrgpcrsSnd)
{
	pdrgpcrsFst->AddRef();
...
CColRefSetArray *
CUtils::PdrgpcrsIntersectEquivClasses(CMemoryPool *mp, CColRefSetArray *pdrgpcrsFst, CColRefSetArray *pdrgpcrsSnd)
{
...
CColRefSetArray *
CUtils::PdrgpcrsCopyChildEquivClasses(CMemoryPool *mp, CExpressionHandle &exprhdl)
{
	CColRefSetArray *pdrgpcrs = GPOS_NEW(mp) CColRefSetArray(mp);
...
CColRefArray *
CUtils::PdrgpcrExcludeColumns(CMemoryPool *mp, CColRefArray *pdrgpcrOriginal, CColRefSet *pcrsExcluded)
{
...
CColRefArray *
CUtils::PdrgpcrRedistributableSubset(CMemoryPool *mp, CColRefArray *colref_array)
{
	GPOS_ASSERT(NULL != colref_array);
	GPOS_ASSERT(0 < colref_array->Size());
...
void
CUtils::GenerateFileName(CHAR *buf, const CHAR *szPrefix, const CHAR *szExt, ULONG length, ULONG ulSessionId,
						 ULONG ulCmdId)
{
	CAutoMemoryPool amp;
	CMemoryPool *mp = amp.Pmp();
	...
	str.AppendFormat(wszFileNameTemplate, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min,
					 tm.tm_sec, ulSessionId, ulCmdId);
...
CColRefSet *
CUtils::PcrsRemap(CMemoryPool *mp, CColRefSet *pcrs, UlongToColRefMap *colref_mapping, BOOL must_exist)
{
...
CColRefArray *
CUtils::PdrgpcrRemap(CMemoryPool *mp, CColRefArray *colref_array, UlongToColRefMap *colref_mapping, BOOL must_exist)
{
	GPOS_ASSERT(NULL != colref_array);
	GPOS_ASSERT(NULL != colref_mapping);
...
CColRefArray *
CUtils::PdrgpcrRemapAndCreate(CMemoryPool *mp, CColRefArray *colref_array, UlongToColRefMap *colref_mapping)
{
	GPOS_ASSERT(NULL != colref_array);
	GPOS_ASSERT(NULL != colref_mapping);
...
// and based on the given mapping
CColRef2dArray *
CUtils::PdrgpdrgpcrRemap(CMemoryPool *mp, CColRef2dArray *pdrgpdrgpcr, UlongToColRefMap *colref_mapping,
						 BOOL must_exist)
{
	GPOS_ASSERT(NULL != pdrgpdrgpcr);
	GPOS_ASSERT(NULL != colref_mapping);
...
CExpressionArray *
CUtils::PdrgpexprRemap(CMemoryPool *mp, CExpressionArray *pdrgpexpr, UlongToColRefMap *colref_mapping)
{
	GPOS_ASSERT(NULL != pdrgpexpr);
	GPOS_ASSERT(NULL != colref_mapping);
...
UlongToColRefMap *
CUtils::PhmulcrMapping(CMemoryPool *mp, CColRefArray *pdrgpcrFrom, CColRefArray *pdrgpcrTo)
{
...
void
CUtils::AddColumnMapping(CMemoryPool *mp, UlongToColRefMap *colref_mapping, CColRefArray *pdrgpcrFrom,
...
CColRefArray *
CUtils::PdrgpcrCopy(CMemoryPool *mp, CColRefArray *colref_array, BOOL fAllComputed, UlongToColRefMap *colref_mapping)
{
	// get column factory from optimizer context object
	CColumnFactory *col_factory = COptCtxt::PoctxtFromTLS()->Pcf();
...
CColRefSet *
CUtils::PcrsCTEProducerColumns(CMemoryPool *mp, CColRefSet *pcrsInput, CLogicalCTEConsumer *popCTEConsumer)
{
...
CExpression *
CUtils::PexprCast(CMemoryPool *mp, CMDAccessor *md_accessor, CExpression *pexpr, IMDId *mdid_dest)
{
	...
	if (pmdcast->GetMDPathType() == IMDCast::EmdtArrayCoerce)
	{
		...
		pexprCast = GPOS_NEW(mp) CExpression(
			mp,
			GPOS_NEW(mp) CScalarArrayCoerceExpr(mp, parrayCoerceCast->GetCastFuncMdId(), mdid_dest,
												parrayCoerceCast->TypeModifier(), parrayCoerceCast->IsExplicit(),
...
CExpression *
CUtils::PexprLogicalJoin(CMemoryPool *mp, EdxlJoinType edxljointype, CExpressionArray *pdrgpexpr)
{
	COperator *pop = NULL;
	switch (edxljointype)
	{
...
CUtils::PhmulcnstrBoolConstOnPartKeys(CMemoryPool *mp, CColRef2dArray *pdrgpdrgpcrPartKey, BOOL value)
{
	...
	for (ULONG ul = 0; ul < ulLevels; ul++)
	{
		...
		if (NULL != pcnstr)
		{
			BOOL result GPOS_ASSERTS_ONLY = phmulcnstr->Insert(GPOS_NEW(mp) ULONG(ul), pcnstr);
			GPOS_ASSERT(result);
		}
	}
...
CPartConstraint *
CUtils::PpartcnstrFromMDPartCnstr(CMemoryPool *mp, CMDAccessor *md_accessor, CColRef2dArray *pdrgpdrgpcrPartKey,
								  ...
								  BOOL fDummyConstraint)
{
	...
	else
	{
		...
		for (ULONG ul = 0; ul < ulLevels && NULL != pcnstr; ul++)
		{
			...
			if (NULL != pcnstrLevel)
			{
				BOOL result GPOS_ASSERTS_ONLY = phmulcnstr->Insert(GPOS_NEW(mp) ULONG(ul), pcnstrLevel);
				GPOS_ASSERT(result);
			}
		}
	...
	return GPOS_NEW(mp) CPartConstraint(mp, phmulcnstr, pbsDefaultParts, mdpart_constraint->IsConstraintUnbounded(),
										pdrgpdrgpcrPartKey);
}
...
BOOL
CUtils::FMotionOverUnresolvedPartConsumers(CMemoryPool *mp, CExpressionHandle &exprhdl, CPartIndexMap *ppimReqd)
{
...
CUtils::FDuplicateHazardMotion(CExpression *pexprMotion)
{
	...
	BOOL fReplicatedInput = CDistributionSpec::EdtReplicated == edtChild || CDistributionSpec::EdtUniversal == edtChild;
...
BOOL
CUtils::FEquivalanceClassesDisjoint(CMemoryPool *mp, const CColRefSetArray *pdrgpcrs)
{
	const ULONG length = pdrgpcrs->Size();
...
BOOL
CUtils::FEquivalanceClassesEqual(CMemoryPool *mp, CColRefSetArray *pdrgpcrsFst, CColRefSetArray *pdrgpcrsSnd)
{
...
CExpression *
CUtils::PexprLimit(CMemoryPool *mp, CExpression *pexpr, ULONG ulOffSet, ULONG count)
{
	GPOS_ASSERT(pexpr);
...
CUtils::FGeneratePartOid(IMDId *mdid)
{
	...
	BOOL fInsertSortOnRows =
		...
		(optimizer_config->GetHint()->UlMinNumOfPartsToRequireSortOnInsert() <= pmdrel->PartitionCount());
...
BOOL
CUtils::IsExprNDVPreserving(CExpression *pexpr, const CColRef **underlying_colref)
{
...
CExpression *
CUtils::PexprMatchEqualityOrINDF(CExpression *pexprToMatch,
...
BOOL
CUtils::Equals(const CExpressionArrays *exprs_arr, const CExpressionArrays *other_exprs_arr)
{
	GPOS_CHECK_STACK_SIZE;
...
CExpressionArrays *
CUtils::GetCombinedExpressionArrays(CMemoryPool *mp, CExpressionArrays *exprs_array,
...
void
CUtils::AddExprs(CExpressionArrays *results_exprs, CExpressionArrays *input_exprs)
{
