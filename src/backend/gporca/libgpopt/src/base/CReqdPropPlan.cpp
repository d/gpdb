...
//---------------------------------------------------------------------------
void
CReqdPropPlan::ComputeReqdCols(CMemoryPool *mp, CExpressionHandle &exprhdl, CReqdProp *prpInput, ULONG child_index,
...
//---------------------------------------------------------------------------
void
CReqdPropPlan::ComputeReqdCTEs(CMemoryPool *mp, CExpressionHandle &exprhdl, CReqdProp *prpInput, ULONG child_index,
							   CDrvdPropArray *pdrgpdpCtxt)
{
...
//---------------------------------------------------------------------------
void
CReqdPropPlan::Compute(CMemoryPool *mp, CExpressionHandle &exprhdl, CReqdProp *prpInput, ULONG child_index,
					   CDrvdPropArray *pdrgpdpCtxt, ULONG ulOptReq)
{
	...
	m_ped = GPOS_NEW(mp) CEnfdDistribution(
		popPhysical->PdsRequired(mp, exprhdl, prppInput->Ped()->PdsRequired(), child_index, pdrgpdpCtxt, ulDistrReq),
...
//---------------------------------------------------------------------------
CPartFilterMap *
CReqdPropPlan::PpfmCombineDerived(CMemoryPool *mp, CExpressionHandle &exprhdl, CReqdPropPlan *prppInput,
...
CReqdPropPlan::InitReqdPartitionPropagation(CMemoryPool *mp, CPartInfo *ppartinfo)
{
	...
	for (ULONG ul = 0; ul < ppartinfo->UlConsumers(); ul++)
	{
		...
		ppim->Insert(scan_id, GPOS_NEW(mp) UlongToPartConstraintMap(mp), CPartIndexMap::EpimConsumer,
					 0,	 //ulExpectedPropagators
					 mdid, pdrgppartkeys, ppartcnstr);
	}

	m_pepp = GPOS_NEW(mp)
		CEnfdPartitionPropagation(GPOS_NEW(mp) CPartitionPropagationSpec(ppim, GPOS_NEW(mp) CPartFilterMap(mp)), eppm,
...
//---------------------------------------------------------------------------
BOOL
CReqdPropPlan::FCompatible(CExpressionHandle &exprhdl, CPhysical *popPhysical, const CDrvdPropRelational *pdprel,
						   const CDrvdPropPlan *pdpplan) const
{
	...
	return m_peo->FCompatible(pdpplan->Pos()) && m_ped->FCompatible(pdpplan->Pds()) &&
		   m_per->FCompatible(pdpplan->Prs()) && pdpplan->Ppim()->FSatisfies(m_pepp->PppsRequired()) &&
