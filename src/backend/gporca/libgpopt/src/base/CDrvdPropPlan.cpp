...
//
//---------------------------------------------------------------------------
CDrvdPropPlan::CDrvdPropPlan() : m_pos(NULL), m_pds(NULL), m_prs(NULL), m_ppim(NULL), m_ppfm(NULL), m_pcm(NULL)
{
}
...
//---------------------------------------------------------------------------
void
CDrvdPropPlan::Derive(CMemoryPool *mp, CExpressionHandle &exprhdl, CDrvdPropCtxt *pdpctxt)
{
...
//---------------------------------------------------------------------------
void
CDrvdPropPlan::CopyCTEProducerPlanProps(CMemoryPool *mp, CDrvdPropCtxt *pdpctxt, COperator *pop)
{
...
CDrvdPropPlan::FSatisfies(const CReqdPropPlan *prpp) const
{
	...
	return m_pos->FSatisfies(prpp->Peo()->PosRequired()) && m_pds->FSatisfies(prpp->Ped()->PdsRequired()) &&
		   m_prs->FSatisfies(prpp->Per()->PrsRequired()) && m_ppim->FSatisfies(prpp->Pepp()->PppsRequired()) &&
...
CDrvdPropPlan::Equals(const CDrvdPropPlan *pdpplan) const
{
	return m_pos->Matches(pdpplan->Pos()) && m_pds->Equals(pdpplan->Pds()) && m_prs->Matches(pdpplan->Prs()) &&
		   m_ppim->Equals(pdpplan->Ppim()) && m_pcm->Equals(pdpplan->GetCostModel());
}
