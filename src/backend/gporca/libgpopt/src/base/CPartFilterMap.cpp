...
//---------------------------------------------------------------------------
void
CPartFilterMap::AddPartFilter(CMemoryPool *mp, ULONG scan_id, CExpression *pexpr, IStatistics *stats)
{
	GPOS_ASSERT(NULL != pexpr);
	CPartFilter *ppf = m_phmulpf->Find(&scan_id);
	if (NULL != ppf)
	...
	BOOL fSuccess GPOS_ASSERTS_ONLY = m_phmulpf->Insert(GPOS_NEW(mp) ULONG(scan_id), ppf);
...
//---------------------------------------------------------------------------
BOOL
CPartFilterMap::FCopyPartFilter(CMemoryPool *mp, ULONG scan_id, CPartFilterMap *ppfmSource, CColRefSet *filter_colrefs)
{
