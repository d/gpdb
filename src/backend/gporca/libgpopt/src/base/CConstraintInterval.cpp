...
CConstraintInterval::CConstraintInterval(CMemoryPool *mp, const CColRef *colref,
										 CRangeArray *pdrgprng, BOOL fIncludesNull)
...
//---------------------------------------------------------------------------
CConstraint *
CConstraintInterval::PcnstrCopyWithRemappedColumns(CMemoryPool *mp,
											   ...
											   BOOL infer_nulls_as)
{
	...
	switch (pexpr->Pop()->Eopid())
	{
		...
		case COperator::EopScalarConst:
		{
			...
			{
				pci = GPOS_NEW(mp) CConstraintInterval(mp, colref, GPOS_NEW(mp) CRangeArray(mp),
													   false /*fIncludesNull*/);
			}
		}
		break;
...
//---------------------------------------------------------------------------
CConstraintInterval *
CConstraintInterval::PcnstrIntervalFromScalarArrayCmp(CMemoryPool *mp, CExpression *pexpr,
	...
	switch (cmp_type)
	{
		...
		case IMDType::EcmptNEq:
		{
			...
			for (ULONG ul = 0; ul < apdatumsortedset->Size(); ul++)
			{
				...
				CRange *prng = GPOS_NEW(mp) CRange(mdid, pcomp, pprevdatum, CRange::EriExcluded,
												   datum, CRange::EriExcluded);
				prgrng->Append(prng);
...
//---------------------------------------------------------------------------
CConstraintInterval *
CConstraintInterval::PciIntervalFromConstraint(CMemoryPool *mp, CConstraint *pcnstr,
											   CColRef *colref)
{
	if (NULL == pcnstr)
	{
...
//---------------------------------------------------------------------------
CConstraintInterval *
CConstraintInterval::PciIntervalFromScalarNullTest(CMemoryPool *mp, CExpression *pexpr,
												   CColRef *colref)
{
	GPOS_ASSERT(NULL != pexpr);
...
//---------------------------------------------------------------------------
CConstraintInterval *
CConstraintInterval::PciIntervalFromColConstCmp(CMemoryPool *mp, CColRef *colref,
...
//---------------------------------------------------------------------------
CConstraintInterval *
CConstraintInterval::PciIntervalFromScalarCmp(CMemoryPool *mp, CExpression *pexpr, CColRef *colref,
											  BOOL infer_nulls_as)
{
	GPOS_ASSERT(NULL != pexpr);
...
//---------------------------------------------------------------------------
CConstraintInterval *
CConstraintInterval::PciIntervalFromScalarBoolOp(CMemoryPool *mp, CExpression *pexpr,
												 CColRef *colref, BOOL infer_nulls_as)
{
	GPOS_ASSERT(NULL != pexpr);
...
//---------------------------------------------------------------------------
CConstraintInterval *
CConstraintInterval::PciIntervalFromScalarBoolOr(CMemoryPool *mp, CExpression *pexpr,
												 CColRef *colref, BOOL infer_nulls_as)
{
	GPOS_ASSERT(NULL != pexpr);
	GPOS_ASSERT(CUtils::FScalarBoolOp(pexpr));
...
//---------------------------------------------------------------------------
CConstraintInterval *
CConstraintInterval::PciIntervalFromScalarBoolAnd(CMemoryPool *mp, CExpression *pexpr,
												  CColRef *colref, BOOL infer_nulls_as)
{
	GPOS_ASSERT(NULL != pexpr);
	GPOS_ASSERT(CUtils::FScalarBoolOp(pexpr));
...
CConstraintInterval::PciDifference(CMemoryPool *mp, CConstraintInterval *pci)
{
	...
	while (ulFst < ulNumRangesFst && ulSnd < ulNumRangesSnd)
	{
		...
		{
			prangeNew = PrangeDiffWithRightResidual(mp, prangeThis, prangeOther, &prangeResidual,
													pdrgprngResidual);
			ulSnd++;
		}
...
CConstraintInterval::PciUnbounded(CMemoryPool *mp, const CColRef *colref, BOOL fIncludesNull)
{
	...
	mdid->AddRef();
	CRange *prange =
		GPOS_NEW(mp) CRange(mdid, COptCtxt::PoctxtFromTLS()->Pcomp(), NULL /*ppointLeft*/,
...
//---------------------------------------------------------------------------
CRange *
CConstraintInterval::PrangeDiffWithRightResidual(CMemoryPool *mp, CRange *prangeFirst,
...
//---------------------------------------------------------------------------
CRangeArray *
CConstraintInterval::PciRangeFromColConstCmp(CMemoryPool *mp, IMDType::ECmpType cmp_type,
