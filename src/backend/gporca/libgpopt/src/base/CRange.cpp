...
//
//---------------------------------------------------------------------------
CRange::CRange(IMDId *mdid, const IComparator *pcomp, IDatum *pdatumLeft, ERangeInclusion eriLeft,
			   IDatum *pdatumRight, ERangeInclusion eriRight)
	: m_mdid(mdid),
	  m_pcomp(pcomp),
...
CRange::PexprScalar(CMemoryPool *mp, const CColRef *colref)
{
	...
	CExpression *pexprRight = PexprScalarCompEnd(mp, m_pdatumRight, m_eriRight, IMDType::EcmptLEq,
...
CRange::PrngDifferenceLeft(CMemoryPool *mp, CRange *prange)
{
	...
	if (NULL != prange->PdatumLeft() && FStartsBefore(prange))
	{
		...
		return GPOS_NEW(mp) CRange(m_mdid, m_pcomp, m_pdatumLeft, m_eriLeft, pdatumRight,
...
CRange::PrngDifferenceRight(CMemoryPool *mp, CRange *prange)
{
	...
	if (NULL != prange->PdatumRight() && FEndsAfter(prange))
	{
		...
		return GPOS_NEW(mp)
			CRange(m_mdid, m_pcomp, pdatumRight, EriInverseInclusion(prange->EriRight()),
