...
//
//---------------------------------------------------------------------------
CMDScalarOpGPDB::CMDScalarOpGPDB(CMemoryPool *mp, IMDId *mdid, CMDName *mdname,
								 IMDId *mdid_type_left, IMDId *mdid_type_right,
								 IMDId *result_type_mdid, IMDId *mdid_func, IMDId *mdid_commute_opr,
								 IMDId *m_mdid_inverse_opr, IMDType::ECmpType cmp_type,
								 BOOL returns_null_on_null_input, IMdIdArray *mdid_opfamilies_array,
								 IMDId *mdid_hash_opfamily, IMDId *mdid_legacy_hash_opfamily,
...
CMDScalarOpGPDB::Serialize(CXMLSerializer *xml_serializer) const
{
	...
	xml_serializer->AddAttribute(CDXLTokens::GetDXLTokenStr(EdxltokenIsNDVPreserving),
								 m_is_ndv_preserving);
	...
	{
		SerializeMDIdAsElem(xml_serializer, CDXLTokens::GetDXLTokenStr(dxl_token_array[ul]),
							mdid_array[ul]);
