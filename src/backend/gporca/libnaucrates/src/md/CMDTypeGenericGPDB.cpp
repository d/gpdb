...
//---------------------------------------------------------------------------
CMDTypeGenericGPDB::CMDTypeGenericGPDB(
	CMemoryPool *mp, IMDId *mdid, CMDName *mdname, BOOL is_redistributable, BOOL is_fixed_length,
	ULONG length, BOOL is_passed_by_value, IMDId *mdid_distr_opfamily,
	IMDId *mdid_legacy_distr_opfamily, IMDId *mdid_op_eq, IMDId *mdid_op_neq, IMDId *mdid_op_lt,
	IMDId *mdid_op_leq, IMDId *mdid_op_gt, IMDId *mdid_op_geq, IMDId *mdid_op_cmp,
	IMDId *mdid_op_min, IMDId *mdid_op_max, IMDId *mdid_op_avg, IMDId *mdid_op_sum,
	IMDId *mdid_op_count, BOOL is_hashable, BOOL is_merge_joinable, BOOL is_composite_type,
	  ...
	  m_datum_null(NULL)
{
	...
	m_mdid->AddRef();
	m_datum_null = GPOS_NEW(m_mp)
		CDatumGenericGPDB(m_mp, m_mdid, default_type_modifier, NULL /*pba*/, 0 /*length*/,
...
CMDTypeGenericGPDB::GetDatumForDXLConstVal(const CDXLScalarConstValue *dxl_op) const
{
	...
	return GPOS_NEW(m_mp)
		CDatumGenericGPDB(m_mp, m_mdid, dxl_datum->TypeModifier(), dxl_datum->GetByteArray(),
...
//---------------------------------------------------------------------------
IDatum *
CMDTypeGenericGPDB::GetDatumForDXLDatum(CMemoryPool *mp, const CDXLDatum *dxl_datum) const
{
	m_mdid->AddRef();
	...
	return GPOS_NEW(m_mp) CDatumGenericGPDB(
		mp, m_mdid, dxl_datum_generic->TypeModifier(), dxl_datum_generic->GetByteArray(),
...
CMDTypeGenericGPDB::GetDatumVal(CMemoryPool *mp, IDatum *datum) const
{
	...
	return CreateDXLDatumVal(mp, m_mdid, md_type, datum_generic->TypeModifier(),
							 datum_generic->IsNull(), pba, length, lValue, dValue);
}
...
//---------------------------------------------------------------------------
CDXLDatum *
CMDTypeGenericGPDB::CreateDXLDatumVal(CMemoryPool *mp, IMDId *mdid, const IMDType *md_type,
									  INT type_modifier, BOOL is_null, BYTE *pba, ULONG length,
									  LINT lValue, CDouble dValue)
{
	...
	{
		return CMDTypeGenericGPDB::CreateDXLDatumStatsIntMappable(mp, mdid, type_modifier, is_null,
																  pba, length, lValue, dValue);
	}
...
//---------------------------------------------------------------------------
CDXLDatum *
CMDTypeGenericGPDB::CreateDXLDatumStatsDoubleMappable(CMemoryPool *mp, IMDId *mdid,
													  INT type_modifier, BOOL is_null,
													  BYTE *byte_array, ULONG length, LINT,
													  CDouble double_value)
{
	GPOS_ASSERT(CMDTypeGenericGPDB::HasByte2DoubleMapping(mdid));
	return GPOS_NEW(mp) CDXLDatumStatsDoubleMappable(mp, mdid, type_modifier, is_null, byte_array,
													 length, double_value);
}
...
//---------------------------------------------------------------------------
CDXLDatum *
CMDTypeGenericGPDB::CreateDXLDatumStatsIntMappable(CMemoryPool *mp, IMDId *mdid, INT type_modifier,
												   BOOL is_null, BYTE *byte_array, ULONG length,
												   LINT lint_value,
												   CDouble	// double_value
)
{
	return GPOS_NEW(mp) CDXLDatumStatsLintMappable(mp, mdid, type_modifier, is_null, byte_array,
												   length, lint_value);
}
...
CMDTypeGenericGPDB::GetDXLDatumNull(CMemoryPool *mp) const
{
	...
	CMDAccessor *md_accessor = COptCtxt::PoctxtFromTLS()->Pmda();
	const IMDType *md_type = md_accessor->RetrieveType(m_mdid);
	return CreateDXLDatumVal(mp, m_mdid, md_type, default_type_modifier, true /*fConstNull*/,
							 NULL /*byte_array*/, 0 /*length*/, 0 /*lint_value */,
							 0 /*double_value */);
}
