...
//
//---------------------------------------------------------------------------
CMDProviderMemory::CMDProviderMemory(CMemoryPool *mp, const CHAR *file_name) : m_mdmap(NULL)
{
...
//
//---------------------------------------------------------------------------
CMDProviderMemory::CMDProviderMemory(CMemoryPool *mp, IMDCacheObjectArray *mdcache_obj_array) : m_mdmap(NULL)
{
...
//---------------------------------------------------------------------------
void
CMDProviderMemory::LoadMetadataObjectsFromArray(CMemoryPool *mp, IMDCacheObjectArray *mdcache_obj_array)
{
	...
	if (NULL == pstrObj)
	{
		...
		switch (mdid->MdidType())
		{
			case IMDId::EmdidRelStats:
			{
				...
				a_pstrResult = CDXLUtils::SerializeMDObj(mp, a_pdxlrelstats.Value(), true /*fSerializeHeaders*/,
														 false /*findent*/);
				break;
			}
			case IMDId::EmdidColStats:
			{
				...
				a_pmdname.Reset();
				a_pstrResult = CDXLUtils::SerializeMDObj(mp, a_pdxlcolstats.Value(), true /*fSerializeHeaders*/,
														 false /*findent*/);
				break;
			}
			default:
...
//---------------------------------------------------------------------------
IMDId *
CMDProviderMemory::MDId(CMemoryPool *mp, CSystemId sysid, IMDType::ETypeInfo type_info) const
{
