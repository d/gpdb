...
//---------------------------------------------------------------------------
CMDRelationExternalGPDB::CMDRelationExternalGPDB(
	CMemoryPool *mp, IMDId *mdid, CMDName *mdname, Ereldistrpolicy rel_distr_policy, CMDColumnArray *mdcol_array,
	ULongPtrArray *distr_col_array, IMdIdArray *distr_opfamilies, BOOL convert_hash_to_random,
	ULongPtr2dArray *keyset_array, CMDIndexInfoArray *md_index_info_array, IMdIdArray *mdid_triggers_array,
	IMdIdArray *mdid_check_constraint_array, INT reject_limit, BOOL is_reject_limit_in_rows, IMDId *mdid_fmt_err_table)
	: m_mp(mp),
	  m_mdid(mdid),
	  m_mdname(mdname),
	  ...
	  m_nondrop_col_pos_array(NULL)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		else
		{
			...
			(void) m_colpos_nondrop_colpos_map->Insert(GPOS_NEW(m_mp) ULONG(ul), GPOS_NEW(m_mp) ULONG(ulPosNonDropped));
			ulPosNonDropped++;
		}

		(void) m_attrno_nondrop_col_pos_map->Insert(GPOS_NEW(m_mp) INT(pmdcol->AttrNum()), GPOS_NEW(m_mp) ULONG(ul));
...
CMDRelationExternalGPDB::Serialize(CXMLSerializer *xml_serializer) const
{
	...
	SerializeMDIdList(xml_serializer, m_mdid_trigger_array, CDXLTokens::GetDXLTokenStr(EdxltokenTriggers),
