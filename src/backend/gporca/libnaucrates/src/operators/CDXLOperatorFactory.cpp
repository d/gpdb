...
CDXLOperatorFactory::MakeDXLNLJoin(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	{
		is_index_nlj =
			ConvertAttrValueToBool(dxl_memory_manager, index_nlj_xml, EdxltokenPhysicalNLJoinIndex,
	...
	{
		nest_params_exists =
			ConvertAttrValueToBool(dxl_memory_manager, nest_param_exists_xml,
										 ...
										 const Attributes &attrs)
{
	...
	if (NULL != duplicate_sensitive_xml)
	{
		is_duplicate_sensitive =
			ConvertAttrValueToBool(dxl_memory_manager, duplicate_sensitive_xml,
...
CDXLOperatorFactory::MakeDXLAgg(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	{
		stream_safe = ConvertAttrValueToBool(dxl_memory_manager, stream_safe_xml,
											 EdxltokenAggStreamSafe, EdxltokenPhysicalAggregate);
	}
										...
										const Attributes &attrs)
{
	...
	{
		// parse spool id
		ULONG spool_id = ExtractConvertAttrValueToUlong(dxl_memory_manager, attrs, EdxltokenSpoolId,
									  ...
									  const Attributes &attrs)
{
	...
	IMDId *op_id = ExtractConvertAttrValueToMdId(dxl_memory_manager, attrs, EdxltokenOpNo,
												 EdxltokenScalarComp);
...
CDXLOperatorFactory::MakeDXLOpExpr(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	{
		return_type_mdid = ExtractConvertAttrValueToMdId(dxl_memory_manager, attrs, EdxltokenOpType,
														 EdxltokenScalarOpExpr);
	}
...
CDXLOperatorFactory::MakeDXLSubPlan(CDXLMemoryManager *dxl_memory_manager, IMDId *mdid,
									...
									EdxlSubPlanType dxl_subplan_type, CDXLNode *dxlnode_test_expr)
{
	// get the memory pool from the memory manager
	CMemoryPool *mp = dxl_memory_manager->Pmp();
...
CDXLOperatorFactory::MakeDXLCast(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	IMDId *mdid_type = ExtractConvertAttrValueToMdId(dxl_memory_manager, attrs, EdxltokenTypeId,
													 EdxltokenScalarCast);

	IMDId *mdid_func = ExtractConvertAttrValueToMdId(dxl_memory_manager, attrs, EdxltokenFuncId,
													 EdxltokenScalarCast);
										   ...
										   const Attributes &attrs)
{
	...
	INT type_modifier =
		ExtractConvertAttrValueToInt(dxl_memory_manager, attrs, EdxltokenTypeMod,
	...
	INT location = ExtractConvertAttrValueToInt(dxl_memory_manager, attrs, EdxltokenLocation,
												EdxltokenScalarCoerceToDomain);

	return GPOS_NEW(mp) CDXLScalarCoerceToDomain(mp, mdid_type, type_modifier,
										...
										const Attributes &attrs)
{
	...
	return GPOS_NEW(mp) CDXLScalarCoerceViaIO(mp, mdid_type, type_modifier,
											  (EdxlCoercionForm) coercion_form, location);
}
											...
											const Attributes &attrs)
{
	...
	BOOL is_explicit = ExtractConvertAttrValueToBool(dxl_memory_manager, attrs, EdxltokenIsExplicit,
													 EdxltokenScalarArrayCoerceExpr);
	ULONG coercion_form = ExtractConvertAttrValueToUlong(
	...
	return GPOS_NEW(mp)
		CDXLScalarArrayCoerceExpr(mp, element_func, mdid_type, type_modifier, is_explicit,
								  (EdxlCoercionForm) coercion_form, location);
}
...
CDXLOperatorFactory::MakeDXLFuncExpr(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	BOOL is_retset = ExtractConvertAttrValueToBool(dxl_memory_manager, attrs, EdxltokenFuncRetSet,
												   EdxltokenScalarFuncExpr);
	...
	INT type_modifier =
		ExtractConvertAttrValueToInt(dxl_memory_manager, attrs, EdxltokenTypeMod,
									 EdxltokenScalarCast, true, default_type_modifier);
...
CDXLOperatorFactory::MakeDXLAggFunc(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	IMDId *agg_mdid = ExtractConvertAttrValueToMdId(dxl_memory_manager, attrs, EdxltokenAggrefOid,
	...
	if (NULL != return_type_xml)
	{
		resolved_rettype = ExtractConvertAttrValueToMdId(dxl_memory_manager, attrs, EdxltokenTypeId,
														 EdxltokenScalarAggref);
	}
...
CDXLOperatorFactory::MakeDXLProjElem(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	// parse column id
	ULONG id = ExtractConvertAttrValueToUlong(dxl_memory_manager, attrs, EdxltokenColId,
...
CDXLOperatorFactory::MakeDXLHashExpr(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	IMDId *mdid_type = ExtractConvertAttrValueToMdId(dxl_memory_manager, attrs, EdxltokenOpfamily,
...
CDXLOperatorFactory::MakeDXLSortCol(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	ULONG colid = ExtractConvertAttrValueToUlong(dxl_memory_manager, attrs, EdxltokenColId,
	...
	IMDId *sort_op_id = ExtractConvertAttrValueToMdId(dxl_memory_manager, attrs, EdxltokenSortOpId,
									   ...
									   const Attributes &attrs)
{
	...
	// parse metadata id
	IMDId *mdid = ExtractConvertAttrValueToMdId(dxl_memory_manager, attrs, EdxltokenMdid,
												EdxltokenTableDescr);
	...
	{
		user_id = ConvertAttrValueToUlong(dxl_memory_manager, execute_as_user_xml,
										  EdxltokenExecuteAsUser, EdxltokenTableDescr);
	}
...
CDXLOperatorFactory::MakeColumnDescr(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	INT type_modifier =
		ExtractConvertAttrValueToInt(dxl_memory_manager, attrs, EdxltokenTypeMod, EdxltokenColDescr,
	...
	if (NULL != col_dropped_xml)
	{
		...
		col_dropped = ConvertAttrValueToBool(dxl_memory_manager, col_dropped_xml,
	...
	if (NULL != col_len_xml)
	{
		col_len = ConvertAttrValueToUlong(dxl_memory_manager, col_len_xml, EdxltokenColWidth,
										  EdxltokenColDescr);
	}
								   ...
								   Edxltoken target_elem)
{
	...
	INT type_modifier = ExtractConvertAttrValueToInt(dxl_memory_manager, attrs, EdxltokenTypeMod,
													 target_elem, true, default_type_modifier);
...
//---------------------------------------------------------------------------
const XMLCh *
CDXLOperatorFactory::ExtractAttrValue(const Attributes &attrs, Edxltoken target_attr,
...
CDXLOperatorFactory::ExtractConvertAttrValueToShortInt(CDXLMemoryManager *dxl_memory_manager,
													   const Attributes &attrs,
													   Edxltoken target_attr, Edxltoken target_elem,
													   BOOL is_optional, SINT default_val)
{
	const XMLCh *attr_val_xml =
		CDXLOperatorFactory::ExtractAttrValue(attrs, target_attr, target_elem, is_optional);
												 ...
												 CHAR *default_value)
{
	...
	return CDXLOperatorFactory::ConvertAttrValueToSz(dxl_memory_manager, attr_val_xml, target_attr,
													 target_elem);
}
										...
										const Attributes &attrs)
{
	...
	INT colid = ExtractConvertAttrValueToInt(dxl_memory_manager, attrs, EdxltokenColId,
									 ...
									 Edxltoken target_attr, Edxltoken target_elem)
{
	...
	IMDId::EMDIdType typ = (IMDId::EMDIdType) ConvertAttrValueToUlong(dxl_memory_manager, mdid_type,
...
CDXLOperatorFactory::GetGPDBMdId(CDXLMemoryManager *dxl_memory_manager,
								 XMLChArray *remaining_tokens, Edxltoken target_attr,
								 Edxltoken target_elem)
{
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::GetDatumVal(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs,
								 Edxltoken target_elem)
{
	...
	BOOL is_const_null = ExtractConvertAttrValueToBool(dxl_memory_manager, attrs, EdxltokenIsNull,
													   target_elem, true, false);
	...
	if (NULL == func)
	{
		...
		if (attr_val_xml)
		{
			return GetDatumStatsLintMappable(dxl_memory_manager, attrs, target_elem, mdid,
											 is_const_null);
		}
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::GetDatumOid(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs,
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::GetDatumInt2(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs,
								  Edxltoken target_elem, IMDId *mdid, BOOL is_const_null)
{
	// get the memory pool from the memory manager
	CMemoryPool *mp = dxl_memory_manager->Pmp();
	...
	if (!is_const_null)
	{
		val = ExtractConvertAttrValueToShortInt(dxl_memory_manager, attrs, EdxltokenValue,
												target_elem);
	}
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::GetDatumInt4(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs,
								  Edxltoken target_elem, IMDId *mdid, BOOL is_const_null)
{
	// get the memory pool from the memory manager
	CMemoryPool *mp = dxl_memory_manager->Pmp();
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::GetDatumInt8(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs,
								  Edxltoken target_elem, IMDId *mdid, BOOL is_const_null)
{
	// get the memory pool from the memory manager
	CMemoryPool *mp = dxl_memory_manager->Pmp();
...
//---------------------------------------------------------------------------
CDXLDatum *
CDXLOperatorFactory::GetDatumBool(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs,
								  Edxltoken target_elem, IMDId *mdid, BOOL is_const_null)
{
	// get the memory pool from the memory manager
	CMemoryPool *mp = dxl_memory_manager->Pmp();
											   ...
											   IMDId *mdid, BOOL is_const_null)
{
	...
	INT type_modifier =
		ExtractConvertAttrValueToInt(dxl_memory_manager, attrs, EdxltokenTypeMod,
									 EdxltokenScalarCast, true, -1 /* default_val value */
		);

	return GPOS_NEW(mp)
		CDXLDatumStatsLintMappable(mp, mdid, type_modifier, is_const_null, data, len, value);
}
...
//---------------------------------------------------------------------------
LINT
CDXLOperatorFactory::Value(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs,
						   Edxltoken target_elem, BYTE *data)
{
	if (NULL == data)
	{
		// unable to decode value. probably not Base64 encoded.
	...
	return ExtractConvertAttrValueToLint(dxl_memory_manager, attrs, EdxltokenLintValue,
										 target_elem);
}
...
//---------------------------------------------------------------------------
BYTE *
CDXLOperatorFactory::GetByteArray(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs,
												 ...
												 IMDId *mdid, BOOL is_const_null)
{
	...
	if (!is_const_null)
	{
		...
		value = ExtractConvertAttrValueToDouble(dxl_memory_manager, attrs, EdxltokenDoubleValue,
												target_elem);
	}
	INT type_modifier =
		ExtractConvertAttrValueToInt(dxl_memory_manager, attrs, EdxltokenTypeMod,
	...
	return GPOS_NEW(mp)
		CDXLDatumStatsDoubleMappable(mp, mdid, type_modifier, is_const_null, data, len, value);
}
...
CDXLOperatorFactory::SetSegmentInfo(CDXLMemoryManager *dxl_memory_manager,
									CDXLPhysicalMotion *motion, const Attributes &attrs,
									Edxltoken target_elem)
{
...
CSystemId
CDXLOperatorFactory::Sysid(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs,
						   Edxltoken target_attr, Edxltoken target_elem)
{
	// extract systemids
	...
	ULONG type = CDXLOperatorFactory::ConvertAttrValueToUlong(dxl_memory_manager, sys_id_comp,
															  target_attr, target_elem);
...
CDXLOperatorFactory::MakeWindowRef(CDXLMemoryManager *dxl_memory_manager, const Attributes &attrs)
{
	...
	return GPOS_NEW(mp)
		CDXLScalarWindowRef(mp, mdid_func, mdid_return_type, is_distinct, is_star_arg,
...
CDXLOperatorFactory::ParseRelationDistPolicy(const XMLCh *xml_val)
{
	...
	else if (0 == XMLString::compareString(xml_val,
										   CDXLTokens::XmlstrToken(EdxltokenRelDistrReplicated)))
	{
