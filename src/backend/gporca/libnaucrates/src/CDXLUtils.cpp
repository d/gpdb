...
//---------------------------------------------------------------------------
CParseHandlerDXL *
CDXLUtils::GetParseHandlerForDXLString(CMemoryPool *mp, const CHAR *dxl_string, const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CParseHandlerDXL *
CDXLUtils::GetParseHandlerForDXLFile(CMemoryPool *mp, const CHAR *dxl_filename, const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CParseHandlerDXL *
CDXLUtils::GetParseHandlerForDXLString(CMemoryPool *mp, const CWStringBase *dxl_string, const CHAR *xsd_file_path)
{
...
//---------------------------------------------------------------------------
CDXLNode *
CDXLUtils::GetPlanDXLNode(CMemoryPool *mp, const CHAR *dxl_string, const CHAR *xsd_file_path, ULLONG *plan_id,
...
//---------------------------------------------------------------------------
CQueryToDXLResult *
CDXLUtils::ParseQueryToQueryDXLTree(CMemoryPool *mp, const CHAR *dxl_string, const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CDXLNode *
CDXLUtils::ParseDXLToScalarExprDXLNode(CMemoryPool *mp, const CHAR *dxl_string, const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
IMDCacheObjectArray *
CDXLUtils::ParseDXLToIMDObjectArray(CMemoryPool *mp, const CHAR *dxl_string, const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
IMDId *
CDXLUtils::ParseDXLToMDId(CMemoryPool *mp, const CWStringBase *dxl_string, const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CMDRequest *
CDXLUtils::ParseDXLToMDRequest(CMemoryPool *mp, const CHAR *dxl_string, const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CMDRequest *
CDXLUtils::ParseDXLToMDRequest(CMemoryPool *mp, const WCHAR *dxl_string, const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
COptimizerConfig *
CDXLUtils::ParseDXLToOptimizerConfig(CMemoryPool *mp, const CHAR *dxl_string, const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
CDXLUtils::ParseDXLToStatsDerivedRelArray(CMemoryPool *mp, const CWStringBase *dxl_string, const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CDXLStatsDerivedRelationArray *
CDXLUtils::ParseDXLToStatsDerivedRelArray(CMemoryPool *mp, const CHAR *dxl_string, const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CStatisticsArray *
CDXLUtils::ParseDXLToOptimizerStatisticObjArray(CMemoryPool *mp, CMDAccessor *md_accessor,
												CDXLStatsDerivedRelationArray *dxl_derived_rel_stats_array)
{
	...
	for (ULONG ulIdxRelStat = 0; ulIdxRelStat < ulRelStat; ulIdxRelStat++)
	{
		...
		for (ULONG column_id_idx = 0; column_id_idx < num_of_columns; column_id_idx++)
		{
			...
			CHistogram *histogram = GPOS_NEW(mp) CHistogram(mp, stats_buckets_array, true /*is_well_defined*/,
...
//---------------------------------------------------------------------------
IDatum *
CDXLUtils::GetDatum(CMemoryPool *mp, CMDAccessor *md_accessor, const CDXLDatum *dxl_datum)
{
...
//---------------------------------------------------------------------------
IMDCacheObjectArray *
CDXLUtils::ParseDXLToIMDObjectArray(CMemoryPool *mp, const CWStringBase *dxl_string, const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
IMDCacheObject *
CDXLUtils::ParseDXLToIMDIdCacheObj(CMemoryPool *mp, const CWStringBase *dxl_string, const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
void
CDXLUtils::SerializeQuery(CMemoryPool *mp, IOstream &os, const CDXLNode *dxl_query_node,
...
//---------------------------------------------------------------------------
void
CDXLUtils::SerializePlan(CMemoryPool *mp, IOstream &os, const CDXLNode *node, ULLONG plan_id, ULLONG plan_space_size,
						 BOOL serialize_header_footer, BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
void
CDXLUtils::SerializeMetadata(CMemoryPool *mp, const IMDCacheObjectArray *imd_obj_array, IOstream &os,
							 BOOL serialize_header_footer, BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CWStringDynamic *
CDXLUtils::SerializeMetadata(CMemoryPool *mp, const IMDId *mdid, BOOL serialize_header_footer, BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CWStringDynamic *
CDXLUtils::SerializeSamplePlans(CMemoryPool *mp, CEnumeratorConfig *enumerator_cfg, BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CWStringDynamic *
CDXLUtils::SerializeCostDistr(CMemoryPool *mp, CEnumeratorConfig *enumerator_cfg, BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
void
CDXLUtils::SerializeMDRequest(CMemoryPool *mp, CMDRequest *md_request, IOstream &os, BOOL serialize_header_footer,
							  BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
	GPOS_ASSERT(NULL != md_request);
...
//---------------------------------------------------------------------------
CWStringDynamic *
CDXLUtils::SerializeStatistics(CMemoryPool *mp, CMDAccessor *md_accessor, const CStatisticsArray *statistics_array,
							   BOOL serialize_header_footer, BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CWStringDynamic *
CDXLUtils::SerializeMetadata(CMemoryPool *mp, const IMDCacheObjectArray *imd_obj_array, BOOL serialize_header_footer,
							 BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
	GPOS_ASSERT(NULL != imd_obj_array);
...
//---------------------------------------------------------------------------
CWStringDynamic *
CDXLUtils::SerializeMDObj(CMemoryPool *mp, const IMDCacheObject *imd_cache_obj, BOOL serialize_header_footer,
						  BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CWStringDynamic *
CDXLUtils::SerializeScalarExpr(CMemoryPool *mp, const CDXLNode *node, BOOL serialize_header_footer, BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
	GPOS_ASSERT(NULL != node);
	CWStringDynamic *dxl_string = GPOS_NEW(mp) CWStringDynamic(mp);
...
//---------------------------------------------------------------------------
CWStringDynamic *
CDXLUtils::CreateDynamicStringFromXMLChArray(CDXLMemoryManager *memory_manager, const XMLCh *xml_string)
{
...
//---------------------------------------------------------------------------
BYTE *
CDXLUtils::CreateStringFrom64XMLStr(CDXLMemoryManager *memory_manager, const XMLCh *xml_string,
...
//---------------------------------------------------------------------------
CWStringDynamic *
CDXLUtils::EncodeByteArrayToString(CMemoryPool *mp, const BYTE *byte, ULONG length)
{
...
//---------------------------------------------------------------------------
BYTE *
CDXLUtils::DecodeByteArrayFromString(CMemoryPool *mp, const CWStringDynamic *dxl_string, ULONG *length)
{
	CAutoP<CDXLMemoryManager> a_pmm(GPOS_NEW(mp) CDXLMemoryManager(mp));
...
CWStringDynamic *
CDXLUtils::SerializeToCommaSeparatedString(CMemoryPool *mp, const CharPtrArray *char_ptr_array)
{
	CWStringDynamic *dxl_string = GPOS_NEW(mp) CWStringDynamic(mp);
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		{
			dxl_string->AppendFormat(GPOS_WSZ_LIT("%c%ls"), value,
...
//---------------------------------------------------------------------------
CHAR *
CDXLUtils::CreateMultiByteCharStringFromWCString(CMemoryPool *mp, const WCHAR *wc)
{
	GPOS_ASSERT(NULL != wc);
