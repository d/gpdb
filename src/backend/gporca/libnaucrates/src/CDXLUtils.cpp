...
//---------------------------------------------------------------------------
CParseHandlerDXL *
CDXLUtils::GetParseHandlerForDXLString(CMemoryPool *mp, const CWStringBase *dxl_string,
									   const CHAR *xsd_file_path)
{
...
//---------------------------------------------------------------------------
CDXLNode *
CDXLUtils::GetPlanDXLNode(CMemoryPool *mp, const CHAR *dxl_string, const CHAR *xsd_file_path,
...
CDXLUtils::ParseDXLToStatsDerivedRelArray(CMemoryPool *mp, const CWStringBase *dxl_string,
										  const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CDXLStatsDerivedRelationArray *
CDXLUtils::ParseDXLToStatsDerivedRelArray(CMemoryPool *mp, const CHAR *dxl_string,
										  const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
	...
	for (ULONG ulIdxRelStat = 0; ulIdxRelStat < ulRelStat; ulIdxRelStat++)
	{
		...
		for (ULONG column_id_idx = 0; column_id_idx < num_of_columns; column_id_idx++)
		{
			...
			CHistogram *histogram =
				GPOS_NEW(mp) CHistogram(mp, stats_buckets_array, true /*is_well_defined*/,
								  ...
								  CDXLStatsDerivedColumn *dxl_derived_col_stats)
{
	...
	for (ULONG ul = 0; ul < num_of_buckets; ul++)
	{
		...
		CBucket *bucket = GPOS_NEW(mp) CBucket(
			point_lower_bound, point_upper_bound, dxl_bucket->IsLowerClosed(),
...
//---------------------------------------------------------------------------
IMDCacheObjectArray *
CDXLUtils::ParseDXLToIMDObjectArray(CMemoryPool *mp, const CWStringBase *dxl_string,
									const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
IMDCacheObject *
CDXLUtils::ParseDXLToIMDIdCacheObj(CMemoryPool *mp, const CWStringBase *dxl_string,
								   const CHAR *xsd_file_path)
{
	GPOS_ASSERT(NULL != mp);
...
CDXLUtils::SerializeQuery(CMemoryPool *mp, IOstream &os, const CDXLNode *dxl_query_node,
						  ...
						  const CDXLNodeArray *cte_producers, BOOL serialize_header_footer,
						  BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
void
CDXLUtils::SerializePlan(CMemoryPool *mp, IOstream &os, const CDXLNode *node, ULLONG plan_id,
...
//---------------------------------------------------------------------------
void
CDXLUtils::SerializeMetadata(CMemoryPool *mp, const IMDCacheObjectArray *imd_obj_array,
...
//---------------------------------------------------------------------------
CWStringDynamic *
CDXLUtils::SerializeMetadata(CMemoryPool *mp, const IMDId *mdid, BOOL serialize_header_footer,
							 BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
CWStringDynamic *
CDXLUtils::SerializeSamplePlans(CMemoryPool *mp, CEnumeratorConfig *enumerator_cfg,
								BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
void
CDXLUtils::SerializeMDRequest(CMemoryPool *mp, CMDRequest *md_request, IOstream &os,
							   ...
							   BOOL serialize_header_footer, BOOL indentation)
{
	...
	CDXLUtils::SerializeStatistics(mp, md_accessor, statistics_array, oss, serialize_header_footer,
								   indentation);
...
void
CDXLUtils::SerializeStatistics(CMemoryPool *mp, CMDAccessor *md_accessor,
							   const CStatisticsArray *statistics_array, IOstream &os,
							   BOOL serialize_header_footer, BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
	GPOS_ASSERT(NULL != statistics_array);
...
//---------------------------------------------------------------------------
CWStringDynamic *
CDXLUtils::SerializeScalarExpr(CMemoryPool *mp, const CDXLNode *node, BOOL serialize_header_footer,
							   BOOL indentation)
{
	GPOS_ASSERT(NULL != mp);
	GPOS_ASSERT(NULL != node);
...
CDXLUtils::SerializeHeader(CMemoryPool *mp, CXMLSerializer *xml_serializer)
{
	...
	namespace_specification_string.AppendFormat(
		GPOS_WSZ_LIT("%ls%ls%ls"), CDXLTokens::GetDXLTokenStr(EdxltokenNamespaceAttr)->GetBuffer(),
...
//---------------------------------------------------------------------------
BYTE *
CDXLUtils::DecodeByteArrayFromString(CMemoryPool *mp, const CWStringDynamic *dxl_string,
									 ULONG *length)
{
	CAutoP<CDXLMemoryManager> a_pmm(GPOS_NEW(mp) CDXLMemoryManager(mp));
