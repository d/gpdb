	...
	: m_mdid(nullptr), m_val(val), m_is_null(is_null)
{
	...
	gpos::Ref<IMDId> mdid = dynamic_cast<const CMDTypeInt2GPDB *>(
...
//
//---------------------------------------------------------------------------
CDatumInt2GPDB::CDatumInt2GPDB(gpos::Ref<IMDId> mdid, SINT val, BOOL is_null)
...
{
	if (!datum->MDId()->Equals(m_mdid.get()))
	{
		return false;
	}
