...
//
//---------------------------------------------------------------------------
CDatumGenericGPDB::CDatumGenericGPDB(CMemoryPool *mp, IMDId *mdid, INT type_modifier,
...
CDatumGenericGPDB::MakeCopy(CMemoryPool *mp) const
{
	...
	return GPOS_NEW(mp) CDatumGenericGPDB(mp, m_mdid, m_type_modifier, m_bytearray_value, m_size,
										  m_is_null, m_stats_comp_val_int, m_stats_comp_val_double);
}
...
CDatumGenericGPDB::MakePaddedDatum(CMemoryPool *mp, ULONG col_len) const
{
	...
	if (gpos::ulong_max != adjusted_col_width && datum_len < adjusted_col_width)
	{
		...
		// create a new datum
		this->MDId()->AddRef();
		CDatumGenericGPDB *datum_new = GPOS_NEW(m_mp)
			CDatumGenericGPDB(mp, this->MDId(), this->TypeModifier(), dest, adjusted_col_width,
