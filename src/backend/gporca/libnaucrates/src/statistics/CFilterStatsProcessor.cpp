...
CDouble
CFilterStatsProcessor::SelectivityOfPredicate(CMemoryPool *mp, CExpression *pred,
											  CTableDescriptor *ptabdesc, CColRefSet *outer_refs)
{
	...
	IStatistics *base_table_stats = md_accessor->Pstats(mp, ptabdesc->MDId(), used_local_col_refs,
	...
	if (NULL != expr_with_outer_refs)
	{
		...
		for (ULONG ul = 0; ul < size; ul++)
		{
			...
			if (CPredicateUtils::FIdentCompareOuterRefExprIgnoreCast(pexpr, outer_refs,
																	 &local_col_ref))
			{
	...
	if (1 < num_outer_ref_preds)
	{
		...
		result = std::min(
			result.Get() /
				CScaleFactorUtils::DampedFilterScaleFactor(stats_config, num_outer_ref_preds).Get(),
			1.0);
...
CStatistics *
CFilterStatsProcessor::MakeStatsFilter(CMemoryPool *mp, const CStatistics *input_stats,
									   CStatsPred *base_pred_stats, BOOL do_cap_NDVs)
{
	...
	else
	{
		histograms_new = MakeHistHashMapConjOrDisjFilter(
			mp, stats_config, histograms_copy, input_rows, base_pred_stats, &scale_factor);
	...
	CStatistics *filter_stats = GPOS_NEW(mp)
		CStatistics(mp, histograms_new, input_stats->CopyWidths(mp), rows_filter,
					input_stats->IsEmpty(), input_stats->GetNumberOfPredicates() + num_predicates);
...
UlongToHistogramMap *
CFilterStatsProcessor::MakeHistHashMapConjOrDisjFilter(CMemoryPool *mp,
													   const CStatisticsConfig *stats_config,
													   UlongToHistogramMap *input_histograms,
													   CDouble input_rows, CStatsPred *pred_stats,
													   CDouble *scale_factor)
{
	...
	if (CStatsPred::EsptConj == pred_stats->GetPredStatsType())
	{
		...
		return MakeHistHashMapConjFilter(mp, stats_config, input_histograms, input_rows,
										 conjunctive_pred_stats, scale_factor);
	}

	CStatsPredDisj *disjunctive_pred_stats = CStatsPredDisj::ConvertPredStats(pred_stats);
	result_histograms = MakeHistHashMapDisjFilter(mp, stats_config, input_histograms, input_rows,
												  disjunctive_pred_stats, scale_factor);
...
CFilterStatsProcessor::MakeHistHashMapConjFilter(
	CMemoryPool *mp, const CStatisticsConfig *stats_config, UlongToHistogramMap *input_histograms,
	CDouble input_rows, CStatsPredConj *conjunctive_pred_stats, CDouble *scale_factor)
{
	GPOS_ASSERT(NULL != stats_config);
	GPOS_ASSERT(NULL != input_histograms);
	...
	for (ULONG ul = 0; ul < filters; ul++)
	{
		...
		if (CStatsPred::EsptDisj != child_pred_stats->GetPredStatsType())
		{
			...
			result_histogram = MakeHistSimpleFilter(mp, child_pred_stats, filter_colids,
													hist_before, &last_scale_factor, &last_colid);
			...
			CStatisticsUtils::AddHistogram(mp, colid, result_histogram, result_histograms,
										   true /* fReplaceOld */);
		...
		else
		{
			...
			UlongToHistogramMap *disjunctive_histograms_after =
				MakeHistHashMapDisjFilter(mp, stats_config, result_histograms, num_disj_input_rows,
...
CFilterStatsProcessor::MakeHistHashMapDisjFilter(
	CMemoryPool *mp, const CStatisticsConfig *stats_config, UlongToHistogramMap *input_histograms,
	CDouble input_rows, CStatsPredDisj *disjunctive_pred_stats, CDouble *scale_factor)
{
	GPOS_ASSERT(NULL != stats_config);
	GPOS_ASSERT(NULL != input_histograms);
	...
	for (ULONG ul = 0; ul < filters; ul++)
	{
		...
		if (IsNewStatsColumn(colid, previous_colid))
		{
			...
			CStatisticsUtils::UpdateDisjStatistics(mp, non_updatable_cols, input_rows,
												   cumulative_rows, previous_histogram,
		...
		if (is_pred_simple)
		{
			...
			disjunctive_child_col_histogram =
				MakeHistSimpleFilter(mp, child_pred_stats, filter_colids, histogram,
		...
		{
			child_histograms =
				MakeHistHashMapConjOrDisjFilter(mp, stats_config, input_histograms, input_rows,
			...
			else
			{
					...
					disjunctive_child_col_histogram->IsWellDefined())
				{
					...
					new_histogram = previous_histogram->MakeUnionHistogramNormalize(
						cumulative_rows, disjunctive_child_col_histogram, num_rows_disj_child,
						&output_rows);
				...
				else
				{
					...
					cumulative_rows =
						CDouble(std::max(num_rows_disj_child.Get(), cumulative_rows.Get()));
				}
		...
		else
		{
			...
			UlongToHistogramMap *merged_histograms =
				CStatisticsUtils::MergeHistogramMapsForDisjPreds(
					mp, non_updatable_cols, disjunctive_result_histograms, child_histograms,
					current_rows_estimate, num_rows_disj_child);
	...
	CStatisticsUtils::UpdateDisjStatistics(mp, non_updatable_cols, input_rows, cumulative_rows,
...
//	an AND/OR predicate
CHistogram *
CFilterStatsProcessor::MakeHistSimpleFilter(CMemoryPool *mp, CStatsPred *pred_stats,
											CBitSet *filter_colids, CHistogram *hist_before,
											CDouble *last_scale_factor, ULONG *target_last_colid)
{
	if (CStatsPred::EsptPoint == pred_stats->GetPredStatsType())
	{
		...
		return MakeHistPointFilter(point_pred_stats, filter_colids, hist_before, last_scale_factor,
								   target_last_colid);
	}
	...
	if (CStatsPred::EsptLike == pred_stats->GetPredStatsType())
	{
		...
		return MakeHistLikeFilter(like_pred_stats, filter_colids, hist_before, last_scale_factor,
								  target_last_colid);
	}
	...
	if (CStatsPred::EsptArrayCmp == pred_stats->GetPredStatsType())
	{
		...
		return MakeHistArrayCmpAnyFilter(mp, arraycmp_pred_stats, filter_colids, hist_before,
	...
	return MakeHistUnsupportedPred(unsupported_pred_stats, filter_colids, hist_before,
								   last_scale_factor, target_last_colid);
}

// create a new histograms after applying the point filter
CHistogram *
CFilterStatsProcessor::MakeHistPointFilter(CStatsPredPoint *pred_stats, CBitSet *filter_colids,
...
CHistogram *
CFilterStatsProcessor::MakeHistUnsupportedPred(CStatsPredUnsupported *pred_stats,
											   CBitSet *filter_colids, CHistogram *hist_before,
											   CDouble *last_scale_factor, ULONG *target_last_colid)
{
	GPOS_ASSERT(NULL != pred_stats);
...
CHistogram *
CFilterStatsProcessor::MakeHistLikeFilter(CStatsPredLike *pred_stats, CBitSet *filter_colids,
										  CHistogram *hist_before, CDouble *last_scale_factor,
										  ULONG *target_last_colid)
{
	GPOS_ASSERT(NULL != pred_stats);
...
CHistogram *
CFilterStatsProcessor::MakeHistArrayCmpAnyFilter(CMemoryPool *mp, CStatsPredArrayCmp *pred_stats,
												 ...
												 ULONG *target_last_colid)
{
	...
	CHistogram *dummy_histogram = GPOS_NEW(mp) CHistogram(
		mp, dummy_histogram_buckets, true /* is_well_defined */, CDouble(0.0) /* null_freq */,
	...
	GPOS_ASSERT(dummy_histogram->IsValid() &&
				dummy_histogram->GetNumDistinct() - dummy_rows < CStatistics::Epsilon);

	// Compute the join'ed histogram
	CHistogram *result_histogram =
		base_histogram->MakeJoinHistogram(pred_stats->GetCmpType(), dummy_histogram);
