...
CHistogram::MakeHistogramInequalityFilter(CPoint *point) const
{
	...
	return GPOS_NEW(m_mp) CHistogram(m_mp, histogram_buckets, true /*is_well_defined*/, null_freq, m_distinct_remaining,
									 m_freq_remaining);
}
...
CHistogram::MakeHistogramEqualFilter(CPoint *point) const
{
	...
	if (point->GetDatum()->IsNull())
	{
		return GPOS_NEW(m_mp) CHistogram(m_mp, GPOS_NEW(m_mp) CBucketArray(m_mp), true /* is_well_defined */,
		...
		0 == histogram_buckets->Size())	 // no match is found in the buckets
	{
		return GPOS_NEW(m_mp)
			CHistogram(m_mp, histogram_buckets,
					   true,															// is_well_defined
					   0.0,																// null_freq
...
CDouble
CHistogram::GetInequalityJoinScaleFactor(CDouble rows, const CHistogram *other_histogram, CDouble rows_other) const
{
	GPOS_ASSERT(NULL != other_histogram);
...
CHistogram::MakeLASJHistogram(CStatsPred::EStatsCmpType stats_cmp_type, const CHistogram *histogram) const
{
	...
	while (idx1 < buckets1 && idx2 < buckets2)
	{
		...
		{
			candidate_bucket =
				(*m_histogram_buckets)[idx1]->MakeBucketCopy(m_mp);	 // candidate bucket in result histogram
			idx1++;
		}
...
CHistogram *
CHistogram::MakeUnionAllHistogramNormalize(CDouble rows, const CHistogram *histogram, CDouble rows_other) const
{
	GPOS_ASSERT(NULL != histogram);
	...
	while (NULL != bucket1 && NULL != bucket2)
	{
		...
		else
		{
			...
			CBucket *merge_bucket = bucket1->SplitAndMergeBuckets(m_mp, bucket2, rows, rows_other, &bucket1_new,
																  &bucket2_new, &result_rows);
...
void
CHistogram::AddBuckets(CMemoryPool *mp, const CBucketArray *src_buckets, CBucketArray *dest_buckets, CDouble rows_old,
...
CHistogram::CombineBuckets(CMemoryPool *mp, CBucketArray *buckets, ULONG desired_num_buckets)
{
	...
	for (ULONG ul = 0; ul < buckets->Size() - 1; ++ul)
	{
			...
			bucket1->IsUpperClosed() ^ bucket2->IsLowerClosed())
		{
			...
			CDouble width1 = bucket1->GetUpperBound()->Width(bucket1->GetLowerBound(), bucket1->IsLowerClosed(),
															 bucket1->IsUpperClosed());
			CDouble freq2 = bucket2->GetFrequency();
			CDouble ndv2 = bucket2->GetNumDistinct();
			CDouble width2 = bucket2->GetUpperBound()->Width(bucket2->GetLowerBound(), bucket2->IsLowerClosed(),
...
CBucket *
CHistogram::GetNextBucket(const CHistogram *histogram, CBucket *new_bucket, BOOL *result_bucket_is_residual,
...
// add buckets from one array to another
void
CHistogram::AddBuckets(CMemoryPool *mp, const CBucketArray *src_buckets, CBucketArray *dest_buckets, CDouble rows,
...
CHistogram::TranslateToDXLDerivedColumnStats(CMDAccessor *md_accessor, ULONG colid, CDouble width) const
{
	...
	for (ULONG ul = 0; ul < num_of_buckets; ul++)
	{
		...
		CDXLBucket *dxl_bucket = GPOS_NEW(m_mp) CDXLBucket(dxl_datum_lower, dxl_datum_upper, bucket->IsLowerClosed(),
														   bucket->IsUpperClosed(), freq, distinct);

		dxl_stats_bucket_array->Append(dxl_bucket);
	}

	return GPOS_NEW(m_mp) CDXLStatsDerivedColumn(colid, width, m_null_freq, m_distinct_remaining, m_freq_remaining,
...
CHistogram::MakeDefaultBoolHistogram(CMemoryPool *mp)
{
	...
	return GPOS_NEW(mp) CHistogram(mp, buckets, true /* is_well_defined */, null_freq, distinct_remaining,
...
void
CHistogram::AddHistograms(CMemoryPool *mp, UlongToHistogramMap *src_histograms, UlongToHistogramMap *dest_histograms)
{
