...
CHistogram *
CHistogram::MakeHistogramLessThanOrLessThanEqualFilter(CStatsPred::EStatsCmpType stats_cmp_type,
													   CPoint *point) const
{
	...
	for (ULONG bucket_index = 0; bucket_index < num_buckets; bucket_index++)
	{
		...
		else
		{
			...
			CBucket *last_bucket = bucket->MakeBucketScaleUpper(
				m_mp, point, CStatsPred::EstatscmptLEq == stats_cmp_type /*include_upper*/);
			if (NULL != last_bucket)
			{
	...
	for (ULONG bucket_index = 0; bucket_index < num_buckets; bucket_index++)
	{
		...
		if (bucket->Contains(point) && !point_is_null)
		{
			CBucket *less_than_bucket =
				bucket->MakeBucketScaleUpper(m_mp, point, false /*include_upper */);
...
CHistogram::MakeHistogramEqualFilter(CPoint *point) const
{
	...
	if (point->GetDatum()->IsNull())
	{
		return GPOS_NEW(m_mp)
			CHistogram(m_mp, GPOS_NEW(m_mp) CBucketArray(m_mp), true /* is_well_defined */,
		...
		if (bucket->Contains(point))
		{
			if (CStatsPred::EstatscmptGEq == stats_cmp_type)
			{
				...
				CBucket *first_bucket =
					bucket->MakeBucketScaleLower(m_mp, point, true /* include_lower */);
...
CBucketArray *
CHistogram::DeepCopyHistogramBuckets(CMemoryPool *mp, const CBucketArray *buckets)
{
...
CDouble
CHistogram::GetInequalityJoinScaleFactor(CDouble rows, const CHistogram *other_histogram,
										 CDouble rows_other) const
{
	...
	CHistogram *equijoin_histogram = MakeJoinHistogramNormalize(
		CStatsPred::EstatscmptEq, rows, other_histogram, rows_other, &scale_factor);
...
// output histogram
CHistogram *
CHistogram::MakeLASJHistogramNormalize(CStatsPred::EStatsCmpType stats_cmp_type, CDouble rows,
...
CHistogram::MakeHistogramFilter(CStatsPred::EStatsCmpType stats_cmp_type, CPoint *point) const
{
	...
	switch (stats_cmp_type)
	{
		...
		{
			result_histogram =
				MakeHistogramGreaterThanOrGreaterThanEqualFilter(stats_cmp_type, point);
			break;
		}
							  ...
							  const CHistogram *histogram) const
{
	...
	return GPOS_NEW(m_mp) CHistogram(m_mp, new_buckets, true /*is_well_defined*/, null_freq,
									 m_distinct_remaining, m_freq_remaining);
}
...
CHistogram::CopyHistogram() const
{
	...
	CHistogram *histogram_copy =
		GPOS_NEW(m_mp) CHistogram(m_mp, m_histogram_buckets, m_is_well_defined, m_null_freq,
								  m_distinct_remaining, m_freq_remaining);
	if (WereNDVsScaled())
	{
...
CHistogram::MakeJoinHistogramEqualityFilter(const CHistogram *histogram) const
{
	...
	while (idx1 < buckets1 && idx2 < buckets2)
	{
		...
		if (bucket1->Intersects(bucket2))
		{
			...
			CBucket *new_bucket =
				bucket1->MakeBucketIntersect(m_mp, bucket2, &freq_intersect1, &freq_intersect2);
	...
	ComputeJoinNDVRemainInfo(this, histogram, join_buckets, hist1_buckets_freq, hist2_buckets_freq,
							 &distinct_remaining, &freq_remaining);

	return GPOS_NEW(m_mp) CHistogram(m_mp, join_buckets, true /*is_well_defined*/,
...
void
CHistogram::ComputeJoinNDVRemainInfo(const CHistogram *histogram1, const CHistogram *histogram2,
										  ...
										  CDouble *result_distinct_values) const
{
	...
	for (ULONG ul = 0; ul < num_of_buckets; ul++)
	{
		...
		CBucket *new_bucket = GPOS_NEW(m_mp)
			CBucket(lower_bound, upper_bound, true /* fClosedLower */, is_upper_closed,
	...
	CHistogram *result_histogram =
		GPOS_NEW(m_mp) CHistogram(m_mp, new_buckets, true /*is_well_defined*/, new_null_freq,
...
CHistogram *
CHistogram::MakeUnionAllHistogramNormalize(CDouble rows, const CHistogram *histogram,
										   CDouble rows_other) const
{
	GPOS_ASSERT(NULL != histogram);
	...
	while (NULL != bucket1 && NULL != bucket2)
	{
		...
		else
		{
			...
			CBucket *merge_bucket = bucket1->SplitAndMergeBuckets(
				m_mp, bucket2, rows, rows_other, &bucket1_new, &bucket2_new, &result_rows);
	...
	idx1 =
		AddResidualUnionAllBucket(new_buckets, bucket1, rows, rows_new, bucket1_is_residual, idx1);
	...
	AddBuckets(m_mp, histogram->m_histogram_buckets, new_buckets, rows_other, rows_new, idx2,
			   num_buckets2);
	...
	CDouble freq_remaining =
		(m_freq_remaining * rows + histogram->GetFreqRemain() * rows_other) / rows_new;

	ULONG max_num_buckets =
		COptCtxt::PoctxtFromTLS()->GetOptimizerConfig()->GetStatsConf()->UlMaxStatsBuckets();
	...
	CHistogram *result_histogram =
		GPOS_NEW(m_mp) CHistogram(m_mp, result_buckets, true /*is_well_defined*/, new_null_freq,
								  distinct_remaining, freq_remaining);
...
ULONG
CHistogram::AddResidualUnionAllBucket(CBucketArray *histogram_buckets, CBucket *bucket,
...
void
CHistogram::AddBuckets(CMemoryPool *mp, const CBucketArray *src_buckets, CBucketArray *dest_buckets,
...
// col < 6  ==>  100 * (.2 + .25 * .4)  = 100 * .5 * .6   = 30 rows
CBucketArray *
CHistogram::CombineBuckets(CMemoryPool *mp, CBucketArray *buckets, ULONG desired_num_buckets)
{
	...
	for (ULONG ul = 0; ul < buckets->Size() - 1; ++ul)
	{
			...
			bucket1->IsUpperClosed() ^ bucket2->IsLowerClosed())
		{
			...
			CDouble width1 = bucket1->GetUpperBound()->Width(
				bucket1->GetLowerBound(), bucket1->IsLowerClosed(), bucket1->IsUpperClosed());
			CDouble freq2 = bucket2->GetFrequency();
			CDouble ndv2 = bucket2->GetNumDistinct();
			CDouble width2 = bucket2->GetUpperBound()->Width(
				bucket2->GetLowerBound(), bucket2->IsLowerClosed(), bucket2->IsUpperClosed());
	...
	while (NULL != (candidate_to_remove = boundary_factors->RemoveBestElement()))
	{
	...
	for (ULONG ul = 0; ul < buckets->Size(); ++ul)
	{
		...
		if (end_bucket_ix > ul)
		{
			...
			merged = GPOS_NEW(mp) CBucket(bucket1->GetLowerBound(), bucket2->GetUpperBound(),
...
CHistogram::GetNextBucket(const CHistogram *histogram, CBucket *new_bucket,
						  BOOL *result_bucket_is_residual, ULONG *current_bucket_index) const
{
	GPOS_ASSERT(NULL != histogram);
	if (NULL != new_bucket)
	{
...
CHistogram *
CHistogram::MakeUnionHistogramNormalize(CDouble rows, const CHistogram *other_histogram,
										CDouble rows_other, CDouble *num_output_rows) const
{
	GPOS_ASSERT(NULL != other_histogram);
	GPOS_ASSERT(this->IsValid());
	...
	while (NULL != bucket1 && NULL != bucket2)
	{
		...
		else if (bucket2->IsBefore(bucket1))
		{
			...
			num_tuples_per_bucket->Append(GPOS_NEW(m_mp)
		...
		else
		{
			...
			CDouble result_rows(0.0);
			merge_bucket =
				bucket1->SplitAndMergeBuckets(m_mp, bucket2, rows, rows_other, &bucket1_new,
											  &bucket2_new, &result_rows, false /* is_union_all */
				);

			// add the estimated number of rows in the merged bucket
			num_tuples_per_bucket->Append(GPOS_NEW(m_mp)
											  CDouble(merge_bucket->GetFrequency() * result_rows));
	...
	idx1 = AddResidualUnionBucket(histogram_buckets, bucket1, rows, bucket1_is_residual, idx1,
								  num_tuples_per_bucket);
	idx2 = AddResidualUnionBucket(histogram_buckets, bucket2, rows_other, bucket2_is_residual, idx2,
								  num_tuples_per_bucket);
	...
	// add any leftover buckets from other histogram
	AddBuckets(m_mp, other_histogram->m_histogram_buckets, histogram_buckets, rows_other,
			   num_tuples_per_bucket, idx2, num_buckets2);

	// add any leftover buckets from this histogram
	AddBuckets(m_mp, m_histogram_buckets, histogram_buckets, rows, num_tuples_per_bucket, idx1,
			   num_buckets1);
	...
	*num_output_rows = std::max(CStatistics::MinRows, total_output_rows);
	...
	CHistogram *result_histogram =
		GPOS_NEW(m_mp) CHistogram(m_mp, result_buckets, true /* is_well_defined */, null_freq,
...
void
CHistogram::AddBuckets(CMemoryPool *mp, const CBucketArray *src_buckets, CBucketArray *dest_buckets,
					   CDouble rows, CDoubleArray *dest_bucket_freqs, ULONG begin, ULONG end)
{
	GPOS_ASSERT(NULL != src_buckets);
	GPOS_ASSERT(NULL != dest_buckets);
	GPOS_ASSERT(begin <= end);
...
CHistogram::TranslateToDXLDerivedColumnStats(CMDAccessor *md_accessor, ULONG colid,
											 CDouble width) const
{
	...
	for (ULONG ul = 0; ul < num_of_buckets; ul++)
	{
		...
		CDXLBucket *dxl_bucket =
			GPOS_NEW(m_mp) CDXLBucket(dxl_datum_lower, dxl_datum_upper, bucket->IsLowerClosed(),
									  bucket->IsUpperClosed(), freq, distinct);

		dxl_stats_bucket_array->Append(dxl_bucket);
	}

	return GPOS_NEW(m_mp) CDXLStatsDerivedColumn(colid, width, m_null_freq, m_distinct_remaining,
...
CHistogram::MakeDefaultBoolHistogram(CMemoryPool *mp)
{
	...
	return GPOS_NEW(mp)
		CHistogram(mp, buckets, true /* is_well_defined */, null_freq, distinct_remaining,
...
void
CHistogram::AddDummyHistogramAndWidthInfo(CMemoryPool *mp, CColumnFactory *col_factory,
...
void
CHistogram::AddEmptyHistogram(CMemoryPool *mp, UlongToHistogramMap *output_histograms,
