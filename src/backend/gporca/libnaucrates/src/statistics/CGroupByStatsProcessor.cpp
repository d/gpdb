...
CStatistics *
CGroupByStatsProcessor::CalcGroupByStats(CMemoryPool *mp, const CStatistics *input_stats, ULongPtrArray *GCs,
										 ULongPtrArray *aggs, CBitSet *keys)
{
	...
	if (input_stats->IsEmpty())
	{
		...
		agg_stats =
			GPOS_NEW(mp) CStatistics(mp, col_histogram_mapping, col_width_mapping, agg_rows, true /* is_empty */);
	}
	else
	{
		...
		CStatisticsUtils::AddGrpColStats(mp, input_stats, groupby_cols_for_stats, col_histogram_mapping,
										 col_width_mapping);
		CStatisticsUtils::AddGrpColStats(mp, input_stats, computed_groupby_cols, col_histogram_mapping,
										 col_width_mapping);
