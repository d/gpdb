...
CStatistics *
CGroupByStatsProcessor::CalcGroupByStats(CMemoryPool *mp, const CStatistics *input_stats,
										 ULongPtrArray *GCs, ULongPtrArray *aggs, CBitSet *keys)
{
	...
	if (input_stats->IsEmpty())
	{
		...
		CHistogram::AddDummyHistogramAndWidthInfo(mp, col_factory, col_histogram_mapping,
												  col_width_mapping, aggs, true /* is_empty */);
		CHistogram::AddDummyHistogramAndWidthInfo(mp, col_factory, col_histogram_mapping,
												  col_width_mapping, GCs, true /* is_empty */);

		agg_stats = GPOS_NEW(mp) CStatistics(mp, col_histogram_mapping, col_width_mapping, agg_rows,
											 true /* is_empty */);
	}
	else
	{
		...
		CHistogram::AddDummyHistogramAndWidthInfo(mp, col_factory, col_histogram_mapping,
												  col_width_mapping, aggs, false /* is_empty */);
		...
		CStatisticsUtils::AddGrpColStats(mp, input_stats, groupby_cols_for_stats,
										 col_histogram_mapping, col_width_mapping);
		...
		CDoubleArray *NDVs = CStatisticsUtils::ExtractNDVForGrpCols(mp, stats_config, input_stats,
																	groupby_cols_for_stats, keys);
