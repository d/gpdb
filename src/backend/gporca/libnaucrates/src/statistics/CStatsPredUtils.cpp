...
//---------------------------------------------------------------------------
CStatsPred *
CStatsPredUtils::GetStatsPredNullTest(CMemoryPool *mp, CExpression *predicate_expr,
									  CColRefSet *	//outer_refs
)
{
...
//---------------------------------------------------------------------------
CStatsPred *
CStatsPredUtils::ExtractPredStats(CMemoryPool *mp, CExpression *scalar_expr, CColRefSet *outer_refs)
{
...
//---------------------------------------------------------------------------
CStatsPred *
CStatsPredUtils::CreateStatsPredDisj(CMemoryPool *mp, CExpression *predicate_expr,
									 CColRefSet *outer_refs)
{
	GPOS_ASSERT(NULL != predicate_expr);
...
//---------------------------------------------------------------------------
void
CStatsPredUtils::AddSupportedStatsFilters(CMemoryPool *mp, CStatsPredPtrArry *pred_stats_array,
	...
	else
	{
		...
		switch (ept)
		{
			...
			case CStatsPredUtils::EptScIdent:
				pred_stats =
					CStatsPredUtils::GetStatsPredFromBoolExpr(mp, predicate_expr, outer_refs);
				break;
			...
			default:
				pred_stats =
					CStatsPredUtils::CreateStatsPredUnsupported(mp, predicate_expr, outer_refs);
				break;
		}
...
CStatsPredUtils::GetPredTypeForExpr(CMemoryPool *mp, CExpression *predicate_expr)
{
	GPOS_ASSERT(NULL != predicate_expr);
	if (CPredicateUtils::FOr(predicate_expr))
	{
...
)
{
	...
	CPredicateUtils::ExtractLikePredComponents(predicate_expr, &expr_scalar_ident,
											   &expr_scalar_const);
								 ...
								 CStatsPredPtrArry *result_pred_stats)
{
	...
	{
		// unsupported predicate for stats calculations
		result_pred_stats->Append(GPOS_NEW(mp)
	...
	for (ULONG ul = 0; ul < num_array_elems; ++ul)
	{
		...
		if (COperator::EopScalarConst == child_expr->Pop()->Eopid())
		{
			...
			{
				CStatsPred *child_pred_stats =
					GPOS_NEW(mp) CStatsPredPoint(mp, col_ref, stats_cmp_type, datum);
				pred_stats->Append(child_pred_stats);
			}
		}
...
//---------------------------------------------------------------------------
CStatsPred *
CStatsPredUtils::GetStatsPredFromBoolExpr(CMemoryPool *mp, CExpression *predicate_expr,
										  CColRefSet *	//outer_refs
)
{
	GPOS_ASSERT(NULL != predicate_expr);
	...
	if (CPredicateUtils::FBooleanScalarIdent(predicate_expr))
	{
		...
		datum = md_accessor->PtMDType<IMDTypeBool>()->CreateBoolDatum(mp, true /* fValue */,
																	  false /* is_null */);
	...
	else
	{
		...
		datum = md_accessor->PtMDType<IMDTypeBool>()->CreateBoolDatum(mp, false /* fValue */,
																	  false /* is_null */);
	...
	CColRefSet *outer_refs, BOOL is_semi_or_anti_join, CExpressionArray *unsupported_expr_array)
{
	...
	BOOL fSupportedScIdentComparison = IsJoinPredSupportedForStatsEstimation(
		join_pred_expr, output_col_refsets, is_semi_or_anti_join, &stats_cmp_type, &col_ref_outer,
		&col_ref_inner);
	...
	CColRefSet *outer_refs, BOOL is_semi_or_antijoin, CStatsPred **unsupported_stats_pred_array)
{
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		CStatsPredJoin *join_stats =
			ExtractJoinStatsFromJoinPred(mp, predicate_expr, output_col_refsets, outer_refs,
	...
	CColRefSet *outer_refs, BOOL is_semi_or_anti_join)
{
	...
	CStatsPredJoinArray *join_pred_stats =
		ExtractJoinStatsFromJoinPredArray(mp, scalar_expr, output_col_refsets, outer_refs,
...
//---------------------------------------------------------------------------
CStatsPredJoinArray *
CStatsPredUtils::ExtractJoinStatsFromExprHandle(CMemoryPool *mp, CExpressionHandle &expr_handle,
												BOOL is_semi_or_anti_join)
{
	...
	CStatsPredJoinArray *join_pred_stats = ExtractJoinStatsFromExpr(
		mp, expr_handle, scalar_expr, output_col_refsets, outer_refs, is_semi_or_anti_join);
