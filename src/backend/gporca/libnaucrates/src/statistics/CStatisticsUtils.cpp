...
//---------------------------------------------------------------------------
CPoint *
CStatisticsUtils::NextPoint(CMemoryPool *mp, CMDAccessor *md_accessor, CPoint *point)
{
									 ...
									 ULONG num_mcv_values)
{
	...
	for (ULONG i = 0; i < num_mcv_values; i++)
	{
		...
		CBucket *bucket = GPOS_NEW(mp) CBucket(
			GPOS_NEW(mp) CPoint(datum), GPOS_NEW(mp) CPoint(datum), true /* is_lower_closed */,
...
//---------------------------------------------------------------------------
CBucketArray *
CStatisticsUtils::MergeMcvHistBucket(CMemoryPool *mp, const CBucketArray *mcv_buckets,
...
//---------------------------------------------------------------------------
void
CStatisticsUtils::AddRemainingBuckets(CMemoryPool *mp, const CBucketArray *src_buckets,
...
//---------------------------------------------------------------------------
void
CStatisticsUtils::SplitHistDriver(CMemoryPool *mp, const CBucket *histogram_bucket,
...
//---------------------------------------------------------------------------
CBucketArray *
CStatisticsUtils::SplitHistBucketGivenMcvBuckets(CMemoryPool *mp, const CBucket *histogram_bucket,
												 const CBucketArray *mcv_buckets)
{
	...
	CBucket *first_bucket = CreateValidBucket(mp, histogram_bucket->GetLowerBound(), mcv_point,
	...
	CBucket *last_bucket = CreateValidBucket(mp, mcv_point, histogram_bucket->GetUpperBound(),
	...
	CBucketArray *complete_buckets = DistributeBucketProperties(
		mp, histogram_bucket->GetFrequency(), total_distinct_values, buckets_after_split);
									...
									BOOL is_upper_closed)
{
	...
	return GPOS_NEW(mp)
		CBucket(bucket_lower_bound, bucket_upper_bound, is_lower_closed, is_upper_closed,
...
//---------------------------------------------------------------------------
BOOL
CStatisticsUtils::IsValidBucket(CPoint *bucket_lower_bound, CPoint *bucket_upper_bound,
								BOOL is_lower_closed, BOOL is_upper_closed)
{
	...
	if (datum->IsDatumMappableToLINT())
	{
		...
		CDouble bound_diff =
			bucket_upper_bound->Width(bucket_lower_bound, is_lower_closed, is_upper_closed);
...
//---------------------------------------------------------------------------
CBucketArray *
CStatisticsUtils::DistributeBucketProperties(CMemoryPool *mp, CDouble total_frequency,
											 CDouble total_distinct_values,
...
//---------------------------------------------------------------------------
void
CStatisticsUtils::PrintColStats(CMemoryPool *mp, CStatsPred *pred_stats, ULONG cond_colid,
								CHistogram *histogram, CDouble last_scale_factor,
...
//---------------------------------------------------------------------------
void
CStatisticsUtils::ExtractUsedColIds(CMemoryPool *mp, CBitSet *colids_bitset, CStatsPred *pred_stats,
...
//---------------------------------------------------------------------------
void
CStatisticsUtils::UpdateDisjStatistics(CMemoryPool *mp, CBitSet *dont_update_stats_bitset,
		...
		!dont_update_stats_bitset->Get(colid))
	{
		...
		if (NULL != result_histogram)
		{
			...
			CHistogram *new_histogram = previous_histogram->MakeUnionHistogramNormalize(
				input_disjunct_rows, result_histogram, local_rows, &output_rows);

			GPOS_DELETE(previous_histogram);
			previous_histogram = new_histogram;
		}

		AddHistogram(mp, colid, previous_histogram, disjunctive_result_histograms,
...
//---------------------------------------------------------------------------
CBitSet *
CStatisticsUtils::GetColsNonUpdatableHistForDisj(CMemoryPool *mp, CStatsPredDisj *pred_stats)
{
	GPOS_ASSERT(NULL != pred_stats);
...
//---------------------------------------------------------------------------
void
CStatisticsUtils::AddHistogram(CMemoryPool *mp, ULONG colid, const CHistogram *histogram,
...
//---------------------------------------------------------------------------
UlongToHistogramMap *
CStatisticsUtils::MergeHistogramMapsForDisjPreds(CMemoryPool *mp, CBitSet *non_updatable_cols,
			...
			if (NULL != histogram1 && !non_updatable_cols->Get(colid))
			{
				...
				if (NULL != histogram2)
				{
					CHistogram *normalized_union_histogram =
						histogram1->MakeUnionHistogramNormalize(rows1, histogram2, rows2,
																&output_rows);

					AddHistogram(mp, colid, normalized_union_histogram, merged_hmap,
								 true /* fReplaceOld */);
...
//---------------------------------------------------------------------------
UlongToHistogramMap *
CStatisticsUtils::CopyHistHashMap(CMemoryPool *mp, UlongToHistogramMap *col_histogram_mapping)
{
	GPOS_ASSERT(NULL != col_histogram_mapping);
...
//---------------------------------------------------------------------------
IStatistics *
CStatisticsUtils::DeriveStatsForDynamicScan(CMemoryPool *mp, CExpressionHandle &expr_handle,
											ULONG part_idx_id, CPartFilterMap *part_filter_map)
{
	...
	if (NULL != unsupported_pred_stats)
	{
		...
		IStatistics *stats_after_join_filter = CFilterStatsProcessor::MakeStatsFilter(
			mp, dynamic_cast<CStatistics *>(left_semi_join_stats), unsupported_pred_stats,
			false /* do_cap_NDVs */);
...
//---------------------------------------------------------------------------
IStatistics *
CStatisticsUtils::DeriveStatsForIndexGet(CMemoryPool *mp, CExpressionHandle &expr_handle,
										 IStatisticsArray *stats_contexts)
{
	...
	CPredicateUtils::SeparateOuterRefs(mp, scalar_expr, outer_col_refset, &local_expr,
									   &outer_refs_expr);
...
//---------------------------------------------------------------------------
IStatistics *
CStatisticsUtils::DeriveStatsForBitmapTableGet(CMemoryPool *mp, CExpressionHandle &expr_handle,
											   IStatisticsArray *stats_contexts)
{
	...
	while (col_refset_iter.Advance())
	{
		...
		if (NULL == keys || keys->Get(colid))
		{
			...
			if (NULL == ndv_colid)
			{
				...
				BOOL fres GPOS_ASSERTS_ONLY = grp_colid_upper_bound_ndv_idx_map->Insert(
...
//---------------------------------------------------------------------------
CDoubleArray *
CStatisticsUtils::ExtractNDVForGrpCols(CMemoryPool *mp, const CStatisticsConfig *stats_config,
...
//---------------------------------------------------------------------------
CDouble
CStatisticsUtils::MaxNumGroupsForGivenSrcGprCols(CMemoryPool *mp,
...
//---------------------------------------------------------------------------
CDouble
CStatisticsUtils::Groups(CMemoryPool *mp, IStatistics *stats, const CStatisticsConfig *stats_config,
...
//---------------------------------------------------------------------------
void
CStatisticsUtils::AddGrpColStats(CMemoryPool *mp, const CStatistics *input_stats,
	...
	while (grp_col_refset_iter.Advance())
	{
		...
		if (NULL != histogram)
		{
			CHistogram *result_histogram =
				histogram->MakeGroupByHistogramNormalize(input_stats->Rows(), &num_distinct_vals);
...
//---------------------------------------------------------------------------
CColRefSet *
CStatisticsUtils::MakeGroupByColsForStats(CMemoryPool *mp, const ULongPtrArray *grouping_columns,
