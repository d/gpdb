...
//
//---------------------------------------------------------------------------
CBucket::CBucket(CPoint *bucket_lower_bound, CPoint *bucket_upper_bound, BOOL is_lower_closed,
...
CBucket::GetOverlapPercentage(const CPoint *point, BOOL include_point) const
{
	...
	CDouble distance_upper =
		m_bucket_upper_bound->Width(m_bucket_lower_bound, m_is_lower_closed, m_is_upper_closed);
...
//---------------------------------------------------------------------------
CBucket *
CBucket::MakeBucketScaleUpper(CMemoryPool *mp, CPoint *point_upper_new, BOOL include_upper) const
{
	GPOS_ASSERT(mp);
	...
	return GPOS_NEW(mp)
		CBucket(this->m_bucket_lower_bound, point_upper_new, this->m_is_lower_closed, include_upper,
...
//---------------------------------------------------------------------------
CBucket *
CBucket::MakeBucketScaleLower(CMemoryPool *mp, CPoint *point_lower_new, BOOL include_lower) const
{
	GPOS_ASSERT(mp);
		...
		(this->IsLowerClosed() && !include_lower))
	{
		...
		CDouble overlap =
			CDouble(1.0) - this->GetOverlapPercentage(point_lower_new, !include_lower);
		frequency_new = frequency_new * overlap;
		distinct_new = distinct_new * overlap;
	}
	...
	return GPOS_NEW(mp) CBucket(point_lower_new, this->m_bucket_upper_bound, include_lower,
...
CBucket::MakeBucketSingleton(CMemoryPool *mp, CPoint *point_singleton) const
{
	...
	return GPOS_NEW(mp) CBucket(point_singleton, point_singleton, true /* is_lower_closed */,
...
CBucket::MakeBucketCopy(CMemoryPool *mp)
{
	...
	return GPOS_NEW(mp) CBucket(m_bucket_lower_bound, m_bucket_upper_bound, m_is_lower_closed,
							 ...
							 CDouble *result_freq_intersect2) const
{
	...
	*result_freq_intersect2 = freq_intersect2;
	...
	return GPOS_NEW(mp) CBucket(lower_new, upper_new, lower_new_is_closed, upper_new_is_closed,
								frequency_new, distinct_new);
}
...
//---------------------------------------------------------------------------
void
CBucket::Difference(CMemoryPool *mp, CBucket *bucket_other, CBucket **result_bucket_lower,
					CBucket **result_bucket_upper)
{
	...
	{
		*result_bucket_lower = this->MakeBucketScaleUpper(mp, bucket_other->GetLowerBound(),
	...
	{
		*result_bucket_upper = this->MakeBucketScaleLower(mp, bucket_other->GetUpperBound(),
														  !bucket_other->IsUpperClosed());
	}
	...
	BOOL is_union_all)
{
	...
	CPoint *minLower =
		CPoint::MinPoint(this->GetLowerBound(), bucket_other->GetLowerBound());	 // lowest point
	...
	CPoint *maxUpper =
		CPoint::MaxPoint(this->GetUpperBound(), bucket_other->GetUpperBound());	 // highest point
		...
		(minLower->Equals(maxLower) && this->IsLowerClosed() != bucket_other->IsLowerClosed()))
	{
			...
			(minLower->Equals(maxLower) && this->IsLowerClosed()))
		{
			...
			*result_rows = rows;
			...
			maxLower->AddRef();
			lower_third =
				GPOS_NEW(mp) CBucket(this->GetLowerBound(), maxLower, this->IsLowerClosed(),
									 include_upper, lower_freq, lower_ndv);

			// use the width the scale the buckets down instead of using
			// the default ndv
			*bucket_new1 =
				this->MakeBucketScaleLower(mp, maxLower, !include_upper /*include_lower*/);
		...
		else
		{
			...
			CDouble lower_percent =
				bucket_other->GetOverlapPercentage(maxLower, false /*include_point*/);
			*result_rows = rows_other;
			...
			lower_third = GPOS_NEW(mp)
				CBucket(bucket_other->GetLowerBound(), maxLower, bucket_other->IsLowerClosed(),
						include_upper, lower_freq, lower_ndv);

			*bucket_new2 =
				bucket_other->MakeBucketScaleLower(mp, maxLower, !include_upper /*include_lower*/);
	...
	if (!minUpper->Equals(maxUpper))
	{
		...
		if (this_singleton)
		{
			...
			bucket_other_overlap =
				bucket_other->GetOverlapPercentage(minUpper, true /*include_point*/);
		}
		...
		else
		{
			...
			bucket_other_overlap =
				bucket_other->GetOverlapPercentage(minUpper, false /*include_point*/);
	...
	{
		merged_freq =
			std::min(CDouble(1.0), std::max(merged_rows_this, merged_rows_other) / total_rows);
	}
	...
	maxLower->AddRef();
	minUpper->AddRef();
	middle_third = GPOS_NEW(mp)
		CBucket(maxLower, minUpper, isLowerClosed, isUpperClosed, merged_freq, merged_ndv);
	...
	if (NULL != upper_third)
	{
			...
			upper_third->IsUpperClosed() == this->IsUpperClosed())
		{
			*bucket_new1 = upper_third;
			...
			GPOS_ASSERT_IMP(
				is_union_all,
				middle_third->GetFrequency() * total_rows + upper_third->GetFrequency() * rows <=
...
CBucket::MakeBucketSingleton(CMemoryPool *mp, IDatum *datum)
{
	...
	return GPOS_NEW(mp)
		CBucket(GPOS_NEW(mp) CPoint(datum), GPOS_NEW(mp) CPoint(datum), true /* is_lower_closed */,
