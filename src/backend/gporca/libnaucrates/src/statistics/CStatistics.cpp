...
void
CStatistics::CreateAndInsertUpperBoundNDVs(CMemoryPool *mp, CStatistics *stats, ULongPtrArray *colids, CDouble rows)
{
...
//	return dummy statistics object
CStatistics *
CStatistics::MakeDummyStats(CMemoryPool *mp, ULongPtrArray *col_histogram_mapping, ULongPtrArray *col_width_mapping,
							CDouble rows)
{
	GPOS_ASSERT(NULL != col_histogram_mapping);
...
BOOL
CStatistics::IsEmptyJoin(const CStatistics *outer_stats, const CStatistics *inner_side_stats, BOOL IsLASJ)
{
...
CStatistics *
CStatistics::CalcLASJoinStats(CMemoryPool *mp, const IStatistics *other_stats, CStatsPredJoinArray *join_preds_stats,
...
CStatistics::ScaleStats(CMemoryPool *mp, CDouble factor) const
{
	...
	CStatistics *scaled_stats =
		GPOS_NEW(mp) CStatistics(mp, histograms_new, widths_new, scaled_num_rows, IsEmpty(), m_num_predicates);
...
IStatistics *
CStatistics::CopyStatsWithRemap(CMemoryPool *mp, UlongToColRefMap *colref_mapping, BOOL must_exist) const
{
	...
	CStatistics *stats_copy =
		GPOS_NEW(mp) CStatistics(mp, histograms_new, widths_new, m_rows, IsEmpty(), m_num_predicates);
...
void
CStatistics::AddWidthInfoWithRemap(CMemoryPool *mp, UlongToDoubleMap *src_width, UlongToDoubleMap *dest_width,
