...
CStatistics::CStatistics(CMemoryPool *mp, UlongToHistogramMap *col_histogram_mapping,
						 UlongToDoubleMap *colid_width_mapping, CDouble rows, BOOL is_empty,
...
CStatistics::MakeDummyStats(CMemoryPool *mp, ULongPtrArray *colids, CDouble rows)
{
	...
	CHistogram::AddDummyHistogramAndWidthInfo(mp, col_factory, col_histogram_mapping,
											  colid_width_mapping, colids, is_empty);
...
//	return dummy statistics object
CStatistics *
CStatistics::MakeDummyStats(CMemoryPool *mp, ULongPtrArray *col_histogram_mapping,
							ULongPtrArray *col_width_mapping, CDouble rows)
{
	...
	CStatistics *stats = GPOS_NEW(mp) CStatistics(mp, result_col_histogram_mapping,
												  colid_width_mapping, rows, false /* is_empty */);
...
BOOL
CStatistics::IsEmptyJoin(const CStatistics *outer_stats, const CStatistics *inner_side_stats,
						 BOOL IsLASJ)
{
...
{
	return CLeftOuterJoinStatsProcessor::CalcLOJoinStatsStatic(mp, this, other_stats,
															   join_preds_stats);
}
...
CStatistics *
CStatistics::CalcLSJoinStats(CMemoryPool *mp, const IStatistics *inner_side_stats,
							 CStatsPredJoinArray *join_preds_stats) const
{
	return CLeftSemiJoinStatsProcessor::CalcLSJoinStatsStatic(mp, this, inner_side_stats,
															  join_preds_stats);
}
...
CStatistics::CalcInnerJoinStats(CMemoryPool *mp, const IStatistics *other_stats,
								CStatsPredJoinArray *join_preds_stats) const
{
	return CInnerJoinStatsProcessor::CalcInnerJoinStatsStatic(mp, this, other_stats,
															  join_preds_stats);
}
...
void
CStatistics::AddNotExcludedHistograms(CMemoryPool *mp, CBitSet *excluded_cols,
...
void
CStatistics::CopyWidthsInto(CMemoryPool *mp, UlongToDoubleMap *colid_width_mapping) const
{
...
IStatistics *
CStatistics::CopyStatsWithRemap(CMemoryPool *mp, UlongToColRefMap *colref_mapping,
								BOOL must_exist) const
{
	...
	AddHistogramsWithRemap(mp, m_colid_histogram_mapping, histograms_new, colref_mapping,
						   must_exist);
...
void
CStatistics::AddHistogramsWithRemap(CMemoryPool *mp, UlongToHistogramMap *src_histograms,
...
CStatistics::AddWidthInfoWithRemap(CMemoryPool *mp, UlongToDoubleMap *src_width,
								   UlongToDoubleMap *dest_width, UlongToColRefMap *colref_mapping,
								   BOOL must_exist)
{
...
CStatistics::GetDxlStatsDrvdRelation(CMemoryPool *mp, CMDAccessor *md_accessor) const
{
