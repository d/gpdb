...
CStatistics *
CLeftOuterJoinStatsProcessor::CalcLOJoinStatsStatic(CMemoryPool *mp,
													...
													CStatsPredJoinArray *join_preds_stats)
{
	...
	UlongToHistogramMap *LOJ_histograms = CLeftOuterJoinStatsProcessor::MakeLOJHistogram(
		mp, result_stats_outer_side, result_stats_inner_side, inner_join_stats, join_preds_stats,
	...
	CStatistics *result_stats_LOJ = GPOS_NEW(mp)
		CStatistics(mp, LOJ_histograms, inner_join_stats->CopyWidths(mp), num_rows_LOJ,
	...
	CStatisticsUtils::ComputeCardUpperBounds(mp, result_stats_outer_side, result_stats_LOJ,
											 num_rows_LOJ,
											 CStatistics::EcbmMin /* card_bounding_method */);
	CStatisticsUtils::ComputeCardUpperBounds(mp, result_stats_inner_side, result_stats_LOJ,
											 num_rows_LOJ,
											 CStatistics::EcbmMin /* card_bounding_method */);
...
UlongToHistogramMap *
CLeftOuterJoinStatsProcessor::MakeLOJHistogram(CMemoryPool *mp, const CStatistics *outer_side_stats,
											   ...
											   CDouble *result_rows_LASJ)
{
	...
	AddHistogramsLOJInner(mp, inner_join_stats, inner_colids_with_stats, num_rows_LASJ,
						  num_rows_inner_join, LOJ_histograms);
...
CLeftOuterJoinStatsProcessor::AddHistogramsLOJInner(
	CMemoryPool *mp, const CStatistics *inner_join_stats, ULongPtrArray *inner_colids_with_stats,
	CDouble num_rows_LASJ, CDouble num_rows_inner_join, UlongToHistogramMap *LOJ_histograms)
{
	GPOS_ASSERT(NULL != inner_join_stats);
	...
	for (ULONG ul = 0; ul < num_inner_cols; ul++)
	{
		...
		CHistogram *null_histogram = GPOS_NEW(mp) CHistogram(
			mp, GPOS_NEW(mp) CBucketArray(mp), true /*is_well_defined*/, 1.0 /*null_freq*/,
