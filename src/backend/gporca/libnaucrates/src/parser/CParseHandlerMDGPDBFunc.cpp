...
//
//---------------------------------------------------------------------------
CParseHandlerMDGPDBFunc::CParseHandlerMDGPDBFunc(CMemoryPool *mp,
									  ...
									  const Attributes &attrs)
{
		...
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBFunc), element_local_name))
	{
		...
		m_returns_set = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenGPDBFuncReturnsSet,
			EdxltokenGPDBFunc);
		// parse whether func is strict
		m_is_strict = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenGPDBFuncStrict,
			EdxltokenGPDBFunc);

		// parse whether func is NDV-preserving
		m_is_ndv_preserving = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenGPDBFuncNDVPreserving,
			EdxltokenGPDBFunc,
			true,  // optional
		...
		m_is_allowed_for_PS = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenGPDBFuncIsAllowedForPS,
			EdxltokenGPDBFunc,
			true,  // optional
			false  // default is false
		);
										   ...
										   element_local_name))
	{
		...
		m_mdid_types_array = CDXLOperatorFactory::ExtractConvertMdIdsToArray(
			m_parse_handler_mgr->GetDXLMemoryManager(), xmlszTypes, EdxltokenTypeIds,
			EdxltokenOutputCols);
	}
}
...
)
{
		...
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBFunc), element_local_name))
	{
		...
		m_imd_obj = GPOS_NEW(m_mp)
			CMDFunctionGPDB(m_mp, m_mdid, m_mdname, m_mdid_type_result, m_mdid_types_array,
