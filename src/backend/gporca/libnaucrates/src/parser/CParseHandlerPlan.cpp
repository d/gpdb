								...
								const XMLCh *const element_qname, const Attributes &attrs)
{
									  ...
									  element_local_name))
	{
		...
		direct_dispatch_parse_handler->startElement(element_uri, element_local_name, element_qname,
													attrs);
		return;
	}
	...
	m_plan_id = CDXLOperatorFactory::ConvertAttrValueToUllong(
		m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_plan_id, EdxltokenPlanId,
		EdxltokenPlan);
	...
	m_plan_space_size = CDXLOperatorFactory::ConvertAttrValueToUllong(
		m_parse_handler_mgr->GetDXLMemoryManager(), xmlszPlanSpaceSize, EdxltokenPlanSpaceSize,
		EdxltokenPlan);
