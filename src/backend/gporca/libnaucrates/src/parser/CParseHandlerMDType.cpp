								  ...
								  const Attributes &attrs)
{
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenMDType), element_local_name))
	{
		...
		if (!IsBuiltInType(m_mdid))
		{
			...
			{
				m_is_composite = CDXLOperatorFactory::ConvertAttrValueToBool(m_parse_handler_mgr->GetDXLMemoryManager(),
																			 attribute_val_xml,
			...
			else
			{
				m_is_text_related = CDXLOperatorFactory::ConvertAttrValueToBool(
					m_parse_handler_mgr->GetDXLMemoryManager(), xml_is_text_related, EdxltokenMDTypeIsTextRelated,
					EdxltokenMDType);
			}

			// get type length
			m_type_length = CDXLOperatorFactory::ExtractConvertAttrValueToInt(
				m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMDTypeLength, EdxltokenMDType);
		}
	}
	else
	{
		...
		switch (pmdidGPDB->Oid())
		{
			...
			default:
				...
				m_imd_obj = GPOS_NEW(m_mp) CMDTypeGenericGPDB(
					...
					m_distr_opfamily, m_legacy_distr_opfamily, m_mdid_eq_op, m_mdid_neq_op, m_mdid_lt_op,
					m_mdid_lteq_op, m_mdid_gt_op, m_mdid_gteq_op, m_mdid_cmp_op, m_mdid_min_op, m_mdid_max_op,
					m_mdid_avg_op, m_mdid_sum_op, m_mdid_count_op, m_is_hashable, m_is_merge_joinable, m_is_composite,
