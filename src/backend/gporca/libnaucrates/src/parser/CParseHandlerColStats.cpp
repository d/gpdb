...
//
//---------------------------------------------------------------------------
CParseHandlerColStats::CParseHandlerColStats(CMemoryPool *mp,
									...
									const XMLCh *const element_qname, const Attributes &attrs)
{
		...
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenColumnStats), element_local_name))
	{
		...
		if (NULL != parsed_null_freq)
		{
			m_null_freq = CDXLOperatorFactory::ConvertAttrValueToDouble(
				m_parse_handler_mgr->GetDXLMemoryManager(), parsed_null_freq, EdxltokenColNullFreq,
				EdxltokenColumnStats);
		}
		...
		if (NULL != parsed_distinct_remaining)
		{
			m_distinct_remaining = CDXLOperatorFactory::ConvertAttrValueToDouble(
				m_parse_handler_mgr->GetDXLMemoryManager(), parsed_distinct_remaining,
		...
		if (NULL != parsed_freq_remaining)
		{
			m_freq_remaining = CDXLOperatorFactory::ConvertAttrValueToDouble(
				m_parse_handler_mgr->GetDXLMemoryManager(), parsed_freq_remaining,
				EdxltokenColFreqRemain, EdxltokenColumnStats);
		}
		...
		if (NULL != parsed_is_column_stats_missing)
		{
			m_is_column_stats_missing = CDXLOperatorFactory::ConvertAttrValueToBool(
				m_parse_handler_mgr->GetDXLMemoryManager(), parsed_is_column_stats_missing,
				EdxltokenColStatsMissing, EdxltokenColumnStats);
		}
	}
										   ...
										   element_local_name))
	{
		...
		parse_handler_base_stats_bucket->startElement(element_uri, element_local_name,
													  element_qname, attrs);
	}
	else
	{
...
)
{
	...
	m_imd_obj = GPOS_NEW(m_mp)
		CDXLColStats(m_mp, m_mdid, m_md_name, m_width, m_null_freq, m_distinct_remaining,
