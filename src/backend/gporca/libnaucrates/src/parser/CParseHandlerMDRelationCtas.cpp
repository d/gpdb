...
//---------------------------------------------------------------------------
void
CParseHandlerMDRelationCtas::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
										  const XMLCh *const element_qname, const Attributes &attrs)
{
	...
	{
		m_mdname_schema =
			CDXLUtils::CreateMDNameFromXMLChar(m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_schema);
	}
	...
	if (NULL != xml_str_has_oids)
	{
		m_has_oids = CDXLOperatorFactory::ConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_has_oids, EdxltokenRelHasOids, EdxltokenRelation);
	}
...
)
{
	...
	m_imd_obj = GPOS_NEW(m_mp)
		CMDRelationCtasGPDB(m_mp, m_mdid, m_mdname_schema, m_mdname, m_is_temp_table, m_has_oids, m_rel_storage_type,
