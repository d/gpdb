...
//
//---------------------------------------------------------------------------
CParseHandlerMDRelationCtas::CParseHandlerMDRelationCtas(CMemoryPool *mp,
										  ...
										  const XMLCh *const element_qname, const Attributes &attrs)
{
	...
	if (NULL != xml_str_has_oids)
	{
		m_has_oids = CDXLOperatorFactory::ConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_has_oids, EdxltokenRelHasOids,
			EdxltokenRelation);
	}
	...
	m_vartypemod_array = CDXLOperatorFactory::ExtractIntsToIntArray(
		m_parse_handler_mgr->GetDXLMemoryManager(), vartypemod_xml, EdxltokenVarTypeModList,
		EdxltokenRelation);
...
)
{
	...
	m_imd_obj = GPOS_NEW(m_mp) CMDRelationCtasGPDB(
		m_mp, m_mdid, m_mdname_schema, m_mdname, m_is_temp_table, m_has_oids, m_rel_storage_type,
		m_rel_distr_policy, md_col_array, m_distr_col_array, distr_opfamilies, m_key_sets_arrays,
