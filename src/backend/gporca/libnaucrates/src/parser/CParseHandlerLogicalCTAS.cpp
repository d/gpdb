...
//---------------------------------------------------------------------------
void
CParseHandlerLogicalCTAS::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
									   const XMLCh *const element_qname, const Attributes &attrs)
{
	...
	if (IMDRelation::EreldistrHash == m_rel_distr_policy)
	{
		...
		m_distr_column_pos_array =
			CDXLOperatorFactory::ExtractIntsToUlongArray(m_parse_handler_mgr->GetDXLMemoryManager(), rel_distr_cols_xml,
	...
	m_has_oids = CDXLOperatorFactory::ExtractConvertAttrValueToBool(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
																	EdxltokenRelHasOids, EdxltokenLogicalCTAS);
...
)
{
	...
	m_dxl_node = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalCTAS(
						   m_mp, m_mdid, m_mdname_schema, m_mdname, dxl_column_descr_array, dxl_ctas_storage_opt,
						   m_rel_distr_policy, m_distr_column_pos_array, distr_opfamilies, m_is_temp_table, m_has_oids,
