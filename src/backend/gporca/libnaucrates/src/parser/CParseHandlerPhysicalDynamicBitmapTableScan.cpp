														  ...
														  const Attributes &attrs)
{
	...
	m_part_index_id = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(m_parse_handler_mgr->GetDXLMemoryManager(),
																		  attrs, EdxltokenPartIndexId,
																		  EdxltokenPhysicalDynamicBitmapTableScan);

	m_part_index_id_printable = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenPartIndexIdPrintable,
		EdxltokenPhysicalDynamicBitmapTableScan,
		true,  //is_optional
		m_part_index_id);
}
...
{
	EndElementHelper(element_local_name, EdxltokenPhysicalDynamicBitmapTableScan, m_part_index_id,
