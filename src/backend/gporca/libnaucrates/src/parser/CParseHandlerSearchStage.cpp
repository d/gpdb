...
//
//---------------------------------------------------------------------------
CParseHandlerSearchStage::CParseHandlerSearchStage(CMemoryPool *mp,
									   ...
									   const XMLCh *const element_qname, const Attributes &attrs)
{
		...
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenSearchStage), element_local_name))
	{
		...
		m_cost_threshold = CCost(CDXLOperatorFactory::ConvertAttrValueToDouble(
			m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_cost, EdxltokenCostThreshold,
		...
		m_time_threshold = CDXLOperatorFactory::ConvertAttrValueToUlong(
			m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_time, EdxltokenTimeThreshold,
			EdxltokenSearchStage);
	}
	else if (0 ==
			 XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenXform), element_local_name))
	{
		...
		xform_set_parse_handler->startElement(element_uri, element_local_name, element_qname,
											  attrs);
	}
	else
	{
