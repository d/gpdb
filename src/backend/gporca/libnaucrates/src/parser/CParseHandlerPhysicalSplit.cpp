										 ...
										 const Attributes &attrs)
{
	...
	m_ctid_colid = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenCtidColId, EdxltokenPhysicalSplit);
	...
	if (NULL != update_with_preserved_oids)
	{
		m_preserve_oids = CDXLOperatorFactory::ConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), update_with_preserved_oids, EdxltokenUpdatePreservesOids,
			EdxltokenPhysicalSplit);
	}
	if (m_preserve_oids)
	{
		m_tuple_oid_col_oid = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenTupleOidColId, EdxltokenPhysicalSplit);
	}
...
)
{
	...
	CDXLPhysicalSplit *dxl_op =
		GPOS_NEW(m_mp) CDXLPhysicalSplit(m_mp, m_deletion_colid_array, m_insert_colid_array, m_action_colid,
