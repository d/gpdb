...
//
//---------------------------------------------------------------------------
CParseHandlerMDGPDBScalarOp::CParseHandlerMDGPDBScalarOp(CMemoryPool *mp,
										  ...
										  const XMLCh *const element_qname, const Attributes &attrs)
{
									  ...
									  element_local_name))
	{
		...
		if (NULL != xml_str_returns_null_on_null_input)
		{
			m_returns_null_on_null_input = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
				m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenReturnsNullOnNullInput,
				EdxltokenGPDBScalarOp);
		}

		// ndv-preserving property is optional
		m_is_ndv_preserving = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenIsNDVPreserving,
			EdxltokenGPDBScalarOp,
			true,  // is optional
			false  // default value
		);
	...
	else if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenOpfamilies),
										   element_local_name))
	{
		...
		opfamilies_list_parse_handler->startElement(element_uri, element_local_name, element_qname,
													attrs);
	}
	else if (0 ==
			 XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpHashOpfamily),
									  element_local_name))
	{
		// parse inverse operator id
		GPOS_ASSERT(NULL != m_mdname);
		...
		m_mdid_hash_opfamily = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
			...
			EdxltokenGPDBScalarOpHashOpfamily);
					  ...
					  CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpLegacyHashOpfamily),
					  element_local_name))
	{
		// parse inverse operator id
		GPOS_ASSERT(NULL != m_mdname);
...
)
{
									  ...
									  element_local_name))
	{
		...
		m_imd_obj = GPOS_NEW(m_mp)
			CMDScalarOpGPDB(m_mp, m_mdid, m_mdname, m_mdid_type_left, m_mdid_type_right,
							m_mdid_type_result, m_func_mdid, m_mdid_commute_opr, m_mdid_inverse_opr,
							m_comparision_type, m_returns_null_on_null_input, mdid_opfamilies_array,
							m_mdid_hash_opfamily, m_mdid_legacy_hash_opfamily, m_is_ndv_preserving);
...
CParseHandlerMDGPDBScalarOp::IsSupportedChildElem(const XMLCh *const xml_str)
{
	return (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpLeftTypeId),
			...
			0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpInverseOpId),
										  xml_str) ||
			0 == XMLString::compareString(
					 CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpHashOpfamily), xml_str) ||
			0 == XMLString::compareString(
					 CDXLTokens::XmlstrToken(EdxltokenGPDBScalarOpLegacyHashOpfamily), xml_str));
}
