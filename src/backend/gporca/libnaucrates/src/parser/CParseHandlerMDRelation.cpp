...
//
//---------------------------------------------------------------------------
CParseHandlerMDRelation::CParseHandlerMDRelation(CMemoryPool *mp,
									  ...
									  const XMLCh *const element_qname, const Attributes &attrs)
{
									  ...
									  element_local_name))
	{
		...
		m_part_constraint_unbounded = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenPartConstraintUnbounded,
			EdxltokenRelation);
	...
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenRelDistrOpfamilies),
									  element_local_name))
	{
		...
		m_opfamilies_parse_handler->startElement(element_uri, element_local_name, element_qname,
												 attrs);
	...
	m_is_temp_table = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenRelTemporary,
		EdxltokenRelation);
	...
	if (NULL != xmlszHasOids)
	{
		m_has_oids = CDXLOperatorFactory::ConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), xmlszHasOids, EdxltokenRelHasOids,
			EdxltokenRelation);
	}
	...
	if (NULL != xmlszPartColumns)
	{
		m_partition_cols_array = CDXLOperatorFactory::ExtractIntsToUlongArray(
			m_parse_handler_mgr->GetDXLMemoryManager(), xmlszPartColumns, EdxltokenPartKeys,
			EdxltokenRelation);
	}
	...
	if (NULL != xmlszPartTypes)
	{
		m_str_part_types_array = CDXLOperatorFactory::ExtractConvertPartitionTypeToArray(
			m_parse_handler_mgr->GetDXLMemoryManager(), xmlszPartTypes, EdxltokenPartTypes,
			EdxltokenRelation);
	}
	...
	if (NULL != xmlszPartitions)
	{
		m_num_of_partitions = CDXLOperatorFactory::ConvertAttrValueToUlong(
			m_parse_handler_mgr->GetDXLMemoryManager(), xmlszPartitions, EdxltokenNumLeafPartitions,
			EdxltokenRelation);
	}
	...
	if (NULL != xmlszConvertHashToRandom)
	{
		m_convert_hash_to_random = CDXLOperatorFactory::ConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), xmlszConvertHashToRandom,
...
)
{
	...
	{
		distr_opfamilies =
			dynamic_cast<CParseHandlerMetadataIdList *>(m_opfamilies_parse_handler)->GetMdIdArray();
		distr_opfamilies->AddRef();
	}

	m_imd_obj = GPOS_NEW(m_mp) CMDRelationGPDB(
		m_mp, m_mdid, m_mdname, m_is_temp_table, m_rel_storage_type, m_rel_distr_policy,
		md_col_array, m_distr_col_array, distr_opfamilies, m_partition_cols_array,
		m_str_part_types_array, m_num_of_partitions, m_convert_hash_to_random, m_key_sets_arrays,
		md_index_info_array, mdid_triggers_array, mdid_check_constraint_array, m_part_constraint,
		m_has_oids);
												 ...
												 Edxltoken dxl_token_element)
{
	...
	if (m_rel_distr_policy == IMDRelation::EreldistrHash)
	{
		...
		m_distr_col_array = CDXLOperatorFactory::ExtractIntsToUlongArray(
			m_parse_handler_mgr->GetDXLMemoryManager(), rel_distr_cols_xml, EdxltokenDistrColumns,
			dxl_token_element);
	}
	...
	if (NULL != xml_str_keys)
	{
		m_key_sets_arrays = CDXLOperatorFactory::ExtractConvertUlongTo2DArray(
			m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_keys, EdxltokenKeys,
			dxl_token_element);
	}
	else
	{
	...
	m_num_of_partitions = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenNumLeafPartitions,
		dxl_token_element, true /* optional */, 0 /* default value */);
}
