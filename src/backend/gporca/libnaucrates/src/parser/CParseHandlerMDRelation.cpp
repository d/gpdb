...
//---------------------------------------------------------------------------
void
CParseHandlerMDRelation::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
									  const XMLCh *const element_qname, const Attributes &attrs)
{
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPartConstraint), element_local_name))
	{
		...
		if (NULL != xmlszDefParts)
		{
			m_level_with_default_part_array =
				CDXLOperatorFactory::ExtractIntsToUlongArray(m_parse_handler_mgr->GetDXLMemoryManager(), xmlszDefParts,
		...
		m_part_constraint_unbounded = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenPartConstraintUnbounded, EdxltokenRelation);
	...
	m_is_temp_table = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenRelTemporary, EdxltokenRelation);
	...
	{
		m_has_oids = CDXLOperatorFactory::ConvertAttrValueToBool(m_parse_handler_mgr->GetDXLMemoryManager(),
																 xmlszHasOids, EdxltokenRelHasOids, EdxltokenRelation);
	}
	...
	if (NULL != xmlszPartitions)
	{
		m_num_of_partitions = CDXLOperatorFactory::ConvertAttrValueToUlong(
			m_parse_handler_mgr->GetDXLMemoryManager(), xmlszPartitions, EdxltokenNumLeafPartitions, EdxltokenRelation);
	}
...
)
{
	...
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenPartConstraint), element_local_name))
	{
		...
		if (pphMdlIndexInfo->GetMdIndexInfoArray()->Size() > 0)
		{
			...
			m_part_constraint = GPOS_NEW(m_mp) CMDPartConstraintGPDB(m_mp, m_level_with_default_part_array,
		...
		else
		{
			...
			m_part_constraint = GPOS_NEW(m_mp)
				CMDPartConstraintGPDB(m_mp, m_level_with_default_part_array, m_part_constraint_unbounded, NULL);
		}
		return;
	}
	...
	m_imd_obj = GPOS_NEW(m_mp)
		CMDRelationGPDB(m_mp, m_mdid, m_mdname, m_is_temp_table, m_rel_storage_type, m_rel_distr_policy, md_col_array,
						m_distr_col_array, distr_opfamilies, m_partition_cols_array, m_str_part_types_array,
						m_num_of_partitions, m_convert_hash_to_random, m_key_sets_arrays, md_index_info_array,
						mdid_triggers_array, mdid_check_constraint_array, m_part_constraint, m_has_oids);
...
//---------------------------------------------------------------------------
void
CParseHandlerMDRelation::ParseRelationAttributes(const Attributes &attrs, Edxltoken dxl_token_element)
{
	// parse table name
	...
	m_num_of_partitions = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenNumLeafPartitions, dxl_token_element,
