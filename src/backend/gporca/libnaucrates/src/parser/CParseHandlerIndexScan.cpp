...
//
//---------------------------------------------------------------------------
CParseHandlerIndexScan::CParseHandlerIndexScan(CMemoryPool *mp,
...
CParseHandlerIndexScan::EndElementHelper(const XMLCh *const element_local_name,
										 Edxltoken token_type, ULONG part_idx_id,
										 ULONG part_idx_id_printable)
{
	...
	else
	{
		...
		dxl_op = GPOS_NEW(m_mp)
			CDXLPhysicalDynamicIndexScan(m_mp, dxl_table_descr, part_idx_id, part_idx_id_printable,
