...
// Ctor
CParseHandlerScalarPartListNullTest::CParseHandlerScalarPartListNullTest(CMemoryPool *mp,
												  ...
												  const Attributes &attrs)
{
	...
	ULONG partition_level = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenPartLevel, EdxltokenScalarPartListNullTest);
	BOOL is_null = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenScalarIsNull, EdxltokenScalarPartListNullTest);

	m_dxl_node =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarPartListNullTest(m_mp, partition_level, is_null));
}
