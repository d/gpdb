...
//
//---------------------------------------------------------------------------
CParseHandlerScalarPartBound::CParseHandlerScalarPartBound(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
										   ...
										   const Attributes &attrs)
{
	...
	ULONG partition_level = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenPartLevel, EdxltokenScalarPartBound);
	IMDId *m_mdid_type = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenMDType, EdxltokenScalarPartBound);
	BOOL is_lower_bound = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenScalarPartBoundLower, EdxltokenScalarPartBound);

	m_dxl_node = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarPartBound(m_mp, partition_level, m_mdid_type, is_lower_bound));
}
