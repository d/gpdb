...
//
//---------------------------------------------------------------------------
CParseHandlerWindowSpec::CParseHandlerWindowSpec(CMemoryPool *mp,
									  ...
									  const XMLCh *const element_qname, const Attributes &attrs)
{
		...
		XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenWindowSpec), element_local_name))
	{
		...
		const XMLCh *xml_part_cols = CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenPartKeys,
																		   EdxltokenPhysicalWindow);
		m_part_by_colid_array = CDXLOperatorFactory::ExtractIntsToUlongArray(
			m_parse_handler_mgr->GetDXLMemoryManager(), xml_part_cols, EdxltokenPartKeys,
			EdxltokenPhysicalWindow);
										   ...
										   element_local_name))
	{
		...
		sort_col_list_parse_handler->startElement(element_uri, element_local_name, element_qname,
												  attrs);
	}
										   ...
										   element_local_name))
	{
		...
		window_frame_parse_handler->startElement(element_uri, element_local_name, element_qname,
												 attrs);
	}
	else
	{
