...
//
//---------------------------------------------------------------------------
CParseHandlerCostModel::CParseHandlerCostModel(CMemoryPool *mp,
									 ...
									 const XMLCh *const element_qname, const Attributes &attrs)
{
									  ...
									  element_local_name))
	{
		m_num_of_segments = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenSegmentsForCosting,
			EdxltokenCostModelConfig);

		m_cost_model_type =
			(ICostModel::ECostModelType) CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
				m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenCostModelType,
				EdxltokenCostModelConfig);
	}
