...
//---------------------------------------------------------------------------
void
CParseHandlerScalarFuncExpr::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
										  const XMLCh *const element_qname, const Attributes &attrs)
{
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenScalarFuncExpr), element_local_name))
	{
		if (!m_inside_func_expr)
		{
			...
			CDXLScalarFuncExpr *dxl_op = (CDXLScalarFuncExpr *) CDXLOperatorFactory::MakeDXLFuncExpr(
