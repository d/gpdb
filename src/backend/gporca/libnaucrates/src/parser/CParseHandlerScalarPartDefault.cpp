...
//
//---------------------------------------------------------------------------
CParseHandlerScalarPartDefault::CParseHandlerScalarPartDefault(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
											 ...
											 const Attributes &attrs)
{
	...
	ULONG partition_level = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenPartLevel, EdxltokenScalarPartDefault);
	m_dxl_node = GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarPartDefault(m_mp, partition_level));
}
