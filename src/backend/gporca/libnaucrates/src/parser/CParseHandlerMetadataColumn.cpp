...
//
//---------------------------------------------------------------------------
CParseHandlerMetadataColumn::CParseHandlerMetadataColumn(CMemoryPool *mp,
										  ...
										  const Attributes &attrs)
{
	...
	m_type_modifier = CDXLOperatorFactory::ExtractConvertAttrValueToInt(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenTypeMod, EdxltokenColDescr,
		true, default_type_modifier);

	// parse attribute number
	m_is_nullable = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenColumnNullable,
	...
	if (NULL != col_len_xml)
	{
		m_width = CDXLOperatorFactory::ConvertAttrValueToUlong(
			m_parse_handler_mgr->GetDXLMemoryManager(), col_len_xml, EdxltokenColWidth,
			EdxltokenColDescr);
	}
	...
	if (NULL != xmlszDropped)
	{
		m_is_dropped = CDXLOperatorFactory::ConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), xmlszDropped, EdxltokenColDropped,
			EdxltokenMetadataColumn);
	}
...
)
{
	...
	m_mdcol = GPOS_NEW(m_mp) CMDColumn(m_mdname, m_attno, m_mdid_type, m_type_modifier,
