...
CParseHandlerOptimizerConfig::CParseHandlerOptimizerConfig(CMemoryPool *mp, CParseHandlerManager *parse_handler_mgr,
	...
	: CParseHandlerBase(mp, parse_handler_mgr, parse_handler_root), m_pbs(NULL), m_optimizer_config(NULL)
{
}
...
//---------------------------------------------------------------------------
void
CParseHandlerOptimizerConfig::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
										   const XMLCh *const element_qname, const Attributes &attrs)
{
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenHint), element_local_name))
	{
		...
		CParseHandlerBase *pphHint = CParseHandlerFactory::GetParseHandler(m_mp, CDXLTokens::XmlstrToken(EdxltokenHint),
