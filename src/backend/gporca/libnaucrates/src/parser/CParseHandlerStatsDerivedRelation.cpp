...
//---------------------------------------------------------------------------
void
CParseHandlerStatsDerivedRelation::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
												const XMLCh *const element_qname, const Attributes &attrs)
{
	...
	else
	{
		...
		// parse rows
		const XMLCh *xml_rows =
			CDXLOperatorFactory::ExtractAttrValue(attrs, EdxltokenRows, EdxltokenStatsDerivedRelation);

		m_rows = CDouble(CDXLOperatorFactory::ConvertAttrValueToDouble(
			m_parse_handler_mgr->GetDXLMemoryManager(), xml_rows, EdxltokenRows, EdxltokenStatsDerivedRelation));
		...
		{
			m_empty =
				CDXLOperatorFactory::ConvertAttrValueToBool(m_parse_handler_mgr->GetDXLMemoryManager(), xml_is_empty,
