											 ...
											 const Attributes &attrs)
{
									  ...
									  element_local_name))
	{
		...
		if (NULL != m_rel_mdid)
		{
			...
			child_parse_handler->startElement(element_uri, element_local_name, element_qname,
											  attrs);
		}
		else
		{
			...
			m_num_of_part_levels = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
				...
				EdxltokenPhysicalPartitionSelectorLevels, EdxltokenPhysicalPartitionSelector);

			// parse scan id
			m_scan_id = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
				m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
				EdxltokenPhysicalPartitionSelectorScanId, EdxltokenPhysicalPartitionSelector);

			// parse handlers for all the scalar children
			CParseHandlerBase *op_list_filters_parse_handler =
				CParseHandlerFactory::GetParseHandler(
					m_mp, CDXLTokens::XmlstrToken(EdxltokenScalarOpList), m_parse_handler_mgr,
					this);
