								   ...
								   const Attributes &attrs)
{
	...
	if (NULL != xmlszItemType)
	{
		m_mdid_item_type = CDXLOperatorFactory::MakeMdIdFromStr(
			m_parse_handler_mgr->GetDXLMemoryManager(), xmlszItemType, EdxltokenIndexItemType,
			EdxltokenIndex);
	}
	...
	m_index_key_cols_array = CDXLOperatorFactory::ExtractIntsToUlongArray(
		m_parse_handler_mgr->GetDXLMemoryManager(), xmlszIndexKeys, EdxltokenIndexKeyCols,
		EdxltokenIndex);
...
)
{
									  ...
									  element_local_name))
	{
		...
		m_part_constraint =
			GPOS_NEW(m_mp) CMDPartConstraintGPDB(m_mp, m_level_with_default_part_array,
	...
	m_imd_obj = GPOS_NEW(m_mp) CMDIndexGPDB(
		m_mp, m_mdid, m_mdname, m_clustered, m_index_type, m_mdid_item_type, m_index_key_cols_array,
