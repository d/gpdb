...
//
//---------------------------------------------------------------------------
CParseHandlerLogicalDelete::CParseHandlerLogicalDelete(CMemoryPool *mp,
										 ...
										 const Attributes &attrs)
{
	...
	m_deletion_colid_array = CDXLOperatorFactory::ExtractIntsToUlongArray(
		m_parse_handler_mgr->GetDXLMemoryManager(), deletion_colids, EdxltokenDeleteCols,
		EdxltokenLogicalDelete);
...
)
{
	...
	m_dxl_node = GPOS_NEW(m_mp)
		CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalDelete(m_mp, table_descr, m_ctid_colid,
