											 ...
											 const Attributes &attrs)
{
	...
	IMDId *mdid_src = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenGPDBCastSrcType,
		EdxltokenGPDBArrayCoerceCast);

	IMDId *mdid_dest = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenGPDBCastDestType,
		EdxltokenGPDBArrayCoerceCast);

	IMDId *mdid_cast_func = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(
		m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenGPDBCastFuncId,
		EdxltokenGPDBArrayCoerceCast);
	...
	EdxlCoercionForm dxl_coercion_form =
		(EdxlCoercionForm) CDXLOperatorFactory::ExtractConvertAttrValueToInt(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenCoercionForm,
			EdxltokenGPDBArrayCoerceCast);
	...
	m_imd_obj = GPOS_NEW(m_mp) CMDArrayCoerceCastGPDB(
		m_mp, mdid, mdname, mdid_src, mdid_dest, is_binary_coercible, mdid_cast_func,
		coerce_path_type, type_modifier, is_explicit, dxl_coercion_form, location);
}
