...
//
//---------------------------------------------------------------------------
CParseHandlerPhysicalDML::CParseHandlerPhysicalDML(CMemoryPool *mp,
									   ...
									   const Attributes &attrs)
{
	...
	if (NULL != input_sort_req_xml)
	{
		m_input_sort_req = CDXLOperatorFactory::ConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), input_sort_req_xml, EdxltokenInputSorted,
...
)
{
	...
	CDXLPhysicalDML *dxl_op = GPOS_NEW(m_mp)
		CDXLPhysicalDML(m_mp, m_dxl_dml_type, table_descr, m_src_colids_array, m_action_colid,
