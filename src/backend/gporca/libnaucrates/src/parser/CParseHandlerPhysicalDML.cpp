									   ...
									   const Attributes &attrs)
{
	...
	m_oid_colid = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(m_parse_handler_mgr->GetDXLMemoryManager(), attrs,
																	  EdxltokenOidColId, token_type);
	m_ctid_colid = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(m_parse_handler_mgr->GetDXLMemoryManager(),
																	   attrs, EdxltokenCtidColId, token_type);
	m_segid_colid = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(m_parse_handler_mgr->GetDXLMemoryManager(),
																		attrs, EdxltokenGpSegmentIdColId, token_type);
	...
	{
		m_preserve_oids =
			CDXLOperatorFactory::ConvertAttrValueToBool(m_parse_handler_mgr->GetDXLMemoryManager(), preserve_oids_xml,
	...
	if (m_preserve_oids)
	{
		m_tuple_oid_col_oid = CDXLOperatorFactory::ExtractConvertAttrValueToUlong(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenTupleOidColId, EdxltokenPhysicalDMLUpdate);
	}
	...
	{
		m_input_sort_req =
			CDXLOperatorFactory::ConvertAttrValueToBool(m_parse_handler_mgr->GetDXLMemoryManager(), input_sort_req_xml,
...
)
{
	...
	CDXLPhysicalDML *dxl_op = GPOS_NEW(m_mp) CDXLPhysicalDML(
		m_mp, m_dxl_dml_type, table_descr, m_src_colids_array, m_action_colid, m_oid_colid, m_ctid_colid, m_segid_colid,
		m_preserve_oids, m_tuple_oid_col_oid, dxl_direct_dispatch_info, m_input_sort_req);
