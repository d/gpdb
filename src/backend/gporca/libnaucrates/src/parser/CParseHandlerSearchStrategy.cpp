...
//
//---------------------------------------------------------------------------
CParseHandlerSearchStrategy::CParseHandlerSearchStrategy(CMemoryPool *mp,
										  ...
										  const XMLCh *const element_qname, const Attributes &attrs)
{
										   ...
										   element_local_name))
	{
		...
		search_stage_parse_handler->startElement(element_uri, element_local_name, element_qname,
												 attrs);
	}
	else
	{
...
)
{
	...
	for (ULONG idx = 0; idx < size; idx++)
	{
		...
		CSearchStage *search_stage =
			GPOS_NEW(m_mp) CSearchStage(xform_set, search_stage_parse_handler->TimeThreshold(),
