...
//
//---------------------------------------------------------------------------
CParseHandlerMDRelationExternal::CParseHandlerMDRelationExternal(CMemoryPool *mp,
...
//---------------------------------------------------------------------------
void
CParseHandlerMDRelationExternal::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
											  const XMLCh *const element_qname, const Attributes &attrs)
{
	...
	{
		m_reject_limit =
			CDXLOperatorFactory::ConvertAttrValueToInt(m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_reject_limit,
		...
		m_is_rej_limit_in_rows = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenExtRelRejLimitInRows,
	...
	if (NULL != xml_str_err_rel_id)
	{
		m_mdid_fmt_err_table =
			CDXLOperatorFactory::MakeMdIdFromStr(m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_err_rel_id,
	...
	if (NULL != xml_str_convert_hash_to_random)
	{
		m_convert_hash_to_random = CDXLOperatorFactory::ConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_convert_hash_to_random, EdxltokenConvertHashToRandom,
			EdxltokenRelationExternal);
	}
...
)
{
	...
	m_imd_obj = GPOS_NEW(m_mp) CMDRelationExternalGPDB(
		m_mp, m_mdid, m_mdname, m_rel_distr_policy, md_col_array, m_distr_col_array, distr_opfamilies,
		m_convert_hash_to_random, m_key_sets_arrays, md_index_info_array, mdid_triggers_array,
		mdid_check_constraint_array, m_reject_limit, m_is_rej_limit_in_rows, m_mdid_fmt_err_table);
