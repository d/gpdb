											  ...
											  const Attributes &attrs)
{
	...
	if (NULL != xml_str_convert_hash_to_random)
	{
		m_convert_hash_to_random = CDXLOperatorFactory::ConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_convert_hash_to_random,
...
)
{
	...
	m_imd_obj = GPOS_NEW(m_mp) CMDRelationExternalGPDB(
		m_mp, m_mdid, m_mdname, m_rel_distr_policy, md_col_array, m_distr_col_array,
		distr_opfamilies, m_convert_hash_to_random, m_key_sets_arrays, md_index_info_array,
		mdid_triggers_array, mdid_check_constraint_array, m_reject_limit, m_is_rej_limit_in_rows,
