...
//
//---------------------------------------------------------------------------
CParseHandlerColStatsBucket::CParseHandlerColStatsBucket(CMemoryPool *mp,
										  ...
										  const Attributes &attrs)
{
									  ...
									  element_local_name))
	{
		...
		m_frequency = CDXLOperatorFactory::ExtractConvertAttrValueToDouble(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenStatsFrequency,
			EdxltokenColumnStatsBucket);
		m_distinct = CDXLOperatorFactory::ExtractConvertAttrValueToDouble(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenStatsDistinct,
			EdxltokenColumnStatsBucket);
	}
										   ...
										   element_local_name))
	{
		...
		m_is_lower_closed = CDXLOperatorFactory::ExtractConvertAttrValueToBool(
			m_parse_handler_mgr->GetDXLMemoryManager(), attrs, EdxltokenStatsBoundClosed,
...
)
{
	if (0 == XMLString::compareString(CDXLTokens::XmlstrToken(EdxltokenColumnStatsBucket),
									  element_local_name))
	{
		m_dxl_bucket = GPOS_NEW(m_mp)
			CDXLBucket(m_lower_bound_dxl_datum, m_upper_bound_dxl_datum, m_is_lower_closed,
