...
//---------------------------------------------------------------------------
void
CParseHandlerMetadata::StartElement(const XMLCh *const element_uri, const XMLCh *const element_local_name,
									const XMLCh *const element_qname, const Attributes &attrs)
{
	...
	else if (0 == XMLString::compareString(element_local_name, CDXLTokens::XmlstrToken(EdxltokenMdid)))
	{
		...
		IMDId *mdid = CDXLOperatorFactory::ExtractConvertAttrValueToMdId(m_parse_handler_mgr->GetDXLMemoryManager(),
																		 attrs, EdxltokenValue, EdxltokenMdid);
...
//---------------------------------------------------------------------------
CSystemIdArray *
CParseHandlerMetadata::GetSrcSysIdArray(const Attributes &attrs, Edxltoken dxl_token_attr, Edxltoken dxl_token_element)
{
	...
	while (NULL != (xml_str_sys_id = xml_str_tokenizer.nextToken()))
	{
		...
		ULONG type = CDXLOperatorFactory::ConvertAttrValueToUlong(m_parse_handler_mgr->GetDXLMemoryManager(),
																  xml_str_type, dxl_token_attr, dxl_token_element);
