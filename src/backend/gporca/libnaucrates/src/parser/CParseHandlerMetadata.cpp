...
//
//---------------------------------------------------------------------------
CParseHandlerMetadata::CParseHandlerMetadata(CMemoryPool *mp,
...
//---------------------------------------------------------------------------
CSystemIdArray *
CParseHandlerMetadata::GetSrcSysIdArray(const Attributes &attrs, Edxltoken dxl_token_attr,
										Edxltoken dxl_token_element)
{
	...
	while (NULL != (xml_str_sys_id = xml_str_tokenizer.nextToken()))
	{
		...
		ULONG type = CDXLOperatorFactory::ConvertAttrValueToUlong(
			m_parse_handler_mgr->GetDXLMemoryManager(), xml_str_type, dxl_token_attr,
			dxl_token_element);
