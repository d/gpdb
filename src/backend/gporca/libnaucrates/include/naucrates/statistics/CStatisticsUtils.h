...
private:
	...
	static CBucketArray *SplitHistBucketGivenMcvBuckets(CMemoryPool *mp,
	...
	static CBucket *CreateValidBucket(CMemoryPool *mp, CPoint *bucket_lower_bound,
	...
	static BOOL IsValidBucket(CPoint *bucket_lower_bound, CPoint *bucket_upper_bound,
							  BOOL is_lower_closed, BOOL is_upper_closed);

	// find the MCVs that fall within the same histogram bucket and perform the split
	static void SplitHistDriver(CMemoryPool *mp, const CBucket *histogram_bucket,
	...
	static CDouble MaxNumGroupsForGivenSrcGprCols(CMemoryPool *mp,
...
public:
	...
	static CHistogram *TransformMCVToHist(CMemoryPool *mp, const IMDType *mdtype,
										  IDatumArray *mcv_datums, CDoubleArray *freq_array,
										  ULONG num_mcv_values);

	// merge MCVs and histogram
	static CHistogram *MergeMCVHist(CMemoryPool *mp, const CHistogram *mcv_histogram,
	...
	static void PrintColStats(CMemoryPool *mp, CStatsPred *pred_stats, ULONG cond_colid,
	...
	static void ExtractUsedColIds(CMemoryPool *mp, CBitSet *colids_bitset, CStatsPred *pred_stats,
								  ULongPtrArray *colids);

	// given the previously generated histogram, update the intermediate
	// result of the disjunction
	static void UpdateDisjStatistics(CMemoryPool *mp, CBitSet *non_updatable_cols,
	...
	static UlongToHistogramMap *MergeHistogramMapsForDisjPreds(CMemoryPool *mp,
	...
	static UlongToHistogramMap *CopyHistHashMap(CMemoryPool *mp,
	...
	static void AddRemainingBuckets(CMemoryPool *mp, const CBucketArray *src_buckets,
	...
	static IStatistics *DeriveStatsForDynamicScan(CMemoryPool *mp, CExpressionHandle &expr_handle,
												  ULONG part_idx_id,
	...
	static UlongToUlongPtrArrayMap *GetGrpColIdToUpperBoundNDVIdxMap(
		CMemoryPool *mp, CStatistics *stats, const CColRefSet *grouping_cols_refset, CBitSet *keys);

	// extract NDVs for the given array of grouping columns
	static CDoubleArray *ExtractNDVForGrpCols(CMemoryPool *mp,
											  const CStatisticsConfig *stats_config,
	...
	static CDouble Groups(CMemoryPool *mp, IStatistics *stats,
						  const CStatisticsConfig *stats_config, ULongPtrArray *grouping_cols,
						  CBitSet *keys);
	...
	static void AddGrpColStats(CMemoryPool *mp, const CStatistics *input_stats,
							   CColRefSet *grp_cols_refset, UlongToHistogramMap *output_histograms,
