...
private:
	...
	static void AddBuckets(CMemoryPool *mp, const CBucketArray *src_buckets,
						   CBucketArray *dest_buckets, CDouble rows_old, CDouble rows_new,
	...
	ULONG AddResidualUnionAllBucket(CBucketArray *histogram_buckets, CBucket *bucket,
									CDouble rows_old, CDouble rows_new, BOOL bucket_is_residual,
									ULONG index) const;

	// add residual union buckets after the merge
	ULONG AddResidualUnionBucket(CBucketArray *histogram_buckets, CBucket *bucket, CDouble rows,
	...
	typedef CDynamicPtrArray<SAdjBucketBoundary, CleanupDelete<SAdjBucketBoundary> >
...
public:
	...
	CHistogram(CMemoryPool *mp, CBucketArray *histogram_buckets, BOOL is_well_defined,
	...
	CHistogram *MakeHistogramFilterNormalize(CStatsPred::EStatsCmpType stats_cmp_type,
											 CPoint *point, CDouble *scale_factor) const;
	...
	CDouble GetInequalityJoinScaleFactor(CDouble rows, const CHistogram *other_histogram,
										 CDouble rows_other) const;

	// left anti semi join with another histogram and normalize
	CHistogram *MakeLASJHistogramNormalize(
		CStatsPred::EStatsCmpType stats_cmp_type, CDouble rows, const CHistogram *other_histogram,
	...
	CHistogram *MakeUnionAllHistogramNormalize(CDouble rows, const CHistogram *other_histogram,
											   CDouble rows_other) const;
	...
	CBucket *GetNextBucket(const CHistogram *histogram, CBucket *new_bucket,
						   BOOL *target_bucket_is_residual, ULONG *current_bucket_index) const;
	...
	CDXLStatsDerivedColumn *TranslateToDXLDerivedColumnStats(CMDAccessor *md_accessor, ULONG colid,
															 CDouble width) const;
	...
	static void AddHistograms(CMemoryPool *mp, UlongToHistogramMap *src_histograms,
