...
public:
	struct SJoinCondition
	{
		...
		gpos::Ref<IMdIdArray> m_oid_pair;

		// true if both sides of the predicate are distribution keys
		BOOL m_dist_keys;

		//ctor
		SJoinCondition(CDouble scale_factor, gpos::Ref<IMdIdArray> mdid_pair,
					   BOOL both_dist_keys)
		...
		{
			return CombineHashes(gpos::HashPtr<IMDId>((*oid_pair)[0].get()),
								 gpos::HashPtr<IMDId>((*oid_pair)[1].get()));
		}
	...
	typedef gpos::UnorderedMap<
		gpos::Ref<IMdIdArray>, gpos::Ref<CDoubleArray>,
		gpos::RefHash<IMdIdArray, SJoinCondition::HashValue>,
		gpos::RefEq<IMdIdArray, SJoinCondition::Equals>>
		OIDPairToScaleFactorArrayMap;

	typedef gpos::UnorderedMap<
		gpos::Ref<IMdIdArray>, gpos::Ref<CDoubleArray>,
		gpos::RefHash<IMdIdArray, SJoinCondition::HashValue>,
		gpos::RefEq<IMdIdArray, SJoinCondition::Equals>>::LegacyIterator
	...
	static gpos::Ref<OIDPairToScaleFactorArrayMap> GenerateScaleFactorMap(
