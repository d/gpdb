...
protected:
	...
	static CDouble CalcJoinCardinality(
		...
		CDouble right_num_rows, CScaleFactorUtils::SJoinConditionArray *join_conds_scale_factors,
	...
	static void JoinHistograms(CMemoryPool *mp, const CHistogram *histogram1,
...
public:
	...
	static IStatistics *CalcAllJoinStats(CMemoryPool *mp, IStatisticsArray *statistics_array,
										 CExpression *expr, COperator *pop);

	// derive statistics for join operation given array of statistics object
	static IStatistics *DeriveJoinStats(CMemoryPool *mp, CExpressionHandle &exprhdl,
