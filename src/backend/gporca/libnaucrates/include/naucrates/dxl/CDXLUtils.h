...
private:
	...
	static CParseHandlerDXL *GetParseHandlerForDXLString(CMemoryPool *,
...
public:
	...
	static CQueryToDXLResult *ParseQueryToQueryDXLTree(CMemoryPool *, const CHAR *dxl_string,
													   const CHAR *xsd_file_path);

	// parse a DXL document containing a scalar expression
	static CDXLNode *ParseDXLToScalarExprDXLNode(CMemoryPool *, const CHAR *dxl_string,
												 const CHAR *xsd_file_path);

	// parse an MD request
	static CMDRequest *ParseDXLToMDRequest(CMemoryPool *mp, const CHAR *dxl_string,
										   const CHAR *xsd_file_path);

	// parse a list of mdids from a MD request message
	static CMDRequest *ParseDXLToMDRequest(CMemoryPool *mp, const WCHAR *dxl_string,
										   const CHAR *xsd_file_path);
	...
	static IMDCacheObjectArray *ParseDXLToIMDObjectArray(CMemoryPool *,
														 const CWStringBase *dxl_string,
														 const CHAR *xsd_file_path);
	...
	static CDXLStatsDerivedRelationArray *ParseDXLToStatsDerivedRelArray(CMemoryPool *,
																		 const CHAR *dxl_string,
																		 const CHAR *xsd_file_path);

	// parse statistics object from the statistics document
	static CDXLStatsDerivedRelationArray *ParseDXLToStatsDerivedRelArray(
		CMemoryPool *, const CWStringBase *dxl_string, const CHAR *xsd_file_path);
	...
	static void SerializePlan(CMemoryPool *mp, IOstream &os, const CDXLNode *node, ULLONG plan_id,
							  ULLONG plan_space_size, BOOL serialize_document_header_footer,
							  BOOL indentation);
	...
	static void SerializeMetadata(CMemoryPool *mp, const IMDCacheObjectArray *imd_obj_array,
								  IOstream &os, BOOL serialize_document_header_footer,
								  BOOL indentation);
	...
	static CWStringDynamic *SerializeMetadata(CMemoryPool *mp, const IMDId *mdid,
											  BOOL serialize_document_header_footer,
											  BOOL indentation);

	// serialize sample plans
	static CWStringDynamic *SerializeSamplePlans(CMemoryPool *mp, CEnumeratorConfig *enumerator_cfg,
												 BOOL indentation);

	// serialize cost distribution plans
	static CWStringDynamic *SerializeCostDistr(CMemoryPool *mp, CEnumeratorConfig *enumerator_cfg,
											   BOOL indentation);

	// serialize a metadata object into DXL
	static CWStringDynamic *SerializeMDObj(CMemoryPool *, const IMDCacheObject *,
										   BOOL serialize_document_header_footer, BOOL indentation);
	...
	static BYTE *CreateStringFrom64XMLStr(CDXLMemoryManager *memory_manager,
										  const XMLCh *xml_string, ULONG *length);
	...
	static CWStringDynamic *EncodeByteArrayToString(CMemoryPool *mp, const BYTE *byte,
													ULONG length);
	...
	static CWStringDynamic *SerializeToCommaSeparatedString(CMemoryPool *mp,
															const CharPtrArray *pdrgpsz);

	// decode a byte array from a string
	static BYTE *DecodeByteArrayFromString(CMemoryPool *mp, const CWStringDynamic *dxl_string,
										   ULONG *length);
...
template <typename T, void (*CleanupFn)(T *)>
CWStringDynamic *
CDXLUtils::Serialize(CMemoryPool *mp, const CDynamicPtrArray<T, CleanupFn> *dynamic_ptr_array)
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		{
			string_var->AppendFormat(GPOS_WSZ_LIT("%d%ls"), value,
