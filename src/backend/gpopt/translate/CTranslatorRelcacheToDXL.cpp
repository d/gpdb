...
//---------------------------------------------------------------------------
IMDCacheObject *
CTranslatorRelcacheToDXL::RetrieveObject(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid)
{
...
//---------------------------------------------------------------------------
IMDCacheObject *
CTranslatorRelcacheToDXL::RetrieveObjectGPDB(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid)
{
...
CMDIndexInfoArray *
CTranslatorRelcacheToDXL::RetrieveRelIndexInfoForNonPartTable(CMemoryPool *mp, Relation rel)
{
	CMDIndexInfoArray *md_index_info_array = GPOS_NEW(mp) CMDIndexInfoArray(mp);
...
//---------------------------------------------------------------------------
IMDRelation *
CTranslatorRelcacheToDXL::RetrieveRel(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid)
{
	OID oid = CMDIdGPDB::CastMdid(mdid)->Oid();
	...
	if (NULL != rel->rd_cdbpolicy && POLICYTYPE_ENTRY != rel->rd_cdbpolicy->ptype &&
		gpdb::GetGPSegmentCount() != rel->rd_cdbpolicy->numsegments)
	{
		// GPORCA does not support partially distributed tables yet
		GPOS_RAISE(gpdxl::ExmaMD, gpdxl::ExmiDXLInvalidAttributeValue, GPOS_WSZ_LIT("Partially Distributed Data"));
	}
	...
	if (IMDRelation::ErelstorageExternal == rel_storage_type)
	{
		...
		md_rel = GPOS_NEW(mp) CMDRelationExternalGPDB(
			mp, mdid, mdname, dist, mdcol_array, distr_cols, distr_op_families, convert_hash_to_random, keyset_array,
			md_index_info_array, mdid_triggers_array, check_constraint_mdids, extentry->rejectlimit,
			('r' == extentry->rejectlimittype), NULL /* it's sufficient to pass NULL here since ORCA
	...
	else
	{
		...
		md_rel = GPOS_NEW(mp) CMDRelationGPDB(mp, mdid, mdname, is_temporary, rel_storage_type, dist, mdcol_array,
											  distr_cols, distr_op_families, part_keys, part_types, num_leaf_partitions,
											  convert_hash_to_random, keyset_array, md_index_info_array,
											  mdid_triggers_array, check_constraint_mdids, mdpart_constraint, has_oids);
	}
...
//---------------------------------------------------------------------------
CMDColumnArray *
CTranslatorRelcacheToDXL::RetrieveRelColumns(CMemoryPool *mp, CMDAccessor *md_accessor, Relation rel,
											 IMDRelation::Erelstoragetype rel_storage_type)
{
	...
	for (ULONG ul = 0; ul < (ULONG) rel->rd_att->natts; ul++)
	{
		...
		if (rel->rd_att->attrs[ul].attgenerated)
			GPOS_RAISE(gpdxl::ExmaMD, gpdxl::ExmiMDObjUnsupported, GPOS_WSZ_LIT("column has GENERATED default value"));
	}
	...
	for (ULONG ul = 0; ul < (ULONG) rel->rd_att->natts; ul++)
	{
		...
		CMDColumn *md_col = GPOS_NEW(mp) CMDColumn(md_colname, att->attnum, mdid_col, att->atttypmod, !att->attnotnull,
												   att->attisdropped, dxl_default_col_val /* default value */, col_len);
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorRelcacheToDXL::GetDefaultColumnValue(CMemoryPool *mp, CMDAccessor *md_accessor, TupleDesc rd_att,
												AttrNumber attno)
{
	GPOS_ASSERT(attno > 0);
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorRelcacheToDXL::RetrieveRelDistributionCols(CMemoryPool *mp, GpPolicy *gp_policy, CMDColumnArray *mdcol_array,
													  ULONG size)
{
...
IMdIdArray *
CTranslatorRelcacheToDXL::RetrieveRelDistributionOpFamilies(CMemoryPool *mp, GpPolicy *gp_policy)
{
...
//---------------------------------------------------------------------------
void
CTranslatorRelcacheToDXL::AddSystemColumns(CMemoryPool *mp, CMDColumnArray *mdcol_array, Relation rel, BOOL is_ao_table)
{
	...
	for (INT i = SelfItemPointerAttributeNumber; i > FirstLowInvalidHeapAttributeNumber; i--)
	{
		...
		CMDColumn *md_col = GPOS_NEW(mp)
			CMDColumn(md_colname, attno, CTranslatorUtils::GetSystemColType(mp, attno), default_type_modifier,
					  false,  // is_nullable
					  false,  // is_dropped
					  NULL,	  // default value
...
//---------------------------------------------------------------------------
IMDIndex *
CTranslatorRelcacheToDXL::RetrieveIndex(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid_index)
{
	...
	CMDIndexGPDB *index = GPOS_NEW(mp) CMDIndexGPDB(mp, mdid_index, mdname, index_clustered, index_type, mdid_item_type,
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorRelcacheToDXL::ComputeIncludedCols(CMemoryPool *mp, const IMDRelation *md_rel)
{
...
//---------------------------------------------------------------------------
ULONG *
CTranslatorRelcacheToDXL::PopulateAttnoPositionMap(CMemoryPool *mp, const IMDRelation *md_rel, ULONG size)
{
...
CTranslatorRelcacheToDXL::RetrieveType(CMemoryPool *mp, IMDId *mdid)
{
	...
	return GPOS_NEW(mp) CMDTypeGenericGPDB(
		mp, mdid, mdname, is_redistributable, is_fixed_length, length, is_passed_by_value, mdid_distr_opfamily,
		mdid_legacy_distr_opfamily, mdid_op_eq, mdid_op_neq, mdid_op_lt, mdid_op_leq, mdid_op_gt, mdid_op_geq,
		mdid_op_cmp, mdid_min, mdid_max, mdid_avg, mdid_sum, mdid_count, is_hashable, is_merge_joinable,
		is_composite_type, is_text_related_type, mdid_type_relid, mdid_type_array, ptce->typlen);
}
...
CTranslatorRelcacheToDXL::RetrieveScOp(CMemoryPool *mp, IMDId *mdid)
{
	...
	CMDScalarOpGPDB *md_scalar_op = GPOS_NEW(mp) CMDScalarOpGPDB(
		mp, mdid, mdname, mdid_type_left, mdid_type_right, result_type_mdid, mdid_func, mdid_commute_opr,
		m_mdid_inverse_opr, cmp_type, returns_null_on_null_input, RetrieveScOpOpFamilies(mp, mdid), mdid_hash_opfamily,
		mdid_legacy_hash_opfamily, is_ndv_preserving);
	return md_scalar_op;
}
...
CTranslatorRelcacheToDXL::RetrieveFunc(CMemoryPool *mp, IMDId *mdid)
{
	...
	CMDFunctionGPDB *md_func =
		GPOS_NEW(mp) CMDFunctionGPDB(mp, mdid, mdname, result_type_mdid, arg_type_mdids, returns_set, stability, access,
...
CTranslatorRelcacheToDXL::RetrieveCheckConstraints(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid)
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		CDXLColDescr *dxl_col_descr =
			GPOS_NEW(mp) CDXLColDescr(mp, md_colname, ul + 1 /*colid*/, md_col->AttrNum(), mdid_col_type,
...
//---------------------------------------------------------------------------
IMDId *
CTranslatorRelcacheToDXL::RetrieveAggIntermediateResultType(CMemoryPool *mp, IMDId *mdid)
{
	OID agg_oid = CMDIdGPDB::CastMdid(mdid)->Oid();
...
CTranslatorRelcacheToDXL::RetrieveRelStats(CMemoryPool *mp, IMDId *mdid)
{
	...
	CDXLRelStats *dxl_rel_stats =
		GPOS_NEW(mp) CDXLRelStats(mp, m_rel_stats_mdid, mdname, CDouble(num_rows), relation_empty);
...
IMDCacheObject *
CTranslatorRelcacheToDXL::RetrieveColStats(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid)
{
	...
	if (0 > attno)
	{
		...
		return GenerateStatsForSystemCols(mp, rel_oid, mdid_col_stats, md_colname, att_type, attno,
...
//---------------------------------------------------------------------------
CDXLColStats *
CTranslatorRelcacheToDXL::GenerateStatsForSystemCols(CMemoryPool *mp, OID rel_oid, CMDIdColStats *mdid_col_stats,
													 ...
													 CDXLBucketArray *dxl_stats_bucket_array, CDouble num_rows)
{
	...
	return GPOS_NEW(mp) CDXLColStats(mp, mdid_col_stats, md_colname, width, null_freq, distinct_remaining,
...
CTranslatorRelcacheToDXL::RetrieveCast(CMemoryPool *mp, IMDId *mdid)
{
	...
	switch (pathtype)
	{
		...
		{
			return GPOS_NEW(mp) CMDArrayCoerceCastGPDB(mp, mdid, mdname, mdid_src, mdid_dest, is_binary_coercible,
	...
	return GPOS_NEW(mp)
		CMDCastGPDB(mp, mdid, mdname, mdid_src, mdid_dest, is_binary_coercible, GPOS_NEW(mp) CMDIdGPDB(cast_fn_oid));
}
...
CTranslatorRelcacheToDXL::RetrieveScCmp(CMemoryPool *mp, IMDId *mdid)
{
	...
	return GPOS_NEW(mp)
		CMDScCmpGPDB(mp, mdid, mdname, mdid_left, mdid_right, cmp_type, GPOS_NEW(mp) CMDIdGPDB(scalar_cmp_oid));
}
...
//---------------------------------------------------------------------------
CDXLBucketArray *
CTranslatorRelcacheToDXL::TransformStatsToDXLBucketArray(CMemoryPool *mp, OID att_type, CDouble num_distinct,
...
//---------------------------------------------------------------------------
CHistogram *
CTranslatorRelcacheToDXL::TransformMcvToOrcaHistogram(CMemoryPool *mp, const IMDType *md_type, const Datum *mcv_values,
													  const float4 *mcv_frequencies, ULONG num_mcv_values)
{
...
//---------------------------------------------------------------------------
CHistogram *
CTranslatorRelcacheToDXL::TransformHistToOrcaHistogram(CMemoryPool *mp, const IMDType *md_type,
													   const Datum *hist_values, ULONG num_hist_values,
...
//---------------------------------------------------------------------------
CDXLBucketArray *
CTranslatorRelcacheToDXL::TransformHistogramToDXLBucketArray(CMemoryPool *mp, const IMDType *md_type,
...
//---------------------------------------------------------------------------
void
CTranslatorRelcacheToDXL::RetrievePartKeysAndTypes(CMemoryPool *mp, Relation rel, OID oid, ULongPtrArray **part_keys,
...
//---------------------------------------------------------------------------
ULONG *
CTranslatorRelcacheToDXL::ConstructAttnoMapping(CMemoryPool *mp, CMDColumnArray *mdcol_array, ULONG max_cols)
{
...
//---------------------------------------------------------------------------
ULongPtr2dArray *
CTranslatorRelcacheToDXL::RetrieveRelKeysets(CMemoryPool *mp, OID oid, BOOL should_add_default_keys,
...
CTranslatorRelcacheToDXL::IsIndexSupported(Relation index_rel)
{
	...
	return gpdb::HeapAttIsNull(tup, Anum_pg_index_indexprs) && gpdb::HeapAttIsNull(tup, Anum_pg_index_indpred) &&
...
CTranslatorRelcacheToDXL::RetrievePartConstraintForIndex(CMemoryPool *mp, CMDAccessor *md_accessor,
														 const IMDRelation *md_rel, Node *part_constraint,
														 ...
														 BOOL is_unbounded)
{
	...
	for (ULONG ul = 0; ul < num_columns; ul++)
	{
		...
		// create a column descriptor for the column
		CDXLColDescr *dxl_col_descr =
			GPOS_NEW(mp) CDXLColDescr(mp, md_colname,
									  ul + 1,  // colid
	...
	CMDPartConstraintGPDB *mdpart_constraint = RetrievePartConstraintFromNode(
		mp, md_accessor, dxl_col_descr_array, part_constraint, level_with_default_part_array, is_unbounded);
														 ...
														 BOOL is_unbounded)
{
	...
	// translate the check constraint expression
	CDXLNode *scalar_dxlnode = CTranslatorScalarToDXL::TranslateStandaloneExprToDXL(mp, md_accessor, var_colid_mapping,
																					(Expr *) part_constraints);
...
{
	return RELKIND_RELATION == rel_kind || RELKIND_SEQUENCE == rel_kind || RELKIND_AOSEGMENTS == rel_kind ||
		   RELKIND_TOASTVALUE == rel_kind || RELKIND_FOREIGN_TABLE == rel_kind || RELKIND_PARTITIONED_TABLE == rel_kind;
}
...
//---------------------------------------------------------------------------
IMdIdArray *
CTranslatorRelcacheToDXL::RetrieveIndexOpFamilies(CMemoryPool *mp, IMDId *mdid_index)
{
...
//---------------------------------------------------------------------------
IMdIdArray *
CTranslatorRelcacheToDXL::RetrieveScOpOpFamilies(CMemoryPool *mp, IMDId *mdid_scalar_op)
{
