...
static const ULONG cmp_type_mappings[][2] = {
	{IMDType::EcmptEq, CmptEq}, {IMDType::EcmptNEq, CmptNEq}, {IMDType::EcmptL, CmptLT},
	{IMDType::EcmptG, CmptGT},	{IMDType::EcmptGEq, CmptGEq}, {IMDType::EcmptLEq, CmptLEq}};
...
CTranslatorRelcacheToDXL::RetrieveRel(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid)
{
	...
	if (IMDRelation::ErelstorageExternal == rel_storage_type)
	{
		...
		md_rel = GPOS_NEW(mp) CMDRelationExternalGPDB(
			...
			convert_hash_to_random, keyset_array, md_index_info_array, mdid_triggers_array,
			check_constraint_mdids, extentry->rejectlimit, ('r' == extentry->rejectlimittype),
	...
	else
	{
		...
		md_rel = GPOS_NEW(mp) CMDRelationGPDB(
			mp, mdid, mdname, is_temporary, rel_storage_type, dist, mdcol_array, distr_cols,
			distr_op_families, part_keys, part_types, num_leaf_partitions, convert_hash_to_random,
			keyset_array, md_index_info_array, mdid_triggers_array, check_constraint_mdids,
											 ...
											 IMDRelation::Erelstoragetype rel_storage_type)
{
	...
	for (ULONG ul = 0; ul < (ULONG) rel->rd_att->natts; ul++)
	{
		...
		CMDColumn *md_col = GPOS_NEW(mp)
			CMDColumn(md_colname, att->attnum, mdid_col, att->atttypmod, !att->attnotnull,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorRelcacheToDXL::GetDefaultColumnValue(CMemoryPool *mp, CMDAccessor *md_accessor,
												TupleDesc rd_att, AttrNumber attno)
{
	GPOS_ASSERT(attno > 0);
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorRelcacheToDXL::RetrieveRelDistributionCols(CMemoryPool *mp, GpPolicy *gp_policy,
													  CMDColumnArray *mdcol_array, ULONG size)
{
...
//---------------------------------------------------------------------------
void
CTranslatorRelcacheToDXL::AddSystemColumns(CMemoryPool *mp, CMDColumnArray *mdcol_array,
										   Relation rel, BOOL is_ao_table)
{
	...
	for (INT i = SelfItemPointerAttributeNumber; i > FirstLowInvalidHeapAttributeNumber; i--)
	{
		...
		CMDColumn *md_col = GPOS_NEW(mp) CMDColumn(
			md_colname, attno, CTranslatorUtils::GetSystemColType(mp, attno), default_type_modifier,
			false,	// is_nullable
			false,	// is_dropped
			NULL,	// default value
...
//---------------------------------------------------------------------------
IMDIndex *
CTranslatorRelcacheToDXL::RetrieveIndex(CMemoryPool *mp, CMDAccessor *md_accessor,
										IMDId *mdid_index)
{
	...
	CMDIndexGPDB *index = GPOS_NEW(mp)
		CMDIndexGPDB(mp, mdid_index, mdname, index_clustered, index_type, mdid_item_type,
...
//---------------------------------------------------------------------------
ULONG *
CTranslatorRelcacheToDXL::PopulateAttnoPositionMap(CMemoryPool *mp, const IMDRelation *md_rel,
												   ULONG size)
{
...
CTranslatorRelcacheToDXL::RetrieveType(CMemoryPool *mp, IMDId *mdid)
{
	...
	return GPOS_NEW(mp) CMDTypeGenericGPDB(
		mp, mdid, mdname, is_redistributable, is_fixed_length, length, is_passed_by_value,
		mdid_distr_opfamily, mdid_legacy_distr_opfamily, mdid_op_eq, mdid_op_neq, mdid_op_lt,
		mdid_op_leq, mdid_op_gt, mdid_op_geq, mdid_op_cmp, mdid_min, mdid_max, mdid_avg, mdid_sum,
		mdid_count, is_hashable, is_merge_joinable, is_composite_type, is_text_related_type,
		mdid_type_relid, mdid_type_array, ptce->typlen);
}
...
CTranslatorRelcacheToDXL::RetrieveScOp(CMemoryPool *mp, IMDId *mdid)
{
	...
	CMDScalarOpGPDB *md_scalar_op = GPOS_NEW(mp)
		CMDScalarOpGPDB(mp, mdid, mdname, mdid_type_left, mdid_type_right, result_type_mdid,
...
CTranslatorRelcacheToDXL::RetrieveFunc(CMemoryPool *mp, IMDId *mdid)
{
	...
	CMDFunctionGPDB *md_func = GPOS_NEW(mp)
		CMDFunctionGPDB(mp, mdid, mdname, result_type_mdid, arg_type_mdids, returns_set, stability,
						access, is_strict, is_ndv_preserving, is_allowed_for_PS);
...
CTranslatorRelcacheToDXL::RetrieveAgg(CMemoryPool *mp, IMDId *mdid)
{
	...
	CMDAggregateGPDB *pmdagg = GPOS_NEW(mp)
		CMDAggregateGPDB(mp, mdid, mdname, result_type_mdid, intermediate_result_type_mdid,
...
CTranslatorRelcacheToDXL::RetrieveCheckConstraints(CMemoryPool *mp, CMDAccessor *md_accessor,
												   IMDId *mdid)
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		CDXLColDescr *dxl_col_descr =
			GPOS_NEW(mp) CDXLColDescr(mp, md_colname, ul + 1 /*colid*/, md_col->AttrNum(),
									  mdid_col_type, md_col->TypeModifier(), false /* fColDropped */
			);
	...
	// translate the check constraint expression
	CDXLNode *scalar_dxlnode = CTranslatorScalarToDXL::TranslateStandaloneExprToDXL(
...
CTranslatorRelcacheToDXL::RetrieveColStats(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid)
{
	...
	(void) gpdb::GetAttrStatsSlot(&hist_slot, stats_tup, STATISTIC_KIND_HISTOGRAM, InvalidOid,
								  ATTSTATSSLOT_VALUES);
	...
	CDXLBucketArray *dxl_stats_bucket_array_transformed = TransformStatsToDXLBucketArray(
		mp, att_type, num_distinct, null_freq, mcv_slot.values, mcv_slot.numbers,
	...
	mdid_col_stats->AddRef();
	CDXLColStats *dxl_col_stats = GPOS_NEW(mp)
		CDXLColStats(mp, mdid_col_stats, md_colname, width, null_freq, distinct_remaining,
...
CDXLColStats *
CTranslatorRelcacheToDXL::GenerateStatsForSystemCols(
	CMemoryPool *mp, OID rel_oid, CMDIdColStats *mdid_col_stats, CMDName *md_colname, OID att_type,
	AttrNumber attno, CDXLBucketArray *dxl_stats_bucket_array, CDouble num_rows)
{
...
CTranslatorRelcacheToDXL::RetrieveCast(CMemoryPool *mp, IMDId *mdid)
{
	...
	switch (pathtype)
	{
		...
		case COERCION_PATH_RELABELTYPE:
			...
			return GPOS_NEW(mp)
				CMDCastGPDB(mp, mdid, mdname, mdid_src, mdid_dest, true /*is_binary_coercible*/,
...
CTranslatorRelcacheToDXL::TransformStatsToDXLBucketArray(
	...
	const float4 *mcv_frequencies, ULONG num_mcv_values, const Datum *hist_values,
	ULONG num_hist_values)
{
	...
	if (has_hist)
	{
		...
		histogram = TransformHistToOrcaHistogram(mp, md_type, hist_values, num_hist_values,
												 num_distinct, hist_freq);
...
//---------------------------------------------------------------------------
CHistogram *
CTranslatorRelcacheToDXL::TransformMcvToOrcaHistogram(CMemoryPool *mp, const IMDType *md_type,
...
CTranslatorRelcacheToDXL::TransformHistToOrcaHistogram(CMemoryPool *mp, const IMDType *md_type,
													   ...
													   ULONG num_hist_values, CDouble num_distinct,
													   CDouble hist_freq)
{
	...
	for (ULONG ul = 0; ul < num_buckets; ul++)
	{
		...
		CBucket *bucket = GPOS_NEW(mp)
			CBucket(GPOS_NEW(mp) CPoint(min_datum), GPOS_NEW(mp) CPoint(max_datum), is_lower_closed,
...
//---------------------------------------------------------------------------
CDXLBucketArray *
CTranslatorRelcacheToDXL::TransformHistogramToDXLBucketArray(CMemoryPool *mp,
															 ...
															 const CHistogram *hist)
{
	...
	for (ULONG ul = 0; ul < num_buckets; ul++)
	{
		...
		CDXLBucket *dxl_bucket = GPOS_NEW(mp)
			CDXLBucket(dxl_lower, dxl_upper, bucket->IsLowerClosed(), bucket->IsUpperClosed(),
...
//---------------------------------------------------------------------------
void
CTranslatorRelcacheToDXL::RetrievePartKeysAndTypes(CMemoryPool *mp, Relation rel, OID oid,
...
//---------------------------------------------------------------------------
ULONG *
CTranslatorRelcacheToDXL::ConstructAttnoMapping(CMemoryPool *mp, CMDColumnArray *mdcol_array,
												ULONG max_cols)
{
...
CTranslatorRelcacheToDXL::RetrievePartConstraintForIndex(
	CMemoryPool *mp, CMDAccessor *md_accessor, const IMDRelation *md_rel, Node *part_constraint,
...
CMDPartConstraintGPDB *
CTranslatorRelcacheToDXL::RetrievePartConstraintFromNode(
	CMemoryPool *mp, CMDAccessor *md_accessor, CDXLColDescrArray *dxl_col_descr_array,
	Node *part_constraints, ULongPtrArray *level_with_default_part_array, BOOL is_unbounded)
{
...
CTranslatorRelcacheToDXL::RelHasSystemColumns(char rel_kind)
{
	return RELKIND_RELATION == rel_kind || RELKIND_SEQUENCE == rel_kind ||
		   ...
		   RELKIND_FOREIGN_TABLE == rel_kind || RELKIND_PARTITIONED_TABLE == rel_kind;
}
