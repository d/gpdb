...
//
//---------------------------------------------------------------------------
CCTEListEntry::CCTEListEntry(CMemoryPool *mp, ULONG query_level, CommonTableExpr *cte,
	...
	: m_query_level(query_level), m_cte_info(NULL)
{
	...
	BOOL result GPOS_ASSERTS_ONLY = m_cte_info->Insert(
		cte->ctename, GPOS_NEW(mp) SCTEProducerInfo(cte_producer, cte_query->targetList));
...
//---------------------------------------------------------------------------
void
CCTEListEntry::AddCTEProducer(CMemoryPool *mp, CommonTableExpr *cte, const CDXLNode *cte_producer)
{
