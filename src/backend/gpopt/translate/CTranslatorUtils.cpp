...
//---------------------------------------------------------------------------
CDXLIndexDescr *
CTranslatorUtils::GetIndexDescr(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid)
{
...
//---------------------------------------------------------------------------
CDXLTableDescr *
CTranslatorUtils::GetTableDescr(CMemoryPool *mp, CMDAccessor *md_accessor, CIdGenerator *id_generator,
...
)
{
	...
	for (ULONG ul = 0; ul < len; ul++)
	{
		...
		CDXLColDescr *dxl_col_descr = GPOS_NEW(mp) CDXLColDescr(mp, col, id_generator->next_id(), md_col->AttrNum(),
...
//---------------------------------------------------------------------------
BOOL
CTranslatorUtils::IsSirvFunc(CMemoryPool *mp, CMDAccessor *md_accessor, OID func_oid)
{
...
//---------------------------------------------------------------------------
CDXLLogicalTVF *
CTranslatorUtils::ConvertToCDXLLogicalTVF(CMemoryPool *mp, CMDAccessor *md_accessor, CIdGenerator *id_generator,
										  const RangeTblEntry *rte)
{
	/*
...
//---------------------------------------------------------------------------
IMdIdArray *
CTranslatorUtils::ResolvePolymorphicTypes(CMemoryPool *mp, IMdIdArray *mdid_array, List *arg_types_list,
										  FuncExpr *funcexpr)
{
	ULONG arg_index = 0;
...
//---------------------------------------------------------------------------
CDXLColDescrArray *
CTranslatorUtils::GetColumnDescriptorsFromRecord(CMemoryPool *mp, CIdGenerator *id_generator, List *col_names,
												 List *col_types, List *col_type_modifiers)
{
	...
	ForThree(col_name, col_names, col_type, col_types, col_type_modifier, col_type_modifiers)
	{
		...
		CDXLColDescr *dxl_col_descr =
			GPOS_NEW(mp) CDXLColDescr(mp, col_mdname, id_generator->next_id(), INT(ul + 1) /* attno */, col_type,
...
//---------------------------------------------------------------------------
CDXLColDescrArray *
CTranslatorUtils::GetColumnDescriptorsFromBase(CMemoryPool *mp, CIdGenerator *id_generator, IMDId *mdid_return_type,
											   INT type_modifier, CMDName *pmdName)
{
	CDXLColDescrArray *column_descrs = GPOS_NEW(mp) CDXLColDescrArray(mp);
	...
	CDXLColDescr *dxl_col_descr = GPOS_NEW(mp) CDXLColDescr(mp, col_mdname, id_generator->next_id(), INT(1) /* attno */,
...
//---------------------------------------------------------------------------
CDXLColDescrArray *
CTranslatorUtils::GetColumnDescriptorsFromComposite(CMemoryPool *mp, CMDAccessor *md_accessor,
													CIdGenerator *id_generator, const IMDType *type)
{
...
//---------------------------------------------------------------------------
CMDColumnArray *
CTranslatorUtils::ExpandCompositeType(CMemoryPool *mp, CMDAccessor *md_accessor, const IMDType *type)
{
...
CTranslatorUtils::GetSystemColName(AttrNumber attno)
{
	...
	switch (attno)
	{
		...
		default:
			GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiPlStmt2DXLConversion, GPOS_WSZ_LIT("Invalid attribute number"));
			return NULL;
	}
}
...
CTranslatorUtils::OidCmpOperator(Expr *expr)
{
	...
	switch (expr->type)
	{
		...
		default:
			GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiPlStmt2DXLConversion, GPOS_WSZ_LIT("Unsupported comparison"));
			return InvalidOid;
	}
}
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorUtils::GetGroupingColidArray(CMemoryPool *mp, CBitSet *group_by_cols,
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorUtils::GenerateColIds(CMemoryPool *mp, List *target_list, IMdIdArray *input_mdid_arr,
...
//---------------------------------------------------------------------------
CDXLColDescrArray *
CTranslatorUtils::GetDXLColumnDescrArray(CMemoryPool *mp, List *target_list, ULongPtrArray *colids,
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorUtils::GetPosInTargetList(CMemoryPool *mp, List *target_list, BOOL keep_res_junked)
{
	GPOS_ASSERT(NULL != target_list);
...
//---------------------------------------------------------------------------
CDXLColDescr *
CTranslatorUtils::GetColumnDescrAt(CMemoryPool *mp, TargetEntry *target_entry, ULONG colid, ULONG pos)
{
	...
	CDXLColDescr *dxl_col_descr = GPOS_NEW(mp) CDXLColDescr(mp, mdname, colid, pos,	 /* attno */
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorUtils::CreateDummyProjectElem(CMemoryPool *mp, ULONG colid_input, ULONG colid_output,
										 CDXLColDescr *dxl_col_descr)
{
	CMDIdGPDB *original_mdid = CMDIdGPDB::CastMdid(dxl_col_descr->MdidType());
	CMDIdGPDB *copy_mdid =
		GPOS_NEW(mp) CMDIdGPDB(original_mdid->Oid(), original_mdid->VersionMajor(), original_mdid->VersionMinor());
	...
	CDXLNode *dxl_project_element = GPOS_NEW(mp) CDXLNode(
		mp,
		GPOS_NEW(mp)
			CDXLScalarProjElem(mp, colid_output, GPOS_NEW(mp) CMDName(mp, dxl_col_descr->MdName()->GetMDName())),
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorUtils::GetOutputColIdsArray(CMemoryPool *mp, List *target_list, IntToUlongMap *attno_to_colid_map)
{
	GPOS_ASSERT(NULL != target_list);
...
//---------------------------------------------------------------------------
BOOL
CTranslatorUtils::IsReferencedInWindowSpec(const TargetEntry *target_entry, List *window_clause_list)
{
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorUtils::CreateDXLProjElemFromInt8Const(CMemoryPool *mp, CMDAccessor *md_accessor, INT val)
{
	GPOS_ASSERT(NULL != mp);
...
//---------------------------------------------------------------------------
BOOL
CTranslatorUtils::IsGroupingColumn(const TargetEntry *target_entry, const SortGroupClause *grouping_clause)
{
...
//---------------------------------------------------------------------------
UlongToUlongMap *
CTranslatorUtils::MakeNewToOldColMapping(CMemoryPool *mp, ULongPtrArray *old_colids, ULongPtrArray *new_colids)
{
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorUtils::CreateDXLProjElemConstNULL(CMemoryPool *mp, CMDAccessor *md_accessor, CIdGenerator *pidgtorCol,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorUtils::CreateDXLProjElemConstNULL(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid, ULONG colid,
...
//---------------------------------------------------------------------------
void
CTranslatorUtils::UpdateGrpColMapping(CMemoryPool *mp, UlongToUlongMap *group_col_pos, CBitSet *group_cols,
