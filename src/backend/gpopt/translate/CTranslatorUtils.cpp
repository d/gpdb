...
)
{
	...
	for (ULONG ul = 0; ul < len; ul++)
	{
		...
		CDXLColDescr *dxl_col_descr =
			GPOS_NEW(mp) CDXLColDescr(mp, col, id_generator->next_id(), md_col->AttrNum(), col_type,
...
//---------------------------------------------------------------------------
CDXLLogicalTVF *
CTranslatorUtils::ConvertToCDXLLogicalTVF(CMemoryPool *mp, CMDAccessor *md_accessor,
										  CIdGenerator *id_generator, const RangeTblEntry *rte)
{
	/*
	...
	if (NULL != rtfunc->funccoltypes)
	{
		...
		column_descrs = GetColumnDescriptorsFromRecord(
			mp, id_generator, rte->eref->colnames, rtfunc->funccoltypes, rtfunc->funccoltypmods);
	}
	...
	else
	{
		...
		column_descrs = GetColumnDescriptorsFromBase(mp, id_generator, mdid_return_type,
...
//---------------------------------------------------------------------------
IMdIdArray *
CTranslatorUtils::ResolvePolymorphicTypes(CMemoryPool *mp, IMdIdArray *mdid_array,
...
//---------------------------------------------------------------------------
CDXLColDescrArray *
CTranslatorUtils::GetColumnDescriptorsFromRecord(CMemoryPool *mp, CIdGenerator *id_generator,
												 ...
												 List *col_type_modifiers)
{
	...
	ForThree(col_name, col_names, col_type, col_types, col_type_modifier, col_type_modifiers)
	{
		...
		CDXLColDescr *dxl_col_descr = GPOS_NEW(mp)
			CDXLColDescr(mp, col_mdname, id_generator->next_id(), INT(ul + 1) /* attno */, col_type,
...
//---------------------------------------------------------------------------
CDXLColDescrArray *
CTranslatorUtils::GetColumnDescriptorsFromBase(CMemoryPool *mp, CIdGenerator *id_generator,
											   ...
											   CMDName *pmdName)
{
	...
	CDXLColDescr *dxl_col_descr =
		GPOS_NEW(mp) CDXLColDescr(mp, col_mdname, id_generator->next_id(), INT(1) /* attno */,
...
)
{
	...
	{
		// simple group by
		CBitSet *col_attnos = CreateAttnoSetForGroupingSet(mp, group_clause_list, num_cols,
...
//---------------------------------------------------------------------------
CDXLColDescr *
CTranslatorUtils::GetColumnDescrAt(CMemoryPool *mp, TargetEntry *target_entry, ULONG colid,
								   ULONG pos)
{
...
//---------------------------------------------------------------------------
UlongToUlongMap *
CTranslatorUtils::MakeNewToOldColMapping(CMemoryPool *mp, ULongPtrArray *old_colids,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorUtils::CreateDXLProjElemConstNULL(CMemoryPool *mp, CMDAccessor *md_accessor,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorUtils::CreateDXLProjElemConstNULL(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid,
											 ...
											 ULONG colid, CHAR *alias_name)
{
	...
	else
	{
		...
		datum_dxl = CMDTypeGenericGPDB::CreateDXLDatumVal(mp, mdid, md_type, default_type_modifier,
														  true /*fConstNull*/, NULL,	 /*pba */
...
//---------------------------------------------------------------------------
void
CTranslatorUtils::UpdateGrpColMapping(CMemoryPool *mp, UlongToUlongMap *group_col_pos,
