...
//---------------------------------------------------------------------------
PlannedStmt *
CTranslatorDXLToPlStmt::GetPlannedStmtFromDXL(const CDXLNode *dxlnode, const Query *orig_query,
											  bool can_set_tag)
{
...
{
	List *params_node_list =
		gpdb::ExtractNodesPlan(plan, T_Param, true /* descend_into_subqueries */);
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	RangeTblEntry *rte =
		TranslateDXLTblDescrToRangeTblEntry(dxl_table_descr, index, &base_table_context);
	...
	if (IMDRelation::ErelstorageExternal == md_rel->RetrieveRelStorageType())
	{
		...
		ForeignScan *foreign_scan =
			gpdb::CreateForeignScanForExternalTable(oidRel, index, qual, targetlist);
...
Plan *
CTranslatorDXLToPlStmt::TranslateDXLIndexScan(
	const CDXLNode *index_scan_dxlnode, CDXLPhysicalIndexScan *physical_idx_scan_dxlop,
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	RangeTblEntry *rte = TranslateDXLTblDescrToRangeTblEntry(
		physical_idx_scan_dxlop->GetDXLTableDescr(), index, &base_table_context);
	GPOS_ASSERT(NULL != rte);
	rte->requiredPerms |= ACL_SELECT;
	m_dxl_to_plstmt_context->AddRTE(rte);
	...
	// translate proj list
	plan->targetlist = TranslateDXLProjList(project_list_dxlnode, &base_table_context,
											NULL /*child_contexts*/, output_context);

	// translate index filter
	plan->qual = TranslateDXLIndexFilter(filter_dxlnode, output_context, &base_table_context,
										 ctxt_translation_prev_siblings);
					   ...
					   CDXLTranslateContextBaseTable *index_context)
{
	...
	for (ULONG ul = 0; ul < md_index->Keys(); ul++)
	{
		...
		Expr *indexvar = (Expr *) gpdb::MakeVar(
			new_varno, col->AttrNum(), CMDIdGPDB::CastMdid(col->MdidType())->Oid(),
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	index_scan->indextlist =
		TranslateDXLIndexTList(md_rel, md_index, index, table_desc, &index_context);

	// translate target list
	plan->targetlist = TranslateDXLProjList(project_list_dxlnode, &index_context,
											NULL /*child_contexts*/, output_context);

	// translate index filter
	plan->qual = TranslateDXLIndexFilter(filter_dxlnode, output_context, &index_context,
										 ctxt_translation_prev_siblings);
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		Expr *index_filter_expr = m_translator_dxl_to_scalar->TranslateDXLToScalar(
...
CTranslatorDXLToPlStmt::TranslateIndexConditions(
	...
	List **index_orig_cond, List **index_strategy_list, List **index_subtype_list)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		Expr *original_index_cond_expr = m_translator_dxl_to_scalar->TranslateDXLToScalar(
			index_cond_dxlnode, &colid_var_mapping);
		Expr *index_cond_expr = m_translator_dxl_to_scalar->TranslateDXLToScalar(
			index_cond_dxlnode, &colid_var_mapping);
		...
		// create index qual
		index_qual_info_array->Append(
			GPOS_NEW(m_mp) CIndexQualInfo(attno, index_cond_expr, original_index_cond_expr,
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		*index_strategy_list =
			gpdb::LAppendInt(*index_strategy_list, index_qual_info->m_index_subtype_oid);
		*index_subtype_list =
													  ...
													  CDXLTranslationContextArray *child_contexts)
{
	...
	// build colid->var mapping
	CMappingColIdVarPlStmt colid_var_mapping(m_mp, NULL /*base_table_context*/, child_contexts,
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		Expr *assert_contraint_expr = m_translator_dxl_to_scalar->TranslateDXLToScalar(
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	CDXLTranslateContext left_dxl_translate_ctxt(m_mp, false,
	...
	Plan *left_plan = TranslateDXLOperatorToPlan(child_plan_dxlnode, &left_dxl_translate_ctxt,
												 ctxt_translation_prev_siblings);
	...
	{
		CMappingColIdVarPlStmt colid_var_mapping(m_mp, NULL, child_contexts, output_context,
												 m_dxl_to_plstmt_context);
		Node *limit_count = (Node *) m_translator_dxl_to_scalar->TranslateDXLToScalar(
	...
	if (NULL != limit_offset_dxlnode && limit_offset_dxlnode->Arity() > 0)
	{
		...
		Node *limit_offset = (Node *) m_translator_dxl_to_scalar->TranslateDXLToScalar(
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	CDXLTranslateContext left_dxl_translate_ctxt(m_mp, false,
												 output_context->GetColIdToParamIdMap());
	CDXLTranslateContext right_dxl_translate_ctxt(m_mp, false,
												  output_context->GetColIdToParamIdMap());

	Plan *left_plan = TranslateDXLOperatorToPlan(left_tree_dxlnode, &left_dxl_translate_ctxt,
												 ctxt_translation_prev_siblings);
	...
	Plan *right_plan = (Plan *) TranslateDXLHash(right_tree_dxlnode, &right_dxl_translate_ctxt,
	...
	else
	{
		...
		for (ULONG ul = 0; ul < arity; ul++)
		{
			...
			Expr *hash_clause_expr =
				(Expr *) m_translator_dxl_to_scalar->TranslateDXLScalarCmpToScalar(
										...
										CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	ForEach(lc_target_entry, target_list)
	{
		...
		rtfunc->funccolnames =
			gpdb::LAppend(rtfunc->funccolnames, gpdb::MakeStringValue(target_entry->resname));
	...
	CDXLTranslateContextBaseTable *base_table_context)
{
	...
	for (ULONG ul = 1; ul < num_of_child; ++ul)
	{
		...
		Expr *pexprFuncArg =
			m_translator_dxl_to_scalar->TranslateDXLToScalar(func_arg_dxlnode, &colid_var_mapping);
	...
	ListCell *lc = NULL;
	List *param_exprs = gpdb::ExtractNodesExpression((Node *) func_expr, T_Param,
	...
	CDXLTranslateContextBaseTable *base_table_context)
{
	...
	for (ULONG ulValue = EdxlValIndexConstStart; ulValue < num_of_child; ulValue++)
	{
		...
		if (NIL == values_collations)
		{
			...
			{
				values_collations =
					gpdb::LAppendOid(values_collations, gpdb::ExprCollation((Node *) value));
			}
		}
	}
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	if (dxl_nlj->IsIndexNLJ())
	{
		...
		for (ULONG ul = 0; ul < ulLen; ul++)
		{
			...
			if (NULL == right_dxl_translate_ctxt.GetParamIdMappingElement(ulColid))
			{
				...
				CMappingElementColIdParamId *pmecolidparamid = GPOS_NEW(m_mp)
					CMappingElementColIdParamId(ulColid, param_id, pmdid, iTypeModifier);
#ifdef GPOS_DEBUG
	...
	if (dxl_nlj->IsIndexNLJ())
	{
		((NestLoop *) plan)->nestParams =
			TranslateNestLoopParamList(dxl_nlj->GetNestLoopParamsColRefs(),
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	if (0 < num_sort_cols)
	{
		...
		TranslateSortCols(sort_col_list_dxl, output_context, motion->sortColIdx,
						  motion->sortOperators, motion->collations, motion->nullsFirst);
	}
	else
	{
		...
		motion_dxlop->GetDXLOperator() == EdxlopPhysicalMotionRandom)
	{
		...
		if (EdxlopPhysicalMotionRedistribute == motion_dxlop->GetDXLOperator())
		{
			...
			TranslateHashExprList(hash_expr_list_dxlnode, &child_context, &hash_expr_list,
								  &hash_expr_opfamilies, output_context);
		}
	...
	if (EdxlopPhysicalMotionRedistribute == motion_dxlop->GetDXLOperator())
	{
		...
		for (ULONG ul = 0; ul < length; ul++)
		{
			...
			else
			{
				...
				Expr *expr = m_translator_dxl_to_scalar->TranslateDXLToScalar(expr_dxlnode,
																			  &colid_var_mapping);
				...
				target_entry =
					gpdb::MakeTargetEntry(expr, gpdb::ListLength(plan->targetlist) + 1,
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	{
		window->partColIdx =
			(AttrNumber *) gpdb::GPDBAlloc(window->partNumCols * sizeof(AttrNumber));
	...
	if (size == 1)
	{
		...
		TranslateSortCols(sort_col_list_dxlnode, &child_context, window->ordColIdx,
						  window->ordOperators, window->ordCollations, is_nulls_first);
							...
							CMDAccessor *md_accessor)
{
	...
	{
		auto step = StepFromDXLConst((*eq_values)[i], supportfnoid, gpdb::ListLength(result),
...
PartitionPruneInfoFromPartitionSelector(const CDXLNode *partition_selector_dxlnode,
										...
										CTranslatorDXLToScalar *translator_dxl_to_scalar)
{
	auto dxl_part_selector =
		dynamic_cast<CDXLPhysicalPartitionSelector *>(partition_selector_dxlnode->GetOperator());
	...
	std::copy(relation->rd_partdesc->oids,
			  relation->rd_partdesc->oids + relation->rd_partdesc->nparts, pinfo->relid_map);
	...
	pinfo->exec_pruning_steps =
		PartPruneStepsFromEqFilters(eq_filters, relation->rd_partkey->partsupfunc[0].fn_oid,
...
Plan *
CTranslatorDXLToPlStmt::TranslateDXLPartSelector(
	const CDXLNode *partition_selector_dxlnode, CDXLTranslateContext *output_context,
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
...
List *
CTranslatorDXLToPlStmt::TranslateDXLFilterList(
	const CDXLNode *filter_list_dxlnode, const CDXLTranslateContextBaseTable *base_table_context,
	CDXLTranslationContextArray *child_contexts, CDXLTranslateContext *output_context)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		Expr *filter_expr = m_translator_dxl_to_scalar->TranslateDXLToScalar(child_filter_dxlnode,
																			 &colid_var_mapping);
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	for (ULONG ul = 0; ul < length; ++ul)
	{
		...
		Var *var =
			gpdb::MakeVar(idxVarno, attno, CMDIdGPDB::CastMdid(sc_ident_dxlop->MdidType())->Oid(),
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	for (ULONG ul = 0; ul < num_of_proj_list_elem; ul++)
	{
		...
		Var *var = gpdb::MakeVar(OUTER_VAR, (AttrNumber)(ul + 1), oid_type,
										...
										CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	RangeTblEntry *rte =
		TranslateDXLTblDescrToRangeTblEntry(table_descr, index, &base_table_context);
	GPOS_ASSERT(NULL != rte);
	...
	{
		AddJunkTargetEntryForColId(&dml_target_list, &child_context, phy_dml_dxlop->GetCtIdColId(),
								   "ctid");
		AddJunkTargetEntryForColId(&dml_target_list, &child_context,
								   phy_dml_dxlop->GetSegmentIdColId(), "gp_segment_id");
	}
	if (m_cmd_type == CMD_UPDATE && phy_dml_dxlop->IsOidsPreserved())
		AddJunkTargetEntryForColId(&dml_target_list, &child_context, phy_dml_dxlop->GetTupleOid(),
								   "oid");
...
CTranslatorDXLToPlStmt::GetDXLDatumGPDBHash(CDXLDatumArray *dxl_datum_array)
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		Const *const_expr =
			(Const *) m_translator_dxl_to_scalar->TranslateDXLDatumToScalar(datum_dxl);
		consts_list = gpdb::LAppend(consts_list, const_expr);
		hashfuncs[ul] =
			m_dxl_to_plstmt_context->GetDistributionHashFuncForType(const_expr->consttype);
	}
...
List *
CTranslatorDXLToPlStmt::TranslateDXLProjList(
	const CDXLNode *project_list_dxlnode, const CDXLTranslateContextBaseTable *base_table_context,
	CDXLTranslationContextArray *child_contexts, CDXLTranslateContext *output_context)
{
	...
	for (ULONG ul = 0; ul < arity; ++ul)
	{
		...
		Expr *expr =
			m_translator_dxl_to_scalar->TranslateDXLToScalar(expr_dxlnode, &colid_var_mapping);
...
List *
CTranslatorDXLToPlStmt::TranslateDXLFilterToQual(
	const CDXLNode *filter_dxlnode, const CDXLTranslateContextBaseTable *base_table_context,
	CDXLTranslationContextArray *child_contexts, CDXLTranslateContext *output_context)
{
	const ULONG arity = filter_dxlnode->Arity();
	if (0 == arity)
	...
	return TranslateDXLScCondToQual(filter_cond_dxlnode, base_table_context, child_contexts,
									output_context);
}
...
List *
CTranslatorDXLToPlStmt::TranslateDXLScCondToQual(
	const CDXLNode *condition_dxlnode, const CDXLTranslateContextBaseTable *base_table_context,
	CDXLTranslationContextArray *child_contexts, CDXLTranslateContext *output_context)
{
	List *quals_list = NIL;
	...
	Expr *expr =
		m_translator_dxl_to_scalar->TranslateDXLToScalar(condition_dxlnode, &colid_var_mapping);
...
CTranslatorDXLToPlStmt::TranslatePlanCosts(const CDXLNode *dxlnode, Plan *plan)
{
	CDXLOperatorCost *costs =
		CDXLPhysicalProperties::PdxlpropConvert(dxlnode->GetProperties())->GetDXLOperatorCost();
	...
	if (GPOS_FTRACE(EopttraceConsiderOpfamiliesForDistribution))
	{
		for (ULONG ul = 0; ul < arity; ul++)
		{
			...
			hash_expr_opfamilies =
				gpdb::LAppendOid(hash_expr_opfamilies, CMDIdGPDB::CastMdid(opfamily)->Oid());
		}
	}
...
//---------------------------------------------------------------------------
void
CTranslatorDXLToPlStmt::AddJunkTargetEntryForColId(List **target_list,
												   ...
												   ULONG colid, const char *resname)
{
	...
	TargetEntry *te_new =
		gpdb::MakeTargetEntry((Expr *) var, resno, resname_str, true /* resjunk */);
...
CTranslatorDXLToPlStmt::TranslateDXLPhyCtasToIntoClause(const CDXLPhysicalCTAS *phy_ctas_dxlop)
{
	...
	into_clause->rel->relpersistence =
		phy_ctas_dxlop->IsTemporary() ? RELPERSISTENCE_TEMP : RELPERSISTENCE_PERMANENT;
	...
	for (ULONG ul = 0; ul < num_of_cols; ++ul)
	{
		...
		col_def->collOid =
			gpdb::TypeCollation(CMDIdGPDB::CastMdid(dxl_col_descr->MdidType())->Oid());
														 ...
														 List *target_list)
{
	...
	GpPolicy *distr_policy = gpdb::MakeGpPolicy(POLICYTYPE_PARTITIONED, num_of_distr_cols_alloc,
	...
	if (IMDRelation::EreldistrHash == dxlop->Ereldistrpolicy())
	{
		...
		for (ULONG ul = 0; ul < num_of_distr_cols; ul++)
		{
			...
			distr_policy->opclasses[ul] =
				m_dxl_to_plstmt_context->GetDistributionHashOpclassForType(typeoid);
		}
	}
	return distr_policy;
}
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	bitmap_tbl_scan->scan.plan.lefttree = TranslateDXLBitmapAccessPath(
		bitmap_access_path_dxlnode, output_context, md_rel, table_descr, &base_table_context,
...
Plan *
CTranslatorDXLToPlStmt::TranslateDXLBitmapAccessPath(
	const CDXLNode *bitmap_access_path_dxlnode, CDXLTranslateContext *output_context,
	const IMDRelation *md_rel, const CDXLTableDescr *table_descr,
	CDXLTranslateContextBaseTable *base_table_context,
	CDXLTranslationContextArray *ctxt_translation_prev_siblings, BitmapHeapScan *bitmap_tbl_scan)
{
	Edxlopid dxl_op_id = bitmap_access_path_dxlnode->GetOperator()->GetDXLOperator();
	if (EdxlopScalarBitmapIndexProbe == dxl_op_id)
	{
		return TranslateDXLBitmapIndexProbe(bitmap_access_path_dxlnode, output_context, md_rel,
											table_descr, base_table_context,
											ctxt_translation_prev_siblings, bitmap_tbl_scan);
	}
	GPOS_ASSERT(EdxlopScalarBitmapBoolOp == dxl_op_id);

	return TranslateDXLBitmapBoolOp(bitmap_access_path_dxlnode, output_context, md_rel, table_descr,
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings, BitmapHeapScan *bitmap_tbl_scan)
{
	...
	Plan *left_plan = TranslateDXLBitmapAccessPath(left_tree_dxlnode, output_context, md_rel,
												   table_descr, base_table_context,
												   ctxt_translation_prev_siblings, bitmap_tbl_scan);
	Plan *right_plan = TranslateDXLBitmapAccessPath(
		right_tree_dxlnode, output_context, md_rel, table_descr, base_table_context,
		ctxt_translation_prev_siblings, bitmap_tbl_scan);
...
Plan *
CTranslatorDXLToPlStmt::TranslateDXLBitmapIndexProbe(
	const CDXLNode *bitmap_index_probe_dxlnode, CDXLTranslateContext *output_context,
	const IMDRelation *md_rel, const CDXLTableDescr *table_descr,
	CDXLTranslateContextBaseTable *base_table_context,
	CDXLTranslationContextArray *ctxt_translation_prev_siblings, BitmapHeapScan *bitmap_tbl_scan)
{
	...
	TranslateIndexConditions(index_cond_list_dxlnode, table_descr, true /*is_bitmap_index_probe*/,
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	RangeTblEntry *rte = TranslateDXLValueScanToRangeTblEntry(value_scan_dxlnode, output_context,
															  &base_table_context);
	GPOS_ASSERT(NULL != rte);
												   ...
												   CDXLTranslateContext *dxltrctxRight)
{
	...
	for (ULONG ul = 0; ul < pdrgdxlcrOuterRefs->Size(); ul++)
	{
		...
		Var *new_var = gpdb::MakeVar(OUTER_VAR, target_entry->resno, old_var->vartype,
