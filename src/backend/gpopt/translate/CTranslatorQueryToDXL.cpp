	  ...
	  m_cteid_at_current_query_level_map(NULL)
{
	...
	m_scalar_translator = GPOS_NEW(m_mp) CTranslatorScalarToDXL(
		m_context, m_md_accessor, m_query_level, m_query_level_to_cte_map, m_dxl_cte_producers);
}
...
CTranslatorQueryToDXL::TranslateSelectQueryToDXL()
{
	...
	else if (0 != gpdb::ListLength(m_query->windowClause))	// translate window clauses
	{
		...
		child_dxlnode = TranslateWindowToDXL(dxlnode, m_query->targetList, m_query->windowClause,
	...
	else
	{
		child_dxlnode = TranslateGroupingSets(
			m_query->jointree, m_query->targetList, m_query->groupClause, m_query->hasAggs,
			sort_group_attno_to_colid_mapping, output_attno_to_colid_mapping);
	}

	// translate limit clause
	CDXLNode *limit_dxlnode =
		TranslateLimitToDXLGroupBy(m_query->sortClause, m_query->limitCount, m_query->limitOffset,
	...
	IntToUlongMap *output_attno_to_colid_mapping, List *group_clause)
{
	...
	return TranslateTargetListToDXLProject(target_list, join_tree_dxlnode,
...
CTranslatorQueryToDXL::TranslateInsertQueryToDXL()
{
	...
	CDXLTableDescr *table_descr = CTranslatorUtils::GetTableDescr(
		m_mp, m_md_accessor, m_context->m_colid_counter, rte, &m_context->m_has_distributed_tables);
...
CTranslatorQueryToDXL::TranslateCTASToDXL()
{
	...
	for (ULONG ul = 0; ul < num_columns; ul++)
	{
		...
		{
			md_colname =
				GPOS_NEW(m_mp) CMDName(m_mp, dxl_ident->GetDXLColRef()->MdName()->GetMDName());
		}
		...
		CDXLColDescr *dxl_col_descr = GPOS_NEW(m_mp)
			CDXLColDescr(m_mp, md_colname, m_context->m_colid_counter->next_id(), resno /* attno */,
	...
	CDXLLogicalCTAS *ctas_dxlop = GPOS_NEW(m_mp) CDXLLogicalCTAS(
		...
		rel_distr_policy, distribution_colids, distr_opfamilies, fTempTable, has_oids,
		rel_storage_type, source_array, var_typmods);
...
CTranslatorQueryToDXL::GetCtidAndSegmentId(ULONG *ctid, ULONG *segment_id)
{
	...
	*ctid = CTranslatorUtils::GetColId(m_query_level, m_query->resultRelation,
									   SelfItemPointerAttributeNumber, mdid, m_var_to_colid_map);
	mdid->Release();

	// segmentid column id
	mdid = CTranslatorUtils::GetSystemColType(m_mp, GpSegmentIdAttributeNumber);
	*segment_id = CTranslatorUtils::GetColId(m_query_level, m_query->resultRelation,
											 GpSegmentIdAttributeNumber, mdid, m_var_to_colid_map);
	mdid->Release();
}
...
CTranslatorQueryToDXL::TranslateDeleteQueryToDXL()
{
	...
	for (ULONG ul = 0; ul < num_of_non_sys_cols; ul++)
	{
		...
		ULONG colid =
			CTranslatorUtils::GetColId(m_query_level, m_query->resultRelation, mdcol->AttrNum(),
...
CTranslatorQueryToDXL::TranslateUpdateQueryToDXL()
{
	...
	for (ULONG ul = 0; ul < num_of_non_sys_cols; ul++)
	{
		...
		ULONG colid = CTranslatorUtils::GetColId(m_query_level, m_query->resultRelation, attno,
												 mdcol->MdidType(), m_var_to_colid_map);
	...
	CDXLLogicalUpdate *pdxlopupdate = GPOS_NEW(m_mp)
		CDXLLogicalUpdate(m_mp, table_descr, ctid_colid, segmentid_colid, delete_colid_array,
...
) const
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		if (is_lead_func || is_lag)
		{
			...
			CDXLWindowSpec *pdxlwsNew = GPOS_NEW(m_mp)
				CDXLWindowSpec(m_mp, window_spec_dxlnode->GetPartitionByColIdArray(), mdname,
												...
												CDXLNode *project_list_dxlnode_node)
{
	...
	ForEach(lc, window_clause)
	{
		...
		ULongPtrArray *part_columns =
			TranslatePartColumns(wc->partitionClause, sort_col_attno_to_colid_mapping);
		...
		if (0 < gpdb::ListLength(wc->orderClause))
		{
			...
			CDXLNodeArray *dxl_sort_cols =
				TranslateSortColumsToDXL(wc->orderClause, sort_col_attno_to_colid_mapping);
		...
		window_frame = m_scalar_translator->TranslateWindowFrameToDXL(
			wc->frameOptions, wc->startOffset, wc->endOffset, m_var_to_colid_map,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateWindowToDXL(CDXLNode *child_dxlnode, List *target_list,
		...
		if (!target_entry->resjunk)
		{
			...
			else if (CTranslatorUtils::IsReferencedInWindowSpec(target_entry, window_clause))
			{
				...
				CMDName *mdname_alias = GPOS_NEW(m_mp)
					CMDName(m_mp, CDXLScalarProjElem::Cast(project_elem_dxlnode->GetOperator())
									  ->GetMdNameAlias()
									  ->GetMDName());
												...
												IntToUlongMap *col_attno_colid_mapping) const
{
	...
	ForEach(lc, sort_clause)
	{
		...
		CDXLScalarSortCol *sc_sort_col_dxlop = GPOS_NEW(m_mp)
			CDXLScalarSortCol(m_mp, colid, op_mdid, GPOS_NEW(m_mp) CWStringConst(str->GetBuffer()),
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateLimitToDXLGroupBy(List *sort_clause, Node *limit_count,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::CreateSimpleGroupBy(List *target_list, List *group_clause,
										   CBitSet *grpby_cols_bitset, BOOL has_aggs,
										   BOOL has_grouping_sets, CDXLNode *child_dxlnode,
										   IntToUlongMap *sort_grpref_to_colid_mapping,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateGroupingSets(FromExpr *from_expr, List *target_list,
											 List *group_clause, BOOL has_aggs,
											 IntToUlongMap *sort_grpref_to_colid_mapping,
											 IntToUlongMap *output_attno_to_colid_mapping)
{
	...
	if (NULL == group_clause)
	{
		...
		CDXLNode *select_project_join_dxlnode =
			TranslateSelectProjectJoinToDXL(target_list, from_expr, sort_grpref_to_colid_mapping,
		...
		CDXLNode *result_dxlnode =
			CreateSimpleGroupBy(target_list, group_clause, bitset, has_aggs,
...
)
{
	...
	for (ULONG ul = 0; ul < num_of_grouping_sets; ul++)
	{
		...
		IntToUlongMap *spj_consumer_output_attno_to_colid_mapping =
			RemapColIds(m_mp, spj_output_attno_to_colid_mapping, op_colid_array_cte_producer,
						colid_array_cte_consumer);
		IntToUlongMap *phmiulSortgrouprefColIdConsumer =
			RemapColIds(m_mp, sort_groupref_to_colid_producer_mapping, op_colid_array_cte_producer,
						colid_array_cte_consumer);
										   ...
										   IntToUlongMap *output_attno_to_colid_mapping)
{
	...
	ULongPtrArray *output_colids =
		CTranslatorUtils::GenerateColIds(m_mp, target_list, mdid_array_leftchild, leftchild_array,
	...
	CDXLNode *dxlnode = CreateDXLSetOpFromColumns(setop_type, target_list, output_colids,
												  input_colids, children_dxlnodes,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateSetOpChild(Node *child_node, ULongPtrArray *colids,
	...
	if (IsA(child_node, RangeTblRef))
	{
		...
		if (RTE_SUBQUERY == rte->rtekind)
		{
			...
			CTranslatorQueryToDXL query_to_dxl_translator(
				m_context, m_md_accessor, m_var_to_colid_map, query_derived_tbl, m_query_level + 1,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateValueScanRTEToDXL(const RangeTblEntry *rte, ULONG rt_index,
												  ULONG current_query_level)
{
	...
	ForEach(lc_tuple, tuples_list)
	{
		...
		ForEach(lc_column, tuple_list)
		{
			...
			else
			{
				...
				if (0 == tuple_pos)
				{
					...
					CDXLColDescr *dxl_col_descr = GPOS_NEW(m_mp)
						CDXLColDescr(m_mp, mdname, colid, col_pos_idx + 1 /* attno */,
		...
		dxlnodes->Append(TranslateColumnValuesToDXL(dxl_datum_array, dxl_column_descriptors,
...
)
{
	...
	if (!IsA(funcexpr, FuncExpr))
	{
		...
		CDXLNode *project_elem_dxlnode = TranslateExprToDXLProject(
			(Expr *) funcexpr, rte->eref->aliasname, true /* insist_new_colids */);
		project_list_dxlnode->AddChild(project_elem_dxlnode);
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateCTEToDXL(const RangeTblEntry *rte, ULONG rt_index,
										 ULONG current_query_level)
{
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateDerivedTablesToDXL(const RangeTblEntry *rte, ULONG rt_index,
												   ULONG current_query_level)
{
	...
	CTranslatorQueryToDXL query_to_dxl_translator(m_context, m_md_accessor, m_var_to_colid_map,
												  query_derived_tbl, m_query_level + 1,
												  IsDMLQuery(), m_query_level_to_cte_map);
	...
	m_var_to_colid_map->LoadDerivedTblColumns(current_query_level, rt_index,
	...
	ForEach(lc, target_list)
	{
		...
		else if (is_expand_aggref_expr && IsA(target_entry->expr, Aggref))
		{
			vars_list = gpdb::ListConcat(
				vars_list, gpdb::ExtractNodesExpression((Node *) target_entry->expr, T_Var,
...
) const
{
	...
	return GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalProject(m_mp),
...
CTranslatorQueryToDXL::CreateDXLProjectGroupingFuncs(
	...
	IntToUlongMap *output_attno_to_colid_mapping, UlongToUlongMap *grpcol_index_to_colid_mapping,
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::StoreAttnoColIdMapping(IntToUlongMap *attno_to_colid_mapping, INT attno,
											  ULONG colid) const
{
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateExprToDXLProject(Expr *expr, const CHAR *alias_name,
...
CTranslatorQueryToDXL::ConstructCTEProducerList(List *cte_list, ULONG cte_query_level)
{
	...
	ForEach(lc, cte_list)
	{
		...
		CTranslatorQueryToDXL query_to_dxl_translator(m_context, m_md_accessor, m_var_to_colid_map,
													  cte_query, cte_query_level + 1, IsDMLQuery(),
		...
		CDXLLogicalCTEProducer *lg_cte_prod_dxlop = GPOS_NEW(m_mp)
			CDXLLogicalCTEProducer(m_mp, m_context->m_cte_id_counter->next_id(), colid_array);
		CDXLNode *cte_producer_dxlnode =
