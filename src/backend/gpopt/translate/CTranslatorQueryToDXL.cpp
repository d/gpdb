...
CTranslatorQueryToDXL::QueryToDXLInstance(CMemoryPool *mp, CMDAccessor *md_accessor, Query *query)
{
...
CTranslatorQueryToDXL::CheckSirvFuncsWithoutFromClause(Query *query)
{
	...
	if ((NULL != query->jointree && 0 < gpdb::ListLength(query->jointree->fromlist)) || NIL == query->targetList)
	{
		return;
	}
...
CTranslatorQueryToDXL::CheckSupportedCmdType(Query *query)
{
	...
	if (CMD_SELECT == query->commandType)
	{
		...
		{
			GPOS_RAISE(gpdxl::ExmaDXL, gpdxl::ExmiQuery2DXLUnsupportedFeature,
					   GPOS_WSZ_LIT("COPY. Copy select statement to file on segment is not supported with GPORCA"));
		}
...
CTranslatorQueryToDXL::TranslateSelectQueryToDXL()
{
	...
	{
		child_dxlnode =
			TranslateGroupingSets(m_query->jointree, m_query->targetList, m_query->groupClause, m_query->hasAggs,
								  sort_group_attno_to_colid_mapping, output_attno_to_colid_mapping);
	}
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateSelectProjectJoinToDXL(List *target_list, FromExpr *from_expr,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateSelectProjectJoinForGrpSetsToDXL(List *target_list, FromExpr *from_expr,
																 IntToUlongMap *sort_group_attno_to_colid_mapping,
...
CTranslatorQueryToDXL::TranslateCTASToDXL()
{
	...
	for (ULONG ul = 0; ul < num_columns; ul++)
	{
		...
		CDXLColDescr *dxl_col_descr =
			GPOS_NEW(m_mp) CDXLColDescr(m_mp, md_colname, m_context->m_colid_counter->next_id(), resno /* attno */,
	...
	{
		GpdbEreport(ERRCODE_SUCCESSFUL_COMPLETION, NOTICE,
					"Table doesn't have 'DISTRIBUTED BY' clause. Creating a NULL policy entry.", NULL);
	}
	...
	CDXLLogicalCTAS *ctas_dxlop = GPOS_NEW(m_mp) CDXLLogicalCTAS(
		...
		rel_distr_policy, distribution_colids, distr_opfamilies, fTempTable, has_oids, rel_storage_type, source_array,
		var_typmods);
...
CTranslatorQueryToDXL::GetCtidAndSegmentId(ULONG *ctid, ULONG *segment_id)
{
	...
	*segment_id = CTranslatorUtils::GetColId(m_query_level, m_query->resultRelation, GpSegmentIdAttributeNumber, mdid,
											 m_var_to_colid_map);
	mdid->Release();
}
...
CTranslatorQueryToDXL::TranslateUpdateQueryToDXL()
{
	...
	CDXLLogicalUpdate *pdxlopupdate =
		GPOS_NEW(m_mp) CDXLLogicalUpdate(m_mp, table_descr, ctid_colid, segmentid_colid, delete_colid_array,
...
CTranslatorQueryToDXL::CreateWindowFramForLeadLag(BOOL is_lead_func, CDXLNode *dxl_offset) const
{
	...
	return GPOS_NEW(m_mp) CDXLWindowFrame(m_mp,
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::UpdateLeadLagWinSpecPos(CDXLNode *project_list_dxlnode,	// project list holding WinRef nodes
...
//---------------------------------------------------------------------------
CDXLWindowSpecArray *
CTranslatorQueryToDXL::TranslateWindowSpecToDXL(List *window_clause, IntToUlongMap *sort_col_attno_to_colid_mapping,
												CDXLNode *project_list_dxlnode_node)
{
	...
	ForEach(lc, window_clause)
	{
		...
		window_frame = m_scalar_translator->TranslateWindowFrameToDXL(wc->frameOptions, wc->startOffset, wc->endOffset,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateWindowToDXL(CDXLNode *child_dxlnode, List *target_list, List *window_clause,
		...
		if (!target_entry->resjunk)
		{
			...
			else if (CTranslatorUtils::IsReferencedInWindowSpec(target_entry, window_clause))
			{
				...
				CMDName *mdname_alias = GPOS_NEW(m_mp) CMDName(
					m_mp, CDXLScalarProjElem::Cast(project_elem_dxlnode->GetOperator())->GetMdNameAlias()->GetMDName());
				CDXLNode *new_project_elem_dxlnode =
					GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarProjElem(m_mp, colid, mdname_alias));
				CDXLNode *project_elem_new_child_dxlnode = GPOS_NEW(m_mp)
					CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarIdent(
									   m_mp, GPOS_NEW(m_mp) CDXLColRef(
												 m_mp, GPOS_NEW(m_mp) CMDName(m_mp, mdname_alias->GetMDName()), colid,
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorQueryToDXL::TranslatePartColumns(List *partition_by_clause, IntToUlongMap *col_attno_colid_mapping) const
{
...
//---------------------------------------------------------------------------
CDXLNodeArray *
CTranslatorQueryToDXL::TranslateSortColumsToDXL(List *sort_clause, IntToUlongMap *col_attno_colid_mapping) const
{
	...
	ForEach(lc, sort_clause)
	{
		...
		const ULONG colid =
			CTranslatorUtils::GetColId((INT) sort_group_clause->tleSortGroupRef, col_attno_colid_mapping);
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateLimitToDXLGroupBy(List *sort_clause, Node *limit_count, Node *limit_offset_node,
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::AddSortingGroupingColumn(TargetEntry *target_entry, IntToUlongMap *sort_grpref_to_colid_mapping,
												ULONG colid) const
{
	if (0 < target_entry->ressortgroupref)
	{
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::CreateSimpleGroupBy(List *target_list, List *group_clause, CBitSet *grpby_cols_bitset,
										   ...
										   IntToUlongMap *output_attno_to_colid_mapping)
{
	...
	return GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalGroupBy(m_mp, grouping_cols),
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateGroupingSets(FromExpr *from_expr, List *target_list, List *group_clause, BOOL has_aggs,
											 IntToUlongMap *sort_grpref_to_colid_mapping,
											 IntToUlongMap *output_attno_to_colid_mapping)
{
...
)
{
	...
	CDXLNode *select_project_join_dxlnode =
		TranslateSelectProjectJoinForGrpSetsToDXL(target_list, from_expr, sort_groupref_to_colid_producer_mapping,
	...
	for (ULONG ul = 0; ul < num_of_grouping_sets; ul++)
	{
		...
		IntToUlongMap *phmiulSortgrouprefColIdConsumer = RemapColIds(
			m_mp, sort_groupref_to_colid_producer_mapping, op_colid_array_cte_producer, colid_array_cte_consumer);

		// construct a CTE consumer
		CDXLNode *cte_consumer_dxlnode = GPOS_NEW(m_mp)
			CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLLogicalCTEConsumer(m_mp, cte_id, colid_array_cte_consumer));

		IntToUlongMap *groupby_attno_to_colid_mapping = GPOS_NEW(m_mp) IntToUlongMap(m_mp);
		CDXLNode *groupby_dxlnode =
			CreateSimpleGroupBy(target_list, group_clause, grouping_set_bitset, has_aggs,
		...
		CDXLNode *project_dxlnode = CreateDXLProjectNullsForGroupingSets(
			target_list, groupby_dxlnode, grouping_set_bitset, phmiulSortgrouprefColIdConsumer,
			groupby_attno_to_colid_mapping, grpcol_index_to_colid_mapping);

		ULongPtrArray *colids_outer_array =
			CTranslatorUtils::GetOutputColIdsArray(m_mp, target_list, groupby_attno_to_colid_mapping);
...
CTranslatorQueryToDXL::DXLDummyConstTableGet() const
{
	...
	CDXLColDescr *dxl_col_descr =
		GPOS_NEW(m_mp) CDXLColDescr(m_mp, mdname, m_context->m_colid_counter->next_id(), 1 /* attno */,
									GPOS_NEW(m_mp) CMDIdGPDB(mdid->Oid()), default_type_modifier, false /* is_dropped */
		);
	dxl_col_descr_array->Append(dxl_col_descr);
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateSetOpToDXL(Node *setop_node, List *target_list,
										   IntToUlongMap *output_attno_to_colid_mapping)
{
	...
	CDXLNode *dxlnode = CreateDXLSetOpFromColumns(setop_type, target_list, output_colids, input_colids,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::CreateDXLSetOpFromColumns(EdxlSetOpType setop_type, List *output_target_list,
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateSetOpChild(Node *child_node, ULongPtrArray *colids, IMdIdArray *input_col_mdids,
										   List *target_list)
{
	...
	if (IsA(child_node, RangeTblRef))
	{
		...
		if (RTE_SUBQUERY == rte->rtekind)
		{
			...
			CTranslatorQueryToDXL query_to_dxl_translator(m_context, m_md_accessor, m_var_to_colid_map,
														  query_derived_tbl, m_query_level + 1, IsDMLQuery(),
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateRTEToDXLLogicalGet(const RangeTblEntry *rte, ULONG rt_index,
...
)
{
	...
	CDXLTableDescr *dxl_table_descr = CTranslatorUtils::GetTableDescr(m_mp, m_md_accessor, m_context->m_colid_counter,
	...
	if (rte->rtekind == RTE_RELATION)
	{
		...
		if (contains_nondefault_hashops)
		{
			...
			GPOS_RAISE(gpdxl::ExmaMD, gpdxl::ExmiMDObjUnsupported,
					   GPOS_WSZ_LIT("Query contains relations with non-default hash opclasses"));
		}
		if (contains_default_hashops && m_context->m_distribution_hashops != DistrUseDefaultHashOps)
		{
			if (m_context->m_distribution_hashops != DistrHashOpsNotDeterminedYet)
				GPOS_RAISE(gpdxl::ExmaMD, gpdxl::ExmiMDObjUnsupported,
						   GPOS_WSZ_LIT("Query contains relations with a mix of default and legacy hash opclasses"));
			m_context->m_distribution_hashops = DistrUseDefaultHashOps;
		}
		if (contains_legacy_hashops && m_context->m_distribution_hashops != DistrUseLegacyHashOps)
		{
			if (m_context->m_distribution_hashops != DistrHashOpsNotDeterminedYet)
				GPOS_RAISE(gpdxl::ExmaMD, gpdxl::ExmiMDObjUnsupported,
						   GPOS_WSZ_LIT("Query contains relations with a mix of default and legacy hash opclasses"));
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateValueScanRTEToDXL(const RangeTblEntry *rte, ULONG rt_index, ULONG current_query_level)
{
	...
	ForEach(lc_tuple, tuples_list)
	{
		...
		ForEach(lc_column, tuple_list)
		{
			...
			if (IsA(expr, Const))
			{
				...
				CDXLColDescr *dxl_col_descr = GPOS_NEW(m_mp) CDXLColDescr(
					m_mp, mdname, colid, col_pos_idx + 1 /* attno */, GPOS_NEW(m_mp) CMDIdGPDB(const_expr->consttype),
			...
			else
			{
				...
				if (0 == tuple_pos)
				{
					...
					CDXLColDescr *dxl_col_descr =
						GPOS_NEW(m_mp) CDXLColDescr(m_mp, mdname, colid, col_pos_idx + 1 /* attno */,
		...
		dxlnodes->Append(
			TranslateColumnValuesToDXL(dxl_datum_array, dxl_column_descriptors, project_elem_dxlnode_array));
		if (fAllConstant)
		{
												  ...
												  CDXLNodeArray *project_elem_dxlnode_array) const
{
	...
	CDXLNode *project_list_dxlnode =
		GPOS_NEW(m_mp) CDXLNode(m_mp, GPOS_NEW(m_mp) CDXLScalarProjList(m_mp), project_elem_dxlnode_array);
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateTVFToDXL(const RangeTblEntry *rte, ULONG rt_index,
										 ULONG	//current_query_level
)
{
	/*
	...
	if (!IsA(funcexpr, FuncExpr))
	{
		...
		CDXLNode *project_elem_dxlnode =
			TranslateExprToDXLProject((Expr *) funcexpr, rte->eref->aliasname, true /* insist_new_colids */);
		project_list_dxlnode->AddChild(project_elem_dxlnode);
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateCTEToDXL(const RangeTblEntry *rte, ULONG rt_index, ULONG current_query_level)
{
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateDerivedTablesToDXL(const RangeTblEntry *rte, ULONG rt_index, ULONG current_query_level)
{
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateTargetListToDXLProject(List *target_list, CDXLNode *child_dxlnode,
													   IntToUlongMap *sort_grpref_to_colid_mapping,
													   ...
													   BOOL is_expand_aggref_expr)
{
	...
	ForEach(lc, target_list)
	{
		...
		{
			vars_list = gpdb::ListConcat(vars_list, gpdb::ExtractNodesExpression((Node *) target_entry->expr, T_Var,
...
) const
{
	...
	ForEach(lc, target_list)
	{
		...
		GPOS_ASSERT_IMP(0 == sort_grouping_col_mapping,
						NULL != sort_grouping_col_mapping->Find(&sort_group_ref) && "Grouping column with no mapping");
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::CreateDXLProjectGroupingFuncs(List *target_list, CDXLNode *child_dxlnode, CBitSet *bitset,
													 IntToUlongMap *output_attno_to_colid_mapping,
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::StoreAttnoColIdMapping(IntToUlongMap *attno_to_colid_mapping, INT attno, ULONG colid) const
{
...
//---------------------------------------------------------------------------
CDXLNodeArray *
CTranslatorQueryToDXL::CreateDXLOutputCols(List *target_list, IntToUlongMap *attno_to_colid_mapping) const
{
	GPOS_ASSERT(NULL != target_list);
	GPOS_ASSERT(NULL != attno_to_colid_mapping);
...
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateExprToDXLProject(Expr *expr, const CHAR *alias_name, BOOL insist_new_colids)
{
...
CTranslatorQueryToDXL::ConstructCTEProducerList(List *cte_list, ULONG cte_query_level)
{
	...
	ForEach(lc, cte_list)
	{
		...
		Query *cte_query =
			CQueryMutators::NormalizeQuery(m_mp, m_md_accessor, (Query *) cte->ctequery, cte_query_level + 1);
...
//---------------------------------------------------------------------------
void
CTranslatorQueryToDXL::ConstructCTEAnchors(CDXLNodeArray *dxlnodes, CDXLNode **dxl_cte_anchor_top,
...
//---------------------------------------------------------------------------
ULongPtrArray *
CTranslatorQueryToDXL::ExtractColIds(CMemoryPool *mp, IntToUlongMap *attno_to_colid_mapping) const
{
...
//---------------------------------------------------------------------------
IntToUlongMap *
CTranslatorQueryToDXL::RemapColIds(CMemoryPool *mp, IntToUlongMap *attno_to_colid_mapping,
								   ULongPtrArray *from_list_colids, ULongPtrArray *to_list_colids) const
{
	GPOS_ASSERT(NULL != attno_to_colid_mapping);
