...
//---------------------------------------------------------------------------
Query *
CQueryMutators::NormalizeGroupByProjList(CMemoryPool *mp, CMDAccessor *md_accessor, const Query *query)
{
...
CQueryMutators::RunIncrLevelsUpMutator(Node *node, SContextIncLevelsupMutator *context)
{
	...
	if (IsA(node, Query))
	{
		...
		Query *query =
			query_tree_mutator((Query *) node, (MutatorWalkerFn) CQueryMutators::RunIncrLevelsUpMutator, context,
							   0  // flags
			);
		context->m_current_query_level--;
...
CQueryMutators::RunGroupingColMutator(Node *node, SContextGrpbyPlMutator *context)
{
	...
	if (IsA(node, SubLink))
	{
		...
		new_sublink->subselect = gpdb::MutateQueryOrExpressionTree(
			old_sublink->subselect, (MutatorWalkerFn) CQueryMutators::RunGroupingColMutator, context,
			0  // flags -- mutate into cte-lists
		);
	...
	{
		Query *query =
			gpdb::MutateQueryTree((Query *) node, (MutatorWalkerFn) CQueryMutators::RunGroupingColMutator, context,
...
//---------------------------------------------------------------------------
Node *
CQueryMutators::FixGroupingCols(Node *node, TargetEntry *orginal_target_entry, SContextGrpbyPlMutator *context)
{
...
//---------------------------------------------------------------------------
TargetEntry *
CQueryMutators::GetTargetEntryForAggExpr(CMemoryPool *mp, CMDAccessor *md_accessor, Node *node, ULONG attno)
{
...
CQueryMutators::RunExtractAggregatesMutator(Node *node, SContextGrpbyPlMutator *context)
{
	...
	if (IsA(node, CommonTableExpr))
	{
		...
		cte->ctequery = gpdb::MutateQueryOrExpressionTree(cte->ctequery, (MutatorWalkerFn) RunExtractAggregatesMutator,
														  (void *) context,
														  0	 // mutate into cte-lists
		);
...
CQueryMutators::FindNodeInGroupByTargetList(Node *node, SContextGrpbyPlMutator *context)
{
	...
	if (NULL != found_tle)
	{
		...
		Var *new_var = gpdb::MakeVar(1 /* varno */, found_tle->resno, gpdb::ExprType((Node *) found_tle->expr),
	...
	if (!rte->subquery->hasAggs && NIL == rte->subquery->groupClause)
	{
		...
		ForEach(lc, rte->subquery->targetList)
		{
			...
			new_subquery->targetList = gpdb::LAppend(
				new_subquery->targetList, (TargetEntry *) gpdb::CopyObject(const_cast<TargetEntry *>(target_entry)));
		}
...
//---------------------------------------------------------------------------
Query *
CQueryMutators::NormalizeQuery(CMemoryPool *mp, CMDAccessor *md_accessor, const Query *query, ULONG query_level)
{
...
CQueryMutators::MakeTopLevelTargetEntry(TargetEntry *old_target_entry, ULONG attno)
{
	...
	TargetEntry *new_target_entry = gpdb::MakeTargetEntry((Expr *) new_var, (AttrNumber) attno,
														  old_target_entry->resname, old_target_entry->resjunk);
...
//---------------------------------------------------------------------------
Query *
CQueryMutators::NormalizeWindowProjList(CMemoryPool *mp, CMDAccessor *md_accessor, const Query *original_query)
{
	Query *query_copy = (Query *) gpdb::CopyObject(const_cast<Query *>(original_query));
