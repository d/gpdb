...
CQueryMutators::NeedsProjListNormalization(const Query *query)
{
	...
	ForEach(lc, query->targetList)
	{
		...
		if (!IsA(target_entry->expr, Aggref) &&
			!CTranslatorUtils::IsGroupingColumn((Node *) target_entry->expr, query->groupClause,
												query->targetList))
		{
			return true;
		}
...
CQueryMutators::ShouldFallback(Node *node, SContextTLWalker *context)
{
	...
	if (NULL != entry &&
		CTranslatorUtils::IsGroupingColumn((Node *) entry->expr, context->m_group_clause,
...
//---------------------------------------------------------------------------
Query *
CQueryMutators::NormalizeGroupByProjList(CMemoryPool *mp, CMDAccessor *md_accessor,
										 const Query *query)
{
	...
	Query *new_query = ConvertToDerivedTable(query_copy, false /*should_fix_target_list*/,
	...
	ForEach(lc, target_list_copy)
	{
		...
		{
			target_entry->expr =
				(Expr *) RunExtractAggregatesMutator((Node *) target_entry->expr, &context);
...
CQueryMutators::RunIncrLevelsUpMutator(Node *node, SContextIncLevelsupMutator *context)
{
	...
	if (IsA(node, Query))
	{
		...
		Query *query = query_tree_mutator(
			(Query *) node, (MutatorWalkerFn) CQueryMutators::RunIncrLevelsUpMutator, context,
			0  // flags
		);
		context->m_current_query_level--;
	...
	return expression_tree_mutator(node, (MutatorWalkerFn) CQueryMutators::RunIncrLevelsUpMutator,
								   context);
}
...
//---------------------------------------------------------------------------
BOOL
CQueryMutators::RunFixCTELevelsUpWalker(Node *node, SContextIncLevelsupMutator *context)
{
	if (NULL == node)
	{
		return false;
	...
	if (IsA(node, Query))
	{
		...
		BOOL result = query_tree_walker(
			(Query *) node, (ExprWalkerFn) CQueryMutators::RunFixCTELevelsUpWalker, context,
	...
	if (IsA(node, TargetEntry) && !context->m_should_fix_top_level_target_list &&
	...
	return expression_tree_walker(node, (ExprWalkerFn) CQueryMutators::RunFixCTELevelsUpWalker,
								  context);
}
...
CQueryMutators::RunGroupingColMutator(Node *node, SContextGrpbyPlMutator *context)
{
	...
	if (IsA(node, SubLink))
	{
		...
		new_sublink->testexpr = gpdb::MutateQueryOrExpressionTree(
			old_sublink->testexpr, (MutatorWalkerFn) CQueryMutators::RunGroupingColMutator,
			(void *) context,
			0  // flags -- mutate into cte-lists
		);
		context->m_current_query_level++;
		...
		new_sublink->subselect = gpdb::MutateQueryOrExpressionTree(
			old_sublink->subselect, (MutatorWalkerFn) CQueryMutators::RunGroupingColMutator,
			context,
			0  // flags -- mutate into cte-lists
		);
	...
	if (IsA(node, CommonTableExpr))
	{
		...
		cte->ctequery = gpdb::MutateQueryOrExpressionTree(
			cte->ctequery, (MutatorWalkerFn) CQueryMutators::RunGroupingColMutator,
			(void *) context,
			0  // flags --- mutate into cte-lists
		);
	...
	if (IsA(node, Query))
	{
		Query *query = gpdb::MutateQueryTree(
			(Query *) node, (MutatorWalkerFn) CQueryMutators::RunGroupingColMutator, context,
	...
	return gpdb::MutateExpressionTree(node, (MutatorWalkerFn) CQueryMutators::RunGroupingColMutator,
									  context);
}
...
//---------------------------------------------------------------------------
TargetEntry *
CQueryMutators::GetTargetEntryForAggExpr(CMemoryPool *mp, CMDAccessor *md_accessor, Node *node,
										 ULONG attno)
{
...
Node *
CQueryMutators::RunExtractAggregatesMutator(Node *node, SContextGrpbyPlMutator *context)
{
	if (NULL == node)
	{
		return NULL;
	...
	if (IsA(node, CommonTableExpr))
	{
		...
		cte->ctequery = gpdb::MutateQueryOrExpressionTree(
			cte->ctequery, (MutatorWalkerFn) RunExtractAggregatesMutator, (void *) context,
			0  // mutate into cte-lists
		);
	...
	if (IsA(node, SubLink))
	{
		...
		new_sublink->subselect = gpdb::MutateQueryOrExpressionTree(
			old_sublink->subselect, (MutatorWalkerFn) RunExtractAggregatesMutator, (void *) context,
			0  // mutate into cte-lists
		);
...
// Create a new entry in the derived table and return its corresponding var
Var *
CQueryMutators::MakeVarInDerivedTable(Node *node, SContextGrpbyPlMutator *context)
{
	GPOS_ASSERT(NULL != node);
	...
	Var *new_var = gpdb::MakeVar(1 /* varno */, attno, gpdb::ExprType((Node *) node),
...
Node *
CQueryMutators::FindNodeInGroupByTargetList(Node *node, SContextGrpbyPlMutator *context)
{
	GPOS_ASSERT(NULL != node);
	GPOS_ASSERT(NULL != context);
...
//---------------------------------------------------------------------------
Query *
CQueryMutators::NormalizeHaving(CMemoryPool *mp, CMDAccessor *md_accessor, const Query *query)
{
	Query *query_copy = (Query *) gpdb::CopyObject(const_cast<Query *>(query));
	...
	Query *new_query = ConvertToDerivedTable(query_copy, true /*should_fix_target_list*/,
...
//---------------------------------------------------------------------------
Query *
CQueryMutators::NormalizeQuery(CMemoryPool *mp, CMDAccessor *md_accessor, const Query *query,
							   ULONG query_level)
{
	...
	Query *pqueryWindowPlNormalized =
		CQueryMutators::NormalizeWindowProjList(mp, md_accessor, pqueryEliminateDistinct);
...
{
	Var *new_var =
		gpdb::MakeVar(1, (AttrNumber) attno, gpdb::ExprType((Node *) old_target_entry->expr),
...
//---------------------------------------------------------------------------
Query *
CQueryMutators::ConvertToDerivedTable(const Query *original_query, BOOL should_fix_target_list,
									  BOOL should_fix_having_qual)
{
	...
	Query *lower_query;
	{
		...
		lower_query =
			gpdb::MutateQueryTree(query_copy, (MutatorWalkerFn) RunIncrLevelsUpMutator, &context1,
								  0	 // flags
			);
	}
	...
	// derived table.
	{
		...
		(void) gpdb::WalkQueryOrExpressionTree((Node *) lower_query,
											   (ExprWalkerFn) RunFixCTELevelsUpWalker, &context2,
...
CQueryMutators::EliminateDistinctClause(const Query *query)
{
	...
	Query *new_query = ConvertToDerivedTable(query, true /*should_fix_target_list*/,
											 true /*should_fix_having_qual*/);
	...
	ForEach(lc, target_entries)
	{
		...
		if (!target_entry->resjunk)
		{
			...
			Var *new_var =
				gpdb::MakeVar(1, target_entry->resno, gpdb::ExprType((Node *) target_entry->expr),
		...
		if (0 < target_entry->ressortgroupref &&
			!CTranslatorUtils::IsGroupingColumn(target_entry, derived_table_query->groupClause) &&
			!CTranslatorUtils::IsReferencedInWindowSpec(target_entry,
...
//---------------------------------------------------------------------------
Query *
CQueryMutators::NormalizeWindowProjList(CMemoryPool *mp, CMDAccessor *md_accessor,
										const Query *original_query)
{
	...
	Query *upper_query = ConvertToDerivedTable(query_copy, false /*should_fix_target_list*/,
											   true /*should_fix_having_qual*/);
	gpdb::GPDBFree(query_copy);

	GPOS_ASSERT(1 == gpdb::ListLength(upper_query->rtable));
	Query *lower_query =
		(Query *) ((RangeTblEntry *) gpdb::ListNth(upper_query->rtable, 0))->subquery;
	...
	ForEach(lc, target_entries)
	{
		...
		if (CTranslatorUtils::IsReferencedInWindowSpec(target_entry, original_query->windowClause))
		{
			...
			SContextIncLevelsupMutator level_context(0,
			...
			projlist_context.m_lower_table_tlist =
				gpdb::LAppend(projlist_context.m_lower_table_tlist, lower_target_entry);

			BOOL is_sorting_col =
				CTranslatorUtils::IsSortingColumn(target_entry, original_query->sortClause);
			...
			if (!target_entry->resjunk || is_sorting_col)
			{
				...
				Var *new_var = gpdb::MakeVar(1, lower_target_entry->resno,
		...
		else
		{
			...
			TargetEntry *upper_target_entry = (TargetEntry *) gpdb::MutateExpressionTree(
				(Node *) target_entry, (MutatorWalkerFn) RunWindowProjListMutator,
				&projlist_context);
...
CQueryMutators::RunWindowProjListMutator(Node *node, SContextGrpbyPlMutator *context)
{
	...
	if (IsA(node, WindowFunc) && 0 == context->m_current_query_level)
	{
		...
		Var *new_var =
			gpdb::MakeVar(1,  // derived query which is now the only table in FROM expression
	...
	if (IsA(node, Var) && ((Var *) node)->varlevelsup == context->m_current_query_level)
	{
		...
		var->varlevelsup = 0;
		TargetEntry *found_tle =
			gpdb::FindFirstMatchingMemberInTargetList((Node *) var, context->m_lower_table_tlist);
	...
	if (IsA(node, Query))
	{
		...
		Query *result = query_tree_mutator((Query *) node,
										   (MutatorWalkerFn) RunWindowProjListMutator, context, 0);
		context->m_current_query_level--;

		return (Node *) result;
	}

	return expression_tree_mutator(node, (MutatorWalkerFn) CQueryMutators::RunWindowProjListMutator,
								   context);
}
