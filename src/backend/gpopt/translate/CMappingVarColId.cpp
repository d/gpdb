...
//---------------------------------------------------------------------------
void
CMappingVarColId::Insert(ULONG query_level, ULONG var_no, INT attrnum, ULONG colid,
...
//---------------------------------------------------------------------------
void
CMappingVarColId::Load(ULONG query_level, ULONG RTE_index, CIdGenerator *id_generator,
					   List *col_names)
{
	...
	ForEach(col_name, col_names)
	{
		...
		this->Insert(query_level, RTE_index, INT(i + 1), id_generator->next_id(),
...
//---------------------------------------------------------------------------
void
CMappingVarColId::LoadDerivedTblColumns(ULONG query_level, ULONG RTE_index,
...
CMappingVarColId::LoadCTEColumns(ULONG query_level, ULONG RTE_index,
								 const ULongPtrArray *CTE_columns, List *target_list)
{
...
CMappingVarColId::CopyMapColId(ULONG query_level) const
{
	...
	while (col_map_iterator.Advance())
	{
		...
		if (gpdb_att_info->GetQueryLevel() <= query_level)
		{
			...
			// insert into hashmap
			BOOL result GPOS_ASSERTS_ONLY = var_colid_mapping->m_gpdb_att_opt_col_mapping->Insert(
