...
Oid
gpdb::ResolveAggregateTransType(Oid aggfnoid, Oid aggtranstype, Oid *inputTypes, int numArguments)
{
	GP_WRAP_START;
	{
...
bool
gpdb::GetCastFunc(Oid src_oid, Oid dest_oid, bool *is_binary_coercible, Oid *cast_fn_oid,
...
gpdb::GetEqualityOp(Oid type_oid)
{
	GP_WRAP_START;
	{
		...
		get_sort_group_operators(type_oid, false, true, false, NULL, &eq_opr, NULL, NULL);
...
gpdb::MakeIntConst(int32 intValue)
{
	GP_WRAP_START;
	{
		return (Node *) makeConst(INT4OID, -1, InvalidOid, sizeof(int32), Int32GetDatum(intValue),
								  false, true);
	}
	GP_WRAP_END;
}
...
void
gpdb::GpdbEreportImpl(int xerrcode, int severitylevel, const char *xerrmsg, const char *xerrhint,
...
void
gpdb::CdbEstimateRelationSize(RelOptInfo *relOptInfo, Relation rel, int32 *attr_widths,
...
ForeignScan *
gpdb::CreateForeignScanForExternalTable(Oid rel_oid, Index scanrelid, List *qual, List *targetlist)
{
	GP_WRAP_START;
	{
		return BuildForeignScanForExternalTable(rel_oid, scanrelid, qual, targetlist);
	}
	GP_WRAP_END;
	return NULL;
}
...
register_mdcache_invalidation_callbacks(void)
{
	...
	{
		CacheRegisterSyscacheCallback(metadata_caches[i], &mdsyscache_invalidation_counter_callback,
									  (Datum) 0);
	}
...
gpdb::GPDBAllocSetContextCreate()
{
	GP_WRAP_START;
	{
		...
		cxt = AllocSetContextCreate(OptimizerMemoryContext, "GPORCA memory pool",
