...
COptTasks::ConvertToPlanStmtFromDXL(CMemoryPool *mp, CMDAccessor *md_accessor,
									const Query *orig_query, const CDXLNode *dxlnode,
									bool can_set_tag, DistributionHashOpsKind distribution_hashops)
{
	...
	CContextDXLToPlStmt dxl_to_plan_stmt_ctxt(mp, &plan_id_generator, &motion_id_generator,
											  &param_id_generator, distribution_hashops);
...
COptTasks::CreateOptimizerConfig(CMemoryPool *mp, ICostModel *cost_model)
{
	...
	return GPOS_NEW(mp) COptimizerConfig(
		...
		GPOS_NEW(mp) CStatisticsConfig(mp, damping_factor_filter, damping_factor_join,
		...
		GPOS_NEW(mp) CHint(gpos::int_max /* optimizer_parts_to_force_sort_on_insert */,
						   ...
						   join_order_threshold, broadcast_threshold,
...
COptTasks::SetCostModelParams(ICostModel *cost_model)
{
	...
	if (optimizer_nestloop_factor > 1.0)
	{
		...
		cost_model->GetCostModelParams()->SetParam(cost_param->Id(), nlj_factor, nlj_factor - 0.5,
												   nlj_factor + 0.5);
	}
	...
	GPOS_TRY
	{
...

		{
			...
			query_to_dxl_translator =
				CTranslatorQueryToDXL::QueryToDXLInstance(mp, &mda, (Query *) opt_ctxt->m_query);
			...
			plan_dxl = COptimizer::PdxlnOptimize(mp, &mda, query_dxl, query_output_dxlnode_array,
			...
			if (opt_ctxt->m_should_serialize_plan_dxl)
			{
				...
				CDXLUtils::SerializePlan(mp, oss, plan_dxl,
			...
			if (opt_ctxt->m_should_generate_plan_stmt)
			{
				...
				opt_ctxt->m_plan_stmt = (PlannedStmt *) gpdb::CopyObject(ConvertToPlanStmtFromDXL(
					mp, &mda, opt_ctxt->m_query, plan_dxl, opt_ctxt->m_query->canSetTag,
...
COptTasks::PrintMissingStatsWarning(CMemoryPool *mp, CMDAccessor *md_accessor,
									IMdIdArray *col_stats, MdidHashSet *rel_stats)
{
	GPOS_ASSERT(NULL != md_accessor);
