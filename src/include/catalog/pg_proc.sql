...
CREATE FUNCTION hyperloglog_accum(anyelement) RETURNS hyperloglog_estimator LANGUAGE internal IMMUTABLE AS 'aggregate_dummy' WITH (OID=7164, proisagg="t", DESCRIPTION="Adds every data value to a hyperloglog counter and returns the counter");
 ...
 CREATE FUNCTION gp_lock_status_segments(OUT locktype text, OUT database oid, OUT relation oid, OUT page int4, OUT tuple int2, OUT virtualxid text, OUT transactionid xid, OUT classid oid, OUT objid oid, OUT objsubid int2, OUT virtualtransaction text, OUT pid int4, OUT mode text, OUT granted bool, OUT mppsessionid int4, OUT mppiswriter bool, OUT gp_segment_id int4) RETURNS SETOF pg_catalog.record LANGUAGE internal VOLATILE STRICT EXECUTE ON ALL SEGMENTS AS 'pg_lock_status' WITH (OID=3167, DESCRIPTION="view system lock information on all segments");

 CREATE FUNCTION gp_lock_status_master(OUT locktype text, OUT database oid, OUT relation oid, OUT page int4, OUT tuple int2, OUT virtualxid text, OUT transactionid xid, OUT classid oid, OUT objid oid, OUT objsubid int2, OUT virtualtransaction text, OUT pid int4, OUT mode text, OUT granted bool, OUT mppsessionid int4, OUT mppiswriter bool, OUT gp_segment_id int4) RETURNS SETOF pg_catalog.record LANGUAGE internal VOLATILE STRICT EXECUTE ON MASTER AS 'pg_lock_status' WITH (OID=3168, DESCRIPTION="view system lock information on master");
