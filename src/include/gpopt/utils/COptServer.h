...
private:
	...
	struct SConnectionDescriptor
	{
		...
		// ctor
		SConnectionDescriptor(CTask *task, CSocket *socket) : m_id((ULONG_PTR) task), m_task(task), m_socket(socket)
		{
		}
	};
	...
	static CQueryContext *RecvQuery(CMemoryPool *mp, CCommunicator *communicator, CMDAccessor *md_accessor);

	// extract query plan, serialize it and send it to client
	static void SendPlan(CMemoryPool *mp, CCommunicator *communicator, CMDAccessor *md_accessor,
						 CQueryContext *query_ctxt, CExpression *plan_expr);
