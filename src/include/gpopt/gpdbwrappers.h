...
void GpdbEreportImpl(int xerrcode, int severitylevel, const char *xerrmsg, const char *xerrhint,
					 const char *filename, int lineno, const char *funcname);
#define GpdbEreport(xerrcode, severitylevel, xerrmsg, xerrhint)                           \
	gpdb::GpdbEreportImpl(xerrcode, severitylevel, xerrmsg, xerrhint, __FILE__, __LINE__, \
						  PG_FUNCNAME_MACRO)
...
void CdbEstimateRelationSize(RelOptInfo *relOptInfo, Relation rel, int32 *attr_widths,
...
ForeignScan *CreateForeignScanForExternalTable(Oid rel_oid, Index scanrelid, List *qual,
											   List *targetlist);
...
#define NewNode(size, tag)                                                              \
	(AssertMacro((size) >= sizeof(Node)), /* need the tag, at least */                  \
	 newNodeMacroHolder = (Node *) Palloc0Fast(size), newNodeMacroHolder->type = (tag), \
	 newNodeMacroHolder)
#endif	// __GNUC__
