...
void CdbEstimateRelationSize(RelOptInfo *relOptInfo, Relation rel, int32 *attr_widths, BlockNumber *pages,
...
#define ForEachWithCount(cell, list, counter) \
	for ((cell) = gpdb::ListHead(list), (counter) = 0; (cell) != NULL; (cell) = lnext(cell), ++(counter))
