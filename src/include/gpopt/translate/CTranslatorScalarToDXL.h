...
class CTranslatorScalarToDXL
{
	...
	typedef CDXLNode *(CTranslatorScalarToDXL::*ExprToDXLFn)(const Expr *expr,
															 const CMappingVarColId *var_colid_mapping);

	// shorthand for functions for translating DXL nodes to GPDB expressions
	typedef CDXLDatum *(DxlDatumFromDatum)(CMemoryPool *mp, const IMDType *md_type, BOOL is_null, ULONG len,
										   Datum datum);
...
private:
	...
	void AddArrayIndexList(CDXLNode *dxlnode, List *list, CDXLScalarArrayRefIndexList::EIndexListBound index_list_bound,
						   const CMappingVarColId *var_colid_mapping);
...
public:
	...
	static BYTE *ExtractByteArrayFromDatum(CMemoryPool *mp, const IMDType *md_type, BOOL is_null, ULONG len,
										   Datum datum);
	...
	static LINT ExtractLintValueFromDatum(const IMDType *md_type, BOOL is_null, BYTE *bytes, ULONG len);
	...
	static CDXLDatum *TranslateGenericDatumToDXL(CMemoryPool *mp, const IMDType *md_type, INT type_modifier,
