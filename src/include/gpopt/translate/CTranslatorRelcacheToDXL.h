...
private:
	...
	public:
		// ctor
		SFuncProps(OID oid, IMDFunction::EFuncStbl stability, IMDFunction::EFuncDataAcc access, BOOL is_strict,
				   BOOL ReturnsSet)
			: m_oid(oid), m_stability(stability), m_access(access), m_is_strict(is_strict), m_returns_set(ReturnsSet)
		{
		}
	...
	static IMDCacheObject *RetrieveObjectGPDB(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid);
	...
	static CDXLNode *GetDefaultColumnValue(CMemoryPool *mp, CMDAccessor *md_accessor, TupleDesc rd_att,
										   AttrNumber attrno);


	// get the distribution columns
	static ULongPtrArray *RetrieveRelDistributionCols(CMemoryPool *mp, GpPolicy *gp_policy, CMDColumnArray *mdcol_array,
													  ULONG size);
	...
	static CMDPartConstraintGPDB *RetrievePartConstraintFromNode(CMemoryPool *mp, CMDAccessor *md_accessor,
	...
	static IMDIndex *RetrievePartTableIndex(CMemoryPool *mp, CMDAccessor *md_accessor, IMDId *mdid_index,
	...
	static IMDIndex *RetrievePartTableIndex(CMemoryPool *mp, CMDAccessor *md_accessor, LogicalIndexInfo *index_info,
	...
	static CDXLColStats *GenerateStatsForSystemCols(CMemoryPool *mp, OID rel_oid, CMDIdColStats *mdid_col_stats,
...
public:
	...
	static void AddSystemColumns(CMemoryPool *mp, CMDColumnArray *mdcol_array, Relation rel, BOOL is_ao_table);
