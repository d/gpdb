...
private:
	...
	public:
		// ctor
		SFuncProps(OID oid, IMDFunction::EFuncStbl stability, IMDFunction::EFuncDataAcc access,
				   BOOL is_strict, BOOL ReturnsSet)
			: m_oid(oid),
	...
	static IMDCacheObject *RetrieveObjectGPDB(CMemoryPool *mp, CMDAccessor *md_accessor,
											  IMDId *mdid);
	...
	static CHistogram *TransformMcvToOrcaHistogram(CMemoryPool *mp, const IMDType *md_type,
	...
	static CHistogram *TransformHistToOrcaHistogram(CMemoryPool *mp, const IMDType *md_type,
													const Datum *hist_values, ULONG num_hist_values,
													CDouble num_distinct, CDouble hist_freq);

	// histogram to array of dxl buckets
	static CDXLBucketArray *TransformHistogramToDXLBucketArray(CMemoryPool *mp,
	...
	static CDXLBucketArray *TransformStatsToDXLBucketArray(
		...
		const Datum *mcv_values, const float4 *mcv_frequencies, ULONG num_mcv_values,
		const Datum *hist_values, ULONG num_hist_values);
	...
	static ULongPtr2dArray *RetrieveRelKeysets(CMemoryPool *mp, OID oid,
											   BOOL should_add_default_keys, BOOL is_partitioned,
											   ULONG *attno_mapping);
	...
	static CMDColumnArray *RetrieveRelColumns(CMemoryPool *mp, CMDAccessor *md_accessor,
											  Relation rel,
											  IMDRelation::Erelstoragetype rel_storage_type);

	// return the dxl representation of the column's default value
	static CDXLNode *GetDefaultColumnValue(CMemoryPool *mp, CMDAccessor *md_accessor,
										   TupleDesc rd_att, AttrNumber attrno);


	// get the distribution columns
	static ULongPtrArray *RetrieveRelDistributionCols(CMemoryPool *mp, GpPolicy *gp_policy,
													  CMDColumnArray *mdcol_array, ULONG size);

	// construct a mapping GPDB attnos -> position in the column array
	static ULONG *ConstructAttnoMapping(CMemoryPool *mp, CMDColumnArray *mdcol_array,
										ULONG max_cols);
	...
	static ULongPtrArray *ComputeIncludedCols(CMemoryPool *mp, const IMDRelation *md_rel);
	...
	static CMDPartConstraintGPDB *RetrievePartConstraintFromNode(
		...
		Node *part_constraint, ULongPtrArray *level_with_default_part_array, BOOL is_unbounded);
	...
	static CMDIndexInfoArray *RetrieveRelIndexInfoForPartTable(CMemoryPool *mp, Relation root_rel);

	// return index info list of indexes defined on regular, external tables or leaf partitions
	static CMDIndexInfoArray *RetrieveRelIndexInfoForNonPartTable(CMemoryPool *mp, Relation rel);

	// retrieve an index over a partitioned table from the relcache
	static IMDIndex *RetrievePartTableIndex(CMemoryPool *mp, CMDAccessor *md_accessor,
											IMDId *mdid_index, const IMDRelation *md_rel,
	...
	static IMDIndex *RetrievePartTableIndex(CMemoryPool *mp, CMDAccessor *md_accessor,
											LogicalIndexInfo *index_info, IMDId *mdid_index,
	...
	static IMdIdArray *RetrieveRelDistributionOpFamilies(CMemoryPool *mp, GpPolicy *policy);
	...
	static CDXLColStats *GenerateStatsForSystemCols(
		CMemoryPool *mp, OID rel_oid, CMDIdColStats *mdid_col_stats, CMDName *md_colname,
		OID att_type, AttrNumber attrnum, CDXLBucketArray *dxl_stats_bucket_array, CDouble rows);
...
public:
	...
	static CMDCheckConstraintGPDB *RetrieveCheckConstraints(CMemoryPool *mp,
