...
private:
	...
	static IMdIdArray *ResolvePolymorphicTypes(CMemoryPool *mp, IMdIdArray *mdid_array, List *arg_types,
											   FuncExpr *func_expr);
...
public:
	...
	static CDXLTableDescr *GetTableDescr(CMemoryPool *mp, CMDAccessor *md_accessor, CIdGenerator *id_generator,
										 const RangeTblEntry *rte, BOOL *is_distributed_table = NULL);
	...
	static CDXLColDescrArray *GetColumnDescriptorsFromRecord(CMemoryPool *mp, CIdGenerator *id_generator,
	...
	static CBitSetArray *GetColumnAttnosForGroupBy(CMemoryPool *mp, List *group_clause, ULONG num_cols,
	...
	static ULongPtrArray *GenerateColIds(CMemoryPool *mp, List *target_list, IMdIdArray *input_mdids,
										 ULongPtrArray *input_nums, BOOL *is_outer_ref, CIdGenerator *colid_generator);
	...
	static BOOL IsApplicableTrigger(CMDAccessor *md_accessor, IMDId *trigger_mdid, const EdxlDmlType dml_type_dxl);
